require('scripts/actions/mobskills/pet_gust_breath')

describe('Pet Gust Breath mob skill', function()
    it('uses its wind breath plan and damages only after processing', function()
        local breath = require('scripts/actions/mobskills/pet_gust_breath')
        ...
        assert(params.element == xi.element.WIND and params.attackType == xi.attackType.BREATH)
        assert(params.damageType == xi.damageType.WIND)
        assert(damage == nil)
        xi.mobskills.processDamage = function() return true end
        breath.onMobWeaponSkill({}, target, {}, {})
        xi.mobskills.mobBreathMove, xi.mobskills.processDamage = move, process
        assert(damage[1] == 123 and damage[3] == xi.attackType.BREATH and damage[4] == xi.damageType.WIND)
    end)
end)
