...
describe('Abyssea Grauberg Cruor Prospector permanent items', function()
    it('offers its regular item and Forbidden Key selections', function()
        ...
        local npc = require('scripts/zones/Abyssea-Grauberg/npcs/Cruor_Prospector')
        npc.onEventFinish(player, 0, xi.abyssea.itemType.ITEM + 65536)
        npc.onEventFinish(player, 0, xi.abyssea.itemType.ITEM + 12 * 65536 + 2 * 16777216)
        npcUtil.giveItem = oldGiveItem
        assert(granted[1][1] == xi.item.UNKAI_KOTE and granted[1][2] == 1)
