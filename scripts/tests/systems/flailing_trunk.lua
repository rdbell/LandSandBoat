describe('Flailing Trunk mob skill', function()
    it('uses its critical slashing plan and damages only after processing', function()
        local flailingTrunk = require('scripts/actions/mobskills/flailing_trunk')
        ...
        assert(flailingTrunk.onMobSkillCheck(target, mob, {}) == 0)
        assert(flailingTrunk.onMobWeaponSkill(mob, target, {}, {}) == 123)
        ...
        assert(params.canCrit and params.criticalChance[1] == 0.5 and params.criticalChance[2] == 0.5 and params.criticalChance[3] == 0.5 and damage == nil)
