require('scripts/actions/mobskills/chains_of_envy')
describe('Chains of Envy mob skill', function()
    it('applies TERROR to vulnerable PCs without the light key item', function()
        local skill = require('scripts/actions/mobskills/chains_of_envy')
        local statusMove = xi.mobskills.mobStatusEffectMove
        local status, msg = nil, nil
        local races = {xi.race.MITHRA}
        local target = {
            isPC = function() return true end,
            getRace = function() return races[1] end,
            hasKeyItem = function(_, ki) return ki == xi.ki.LIGHT_OF_DEM end,
        }
