require('scripts/actions/mobskills/deafening_tantara')

describe('Deafening Tantara mob skill', function()
    it('rejects hornless Imps and otherwise applies fixed Silence', function()
        ...
        local tantara = require('scripts/actions/mobskills/deafening_tantara')
        ...
        assert(tantara.onMobSkillCheck({}, mob, skill) == 0)
        assert(tantara.onMobWeaponSkill(mob, {}, skill, {}) == xi.effect.SILENCE)

        xi.mobskills.mobStatusEffectMove = effectMove
        assert(calls[1][1] == xi.effect.SILENCE and calls[1][2] == 1 and calls[1][3] == 0 and calls[1][4] == 30)
        assert(message == 77)
    end)
end)
