require('scripts/actions/mobskills/cursed_sphere')

describe('Cursed Sphere mob skill', function()
    it('uses fixed Dark magical parameters and damages only after processing succeeds', function()
        local cursedSphere = require('scripts/actions/mobskills/cursed_sphere')
        ...
        assert(cursedSphere.onMobSkillCheck(target, mob, skill) == 0)
        assert(cursedSphere.onMobWeaponSkill(mob, target, skill, action) == 123)
        ...
        assert(params.damageType == xi.damageType.DARK and params.shadowBehavior == xi.mobskills.shadowBehavior.IGNORE_SHADOWS)
        assert(damage == nil)
