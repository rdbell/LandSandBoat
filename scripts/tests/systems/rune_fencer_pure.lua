...
describe('Rune Fencer pure pins', function()
    it('bases', function()
        ...
        assert(r.requireRuneMessage == 666)

        local msg, param = r.checkHaveRunesFromParams({ activeRuneCount = 0 })
        assert(msg == 666 and param == 0)
        msg, param = r.checkHaveRunesFromParams({ activeRuneCount = 1 })
        assert(msg == 0 and param == 0)
        msg, param = r.checkHaveRunesFromParams({ activeRuneCount = 255 })
        assert(msg == 0 and param == 0)
    end)
end)
