require('scripts/actions/mobskills/crystal_weapon_fire')

describe('Crystal Weapon Fire mob skill', function()
    it('uses fixed Fire magical parameters and damages only after processing succeeds', function()
        local crystalWeaponFire = require('scripts/actions/mobskills/crystal_weapon_fire')
        ...
        assert(crystalWeaponFire.onMobSkillCheck(target, mob, skill) == 0)
        assert(crystalWeaponFire.onMobWeaponSkill(mob, target, skill, action) == 123)
        ...
        assert(params.damageType == xi.damageType.FIRE and params.shadowBehavior == xi.mobskills.shadowBehavior.IGNORE_SHADOWS)
        assert(damage == nil)
