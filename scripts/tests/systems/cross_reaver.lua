require('scripts/actions/mobskills/cross_reaver')

describe('Cross Reaver mob skill', function()
    it('uses fixed physical parameters and applies damage and Stun only after processing succeeds', function()
        local crossReaver = require('scripts/actions/mobskills/cross_reaver')
        ...
        assert(crossReaver.onMobSkillCheck(target, mob, skill) == 0)
        assert(crossReaver.onMobWeaponSkill(mob, target, skill, action) == 123)
        ...
        assert(effect[1] == mob and effect[2] == target and effect[3] == xi.effect.STUN)
        assert(effect[4] == 1 and effect[5] == 0 and effect[6] == 4)
    end)
end)
