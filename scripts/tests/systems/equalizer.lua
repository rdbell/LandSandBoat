require('scripts/actions/mobskills/equalizer')

describe('Equalizer mob skill', function()
    it('uses its one-hit physical plan and damages only after processing succeeds', function()
        local equalizer = require('scripts/actions/mobskills/equalizer')
        ...
        assert(equalizer.onMobSkillCheck({}, {}, {}) == 0)
        assert(equalizer.onMobWeaponSkill(mob, target, {}, {}) == 123)
        assert(params.baseDamage == 77 and params.numHits == 1 and params.fTP[1] == 3 and params.fTP[2] == 3 and params.fTP[3] == 3)
        assert(params.attackType == xi.attackType.PHYSICAL and params.damageType == xi.damageType.BLUNT and params.shadowBehavior == xi.mobskills.shadowBehavior.NUMSHADOWS_3 and damage == nil)
