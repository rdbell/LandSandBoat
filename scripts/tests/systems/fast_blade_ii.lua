describe('Fast Blade II mob skill', function()
    it('announces Fast Blade and applies its two-hit Slashing plan only after processing', function()
        local fastBladeII = require('scripts/actions/mobskills/fast_blade_ii')
        ...
        assert(fastBladeII.onMobSkillCheck(target, mob, {}) == 0)
        assert(ready[1] == xi.msg.basic.READIES_WS and ready[2] == 0 and ready[3] == xi.weaponskill.FAST_BLADE)
        assert(fastBladeII.onMobWeaponSkill(mob, target, {}, {}) == 123)
        assert(params.baseDamage == 77 and params.numHits == 2 and params.fTP[1] == 1.8 and params.fTP[2] == 3.5 and params.fTP[3] == 5)
