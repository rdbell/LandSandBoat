require('scripts/actions/mobskills/cyclone')

describe('Cyclone mob skill', function()
    it('uses fixed Wind magical parameters and damages only after processing succeeds', function()
        local cyclone = require('scripts/actions/mobskills/cyclone')
        ...
        assert(cyclone.onMobSkillCheck(target, mob, skill) == 0)
        assert(cyclone.onMobWeaponSkill(mob, target, skill, action) == 123)
        assert(params.baseDamage == 52 and params.fTP[1] == 1 and params.fTP[2] == 2.375 and params.fTP[3] == 2.875)
