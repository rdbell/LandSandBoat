describe('Frog Chorus mob skill', function()
    it('misses non-players and charms then costumes players only after enfeebling success', function()
        local chorus = require('scripts/actions/mobskills/frog_chorus')
        ...
        assert(chorus.onMobSkillCheck({}, mob, skill) == 0 and chorus.onMobWeaponSkill(mob, npc, skill, {}) == xi.effect.CHARM_I and skill.message == xi.msg.basic.SKILL_MISS)
        ...
        assert(chorus.onMobWeaponSkill(mob, player, skill, {}) == xi.effect.CHARM_I and not player.charmed and player.costume == nil)
        xi.mobskills.mobStatusEffectMove = function() return xi.msg.basic.SKILL_ENFEEB_IS end
        assert(chorus.onMobWeaponSkill(mob, player, skill, {}) == xi.effect.CHARM_I and player.charmed and player.costume[1] == xi.effect.COSTUME and player.costume[2].power == 1812 and player.costume[2].duration == 60 and player.costume[2].origin == mob and skill.message == xi.msg.basic.SKILL_ENFEEB_IS)
