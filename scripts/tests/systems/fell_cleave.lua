describe('Fell Cleave mob skill', function()
    it('uses its one-hit Slashing plan and damages only after processing', function()
        local fellCleave = require('scripts/actions/mobskills/fell_cleave')
        ...
        assert(fellCleave.onMobSkillCheck(target, mob, {}) == 0)
        assert(fellCleave.onMobWeaponSkill(mob, target, {}, {}) == 123)
