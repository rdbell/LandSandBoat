require('scripts/actions/mobskills/pet_sand_breath')

describe('Pet Sand Breath mob skill', function()
    it('uses its earth breath plan and damages only after processing', function()
        local breath = require('scripts/actions/mobskills/pet_sand_breath')
        ...
        assert(params.element == xi.element.EARTH and params.attackType == xi.attackType.BREATH)
        assert(params.damageType == xi.damageType.EARTH)
        assert(damage == nil)
        xi.mobskills.processDamage = function() return true end
        breath.onMobWeaponSkill({}, target, {}, {})
        xi.mobskills.mobBreathMove, xi.mobskills.processDamage = move, process
        assert(damage[1] == 123 and damage[3] == xi.attackType.BREATH and damage[4] == xi.damageType.EARTH)
    end)
end)
