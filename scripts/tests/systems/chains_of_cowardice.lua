require('scripts/actions/mobskills/chains_of_cowardice')
describe('Chains of Cowardice mob skill', function()
    it('applies TERROR to vulnerable PCs without the light key item', function()
        local skill = require('scripts/actions/mobskills/chains_of_cowardice')
        local statusMove = xi.mobskills.mobStatusEffectMove
        local status, msg = nil, nil
        local races = {xi.race.TARU_M, xi.race.TARU_F}
        local target = {
            isPC = function() return true end,
            getRace = function() return races[1] end,
            hasKeyItem = function(_, ki) return ki == xi.ki.LIGHT_OF_HOLLA end,
        }
