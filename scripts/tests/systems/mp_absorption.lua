require('scripts/actions/mobskills/mp_absorption')

describe('MP Absorption mob skill', function()
    it('uses its magical MP-drain request and only drains after processing', function()
        ...
        local skillObject = require('scripts/actions/mobskills/mp_absorption')
        assert(skillObject.onMobSkillCheck(target, mob, skill) == 0 and skillObject.onMobWeaponSkill(mob, target, skill, {}) == 123)
        assert(params.baseDamage == 75 and params.fTP[1] == 1.5 and params.fTP[2] == 1.5 and params.fTP[3] == 1.5)
