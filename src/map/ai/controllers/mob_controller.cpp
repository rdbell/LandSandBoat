...
auto CMobController::CanDetectTarget(CBattleEntity* PTarget, const bool forceSight) const -> bool
{
    ...
    const auto illusionState = mobcontrollerillusiondetection::Apply(
        PTarget->StatusEffectContainer->HasStatusEffect(xi::StatusEffect::Illusion),
        hasInvisible,
        hasSneak,
        [&]() { return PMob->getMobMod(MOBMOD_SEES_THROUGH_ILLUSION) != 0; });
...
auto CMobController::TryCastSpell() -> bool
{
    ...
    switch (mobcontrollertrycasttargetsource::Select(
        (PSpell->getValidTarget() & TARGET_SELF) != 0,
        maybeTargetOverride.has_value()))
    {
        case mobcontrollertrycasttargetsource::Source::BattleTarget:
            PCastTarget = PTarget;
            break;
        case mobcontrollertrycasttargetsource::Source::Self:
            PCastTarget = PMob;
            break;
        case mobcontrollertrycasttargetsource::Source::Override:
...
void CMobController::Move()
{
    ...
    if (mobcontrollerscriptedpath::ShouldFollow(
            PMob->PAI->CanFollowPath(),
            PMob->PAI->PathFind->IsFollowingScriptedPath()))
    {
        PMob->PAI->PathFind->FollowPath(m_Tick);
        return;
    }
    ...
    else if (PTarget)
    {
        ...
        if (mobcontrollermovemententry::ShouldEnter(
                ...
                PMob->PAI->CanFollowPath()))
        {
            if (mobcontrollerchasemovementadmission::CanEnter(
                    PMob->GetSpeed() != 0,
                    PMob->getMobMod(MOBMOD_NO_MOVE) == 0,
                    m_Tick >= m_LastSpecialTime))
            {
...
auto CMobController::DoRoamTick(timer::time_point tick) -> Task<void>
{
            ...
            m_Tick, m_mobHealTime, PMob->getMobMod(MOBMOD_NO_REST) != 0, PMob->CanRest()))
    {
        ...
        if (mobcontrollerroamrestfullhealth::ShouldResetExperience(PMob->GetHPP()))
        {
    ...
    if (m_Tick >= m_WaitTime)
    {
                     ...
                     m_Tick, m_LastActionTime, std::chrono::seconds(PMob->getMobMod(MOBMOD_ROAM_COOL))))
        {
            ...
            if (mobcontrollerroamcallforhelp::ShouldClear(PMob->GetCallForHelpFlag()))
            {
            ...
            if (roamHomeAction == mobcontrollerroamhomeaction::Action::PathHome)
            {
                ...
                m_LastActionTime = mobcontrollerroamhomeretryschedule::Schedule(
