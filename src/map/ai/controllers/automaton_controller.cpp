...
auto CAutomatonController::TrySpellcast(const CurrentManeuvers& maneuvers) -> bool
{
    ...
    switch (PAutomaton->head())
    {
        ...
        case AutomatonHead::Stormwaker:
        {
            ...
            else if (automatoncontrollerstormwakerelementalfallback::ShouldUseElementalFallback(lowHP, maneuvers.ice) && TryElemental(maneuvers))
            {
                m_LastElementalTime = m_Tick;
                return true;
            }
        ...
        case AutomatonHead::Spiritreaver:
        {
            ...
            else if (automatoncontrollerspiritreaverenfeeblepriority::ShouldPrioritizeEnfeeble(maneuvers.dark, PAutomaton->GetHPP(), PAutomaton->GetMPP()) &&
...
auto CAutomatonController::TryEnfeeble(const CurrentManeuvers& maneuvers) -> bool
{
    ...
    switch (PAutomaton->head())
    {
        ...
        default:
        {
            ...
            if (automatoncontrollersoulsootherparalyze::CanPrioritize(maneuvers.ice))
            { // Ice -> Paralyze
        ...
        case AutomatonHead::Spiritreaver:
        {
            ...
            if (maneuvers.dark) // Dark -> Access to Enfeebles
            {
                ...
                if (automatoncontrollerspiritreaversilence::CanPrioritize(maneuvers.wind))
                { // 2 Wind -> Silence
                    castPriority.emplace_back(SpellID::Silence);
                }
                else
                ...
                if (automatoncontrollerspiritreaverslow::CanPrioritize(maneuvers.earth))
                { // 2 Earth -> Slow
                    castPriority.emplace_back(SpellID::Slow);
                }
                else
                ...
                if (automatoncontrollerspiritreaverparalyze::CanPrioritize(maneuvers.ice))
                { // 2 Ice -> Paralyze
                    castPriority.emplace_back(SpellID::Paralyze);
                }
                else
