...
{
public:
    using Cell = std::variant<std::monostate, int64, double, std::string>;
    using Row  = std::vector<Cell>;

    SQLiteResultSet(const std::string& query, std::shared_ptr<const SQLiteColumnSchema> schema, std::vector<Row> rows);
    SQLiteResultSet(std::size_t rowsAffected, const std::string& query);
