#include "test_enmity_zero_out_of_range_3427.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "enmity ShouldZeroEnmityOutOfRange 3427 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline UpdateEnmity out-of-range CE/VE zero gate for dual-wire cross-check
// (slice 3427 dedicated expand residual 2927; prior dedicated expand 3333 /
...
// Compact dual-wire pin matching free function / capacity body (slice 3427).
...
//   - residual 1357 / 2927 / prior dedicated 3157 / prior expand 3333 pins still hold
auto runEnmityZeroOutOfRange3427SelfTests() -> bool
{
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinShouldZeroEnmityOutOfRange3427(p.withinRange);
        ...
        ok = expect(got == pinShouldZeroEnmityOutOfRange3333(p.withinRange),
                    "ShouldZeroEnmityOutOfRange == prior expand pin") &&
             ok;
    ...
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3427(false), "free==pin false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3427(true), "free==pin true") && ok;
    ...
    for (const bool withinRange : { false, true })
    {
        ...
        const bool pin     = pinShouldZeroEnmityOutOfRange3427(withinRange);
        ...
        ok                 = expect(got == pinShouldZeroEnmityOutOfRange3333(withinRange), "dense free == prior expand pin") && ok;
    }
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3427(within), "compose free == pin") && ok;
    }

    // Residual 2927 / prior dedicated expand 3333 / prior dedicated 3157 /
    // pure 1357 still hold under expand suite.
    ok = expect(ShouldZeroEnmityOutOfRange(false) && !ShouldZeroEnmityOutOfRange(true),
                "residual 2927/3333/3157/1357 pins failed under 3427 suite") &&
         ok;
