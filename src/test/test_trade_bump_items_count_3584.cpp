#include "test_trade_bump_items_count_3584.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "trade ShouldBumpItemsCountOnSetEntry 3584 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldBumpItemsCountOnSetEntry3584 / C++ capacity:
//   slotInRange
...
// Prior dedicated 3540 inline pin for independence cross-check.
auto inlineShouldBumpItemsCountOnSetEntry3540(const bool slotInRange) -> bool
{
    return slotInRange;
}

// Prior dedicated 3540 compact pin for free == inline == pin == pin3540 cross-check.
auto pinShouldBumpItemsCountOnSetEntry3540(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3584 expand residual 2997; prior dedicated 3540 / 3474 / 3420 / 3372).
//
// Coverage:
//   - free == inline == pin == pin3540 == slotInRange (identity)
//   - residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 pins still hold
//   - prior dedicated 3540 independence (free == prior inline == prior pin)
...
auto runTradeBumpItemsCount3584SelfTests() -> bool
{
    ...
    // Residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3540 ---
    const struct
    {
        bool        slotInRange;
        bool        want;
        ...
        // Residual 2997 / 2812 / prior 3372 / 3420 / 3474 / 3540 poles (full 2^1 surface).
        ...
        // Prior dedicated 3540 poles still hold.
        { true, true, "prior 3540 in range bumps" },
        { false, false, "prior 3540 out of range no bump" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3540 = pinShouldBumpItemsCountOnSetEntry3540(c.slotInRange);
        const bool wantPin = c.slotInRange;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3540.
        ok = expect(got == inlineF && got == pin && got == pin3540,
                    "dual-wire free == inline == pin == pin3540") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula identity") && ok;
    }

    // Free == pin == pin3540 across residual poles.
    ...
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry(false),
                "free == pin for false") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3540(true),
                "free == pin3540 for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3540(false),
                "free == pin3540 for false") &&
         ok;

    // Prior dedicated 3540 independence: free still matches prior inline/pin.
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == inlineShouldBumpItemsCountOnSetEntry3540(true) &&
                    ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3540(true),
                "prior 3540 free/inline/pin mismatch for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == inlineShouldBumpItemsCountOnSetEntry3540(false) &&
                    ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3540(false),
                "prior 3540 free/inline/pin mismatch for false") &&
         ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3540.
    for (const bool slotInRange : { false, true })
    {
        ...
        const bool pin3540 = pinShouldBumpItemsCountOnSetEntry3540(slotInRange);
        const bool want    = slotInRange;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3540,
                                    "compose free == inline == pin == pin3540") &&
             ok;
        // Prior 3540 independence across dense poles.
        ok = expect(got == inlineShouldBumpItemsCountOnSetEntry3540(slotInRange) &&
                        got == pinShouldBumpItemsCountOnSetEntry3540(slotInRange),
                    "compose free == prior 3540 inline/pin") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3540 for multi-arg setItem bump.
    for (const bool slotInRange : { false, true })
    {
        const bool got     = ShouldBumpItemsCountOnSetEntry(slotInRange);
        const bool inlineF = inlineShouldBumpItemsCountOnSetEntry(slotInRange);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3540,
                                    "host inject free == inline == pin == pin3540") &&
             ok;
    }
