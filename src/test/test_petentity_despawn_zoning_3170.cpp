#include "test_petentity_despawn_zoning_3170.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petentity ShouldDespawnForZoning 3170 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CPetEntity::Die zoning-despawn formula for dual-wire cross-check
// (dedicated slice 3170):
...
// Positive if/else pin matching free function / capacity body (slice 3170).
...
// (CPetEntity::Die living player-pet zoning-despawn gate; dedicated slice 3170;
// residual expand 2951 / pure 1414 / 2261).
//
// Coverage:
...
auto runPetentityDespawnZoning3170SelfTests() -> bool
{
    ...
    // Sibling residual 2987 still holds (not re-expanded under 3170).
