#include "test_portermoogle_can_retrieve_item_3613.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "portermoogle CanRetrieveItem 3613 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua porter_moogle hasItem / freeSlots gate for dual-wire cross-check
// (dedicated slice 3613):
//   !hasItem && freeSlots > 0
...
// Compact dual-wire pin matching Go pinCanRetrieveItem3613 / C++ capacity
...
// Compact dual-wire pin matching Go pinCanRetrieveItem3568 (prior dedicated):
//   !hasItem && freeSlots > 0
auto pinCanRetrieveItem3568(const bool hasItem, const int32 freeSlots) -> bool
{
    return !hasItem && freeSlots > 0;
}
...
// (Lua porter_moogle hasItem + freeSlots retrieve gate; dedicated slice 3613
// expand residual 2966 / prior dedicated ~3568 / 3512 / 3463 / 3409 / 3316 / 3286 / 3243 / 3171 / pure 1144 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3568 == (!hasItem && freeSlots > 0) (pin direct return)
//   - residual 1144 / 2966 / prior 3171 / 3243 / 3286 / 3316 / 3409 / 3463 / 3512 / 3568 pins still hold
...
auto runPortermoogleCanRetrieveItem3613SelfTests() -> bool
{
    ...
    // Residual 1144 / 2966 / prior 3171 / 3243 / 3286 / 3316 / 3409 / 3463 / 3512 / 3568 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3568 ---
    ...
    const struct
    {
        ...
        // Residual 2966 / prior 3171 / 3243 / 3286 / 3316 / 3409 / 3463 / 3512 / 3568 happy / multi free (n).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3568 = pinCanRetrieveItem3568(c.hasItem, c.freeSlots);
        const bool wantPin = !c.hasItem && c.freeSlots > 0;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3568, "CanRetrieveItem dual-wire free == inline == pin == pin3568") && ok;
    ...
    // Dense 2×slots: hasItem × freeSlots {0, 1, n, negative} free == inline == pin == pin3568.
    ...
    for (const bool hasItem : { false, true })
    {
        for (const int32 freeSlots : freePoles)
        {
            ...
            const bool pin3568 = pinCanRetrieveItem3568(hasItem, freeSlots);
            const bool want    = !hasItem && freeSlots > 0;
            ok                 = expect(got == want, "dense 2xslots free == pin formula") && ok;
            ok                 = expect(got == inlineF && got == pin && got == pin3568, "dense 2xslots free == inline == pin == pin3568") && ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3568 for host poles.
    for (const bool hasItem : { false, true })
    {
        for (const int32 freeSlots : freePoles)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3568, "host inject free == inline == pin == pin3568") && ok;
        }
    }
    ...
    for (const bool hasItem : { false, true })
    {
        for (const int32 freeSlots : freePoles)
        {
            ...
            ok                   = expect(can == inlineF && can == pin && can == pin3568, "buy polarity free == inline == pin == pin3568") && ok;
        }
    }
