#include "test_follow_can_follow_3506.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3506 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3506 dedicated expand residual 2883; prior dedicated 3084 / 3237 / 3331 / 3405 / 3459):
...
// Compact dual-wire pin matching free function / capacity body (slice 3506).
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3506 dedicated
// expand residual 2883; prior dedicated 3084 / 3237 / 3331 / 3405 / 3459). Formula
// unchanged.
//
// Coverage:
//   - free == inline == pin (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 poles still hold
...
auto runFollowCanFollow3506SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3506(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3506(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
