#include "test_follow_can_follow_3737.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3737 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3737 dedicated expand residual 2883; prior dedicated 3692 / 3647 /
// 3602 / 3557 / 3506 / 3459 / 3405 / 3331 / 3237 / 3084):
...
// Compact dual-wire pin matching free function / capacity body (slice 3737).
...
// Prior dedicated 3692 pin retained for free == pin3692 dual-wire agreement.
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3737 dedicated
// expand residual 2883; prior dedicated 3692 / 3647 / 3602 / 3557 / 3506 /
// 3459 / 3405 / 3331 / 3237 / 3084). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3692 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 / 3692 poles still hold
...
auto runFollowCanFollow3737SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 / 3692 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3737(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3692 = pinCanFollow3692(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3737(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3692 = pinCanFollow3692(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3692, "dense free == pin3692") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3692 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3692, "host inject dual-wire free == pin3692") && ok;
    }
