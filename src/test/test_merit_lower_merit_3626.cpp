#include "test_merit_lower_merit_3626.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "merit ShouldLowerMerit 3626 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline LowerMerit count-decrement formula for dual-wire cross-check
// (slice 3626 dedicated expand residual 3054; prior dedicated 3581 / 3537):
//   count > 0
...
// Retained under dedicated expand residual 3626.
...
// (count > 0 LowerMerit count-decrement gate; OmegaXI internal/merit; slice 3626
// dedicated expand residual 3054; prior dedicated 3581 / 3537). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3581 (direct return formula)
//   - residual 3054 / 2810 / prior dedicated 3581 / 3537 poles still hold
...
auto runMeritLowerMerit3626SelfTests() -> bool
{
    ...
    // Residual 3054 / 2810 / prior dedicated 3581 / 3537 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3054 / 2810 / prior 3581 / 3537 classic poles (0, 1, n).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin           = pinShouldLowerMerit3626(p.count);
    ...
    // Dense poles: free == inline == pin == pin3581 (direct return) across residual + interior.
                               ...
                               uint8{ 255 } })
    {
        ...
        const bool pin          = pinShouldLowerMerit3626(count);
    ...
    for (const bool present : { false, true })
    {
        for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 15 }, uint8{ 255 } })
        {
            ...
            ok             = expect(got == inlineShouldLowerMerit(count),
                        "compose free == inline") &&
                 ok;
            ok = expect(got == pinShouldLowerMerit3626(count), "compose free == pin") && ok;
