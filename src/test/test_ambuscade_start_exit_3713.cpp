#include "test_ambuscade_start_exit_3713.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ambuscade start-exit 3713 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onInstanceComplete / onInstanceFailure formula for dual-wire
// checks (slice 3713): player:startEvent(10001) is always called for every char.
...
// Compact dual-wire pin matching C++ capacity formula / Go pinShouldStartExitEvent3713:
//   return true
...
// Prior dedicated pin3668 (always true; free == pin3668 always holds).
auto pinShouldStartExitEvent3668() -> bool
{
    return true;
}
...
// OmegaXI internal/ambuscade; dedicated slice 3713 expand residual 2917; prior
// dedicated 3668/3623/3578/3534/3488/3438/3382/3109). Formula unchanged:
//
//   ShouldStartExitEvent() = true
//
// Coverage:
//   - free == inline == pin == pin3668 == true
//   - residual 2917 / 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 3623 / 3668 / 1089 pins still hold
...
auto runAmbuscadeStartExit3713SelfTests() -> bool
{
    ...
    // Residual constant / 1089 / 2917 / prior dedicated 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 3623 / 3668 pins still hold.
    ...
    // Unconditional gate always true; free == inline == pin == pin3668 (direct return true).
    {
        ...
        const bool pin3668   = pinShouldStartExitEvent3668();
        ...
        ok = expect(got == pin3668, "dual-wire free == pin3668") && ok;
        ok = expect(got == true, "dual-wire free == true") && ok;
    }

    // Repeated calls stay true (no state); free == inline == pin == pin3668 matching C++.
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        ok             = expect(got == pinShouldStartExitEvent3668(), "free == pin3668 across calls") && ok;
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pinShouldStartExitEvent3668(), "compose free == pin3668") && ok;
    ...
    // Dense always-true poles: free == inline == pin == pin3668 for unconditional domain.
    for (int i = 0; i < 2; ++i)
    {
        const bool got = ShouldStartExitEvent();
        ok             = expect(got == pinShouldStartExitEvent(), "dense free == pin") && ok;
        ok             = expect(got == inlineShouldStartExitEvent(), "dense free == inline") && ok;
        ok             = expect(got == pinShouldStartExitEvent3668(), "dense free == pin3668") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3668(), "free == pin3668 always-true") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3668(), "host path free == pin3668") && ok;

    // Residual 2917 / prior dedicated 3668/3623/3578/3534/3488/3438/3382/3109 still hold under dedicated 3713.
    ok = expect(ShouldStartExitEvent(), "residual 2917/3668/3623/3578/3534/3488/3438/3382/3109 pins under 3713") && ok;
