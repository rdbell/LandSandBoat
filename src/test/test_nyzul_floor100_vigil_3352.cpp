#include "test_nyzul_floor100_vigil_3352.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "nyzul floor100 vigil 3352 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua vigilWeaponDrop floor-100 gate for dual-wire cross-check
// (slice 3352 dedicated expand residual 2900):
...
// Compact dual-wire pin matching free function / capacity body (slice 3352).
...
// (Lua vigilWeaponDrop floor-100 guaranteed-drop gate; slice 3352 dedicated
// expand residual 2900). Formula unchanged.
//
// Coverage:
...
auto runNyzulFloor100Vigil3352SelfTests() -> bool
{
    ...
    // Residual 2900 / 1088 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3352(p.floor);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3352(e.floor);
    ...
    for (int32 floor = -2; floor <= Floor100 + 5; ++floor)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3352(floor);
    ...
    ok = expect(ShouldDropFloor100VigilWeapons(100) == pinShouldDropFloor100VigilWeapons3352(100),
                "free == pin 100") &&
         ok;
    ok = expect(ShouldDropFloor100VigilWeapons(99) == pinShouldDropFloor100VigilWeapons3352(99),
                "free == pin 99") &&
         ok;
