#include "test_pet_detach_player_master_3414.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "pet ShouldDetachPlayerMaster 3414 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CPetEntity::Die detach-player-master formula for dual-wire cross-check
// (dedicated slice 3414 expand residual 2987; prior dedicated 3337) — direct
// return AND-chain:
...
// Positive if/else pin matching free function / capacity body (slice 3414).
...
// Prior dedicated 3337 pin (independence under 3414 expand).
...
// (CPetEntity::Die player-master active-pet detach gate; dedicated slice 3414;
// residual expand 2987 / pure 1414 / 2262; prior dedicated 3337).
//
// Coverage:
//   - free == inline == pin (positive if/else form; avoid QF1001)
//   - residual 1414 / 2262 / 2987 / prior 3337 pins still hold
//   - dense 2^3 poles
...
auto runPetDetachPlayerMaster3414SelfTests() -> bool
{
    ...
    // Residual 1414 / 2262 / 2987 / prior 3337 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1414 / 2262 / 2987 / prior 3337 single-false poles.
    ...
    // Sibling residual 3233 still holds (not re-expanded under 3414).
    ...
    ok = expect(ShouldDetachPlayerMaster(true, true, true) ==
                ...
                "prior 3337 pin under 3414 for full accept") &&
         ok;
    ok = expect(ShouldDetachPlayerMaster(false, true, true) ==
                    pinShouldDetachPlayerMaster3337(false, true, true),
                "prior 3337 pin under 3414 for missing master") &&
         ok;
    ok = expect(ShouldDetachPlayerMaster(true, false, true) ==
                    pinShouldDetachPlayerMaster3337(true, false, true),
                "prior 3337 pin under 3414 for different active pet") &&
         ok;
    ok = expect(ShouldDetachPlayerMaster(true, true, false) ==
                    pinShouldDetachPlayerMaster3337(true, true, false),
                "prior 3337 pin under 3414 for non-player master") &&
         ok;

    // Residual 2987 suite still holds under dedicated expand.
    ok = expect(ShouldDetachPlayerMaster(true, true, true), "residual 2987 attached under 3414") && ok;
    ok = expect(!ShouldDetachPlayerMaster(false, true, true), "residual 2987 missing master under 3414") && ok;
    ok = expect(!ShouldDetachPlayerMaster(true, false, true), "residual 2987 different active pet under 3414") && ok;
    ok = expect(!ShouldDetachPlayerMaster(true, true, false), "residual 2987 non-player master under 3414") && ok;
