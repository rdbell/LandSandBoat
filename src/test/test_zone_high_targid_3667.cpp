#include "test_zone_high_targid_3667.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "zone ShouldRejectHighCharTargid 3667 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline IncreaseZoneCounter high-char-targid formula for dual-wire cross-check
// (dedicated slice 3667 expand residual 2949; prior dedicated 3622 / 3577 /
// 3520 / 3457 / 3384):
...
// Compact dual-wire pin matching Go pinShouldRejectHighCharTargid3667 /
...
// Prior dedicated 3622 pin form (free == inline == pin == pin3622):
//   targid >= CharTargidHighThreshold  // pin 0x700
auto pinShouldRejectHighCharTargid3622(const uint16 targid) -> bool
{
    return targid >= zonehelpers::CharTargidHighThreshold;
}
...
// dedicated slice 3667 expand residual 2949; prior dedicated 3622 / 3577 /
// 3520 / 3457 / 3384 / pure 1363).
//
// Coverage:
//   - free == inline == pin == pin3622 == (targid >= CharTargidHighThreshold)
//   - residual 1363 / 2949 / prior 3384 / prior 3457 / prior 3520 / prior 3577 / prior 3622 poles still hold
...
auto runZoneHighTargid3667SelfTests() -> bool
{
    ...
    // Residual 1363 / 2949 / prior 3384 / prior 3457 / prior 3520 / prior 3577 / prior 3622 pins still hold.
    ...
    // --- Composition table: free == inline == pin == pin3622 ---
    const struct
    {
        uint16      targid;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinShouldRejectHighCharTargid3622(c.targid);
        const bool wantPin  = c.targid >= CharTargidHighThreshold;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3622.
        ok = expect(got == inlineF && got == pin && got == pinPrior, "dual-wire free == inline == pin == pin3622") && ok;
        ok = expect(got == wantPin, "free == pin formula targid>=0x700") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3622 (0x6FF / 0x700).
    const struct
    {
        uint16      targid;
        bool        want;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pinPrior = pinShouldRejectHighCharTargid3622(p.targid);
        ok                  = expect(got == p.want, p.label) && ok;
        ok                  = expect(got == inlineF && got == pin && got == pinPrior, "pole free == inline == pin == pin3622") && ok;
    }

    // Dense dual-wire sweep around the threshold boundary:
    // free == inline == pin == pin3622 for targid in [0x6F0, 0x710].
    for (uint16 targid = 0x6F0; targid <= 0x710; ++targid)
    {
        ...
        const bool pinPrior = pinShouldRejectHighCharTargid3622(targid);
        const bool want     = targid >= 0x700;
        ok                  = expect(got == want, "boundary free == pin") && ok;
        ok                  = expect(got == inlineF && got == pin && got == pinPrior, "boundary free == inline == pin == pin3622") && ok;
    }

    // Host-style inject poles: free == inline == pin == pin3622 for IncreaseZoneCounter.
    ...
    for (const uint16 targid : samples)
    {
        ...
        ok                  = expect(got == inlineF && got == pin && got == pinPrior, "host inject free == inline == pin == pin3622") && ok;
