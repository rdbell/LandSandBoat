#include "test_notoriety_add_member_3492.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "notoriety ShouldAddNotorietyMember 3492 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CNotorietyContainer::add admission for dual-wire cross-check
// (dedicated 3492 expand residual 2959; prior dedicated 3327):
//   ownerPresent && entityPresent && differentAllegiance
auto inlineShouldAddNotorietyMember3492(const bool ownerPresent, const bool entityPresent,
                                        const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}

// Compact dual-wire pin matching Go pinShouldAddNotorietyMember3492 / C++ capacity
// direct AND-chain (formula unchanged from 2818 / 2959 / 3165 / 3267 / 3297 / 3327):
//   ownerPresent && entityPresent && differentAllegiance
auto pinShouldAddNotorietyMember3492(const bool ownerPresent, const bool entityPresent,
                                     const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}

// Prior dedicated 3327 pin/inline for independence (formula identical).
auto inlineShouldAddNotorietyMember3327(const bool ownerPresent, const bool entityPresent,
                                        const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}

auto pinShouldAddNotorietyMember3327(const bool ownerPresent, const bool entityPresent,
                                     const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}
...
// OmegaXI internal/notoriety; dedicated slice 3492 expand residual 2959 /
// pure 2818; prior dedicated expand 3327 / 3297 / 3267 / 3165 retained).
//
// Coverage:
//   - free == inline == pin direct AND-chain
//   - residual 2959 / 2818 pins still hold
//   - prior dedicated 3327 independence (free == prior inline == prior pin)
...
auto runNotorietyAddMember3492SelfTests() -> bool
{
    ...
    // Residual 2818 / 2959 / prior dedicated 3327 pins still hold under dedicated dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3327 re-pins.
        { true, true, true, true, "residual 3327 admit owner+entity+diff" },
        { false, true, true, false, "residual 3327 reject nil owner" },
        { true, false, true, false, "residual 3327 reject nil entity" },
        { true, true, false, false, "residual 3327 reject same allegiance" },
        { false, false, false, false, "residual 3327 reject all false" },
    };

    for (const auto& c : cases)
    {
        const bool got     = ShouldAddNotorietyMember(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool inlineF = inlineShouldAddNotorietyMember3492(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool pin     = pinShouldAddNotorietyMember3492(c.ownerPresent, c.entityPresent, c.differentAllegiance);
    ...
    ok = expect(ShouldAddNotorietyMember(true, true, true) == pinShouldAddNotorietyMember3492(true, true, true),
                "free == pin admit all true") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(false, true, true) == pinShouldAddNotorietyMember3492(false, true, true),
                "free == pin nil owner") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(true, false, true) == pinShouldAddNotorietyMember3492(true, false, true),
                "free == pin nil entity") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(true, true, false) == pinShouldAddNotorietyMember3492(true, true, false),
    ...
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            for (const bool differentAllegiance : { false, true })
            {
                ...
                const bool inlineF = inlineShouldAddNotorietyMember3492(ownerPresent, entityPresent, differentAllegiance);
                const bool pin     = pinShouldAddNotorietyMember3492(ownerPresent, entityPresent, differentAllegiance);
    ...
    // Prior dedicated 3327 independence: free still matches prior pin/inline.
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            ...
            {
                const bool got         = ShouldAddNotorietyMember(ownerPresent, entityPresent, differentAllegiance);
                const bool priorInline = inlineShouldAddNotorietyMember3327(ownerPresent, entityPresent, differentAllegiance);
                const bool priorPin    = pinShouldAddNotorietyMember3327(ownerPresent, entityPresent, differentAllegiance);
                ok                     = expect(got == priorPin, "prior 3327 independence free == prior pin") && ok;
                ok                     = expect(got == priorInline, "prior 3327 independence free == prior inline") && ok;
            }
        }
    }
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3492(p.owner, p.entity, p.diff);
        const bool pin     = pinShouldAddNotorietyMember3492(p.owner, p.entity, p.diff);
