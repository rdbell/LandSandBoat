#include "test_status_reject_null_3562.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "status effect ShouldRejectNullStatusEffect 3562 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddStatusEffect null-effect formula for dual-wire cross-check
// (dedicated slice 3562 expand residual 3080; prior dedicated 3526 / 3481 / 3428 / 3348):
//   isNull
...
// Compact dual-wire pin matching Go pinShouldRejectNullStatusEffect3562 /
// C++ capacity identity form (formula unchanged from 1371 / 3080 / 3348 / 3428 / 3481 / 3526):
//   isNull
...
// Prior dedicated 3526 pin (retained) for free==inline==pin==pin3526.
auto pinShouldRejectNullStatusEffect3526(const bool isNull) -> bool
{
    return isNull;
}
...
// (isNull identity; OmegaXI internal/statuseffect; dedicated slice 3562
// expand residual 3080 / prior dedicated 3526 / 3481 / 3428 / 3348 / pure 1371).
//
// Coverage:
//   - free == inline == pin == pin3526 identity form
//   - poles: null rejects; non-null proceeds
//   - residual 3080 / 1371 pins still hold
//   - prior 3348 / 3428 / 3481 / 3526 dedicated poles still hold
...
auto runStatusRejectNull3562SelfTests() -> bool
{
    ...
    // --- Core poles: free == inline == pin == pin3526 identity form ---
    const struct
    {
        bool        isNull;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3526 = pinShouldRejectNullStatusEffect3526(c.isNull);
        ...
        ok = expect(got == pin3526, "dual-wire free==pin3526") && ok;
    ...
    // Free == pin3526 across residual poles (prior dedicated retained).
    ok = expect(ShouldRejectNullStatusEffect(true) == pinShouldRejectNullStatusEffect3526(true),
                "free==pin3526 null effect") &&
         ok;
    ok = expect(ShouldRejectNullStatusEffect(false) == pinShouldRejectNullStatusEffect3526(false),
                "free==pin3526 non-null effect") &&
         ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3526.
    for (const bool isNull : { false, true })
    {
        ...
        const bool pin3526 = pinShouldRejectNullStatusEffect3526(isNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3526, "compose free==pin3526") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3526, "host compose free==pin3526") && ok;
    }
    ...
    // 3049 / 3069 / 3113 / 3135 left alone): null reject is distinct from
    ...
    for (const bool isNull : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3526,
                    "dense free==inline==pin==pin3526") &&
             ok;
        ok = expect(ShouldExpireEffect(true, 100, 100),
