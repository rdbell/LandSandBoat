#include "test_mapsession_destroy_pending_ptr_3542.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapsession ShouldDestroyPendingByPointer 3542 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapSessionContainer::destroyPendingSession(MapSession*) gate for
// dual-wire cross-check (dedicated slice 3542):
...
// Compact dual-wire pin matching Go pinShouldDestroyPendingByPointer3542 /
...
// Prior dedicated slice 3480 inline/pin for independence cross-check.
auto inlineShouldDestroyPendingByPointer3480(const bool found, const bool pointerMatches) -> bool
{
    return found && pointerMatches;
}

auto pinShouldDestroyPendingByPointer3480(const bool found, const bool pointerMatches) -> bool
{
    if (!found)
    {
        return false;
...
// (found && pointerMatches; dedicated slice 3542 expand residual 3056 /
// pure 2787; prior dedicated 3480 / 3426 / 3376 retained).
//
// Coverage:
//   - free == inline == pin == (found && pointerMatches)
//   - residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 pins still hold
//   - full 2² bool poles (found × pointerMatches)
//   - prior dedicated 3480 independence (free == 3480 inline/pin)
...
auto runMapsessionDestroyPendingPtr3542SelfTests() -> bool
{
    ...
    // Residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 pins still hold under dual-wire.
    ok = expect(ShouldDestroyPendingByPointer(true, true), "residual 3480/3426/3376/3056/2787: found match destroys") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(true, false), "residual 3480/3426/3376/3056/2787: found mismatch rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, false), "residual 3480/3426/3376/3056/2787: missing rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, true), "residual 3480/3426/3376/3056/2787: missing ignores match flag") && ok;
    ...
    // Residual independence (2787 / 3056 / 3066 / 3376 / 3426 / 3480): pointer path needs identity;
    // charID path needs presence only. Sibling dual-wires left alone.
    // Prior dedicated 3480 independence: free == prior suite inline/pin forms.
    ...
    for (const bool found : { false, true })
    {
        ...
        {
            const bool got = ShouldDestroyPendingByPointer(found, pointerMatches);
            ok             = expect(got == inlineShouldDestroyPendingByPointer3480(found, pointerMatches),
                        "prior 3480 independence: free == 3480 inline") &&
                 ok;
            ok = expect(got == pinShouldDestroyPendingByPointer3480(found, pointerMatches),
                        "prior 3480 independence: free == 3480 pin") &&
                 ok;
        }
    }
