#include "test_garrison_fail_by_death_3650.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3650 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// prior dedicated expand 3560 / prior dedicated expand 3605 / dedicated 3650):
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3650:
//   allNPCsDead || allPlayersDead
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3650;
...
//
// Coverage:
//   - free == inline == pin == pin3605 (direct return positive OR; no De Morgan)
//   - 4-row truth table + residual poles + dense 2² free == inline == pin == pin3605
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
//     prior dedicated expand 3330 / prior dedicated expand 3406 /
//     prior dedicated expand 3461 / prior dedicated expand 3514 /
//     prior dedicated expand 3560 / prior dedicated expand 3605 pins still hold
...
auto runGarrisonFailByDeath3650SelfTests() -> bool
{
    ...
    // prior dedicated expand 3330 / prior dedicated expand 3406 /
    // prior dedicated expand 3461 / prior dedicated expand 3514 /
    // prior dedicated expand 3560 / prior dedicated expand 3605 pins.
    ...
    // 4-row truth table (+ residual re-pins): free == inline == pin == pin3605.
    const struct
    {
        bool        allNPCsDead;
        bool        allPlayersDead;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3605 = pinShouldFailByDeath3605(c.allNPCsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3605, "ShouldFailByDeath dual-wire == pin3605") && ok;
    ...
    // Free == pin == pin3605 across residual poles (4-row truth table).
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath(false, false) &&
                    ShouldFailByDeath(false, false) == pinShouldFailByDeath3605(false, false),
                "free == pin/pin3605 neither dead") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath(true, false) &&
                    ShouldFailByDeath(true, false) == pinShouldFailByDeath3605(true, false),
                "free == pin/pin3605 npcs dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath(false, true) &&
                    ShouldFailByDeath(false, true) == pinShouldFailByDeath3605(false, true),
                "free == pin/pin3605 players dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath(true, true) &&
                    ShouldFailByDeath(true, true) == pinShouldFailByDeath3605(true, true),
                "free == pin/pin3605 both dead") &&
         ok;

    // Dense compose: full 2^2 boolean space free == inline == pin == pin3605.
    for (const bool allNPCsDead : { false, true })
    {
        for (const bool allPlayersDead : { false, true })
        {
            ...
            ok = expect(got == pinShouldFailByDeath(allNPCsDead, allPlayersDead),
                        "compose free == pin") &&
                 ok;
            ok = expect(got == pinShouldFailByDeath3605(allNPCsDead, allPlayersDead),
                        "compose free == pin3605") &&
                 ok;
        }
    }
    ...
    // prior dedicated expand 3330 / prior dedicated expand 3406 /
    // prior dedicated expand 3461 / prior dedicated expand 3514 /
    // prior dedicated expand 3560 / prior dedicated expand 3605 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236/3330/3406/3461/3514/3560/3605 wipe gate pins under 3650") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3605(false, false),
                "prior 3605 pin neither dead still holds under 3650") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3605(true, false),
                "prior 3605 pin npcs dead still holds under 3650") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3605(false, true),
                "prior 3605 pin players dead still holds under 3650") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3605(true, true),
                "prior 3605 pin both dead still holds under 3650") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3650.
    ...
    // Explicit residual compose dual-wire free == inline == pin == pin3605 for host poles.
    const struct
    {
        int32       npcCount;
        int32       deadNPCCount;
    ...
    for (const auto& c : composeCases)
    {
        ...
        const bool pin3605   = pinShouldFailByDeath3605(npcsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3605, "compose dual-wire free == pin3605") && ok;
    }
