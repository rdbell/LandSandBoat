#include "test_follow_can_follow_3827.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3827 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3827 dedicated expand residual 2883; prior dedicated 3782 / 3737 /
// 3692 / 3647 / 3602 / 3557 / 3506 / 3459 / 3405 / 3331 / 3237 / 3084):
...
// Compact dual-wire pin matching free function / capacity body (slice 3827).
...
// Prior dedicated 3782 pin retained for free == pin3782 dual-wire agreement.
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3827 dedicated
// expand residual 2883; prior dedicated 3782 / 3737 / 3692 / 3647 / 3602 /
// 3557 / 3506 / 3459 / 3405 / 3331 / 3237 / 3084). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3782 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 / 3692 / 3737 / 3782 poles still hold
...
auto runFollowCanFollow3827SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 / 3692 / 3737 / 3782 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3827(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3782 = pinCanFollow3782(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3827(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3782 = pinCanFollow3782(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3782, "dense free == pin3782") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3782 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3782, "host inject dual-wire free == pin3782") && ok;
    }
