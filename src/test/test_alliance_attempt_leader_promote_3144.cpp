#include "test_alliance_attempt_leader_promote_3144.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3144 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check (slice 3144):
//   isMainParty
...
auto runAllianceAttemptLeaderPromote3144SelfTests() -> bool
{
    ...
    // Residual 1341 / prior dual-wire 2979 pins still hold under dual-wire.
