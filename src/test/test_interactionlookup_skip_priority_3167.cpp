#include "test_interactionlookup_skip_priority_3167.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "interactionlookup ShouldSkipPrioritySelection 3167 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline interaction_lookup.lua getHighestPriorityActions early-return gate
// for dual-wire cross-check (dedicated 3167):
//   empty || firstIsNumber
...
// Compact dual-wire pin matching Go pinShouldSkipPrioritySelection3167 /
// C++ capacity (formula unchanged from 1094 / 2953):
//   empty || firstIsNumber
...
// OmegaXI internal/interactionlookup; dedicated slice 3167; residual expand
// 2953 / pure 1094).
//
// Coverage:
//   - free == inline == pin positive form (empty || firstIsNumber)
//   - poles: empty / firstIsNumber / both / neither (full 2^2)
//   - residual 2953 / 1094 pins still hold
...
auto runInteractionlookupSkipPriority3167SelfTests() -> bool
{
    ...
    // Residual 1094 / 2953 pins still hold under dedicated dual-wire.
    ...
    // Residual 1094 / 2953 poles still hold under dual-wire.
