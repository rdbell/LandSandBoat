#include "test_ranged_delete_unlimited_shot_3890.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranged ShouldDeleteUnlimitedShot 3890 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack UnlimitedShot delete gate for dual-wire cross-check
// (slice 3890 dedicated expand residual 3000; prior 3845 retained):
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto inlineShouldDeleteUnlimitedShot3890(
...
// (dedicated slice 3890). Early-return form only — same formula.
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto pinShouldDeleteUnlimitedShot3890(
    const bool hasUnlimitedShot,
    const bool hitOccured,
    const int16 retainUnlimitedShotMod) -> bool
{
...
// Prior 3845 pin retained for independence cross-check (same formula).
// free == inline == pin == pin3845.
// Local test pin only — no C++ capacity pin redefs.
auto pinShouldDeleteUnlimitedShot3845(
    const bool hasUnlimitedShot,
    const bool hitOccured,
    const int16 retainUnlimitedShotMod) -> bool
{
...
// slice 3890 dedicated expand residual 3000; prior dedicated 3845 retained).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3845 (early-return formula)
//   - residual poles: no US; hit always delete; miss retain <=0 / >0; residual 1390/3000/3399/3451/3500/3575/3620/3665/3710/3755/3800/3845
//   - dense hasUS × hit × retain compose
//   - prior 3845 independence (suite retained)
...
//   - prior dedicated dual-wire 3800 suite retained (test_ranged_delete_unlimited_shot_3800)
//   - prior dedicated dual-wire 3845 suite retained (test_ranged_delete_unlimited_shot_3845)
...
auto runRangedDeleteUnlimitedShot3890SelfTests() -> bool
{
    ...
    // Residual 1390 / residual dual-wire 3000 / dedicated 3399 / 3451 / 3500 / 3575 / 3620 / 3665 / 3710 / 3755 / 3800 / 3845 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1390 / 3000 / 3399 / 3451 / 3500 / 3575 / 3620 / 3665 / 3710 / 3755 / 3800 / 3845 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDeleteUnlimitedShot3890(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pinF    = pinShouldDeleteUnlimitedShot3890(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pin3845 = pinShouldDeleteUnlimitedShot3845(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
    ...
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3890(false, true, 0),
                "free == pin residual no US") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3890(true, true, 5),
                "free == pin residual hit delete") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3890(true, false, 1),
                "free == pin residual retain keep") &&
         ok;

    // Prior 3845 independence: free == prior 3845 pin poles (suite retained).
    // free == inline == pin == pin3845.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3845(true, true, 5),
                "prior 3845 independence: free == 3845 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3845(false, true, 0),
                "prior 3845 independence: free == 3845 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3845(true, false, 1),
                "prior 3845 independence: free == 3845 pin retain keep") &&
         ok;
    ok = expect(pinShouldDeleteUnlimitedShot3890(true, false, 0) == pinShouldDeleteUnlimitedShot3845(true, false, 0),
                "prior 3845 independence: 3890 pin == 3845 pin miss retain 0") &&
         ok;
    ok = expect(inlineShouldDeleteUnlimitedShot3890(true, true, 5) == pinShouldDeleteUnlimitedShot3845(true, true, 5),
                "prior 3845 independence: 3890 inline == 3845 pin del hit") &&
         ok;

    // Dense compose: hasUnlimitedShot × hitOccured × retain poles — free == inline == pin == pin3845.
    for (const bool hasUS : { false, true })
    {
        for (const bool hit : { false, true })
        {
                                        ...
                                        static_cast<int16>(1), static_cast<int16>(5), static_cast<int16>(100) })
            {
                ...
                ok              = expect(got == inlineShouldDeleteUnlimitedShot3890(hasUS, hit, retain),
                            "compose free == inline") &&
                     ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3890(hasUS, hit, retain), "compose free == pin") && ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3845(hasUS, hit, retain), "compose free == pin3845") && ok;
            }
        }
    }
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pin3845, "compose free == pin3845") && ok;
    }
    ...
    // Prior 3845 independence under compose: free == 3845 pin residual poles.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3845(true, true, 5),
                "prior 3845 independence compose: free == 3845 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3845(false, true, 0),
                "prior 3845 independence compose: free == 3845 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3845(true, false, 1),
                "prior 3845 independence compose: free == 3845 pin retain keep") &&
         ok;
