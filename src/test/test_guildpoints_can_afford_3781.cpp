#include "test_guildpoints_can_afford_3781.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "guildpoints CanAfford 3781 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua guild_points getCurrency gate for dual-wire cross-check
// (dedicated 3781 expand residual 2944; prior 3736 / 3691 / 3646 / 3601 / 3556 / 3507 / 3458 / 3404 / 3368 / 3221):
//   currency >= cost
...
// Compact dual-wire pin matching Go pinCanAfford3781 / C++ capacity:
//   currency >= cost
...
// Prior dedicated 3736 pin (positive form) for free == pin3736 cross-check:
//   currency >= cost
auto pinCanAfford3736(const int32 currency, const int32 cost) -> bool
{
    return currency >= cost;
}
...
// internal/guildpoints; dedicated slice 3781 expand residual 2944;
// prior dedicated 3736 / 3691 / 3646 / 3601 / 3556 / 3507 / 3458 / 3404 / 3368 / 3221 retained).
//
// Coverage:
//   - free == inline == pin == pin3736 == (currency >= cost)
//   - residual 2944 / 3736 / 3691 / 3646 / 3601 / 3556 / 3507 / 3458 / 3404 / 3368 / 3221 / 1016 pins still hold
...
auto runGuildpointsCanAfford3781SelfTests() -> bool
{
    ...
    // Residual 2944 / 3736 / 3691 / 3646 / 3601 / 3556 / 3507 / 3458 / 3404 / 3368 / 3221 / 1016 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3736 ---
    const struct
    {
        int32       currency;
        int32       cost;
        ...
        // Residual 2944 / 3736 / 3691 / 3646 / 3601 / 3556 / 3507 / 3458 / 3404 / 3368 / 3221 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3736 = pinCanAfford3736(c.currency, c.cost);
        const bool wantPin = c.currency >= c.cost;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3736.
        ok = expect(got == inlineF && got == pin && got == pin3736, "dual-wire free == inline == pin == pin3736") && ok;
        ok = expect(got == wantPin, "free == pin formula currency >= cost") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3736.
    const struct
    {
        int32       currency;
        int32       cost;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin3736 = pinCanAfford3736(p.currency, p.cost);
        const bool want    = p.currency >= p.cost;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3736, "pole free == inline == pin == pin3736") && ok;
    }

    // Dense compose free == inline == pin == pin3736 over required pairs + neighbors.
    const int32 dense[][2] = {
        { 0, 0 },
        { 0, 1 },
        { 1, 0 },
    ...
    for (const auto& p : dense)
    {
        ...
        const bool  pin3736  = pinCanAfford3736(currency, cost);
        const bool  want     = currency >= cost;
        ok                   = expect(got == want, "dense free == pin formula") && ok;
        ok                   = expect(got == inlineF && got == pin && got == pin3736, "dense free == inline == pin == pin3736") && ok;
    }
    ...
    for (const auto& p : inject)
    {
        ...
        ok                   = expect(got == inlineF && got == pin && got == pin3736, "inject free == inline == pin == pin3736") && ok;
    }
