#include "test_colonizationreive_spawn_on_enable_3593.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "colonizationreive ShouldSpawnOnEnable 3593 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua enableReive spawn gate for dual-wire cross-check
// (dedicated 3593 expand residual 2882 / prior 3499 / prior 3465 / prior 3411 /
...
// Compact dual-wire pin matching Go pinShouldSpawnOnEnable3593 / C++ capacity
// (return !isAlive):
//   !isAlive
...
// Prior dedicated 3499 pin (independence / free==inline==pin==pin3499):
//   !isAlive
...
// OmegaXI internal/colonizationreive; dedicated slice 3593 expand residual
// 2882 / prior dedicated 3499 / prior dedicated 3465 / prior dedicated 3411 /
// prior dedicated 3335 / prior dedicated 3163 — formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3499 == !isAlive
//   - residual poles: isAlive true / false
//   - dense 2^1 boolean space
//   - residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 pins still hold
...
auto runColonizationreiveSpawnOnEnable3593SelfTests() -> bool
{
    ...
    // Residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3499 = pinShouldSpawnOnEnable3499(c.isAlive);
        ...
        ok = expect(got == pin3499, "ShouldSpawnOnEnable dual-wire == pin3499") && ok;
    ...
    ok = expect(ShouldSpawnOnEnable(false) == pinShouldSpawnOnEnable3499(false), "free==pin3499 false") && ok;
    ok = expect(ShouldSpawnOnEnable(true) == pinShouldSpawnOnEnable3499(true), "free==pin3499 true") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3499 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3499 = pinShouldSpawnOnEnable3499(pole);
        ok                 = expect(got == !pole, "pole free == !isAlive") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3499, "pole free == pin3499") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3499.
    for (const bool isAlive : { false, true })
    {
        ...
        ok              = expect(got == pinShouldSpawnOnEnable3499(isAlive), "compose free == pin3499") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable(isAlive),
             ...
             ok;
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable3499(isAlive),
                    "host inject free == pin3499") &&
             ok;
