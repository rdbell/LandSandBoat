#include "test_mobutils_can_drop_gil_3309.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mobutils CanDropGil 3309 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CMobEntity::CanDropGil formula for dual-wire cross-check (slice 3309):
...
// Compact dual-wire pin matching free function / capacity body (slice 3309).
// Direct return only — same formula as production CanDropGil.
auto pinCanDropGil3309(const int16 gilMin, const int16 gilMax, const int16 gilBonus) -> bool
{
    return gilMax >= 0 && (gilMin > 0 || gilMax != 0 || gilBonus > 0);
}
...
// (gilMin/gilMax/gilBonus eligibility gate; slice 3309 dedicated expand
// residual 2960). CanStealGil dual-wires through the same free function.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2960 / 2653 / prior dedicated 3158 / prior expand 3231 / 3279 poles
...
auto runMobutilsCanDropGil3309SelfTests() -> bool
{
    ...
    // prior dedicated expand residual 2960 3231 / 3279 pins.
    ...
    const struct
    {
        ...
        // Residual 2960 / 2653 / prior 3158 / prior expand 3231 / 3279 re-pins.
        { 0, 0, 0, false, "residual 2960/2653/3158/3231/3279 all zero" },
        { 1, 0, 0, true, "residual 2960/2653/3158/3231/3279 positive min" },
        { 0, 1, 0, true, "residual 2960/2653/3158/3231/3279 positive max" },
        { 0, 0, 1, true, "residual 2960/2653/3158/3231/3279 positive bonus" },
        { 1, -1, 1, false, "residual 2960/2653/3158/3231/3279 neg max suppress" },
        { 0, 0, -1, false, "residual 2960/2653/3158/3231/3279 neg bonus" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinCanDropGil3309(c.gilMin, c.gilMax, c.gilBonus);
    ...
    ok = expect(CanDropGil(1, 0, 0) == pinCanDropGil3309(1, 0, 0), "free == pin residual min only") && ok;
    ok = expect(CanDropGil(0, 1, 0) == pinCanDropGil3309(0, 1, 0), "free == pin residual max only") && ok;
    ok = expect(CanDropGil(0, 0, 1) == pinCanDropGil3309(0, 0, 1), "free == pin residual bonus only") && ok;
    ok = expect(CanDropGil(0, 0, 0) == pinCanDropGil3309(0, 0, 0), "free == pin residual zero all") && ok;
    ok = expect(CanDropGil(1, -1, 1) == pinCanDropGil3309(1, -1, 1),
    ...
    for (const int16 gilMin : poles)
    {
        for (const int16 gilMax : poles)
        {
            for (const int16 gilBonus : poles)
            {
                ...
                const bool pin = pinCanDropGil3309(gilMin, gilMax, gilBonus);
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin = pinCanDropGil3309(h.gilMin, h.gilMax, h.gilBonus);
