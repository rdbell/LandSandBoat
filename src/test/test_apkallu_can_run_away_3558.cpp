#include "test_apkallu_can_run_away_3558.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "apkallu can run away 3558 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanRunAway3558:
...
// dedicated slice 3558 expand residual 2880 / prior dedicated ~3505 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3505 == GetHateTier>=3 == hate>=45
//   - residual poles across hate tiers 0..3+
//   - dense hate range compose
//   - residual 0925 / 2880 / prior 3149 / prior 3245 / prior 3287 / prior 3317 / prior 3407 / prior 3460 / prior 3505 pins still hold
auto runApkalluCanRunAway3558SelfTests() -> bool
{
    ...
    // prior 3407 / prior 3460 / prior 3505 CanRunAway pins still hold under dual-wire
    ...
    // --- Composition table: free == inline == pin == pin3505 == compose ---
    ...
    for (const auto& c : runCases)
    {
        ...
        ok = expect(got == pin3505, "CanRunAway dual-wire free == pin3505") && ok;
    ...
    // Free == pin3505 across residual poles (positive pin form).
    ok = expect(CanRunAway(45) == pinCanRunAway3505(45), "free == pin3505 residual 45") && ok;
    ok = expect(CanRunAway(100) == pinCanRunAway3505(100), "free == pin3505 residual 100") && ok;
    ok = expect(CanRunAway(44) == pinCanRunAway3505(44), "free == pin3505 residual 44") && ok;
    ok = expect(CanRunAway(0) == pinCanRunAway3505(0), "free == pin3505 hate 0") && ok;

    // Dense compose: free CanRunAway matches GetHateTier(hate) >= 3, inline, pin, pin3505, and hate >= 45.
    for (int32 hate = -20; hate <= 120; ++hate)
    {
        ...
        ok = expect(got == pinCanRunAway3505(hate), "dense free == pin3505") && ok;
        ok = expect(got == (hate >= kHateTier3Min), "dense free == hate>=45") && ok;
    }

    // Prior dedicated 3149 / 3245 / 3287 / 3317 / 3407 / 3460 / 3505 independence:
