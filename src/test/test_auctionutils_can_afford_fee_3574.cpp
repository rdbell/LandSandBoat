#include "test_auctionutils_can_afford_fee_3574.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "auctionutils CanAffordFee 3574 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline production ProofOfPurchase fee formula for dual-wire cross-checks
// (dedicated slice 3574; residual expand 2924; prior dedicated 3503 / 3310 / 3280):
...
// (slice 3574). Multi-branch: positive if/else only (avoid QF1001 De Morgan
...
// Prior dedicated 3503 pin (positive multi-branch if/else) for
// free == pin3503 cross-check.
auto pinCanAffordFee3503(const uint32 gilQuantity, const uint32 gilReserve, const uint32 fee) -> bool
{
    if (gilQuantity < fee)
    {
        return false;
...
// dedicated slice 3574 expand residual 2924 — formula unchanged;
// prior dedicated ~3503 / 3310 / 3280).
//
// Coverage:
//   - free == inline == pin == pin3503 (multi-branch positive if/else)
//   - residual poles: exact, short, surplus, zero fee, reserve block
//   - residual 1135 / 2924 / prior dedicated 3280 / 3310 / 3503 pins still hold
...
auto runAuctionutilsCanAffordFee3574SelfTests() -> bool
{
    ...
    // Residual 1135 / 2924 / prior dedicated 3280 / 3310 / 3503 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3503 (multi-branch if/else) ---
    ...
    const struct
    {
        ...
        // residual 2924 / prior dedicated 3280 / 3310 / 3503 re-pins
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3503 = pinCanAffordFee3503(c.gilQuantity, c.gilReserve, c.fee);

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3503.
        ok = expect(got == inlineC && got == pinGot && got == pin3503, "dual-wire free==inline==pin==pin3503") && ok;
    }
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pin3503 = pinCanAffordFee3503(pole.gilQuantity, pole.gilReserve, pole.fee);
        ok                 = expect(got == inlineC && got == pinGot && got == pin3503, "pole free==inline==pin==pin3503") && ok;
    }

    // Dense compose: free == inline == pin == pin3503 over required poles + neighbors.
    const struct
    {
        uint32 gilQuantity;
        uint32 gilReserve;
    ...
    for (const auto& p : dense)
    {
        ...
        const bool pin3503 = pinCanAffordFee3503(p.gilQuantity, p.gilReserve, p.fee);
        ...
        ok                 = expect(got == pin3503, "dense free == pin3503") && ok;
    }
