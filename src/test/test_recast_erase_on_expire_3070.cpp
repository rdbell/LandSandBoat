#include "test_recast_erase_on_expire_3070.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "recast ShouldEraseOnExpire 3070 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Check erase-vs-retain gate for dual-wire cross-check (slice 3070):
//   !isAbility
...
// (magic erase vs ability zero-retain Check gate; slice 3070).
auto runRecastEraseOnExpire3070SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // Residual 1370 pins.
        { false, true, "residual 1370 erase non-ability" },
        { true, false, "residual 1370 retain ability" },
    };
