#include "test_world_campaign_event_1283.h"

#include "world/campaign_event.h"
...
auto expect(const bool condition, const char* label) -> bool
{
    if (!condition)
    {
        std::cerr << "world CampaignEvent 1283 self-test failed: " << label << '\n';
    }
    return condition;
}
...
auto testDispatch() -> bool
{
    ...
    const ipc::CampaignEvent message{
        .type = static_cast<CampaignMessage>(std::numeric_limits<uint8>::max()),
        .payload = { 0x00, 0x7F, 0x80, 0xFF },
    };
    int calls{};
    CampaignMessage deliveredType{};
    IPP deliveredSource{};
    std::vector<uint8> deliveredPayload{};
    worldipc::HandleCampaignEvent(
        source,
        message,
        [&](CampaignMessage type, IPPMessage&& delivered)
        {
            ++calls;
            deliveredType = type;
