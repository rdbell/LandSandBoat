#include "test_enmity_zero_out_of_range_3724.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "enmity ShouldZeroEnmityOutOfRange 3724 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline UpdateEnmity out-of-range CE/VE zero gate for dual-wire cross-check
// (slice 3724 dedicated expand residual 2927; prior dedicated expand 3679 /
// 3634 / 3589 / 3545 / 3485 / 3427 / 3333 / prior dedicated 3157):
//   !withinRange
// Direct return form matching production free function / capacity.
auto inlineShouldZeroEnmityOutOfRange3724(const bool withinRange) -> bool
{
    return !withinRange;
}

// Compact dual-wire pin matching free function / capacity body (slice 3724).
...
// Positive form: free == inline == pin == pin3679.
...
//
// Coverage:
//   - free == inline == pin == pin3679 (direct return !withinRange identity-not)
...
//     prior expand 3589 / prior expand 3634 / prior expand 3679 pins still hold
auto runEnmityZeroOutOfRange3724SelfTests() -> bool
{
    ...
    // Residual poles: free == inline == pin == pin3679 (direct return).
    const struct
    {
        bool        withinRange;
        bool        want;
        ...
        // Prior dedicated expand 3679 poles (independence).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3724(p.withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3724(p.withinRange);
    ...
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3724(false), "free==pin false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3724(true), "free==pin true") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3679(false), "free==pin3679 false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3679(true), "free==pin3679 true") && ok;

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3679.
    for (const bool withinRange : { false, true })
    {
        const bool got     = ShouldZeroEnmityOutOfRange(withinRange);
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3724(withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3724(withinRange);
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok                = expect(got == inlineShouldZeroEnmityOutOfRange3724(within), "compose free == inline") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3724(within), "compose free == pin") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3679(within), "compose free == pin3679") && ok;
    }

    // Residual 2927 / prior dedicated expand 3679 / 3634 / 3589 / 3545 / 3485 / 3427 / 3333 / prior dedicated 3157 /
    // pure 1357 still hold under expand suite.
    ok = expect(ShouldZeroEnmityOutOfRange(false) && !ShouldZeroEnmityOutOfRange(true),
                "residual 2927/3679/3634/3589/3545/3485/3427/3333/3157/1357 pins failed under 3724 suite") &&
         ok;
