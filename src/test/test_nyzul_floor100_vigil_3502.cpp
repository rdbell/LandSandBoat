#include "test_nyzul_floor100_vigil_3502.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "nyzul floor100 vigil 3502 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua vigilWeaponDrop floor-100 gate for dual-wire cross-check
// (slice 3502 dedicated expand residual 2900; prior dedicated 3352 / 3449):
...
// Compact dual-wire pin matching free function / capacity body (slice 3502).
...
// Prior dedicated 3449 pin (retained cross-check under 3502).
auto pinShouldDropFloor100VigilWeapons3449(const int32 currentFloor) -> bool
{
    return currentFloor == nyzulhelpers::Floor100;
}

// Prior dedicated 3352 pin (retained cross-check under 3502).
...
// (Lua vigilWeaponDrop floor-100 guaranteed-drop gate; slice 3502 dedicated
// expand residual 2900; prior dedicated 3352 / 3449). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2900 / 1088 poles still hold (100 grants; non-100 misses)
//   - prior dedicated 3352 / 3449 pins still hold
...
auto runNyzulFloor100Vigil3502SelfTests() -> bool
{
    ...
    // Residual 2900 / 1088 / prior dedicated 3352 / 3449 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin      = pinShouldDropFloor100VigilWeapons3502(p.floor);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3502(e.floor);
    ...
    for (int32 floor = -2; floor <= Floor100 + 5; ++floor)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3502(floor);
    ...
    // Residual 2900 / prior dedicated 3352 / 3449 still hold under dedicated 3502.
    ok = expect(ShouldDropFloor100VigilWeapons(100) && !ShouldDropFloor100VigilWeapons(99),
                "residual 2900 / prior 3352 / 3449 pins under 3502") &&
         ok;
