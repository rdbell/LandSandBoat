#include "test_status_reject_null_3787.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "status effect ShouldRejectNullStatusEffect 3787 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddStatusEffect null-effect formula for dual-wire cross-check
// (dedicated slice 3787 expand residual 3080; prior dedicated 3742 / 3697 / 3652 / 3607 / 3562 / 3526 / 3481 / 3428 / 3348):
//   isNull
...
// Compact dual-wire pin matching Go pinShouldRejectNullStatusEffect3787 /
// C++ capacity identity form (formula unchanged from 1371 / 3080 / 3348 / 3428 / 3481 / 3526 / 3562 / 3607 / 3652 / 3697 / 3742):
//   isNull
...
// Prior dedicated 3742 pin (retained) for free==inline==pin==pin3742.
auto pinShouldRejectNullStatusEffect3742(const bool isNull) -> bool
{
    return isNull;
}
...
// (isNull identity; OmegaXI internal/statuseffect; dedicated slice 3787
// expand residual 3080 / prior dedicated 3742 / 3697 / 3652 / 3607 / 3562 / 3526 / 3481 / 3428 / 3348 / pure 1371).
//
// Coverage:
//   - free == inline == pin == pin3742 identity form
//   - poles: null rejects; non-null proceeds
//   - residual 3080 / 1371 pins still hold
//   - prior 3348 / 3428 / 3481 / 3526 / 3562 / 3607 / 3652 / 3697 / 3742 dedicated poles still hold
...
auto runStatusRejectNull3787SelfTests() -> bool
{
    ...
    // --- Core poles: free == inline == pin == pin3742 identity form ---
    const struct
    {
        bool        isNull;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3742 = pinShouldRejectNullStatusEffect3742(c.isNull);
        ...
        ok = expect(got == pin3742, "dual-wire free==pin3742") && ok;
    ...
    // Free == pin3742 across residual poles (prior dedicated retained).
    ok = expect(ShouldRejectNullStatusEffect(true) == pinShouldRejectNullStatusEffect3742(true),
                "free==pin3742 null effect") &&
         ok;
    ok = expect(ShouldRejectNullStatusEffect(false) == pinShouldRejectNullStatusEffect3742(false),
                "free==pin3742 non-null effect") &&
         ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3742.
    for (const bool isNull : { false, true })
    {
        ...
        const bool pin3742 = pinShouldRejectNullStatusEffect3742(isNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3742, "compose free==pin3742") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3742, "host compose free==pin3742") && ok;
    }
    ...
    // 3562 / 3607 / 3652 / 3697 / 3742; 3049 / 3069 / 3113 / 3135 left alone): null reject is distinct from
    ...
    for (const bool isNull : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3742,
                    "dense free==inline==pin==pin3742") &&
             ok;
        ok = expect(ShouldExpireEffect(true, 100, 100),
