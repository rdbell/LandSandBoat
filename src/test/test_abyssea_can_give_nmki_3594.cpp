#include "test_abyssea_can_give_nmki_3594.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "abyssea CanGiveNMKI 3594 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.abyssea.canGiveNMKI formula for dual-wire checks (slice 3594).
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3594 / C++ capacity
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3549 (prior dedicated
// expand residual 2861 independence pins).
auto pinCanGiveNMKI3549(const int32 roll1to100, const int32 dropChance, const bool redProc) -> bool
{
    return roll1to100 <= dropChance || redProc;
}
...
// dedicated slice 3594 expand residual 2861 / prior 3549 / 3491 / 3434 /
// 3314 / 3284 / 3238 / 3089 — formula unchanged).
//
// Coverage (clones 3549 density on CanGiveNMKI primary + prior 3549 independence):
//   - free == inline == pin == pin3549 == (roll <= chance || redProc)  // positive OR
...
//   - residual 1041 / 2861 / prior 3089 / prior 3238 / 3284 / 3314 / 3434 / 3491 / 3549 pins still hold
...
auto runAbysseaCanGiveNMKI3594SelfTests() -> bool
{
    ...
    // Residual constant / 1041 / 2861 / 3089 / 3238 / 3284 / 3314 / 3434 / 3491 / 3549 pins still hold.
    ...
    // --- Composition table: free == inline == pin == pin3549 (positive OR form) ---
    const struct
    {
        int32       roll;
        int32       chance;
        ...
        // Residual 2861 / 3089 / 3434 / 3491 / 3549 poles.
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pin3549   = pinCanGiveNMKI3549(pole.roll, pole.chance, pole.redProc);
        const bool want      = pole.roll <= pole.chance || pole.redProc;
        ok                   = expect(got == want, pole.label) && ok;
        ok                   = expect(got == inlineGot && got == pinGot && got == pin3549,
                                      "pole free == inline == pin == pin3549") &&
             ok;
    }

    // Dense rolls 1..100 edges for normal + atma chances: free == inline == pin == pin3549.
    for (const int32 chance : { NormalNMKIDropChance, AtmaNMKIDropChance })
    {
        for (int32 roll = 1; roll <= 100; ++roll)
        {
            for (const bool red : { false, true })
            {
                ...
                const bool pin3549   = pinCanGiveNMKI3549(roll, chance, red);
                const bool want      = roll <= chance || red;
                ok                   = expect(got == want, "dense free == formula") && ok;
                ok                   = expect(got == inlineGot && got == pinGot && got == pin3549,
                                              "dense free == inline == pin == pin3549") &&
                     ok;
            }
        }
    }
    ...
    for (const int32 chance : { NormalNMKIDropChance, AtmaNMKIDropChance, 0, 50, 100 })
    {
        for (const int32 roll : { 0, -1, 101, 200 })
        {
            for (const bool red : { false, true })
            {
                ...
                const bool pin3549   = pinCanGiveNMKI3549(roll, chance, red);
                const bool want      = roll <= chance || red;
                ok                   = expect(got == want && got == inlineGot && got == pinGot && got == pin3549,
                                              "edge free == inline == pin == pin3549 == formula") &&
                     ok;
            }
        }
    }
    ...
    // Prior dedicated expand residual 3549 pin helpers still agree with free dual-wire.
    ok = expect(CanGiveNMKI(20, 20, false) == pinCanGiveNMKI3549(20, 20, false),
                "free == prior 3549 pin boundary") &&
         ok;
    ok = expect(CanGiveNMKI(21, 20, false) == pinCanGiveNMKI3549(21, 20, false),
                "free == prior 3549 pin short") &&
         ok;
    ok = expect(CanGiveNMKI(100, 0, true) == pinCanGiveNMKI3549(100, 0, true),
                "free == prior 3549 pin red-proc force") &&
         ok;
    ok = expect(pinCanGiveNMKI(20, 20, false) == pinCanGiveNMKI3549(20, 20, false),
                "3594 pin == prior 3549 pin boundary") &&
         ok;
    ok = expect(pinCanGiveNMKI(21, 20, false) == pinCanGiveNMKI3549(21, 20, false),
                "3594 pin == prior 3549 pin short") &&
         ok;
    ok = expect(pinCanGiveNMKI(100, 0, true) == pinCanGiveNMKI3549(100, 0, true),
                "3594 pin == prior 3549 pin red-proc force") &&
         ok;
