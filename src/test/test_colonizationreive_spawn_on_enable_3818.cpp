#include "test_colonizationreive_spawn_on_enable_3818.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "colonizationreive ShouldSpawnOnEnable 3818 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua enableReive spawn gate for dual-wire cross-check
// (dedicated 3818 expand residual 2882 / prior 3773 / prior 3728 / prior 3683 /
...
// Compact dual-wire pin matching Go pinShouldSpawnOnEnable3818 / C++ capacity
// (return !isAlive):
//   !isAlive
...
// Prior dedicated 3773 pin (independence / free==inline==pin==pin3773):
//   !isAlive
...
// OmegaXI internal/colonizationreive; dedicated slice 3818 expand residual
// 2882 / prior dedicated 3773 / prior dedicated 3728 / prior dedicated 3683 /
...
// prior dedicated 3163 — formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3773 == !isAlive
//   - residual poles: isAlive true / false
//   - dense 2^1 boolean space
//   - residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 / prior 3638 / prior 3683 / prior 3728 / prior 3773 pins still hold
...
auto runColonizationreiveSpawnOnEnable3818SelfTests() -> bool
{
    ...
    // Residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 / prior 3638 / prior 3683 / prior 3728 / prior 3773 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3773 = pinShouldSpawnOnEnable3773(c.isAlive);
        ...
        ok = expect(got == pin3773, "ShouldSpawnOnEnable dual-wire == pin3773") && ok;
    ...
    ok = expect(ShouldSpawnOnEnable(false) == pinShouldSpawnOnEnable3773(false), "free==pin3773 false") && ok;
    ok = expect(ShouldSpawnOnEnable(true) == pinShouldSpawnOnEnable3773(true), "free==pin3773 true") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3773 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3773 = pinShouldSpawnOnEnable3773(pole);
        ok                 = expect(got == !pole, "pole free == !isAlive") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3773, "pole free == pin3773") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3773.
    for (const bool isAlive : { false, true })
    {
        ...
        ok              = expect(got == pinShouldSpawnOnEnable3773(isAlive), "compose free == pin3773") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable(isAlive),
             ...
             ok;
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable3773(isAlive),
                    "host inject free == pin3773") &&
             ok;
