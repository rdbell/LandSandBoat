#include "test_garrison_fail_by_death_3236.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3236 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua BATTLE fail-by-death formula for dual-wire checks
// (residual 2870 / prior dedicated 3073 / dedicated 3236):
//   allNPCsDead || allPlayersDead
// Direct return of the expression (not if/else).
auto inlineShouldFailByDeath(const bool allNPCsDead, const bool allPlayersDead) -> bool
{
    return allNPCsDead || allPlayersDead;
}

// Compact dual-wire pin matching Go pinShouldFailByDeath3236:
//   allNPCsDead || allPlayersDead
// Direct return OR (not if/else).
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3236;
// expand residual 2870 / pure residual suite; prior dedicated 3073).
//
// Coverage:
//   - free == inline == pin (direct return OR)
//   - residual poles + dense 2² free == inline == pin
//   - residual 2870 / prior dedicated 3073 pins still hold
...
auto runGarrisonFailByDeath3236SelfTests() -> bool
{
    ...
    // Residual 2870 / prior dedicated 3073 pins still hold under dual-wire.
    ...
    // Residual 2870 / prior dedicated 3073 still hold under dedicated suite.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073 wipe gate pins under 3236") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3073(false, false),
                "prior 3073 pin neither dead still holds under 3236") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3073(true, false),
                "prior 3073 pin npcs dead still holds under 3236") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3073(false, true),
                "prior 3073 pin players dead still holds under 3236") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3073(true, true),
                "prior 3073 pin both dead still holds under 3236") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3236.
