#include "test_item_increment_count_insert_3535.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "itemcontainer ShouldIncrementCountOnInsertAt 3535 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertItem count-bump gate for dual-wire cross-check (dedicated 3535):
//   slotEmpty && slotID != 0
...
// Compact dual-wire pin matching Go pinShouldIncrementCountOnInsertAt3535 / C++ capacity:
//   slotEmpty && slotID != 0
...
// dedicated slice 3535 expand residual 3021; prior dedicated 3486 / 3432 / 3374 retained).
//
// Coverage:
//   - free == inline == pin == (slotEmpty && slotID != 0)
//   - residual 2802 / 3021 pins still hold
//   - prior 3486 dedicated poles still hold
...
auto runItemIncrementCountInsert3535SelfTests() -> bool
{
    ...
    // Prior dedicated 3486 poles still hold.
    ok = expect(ShouldIncrementCountOnInsertAt(true, 1), "prior 3486 inc empty nonzero") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 1), "prior 3486 no inc occupied") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(true, 0), "prior 3486 no inc empty zero") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 0), "prior 3486 no inc occupied zero") && ok;
    ok = expect(ShouldIncrementCountOnInsertAt(true, 255), "prior 3486 inc empty max") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 255), "prior 3486 no inc occupied max") && ok;
    ...
    const struct
    {
        ...
        // Prior dedicated 3486 poles still hold.
        { true, 1, true, "prior 3486 empty nonzero" },
        { false, 1, false, "prior 3486 occupied nonzero" },
        { true, 0, false, "prior 3486 empty zero" },
        { false, 0, false, "prior 3486 occupied zero" },
        { true, 120, true, "prior 3486 empty max-container slot" },
        { true, 255, true, "prior 3486 empty max uint8" },
        { false, 120, false, "prior 3486 occupied max-container slot" },
        { false, 255, false, "prior 3486 occupied max uint8" },
