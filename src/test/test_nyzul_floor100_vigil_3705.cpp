#include "test_nyzul_floor100_vigil_3705.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "nyzul floor100 vigil 3705 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua vigilWeaponDrop floor-100 gate for dual-wire cross-check
// (slice 3705 dedicated expand residual 2900; prior dedicated 3352 / 3449 / 3502 / 3570 / 3615 / 3660):
...
// Compact dual-wire pin matching free function / capacity body (slice 3705).
...
// (Lua vigilWeaponDrop floor-100 guaranteed-drop gate; slice 3705 dedicated
// expand residual 2900; prior dedicated 3352 / 3449 / 3502 / 3570 / 3615 / 3660). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2900 / 1088 poles still hold (100 grants; non-100 misses)
//   - prior dedicated 3352 / 3449 / 3502 / 3570 / 3615 / 3660 pins still hold
...
auto runNyzulFloor100Vigil3705SelfTests() -> bool
{
    ...
    // Residual 2900 / 1088 / prior dedicated 3352 / 3449 / 3502 / 3570 / 3615 / 3660 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin      = pinShouldDropFloor100VigilWeapons3705(p.floor);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3705(e.floor);
    ...
    for (int32 floor = -2; floor <= Floor100 + 5; ++floor)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3705(floor);
    ...
    ok = expect(ShouldDropFloor100VigilWeapons(99) == pinShouldDropFloor100VigilWeapons3705(99),
                "free == pin 99") &&
         ok;
    ok = expect(ShouldDropFloor100VigilWeapons(100) == pinShouldDropFloor100VigilWeapons3660(100),
                "free == prior pin3660 100") &&
         ok;
    ok = expect(ShouldDropFloor100VigilWeapons(99) == pinShouldDropFloor100VigilWeapons3660(99),
                "free == prior pin3660 99") &&
         ok;
    ...
    // Residual 2900 / prior dedicated 3352 / 3449 / 3502 / 3570 / 3615 / 3660 still hold under dedicated 3705.
    ok = expect(ShouldDropFloor100VigilWeapons(100) && !ShouldDropFloor100VigilWeapons(99),
                "residual 2900 / prior 3352 / 3449 / 3502 / 3570 / 3615 / 3660 pins under 3705") &&
         ok;
