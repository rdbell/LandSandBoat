#include "test_universal_clear_slot_3566.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "universal ShouldClearSlot 3566 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ClearSlot range gate for dual-wire cross-check (dedicated 3566):
//   slotInRange
...
// Compact dual-wire pin matching Go pinShouldClearSlot3566 / C++ capacity:
//   slotInRange
...
// Prior 3532 pin for independence cross-check (formula unchanged):
//   slotInRange
auto pinShouldClearSlot3532(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3566 expand residual 2813 / prior 2980 / prior 3354 /
// prior 3415 / prior 3469 / prior 3532).
//
// Coverage:
//   - free == inline == pin == pin3532 == slotInRange
//   - residual poles (2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 / prior 3532) still hold
//   - prior 3532 pin independence
...
auto runUniversalClearSlot3566SelfTests() -> bool
{
    ...
    // Residual 2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 / prior 3532 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Classic dual poles / residual 2813+prior 2980+prior 3354+prior 3415+prior 3469+prior 3532 surface.
        { true, true, "in-range accepts" },
        { false, false, "out-of-range rejects" },

        // Residual 2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 / prior 3532 pins (full 2^1 surface, labeled).
    ...
    // Prior 3532 pin independence: free == prior 3532 pin.
    for (const bool slotInRange : { false, true })
    {
        const bool got      = ShouldClearSlot(slotInRange);
        const bool priorPin = pinShouldClearSlot3532(slotInRange);
        ok                  = expect(got == priorPin, "prior 3532 pin independence") && ok;
    }
