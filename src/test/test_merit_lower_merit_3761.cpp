#include "test_merit_lower_merit_3761.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "merit ShouldLowerMerit 3761 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline LowerMerit count-decrement formula for dual-wire cross-check
// (slice 3761 dedicated expand residual 3054; prior dedicated 3716 / 3671):
//   count > 0
...
// Retained under dedicated expand residual 3761.
...
// (count > 0 LowerMerit count-decrement gate; OmegaXI internal/merit; slice 3761
// dedicated expand residual 3054; prior dedicated 3716 / 3671). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3716 (direct return formula)
//   - residual 3054 / 2810 / prior dedicated 3716 / 3671 poles still hold
...
auto runMeritLowerMerit3761SelfTests() -> bool
{
    ...
    // Residual 3054 / 2810 / prior dedicated 3716 / 3671 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3054 / 2810 / prior 3716 / 3671 classic poles (0, 1, n).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin           = pinShouldLowerMerit3761(p.count);
        const bool priorPin3716  = pinShouldLowerMerit3716(p.count);
        ...
        ok = expect(got == priorPin3716, "ShouldLowerMerit == prior 3716 pin") && ok;
    ...
    // Dense poles: free == inline == pin == pin3716 (direct return) across residual + interior.
                               ...
                               uint8{ 255 } })
    {
        ...
        const bool pin          = pinShouldLowerMerit3761(count);
        const bool priorPin3716 = pinShouldLowerMerit3716(count);
        ...
        ok = expect(got == priorPin3716, "dense free == prior 3716 pin") && ok;
    ...
    for (const bool present : { false, true })
    {
        for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 15 }, uint8{ 255 } })
        {
            ...
            ok             = expect(got == inlineShouldLowerMerit(count),
                        "compose free == inline") &&
                 ok;
            ok = expect(got == pinShouldLowerMerit3761(count), "compose free == pin") && ok;
            ok = expect(got == pinShouldLowerMerit3716(count), "compose free == prior 3716 pin") && ok;
    ...
    // Prior dedicated 3716 independence: free still matches prior pin.
    for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 2 }, uint8{ 15 },
                               uint8{ 128 }, uint8{ 254 }, uint8{ 255 } })
    {
        const bool got      = ShouldLowerMerit(count);
        const bool priorPin = pinShouldLowerMerit3716(count);
        ok                  = expect(got == priorPin, "prior 3716 independence") && ok;
    }
