#include "test_recast_update_charge_time_3104.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "recast ShouldUpdateChargeTime 3104 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Load chargeTime update gate for dual-wire cross-check (slice 3104):
//   chargeTimeNonzero
...
// (chargeTime != 0 update on existing Load entry; slice 3104).
auto runRecastUpdateChargeTime3104SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // Residual 1370 pins.
        { true, true, "residual 1370 update charge" },
        { false, false, "residual 1370 no update charge" },
    };
