#include "test_dynamis_can_unlock_sj_3897.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "dynamis CanUnlockSJ 3897 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua formula for dual-wire cross-check (slice 3897 expand residual 2921):
...
// Compact dual-wire pin matching Go pinCanUnlockSJ3897:
...
// Prior dedicated expand residual 2921 pin 3672 (retained under expand):
//   hasSJRestriction ? 1 : 0
auto pinCanUnlockSJ3672(const bool hasSJRestriction) -> int
{
    return hasSJRestriction ? 1 : 0;
}

// Prior dedicated expand residual 2921 pin 3717 (retained under expand):
//   hasSJRestriction ? 1 : 0
auto pinCanUnlockSJ3717(const bool hasSJRestriction) -> int
{
    return hasSJRestriction ? 1 : 0;
}

// Prior dedicated expand residual 2921 pin 3762 (retained under expand):
//   hasSJRestriction ? 1 : 0
auto pinCanUnlockSJ3762(const bool hasSJRestriction) -> int
{
    return hasSJRestriction ? 1 : 0;
}

// Prior dedicated expand residual 2921 pin 3807 (retained under expand):
//   hasSJRestriction ? 1 : 0
auto pinCanUnlockSJ3807(const bool hasSJRestriction) -> int
{
    return hasSJRestriction ? 1 : 0;
}

// Prior dedicated expand residual 2921 pin 3852 (retained under expand):
//   hasSJRestriction ? 1 : 0
// free==inline==pin==pin3852 required for residual expand 3897.
auto pinCanUnlockSJ3852(const bool hasSJRestriction) -> int
{
    return hasSJRestriction ? 1 : 0;
}
...
// slice 3897 dedicated expand residual 2921; prior dedicated 3852 / 3807 / 3762 / 3717 / 3672 / 3627 / 3582 / 3538 / 3476 / 3422 / 3151 retained).
//
// Coverage:
//   - has SJ_RESTRICTION → 1; no restriction → 0
//   - free == inline == pin == pin3852 == hasSJRestriction ? 1 : 0
...
//   - prior dedicated expand residual 2921 pin 3672 still holds
//   - prior dedicated expand residual 2921 pin 3717 still holds
//   - prior dedicated expand residual 2921 pin 3762 still holds
//   - prior dedicated expand residual 2921 pin 3807 still holds
//   - prior dedicated expand residual 2921 pin 3852 still holds
...
auto runDynamisCanUnlockSJ3897SelfTests() -> bool
{
    ...
    // --- Core branches: free == inline == pin == pin3852 == formula ---
    ...
    const struct
    {
        ...
        // prior dedicated expand residual 2921 poles (3672)
        { true, 1, "prior 3672 true → 1 (unlock available)" },
        { false, 0, "prior 3672 false → 0 (no unlock option)" },
        // prior dedicated expand residual 2921 poles (3717)
        { true, 1, "prior 3717 true → 1 (unlock available)" },
        { false, 0, "prior 3717 false → 0 (no unlock option)" },
        // prior dedicated expand residual 2921 poles (3762)
        { true, 1, "prior 3762 true → 1 (unlock available)" },
        { false, 0, "prior 3762 false → 0 (no unlock option)" },
        // prior dedicated expand residual 2921 poles (3807)
        { true, 1, "prior 3807 true → 1 (unlock available)" },
        { false, 0, "prior 3807 false → 0 (no unlock option)" },
        // prior dedicated expand residual 2921 poles (3852)
        { true, 1, "prior 3852 true → 1 (unlock available)" },
        { false, 0, "prior 3852 false → 0 (no unlock option)" },
    ...
    for (const auto& c : cases)
    {
        ...
        ok = expect(got == pinCanUnlockSJ3672(c.hasSJ), "prior 3672 pin still holds") && ok;
        ok = expect(got == pinCanUnlockSJ3717(c.hasSJ), "prior 3717 pin still holds") && ok;
        ok = expect(got == pinCanUnlockSJ3762(c.hasSJ), "prior 3762 pin still holds") && ok;
        ok = expect(got == pinCanUnlockSJ3807(c.hasSJ), "prior 3807 pin still holds") && ok;
        ok = expect(got == pinCanUnlockSJ3852(c.hasSJ), "prior 3852 pin still holds") && ok;
    }
    ...
    // free == pin3852 (required dual-wire identity for residual expand).
    ok = expect(CanUnlockSJ(true) == pinCanUnlockSJ3852(true), "free==pin3852 hasSJ=true") && ok;
    ok = expect(CanUnlockSJ(false) == pinCanUnlockSJ3852(false), "free==pin3852 hasSJ=false") && ok;
    ...
    // Dense 2^1 compose: free == inline == pin == pin3852 == formula
    // (and prior 3852 / 3807 / 3762 / 3717 / 3672 / 3627 / 3582 / 3538 / 3476 / 3422 / 3151 pins).
    for (const bool hasSJ : { false, true })
    {
        ...
        ok = expect(got == pinCanUnlockSJ3672(hasSJ), "compose prior 3672 pin") && ok;
        ok = expect(got == pinCanUnlockSJ3717(hasSJ), "compose prior 3717 pin") && ok;
        ok = expect(got == pinCanUnlockSJ3762(hasSJ), "compose prior 3762 pin") && ok;
        ok = expect(got == pinCanUnlockSJ3807(hasSJ), "compose prior 3807 pin") && ok;
        ok = expect(got == pinCanUnlockSJ3852(hasSJ), "compose prior 3852 pin") && ok;
    }
