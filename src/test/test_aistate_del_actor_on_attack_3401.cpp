#include "test_aistate_del_actor_on_attack_3401.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3401 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3401 expand residual 3044):
//   emittedHostile
auto inlineShouldDelActorOnAttack3401(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3401 / C++ capacity
// identity form (formula unchanged from 1631 / 3044 / 3178):
//   emittedHostile
auto pinShouldDelActorOnAttack3401(const bool emittedHostile) -> bool
{
    return emittedHostile;
}
...
// OmegaXI internal/aistate; dedicated slice 3401 expand residual 3044;
// prior dedicated 3178; pure 1631 / 1709 / 2306). Formula unchanged.
//
// Coverage:
//   - free == inline == pin
//   - residual 3044 / 1631 / prior 3178 pins still hold
...
auto runAistateDelActorOnAttack3401SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 / prior 3178 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044/3178: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044/3178: emittedHostile false → no del") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3401(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3401(c.emittedHostile);
    ...
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3401(true),
                "free == pin for emittedHostile true") &&
         ok;
    ok = expect(!ShouldDelActorOnAttack(false), "emittedHostile false must not del") && ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3401(false),
    ...
    for (const bool emitted : { false, true })
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3401(emitted);
        const bool pin     = pinShouldDelActorOnAttack3401(emitted);
