#include "test_nyzul_claim_rune_3311.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "nyzul claim rune 3311 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua Rune of Transfer onEventUpdate first-claimer gate for dual-wire
// cross-check (slice 3311 dedicated expand residual 2902; prior dedicated
// 3281):
...
// Compact dual-wire pin matching free function / capacity body (slice 3311).
// Direct return only — same formula as production CanClaimRuneHandler.
auto pinCanClaimRuneHandler3311(const int32 runeHandler) -> bool
{
    return runeHandler == 0;
}
...
// (Lua Rune of Transfer onEventUpdate first-claimer gate; slice 3311
// dedicated expand residual 2902; prior dedicated 3281). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2902 / 1088 poles still hold (0 true, non-zero false)
//   - prior dedicated 3240 / 3281 pins still hold
...
auto runNyzulClaimRune3311SelfTests() -> bool
{
    ...
    // Residual 2902 / 1088 / prior dedicated 3240 / 3281 pins still hold.
    ...
    const struct
    {
        ...
        // Prior dedicated 3281 re-pins.
        { 0, true, "prior 3281 unset claims" },
        { 1, false, "prior 3281 player id 1 blocks" },
        { 42, false, "prior 3281 player id 42 blocks" },
        { -1, false, "prior 3281 defensive negative blocks" },
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimRuneHandler3311(p.runeHandler);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinCanClaimRuneHandler3311(e.runeHandler);
    ...
    for (int32 rh = -4; rh <= 4; ++rh)
    {
        ...
        const bool pin     = pinCanClaimRuneHandler3311(rh);
    ...
    ok = expect(CanClaimRuneHandler(0) == pinCanClaimRuneHandler3311(0), "free == pin 0") && ok;
    ok = expect(CanClaimRuneHandler(1) == pinCanClaimRuneHandler3311(1), "free == pin 1") && ok;
    ...
    // Residual 2902 / prior dedicated 3240 / 3281 still hold under dedicated 3311.
    ok = expect(CanClaimRuneHandler(0) && !CanClaimRuneHandler(1),
                "residual 2902 / prior 3240 / 3281 pins under 3311") &&
         ok;
