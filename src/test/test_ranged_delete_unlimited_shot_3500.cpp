#include "test_ranged_delete_unlimited_shot_3500.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranged ShouldDeleteUnlimitedShot 3500 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack UnlimitedShot delete gate for dual-wire cross-check
// (slice 3500 dedicated expand residual 3000; prior 3451 retained):
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto inlineShouldDeleteUnlimitedShot3500(
...
// (dedicated slice 3500). Early-return form only — same formula.
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
...
// Prior 3451 pin retained for independence cross-check (same formula).
auto pinShouldDeleteUnlimitedShot3451(
    const bool hasUnlimitedShot,
    const bool hitOccured,
    const int16 retainUnlimitedShotMod) -> bool
{
...
// slice 3500 dedicated expand residual 3000; prior dedicated 3451 retained).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin (early-return formula)
//   - residual poles: no US; hit always delete; miss retain <=0 / >0; residual 1390/3000/3399/3451
//   - dense hasUS × hit × retain compose
//   - prior 3451 independence (suite retained)
...
auto runRangedDeleteUnlimitedShot3500SelfTests() -> bool
{
    ...
    // Residual 1390 / residual dual-wire 3000 / dedicated 3399 / 3451 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1390 / 3000 / 3399 / 3451 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDeleteUnlimitedShot3500(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pinF    = pinShouldDeleteUnlimitedShot3500(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
    ...
    // Prior 3451 independence: free == prior 3451 pin poles (suite retained).
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3451(true, true, 5),
                "prior 3451 independence: free == 3451 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3451(false, true, 0),
                "prior 3451 independence: free == 3451 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3451(true, false, 1),
                "prior 3451 independence: free == 3451 pin retain keep") &&
         ok;
    ok = expect(pinShouldDeleteUnlimitedShot3500(true, false, 0) == pinShouldDeleteUnlimitedShot3451(true, false, 0),
                "prior 3451 independence: 3500 pin == 3451 pin miss retain 0") &&
         ok;

    // Dense compose: hasUnlimitedShot × hitOccured × retain poles — free == inline == pin.
    for (const bool hasUS : { false, true })
    {
        for (const bool hit : { false, true })
        {
                                        ...
                                        static_cast<int16>(1), static_cast<int16>(5), static_cast<int16>(100) })
            {
                ...
                ok              = expect(got == inlineShouldDeleteUnlimitedShot3500(hasUS, hit, retain),
                            "compose free == inline") &&
                     ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3500(hasUS, hit, retain), "compose free == pin") && ok;
            }
        }
    }
    ...
    // Prior 3451 independence under compose: free == 3451 pin residual poles.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3451(true, true, 5),
                "prior 3451 independence compose: free == 3451 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3451(false, true, 0),
                "prior 3451 independence compose: free == 3451 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3451(true, false, 1),
                "prior 3451 independence compose: free == 3451 pin retain keep") &&
         ok;
