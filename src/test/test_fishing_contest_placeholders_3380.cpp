#include "test_fishing_contest_placeholders_3380.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "fishing contest placeholders 3380 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline GeneratePlaceholderEntries gate for dual-wire cross-check (dedicated 3380).
// Capacity takes presenting bool; host injects status == PRESENTING:
//   presenting && realEntries < maxEntries
auto inlineShouldGeneratePlaceholderEntries3380(
...
// Compact dual-wire pin matching Go pinShouldGeneratePlaceholderEntries3380 /
// C++ capacity AND form (formula unchanged from 2645 / 2855):
//   presenting && realEntries < maxEntries
...
// OmegaXI internal/fishingcontest; dedicated slice 3380; residual expand 2855 /
// pure 2645).
//
// Coverage:
//   - free == inline == pin == (presenting && realEntries < maxEntries)
//   - residual 2855 / 2645 pins still hold
...
auto runFishingContestPlaceholders3380SelfTests() -> bool
{
    ...
    // Residual 2855 / 2645 pins still hold under dedicated dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldGeneratePlaceholderEntries3380(c.presenting, c.maxEntries, c.realEntries);
        const bool pin     = pinShouldGeneratePlaceholderEntries3380(c.presenting, c.maxEntries, c.realEntries);
    ...
    for (const bool presenting : { false, true })
    {
        for (const auto& c : countEdges)
        {
            ...
            const bool inlineF = inlineShouldGeneratePlaceholderEntries3380(presenting, c.maxEntries, c.realEntries);
            const bool pin     = pinShouldGeneratePlaceholderEntries3380(presenting, c.maxEntries, c.realEntries);
    ...
    ok = expect(!ShouldGeneratePlaceholderEntries(true, 4, 4) &&
                    !pinShouldGeneratePlaceholderEntries3380(true, 4, 4),
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF    = inlineShouldGeneratePlaceholderEntries3380(presenting, p.maxEntries, p.realEntries);
        const bool pin        = pinShouldGeneratePlaceholderEntries3380(presenting, p.maxEntries, p.realEntries);
