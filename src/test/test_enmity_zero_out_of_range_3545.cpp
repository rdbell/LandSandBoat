#include "test_enmity_zero_out_of_range_3545.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "enmity ShouldZeroEnmityOutOfRange 3545 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline UpdateEnmity out-of-range CE/VE zero gate for dual-wire cross-check
// (slice 3545 dedicated expand residual 2927; prior dedicated expand 3485 /
// 3427 / 3333 / prior dedicated 3157):
//   !withinRange
// Direct return form matching production free function / capacity.
auto inlineShouldZeroEnmityOutOfRange3545(const bool withinRange) -> bool
{
    return !withinRange;
}

// Compact dual-wire pin matching free function / capacity body (slice 3545).
...
//     prior expand 3427 / prior expand 3485 pins still hold
auto runEnmityZeroOutOfRange3545SelfTests() -> bool
{
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3545(p.withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3545(p.withinRange);
    ...
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3545(false), "free==pin false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3545(true), "free==pin true") && ok;
    ...
    for (const bool withinRange : { false, true })
    {
        ...
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3545(withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3545(withinRange);
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok                = expect(got == inlineShouldZeroEnmityOutOfRange3545(within), "compose free == inline") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3545(within), "compose free == pin") && ok;
    }

    // Residual 2927 / prior dedicated expand 3485 / 3427 / 3333 / prior dedicated 3157 /
    // pure 1357 still hold under expand suite.
    ok = expect(ShouldZeroEnmityOutOfRange(false) && !ShouldZeroEnmityOutOfRange(true),
                "residual 2927/3485/3427/3333/3157/1357 pins failed under 3545 suite") &&
         ok;
