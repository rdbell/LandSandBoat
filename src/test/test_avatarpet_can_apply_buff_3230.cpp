#include "test_avatarpet_can_apply_buff_3230.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "avatarpet CanApplyBuff 3230 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline tryBuffSpell canApplyBuff formula for dual-wire cross-check
// (slice 3230; residual 2968):
...
// Positive if/else pin matching free function / capacity body (slice 3230).
...
// dedicated slice 3230 expand residual 2968).
//
// Coverage:
