#include "test_ranger_strip_partial_retain_3567.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranger ShouldStripPartialStealthOnRetain 3567 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack retain-success partial-stealth gate for dual-wire
// cross-check (dedicated 3567 expand residual 3082; prior dedicated 3510 / 3445 / 3394):
//   !stripAllDetectable
auto inlineShouldStripPartialStealthOnRetain3567(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Compact dual-wire pin matching Go pinShouldStripPartialStealthOnRetain3567 /
// C++ capacity form (formula unchanged from 1391 / 3082 / 3394 / 3445 / 3510):
//   !stripAllDetectable
...
// dedicated slice 3567 expand residual 3082; prior dedicated 3510 / 3445 / 3394 / pure 1391 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3510
//   - residual 3510 / 3445 / 3394 / 3082 / 1391 pins still hold
...
// test_ranger_strip_partial_retain_3510.
...
auto runRangerStripPartialRetain3567SelfTests() -> bool
{
    ...
    // Prior dedicated 3394 edges still hold under expand residual 3567.
    ...
    // Prior dedicated 3445 edges still hold under expand residual 3567.
    ...
    // Prior dedicated 3510 edges still hold under expand residual 3567.
    ...
    // --- Core poles: free == inline == pin == pin3510 (required edges + stability) ---
    const struct
    {
        ...
        // Residual 1391 / 3082 / prior 3394 / prior 3445 / prior 3510 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3567(c.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3567(c.stripAllDetectable);
    ...
    for (const bool stripAll : { false, true })
    {
        ...
        const bool pin     = pinShouldStripPartialStealthOnRetain3567(stripAll);
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3567(stripAll);
    ...
    // Dense compose over strip-all poles from retainChance/roll: free == inline == pin == pin3510.
    const struct
    {
        int16 retainChance;
        int   roll0to99;
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3567(h.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3567(h.stripAllDetectable);
