#include "test_ambuscade_start_exit_3758.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ambuscade start-exit 3758 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onInstanceComplete / onInstanceFailure formula for dual-wire
// checks (slice 3758): player:startEvent(10001) is always called for every char.
...
// Compact dual-wire pin matching C++ capacity formula / Go pinShouldStartExitEvent3758:
//   return true
...
// Prior dedicated pin3713 (always true; free == pin3713 always holds).
auto pinShouldStartExitEvent3713() -> bool
{
    return true;
}
...
// OmegaXI internal/ambuscade; dedicated slice 3758 expand residual 2917; prior
// dedicated 3713/3668/3623/3578/3534/3488/3438/3382/3109). Formula unchanged:
//
//   ShouldStartExitEvent() = true
//
// Coverage:
//   - free == inline == pin == pin3713 == true
//   - residual 2917 / 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 3623 / 3668 / 3713 / 1089 pins still hold
...
auto runAmbuscadeStartExit3758SelfTests() -> bool
{
    ...
    // Residual constant / 1089 / 2917 / prior dedicated 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 3623 / 3668 / 3713 pins still hold.
    ...
    // Unconditional gate always true; free == inline == pin == pin3713 (direct return true).
    {
        ...
        const bool pin3713   = pinShouldStartExitEvent3713();
        ...
        ok = expect(got == pin3713, "dual-wire free == pin3713") && ok;
        ok = expect(got == true, "dual-wire free == true") && ok;
    }

    // Repeated calls stay true (no state); free == inline == pin == pin3713 matching C++.
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        ok             = expect(got == pinShouldStartExitEvent3713(), "free == pin3713 across calls") && ok;
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pinShouldStartExitEvent3713(), "compose free == pin3713") && ok;
    ...
    // Dense always-true poles: free == inline == pin == pin3713 for unconditional domain.
    for (int i = 0; i < 2; ++i)
    {
        const bool got = ShouldStartExitEvent();
        ok             = expect(got == pinShouldStartExitEvent(), "dense free == pin") && ok;
        ok             = expect(got == inlineShouldStartExitEvent(), "dense free == inline") && ok;
        ok             = expect(got == pinShouldStartExitEvent3713(), "dense free == pin3713") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3713(), "free == pin3713 always-true") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3713(), "host path free == pin3713") && ok;

    // Residual 2917 / prior dedicated 3713/3668/3623/3578/3534/3488/3438/3382/3109 still hold under dedicated 3758.
    ok = expect(ShouldStartExitEvent(), "residual 2917/3713/3668/3623/3578/3534/3488/3438/3382/3109 pins under 3758") && ok;
