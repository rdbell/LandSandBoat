#include "test_garrison_fail_by_death_3560.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3560 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// prior dedicated expand 3461 / prior dedicated expand 3514 / dedicated 3560):
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3560:
//   allNPCsDead || allPlayersDead
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3560;
...
//
// Coverage:
//   - free == inline == pin == pin3514 (direct return positive OR; no De Morgan)
//   - 4-row truth table + residual poles + dense 2² free == inline == pin == pin3514
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
//     prior dedicated expand 3330 / prior dedicated expand 3406 /
//     prior dedicated expand 3461 / prior dedicated expand 3514 pins still hold
...
auto runGarrisonFailByDeath3560SelfTests() -> bool
{
    ...
    // Residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
    // prior dedicated expand 3330 / prior dedicated expand 3406 /
    // prior dedicated expand 3461 / prior dedicated expand 3514 pins.
    ...
    // 4-row truth table (+ residual re-pins): free == inline == pin == pin3514.
    const struct
    {
        bool        allNPCsDead;
        bool        allPlayersDead;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3514 = pinShouldFailByDeath3514(c.allNPCsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3514, "ShouldFailByDeath dual-wire == pin3514") && ok;
    ...
    // Free == pin == pin3514 across residual poles (4-row truth table).
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath(false, false) &&
                    ShouldFailByDeath(false, false) == pinShouldFailByDeath3514(false, false),
                "free == pin/pin3514 neither dead") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath(true, false) &&
                    ShouldFailByDeath(true, false) == pinShouldFailByDeath3514(true, false),
                "free == pin/pin3514 npcs dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath(false, true) &&
                    ShouldFailByDeath(false, true) == pinShouldFailByDeath3514(false, true),
                "free == pin/pin3514 players dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath(true, true) &&
                    ShouldFailByDeath(true, true) == pinShouldFailByDeath3514(true, true),
                "free == pin/pin3514 both dead") &&
         ok;

    // Dense compose: full 2^2 boolean space free == inline == pin == pin3514.
    for (const bool allNPCsDead : { false, true })
    {
        for (const bool allPlayersDead : { false, true })
        {
            ...
            ok = expect(got == pinShouldFailByDeath(allNPCsDead, allPlayersDead),
                        "compose free == pin") &&
                 ok;
            ok = expect(got == pinShouldFailByDeath3514(allNPCsDead, allPlayersDead),
                        "compose free == pin3514") &&
                 ok;
        }
    }
    ...
    // Residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
    // prior dedicated expand 3330 / prior dedicated expand 3406 /
    // prior dedicated expand 3461 / prior dedicated expand 3514 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236/3330/3406/3461/3514 wipe gate pins under 3560") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3514(false, false),
                "prior 3514 pin neither dead still holds under 3560") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3514(true, false),
                "prior 3514 pin npcs dead still holds under 3560") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3514(false, true),
                "prior 3514 pin players dead still holds under 3560") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3514(true, true),
                "prior 3514 pin both dead still holds under 3560") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3560.
    ...
    // Explicit residual compose dual-wire free == inline == pin == pin3514 for host poles.
    const struct
    {
        int32       npcCount;
        int32       deadNPCCount;
    ...
    for (const auto& c : composeCases)
    {
        ...
        const bool pin3514   = pinShouldFailByDeath3514(npcsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3514, "compose dual-wire free == pin3514") && ok;
    }
