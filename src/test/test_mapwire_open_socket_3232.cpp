#include "test_mapwire_open_socket_3232.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapwire ShouldOpenSocket 3232 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapNetworking constructor socket-gate formula for dual-wire
// cross-check (residual 2948 / prior dedicated 3169 / dedicated 3232):
//   !isTestServer
...
// Compact dual-wire pin matching Go pinShouldOpenSocket3232:
//   !isTestServer
...
// (!isTestServer; OmegaXI internal/mapwire; dedicated slice 3232;
// expand residual 2948 / pure 2660; prior dedicated 3169).
//
// Coverage:
//   - free == inline == pin == !isTestServer (identity-not)
//   - residual poles true/false
//   - residual 2660 / 2948 / prior 3169 pins still hold
...
auto runMapwireOpenSocket3232SelfTests() -> bool
{
    ...
    // Residual 2948 / 2660 / 3169 still hold under dedicated suite.
    ok = expect(ShouldOpenSocket(false), "residual production open under 3232") && ok;
    ok = expect(!ShouldOpenSocket(true), "residual test skip under 3232") && ok;
