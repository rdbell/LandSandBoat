#include "test_alliance_attempt_leader_promote_3681.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3681 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check
// (slice 3681 dedicated dual-wire expand residual 2979):
//   isMainParty
...
// Prior dedicated expand residual 2979 pin (slice 3636; formula UNCHANGED).
// free == inline == pin == pin3636.
auto pinShouldAttemptAllianceLeaderPromote3636(const bool isMainParty) -> bool
{
    return isMainParty;
}

// This slice's dual-wire pin (slice 3681; formula isMainParty identity —
// UNCHANGED). free == inline == pin == pin3636.
auto pinShouldAttemptAllianceLeaderPromote3681(const bool isMainParty) -> bool
{
    return isMainParty;
}
...
// (isMainParty identity; slice 3681 dedicated dual-wire expand residual 2979).
//
// Coverage:
//   - main party → attempt promote true
//   - non-main → attempt promote false
//   - free == inline == pin == pin3636 (isMainParty identity)
//   - residual 1341 / 1346 / 2979 / 3144 / 3393 / 3447 / 3490 / 3547 / 3591 / 3636 pins still hold
...
auto runAllianceAttemptLeaderPromote3681SelfTests() -> bool
{
    ...
    // dedicated 3547 / prior dedicated 3591 / prior dedicated 3636 pins still
    // hold.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3681(c.isMainParty);
        const bool pin3636 = pinShouldAttemptAllianceLeaderPromote3636(c.isMainParty);

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3636,
                    "ShouldAttemptAllianceLeaderPromote free == inline == pin == pin3636") &&
             ok;
    }
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3636.
    for (const bool isMainParty : { false, true })
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3681(isMainParty);
        const bool pin3636 = pinShouldAttemptAllianceLeaderPromote3636(isMainParty);
        const bool want    = isMainParty;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3636,
                    "compose free == inline == pin == pin3636") &&
             ok;
    }
    ...
    for (const bool isMainParty : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3636,
                    "host inject dual-wire free == inline == pin == pin3636") &&
             ok;
    }
