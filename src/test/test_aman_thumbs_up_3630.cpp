#include "test_aman_thumbs_up_3630.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3630 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3630 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3630 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3585 pin for independence cross-check.
auto pinCanThumbsUp3585(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3585(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3630 expand residual 2967; prior dedicated 3585 / 3541 / 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == pin3585 == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 pins still hold
//   - prior 3585 independence (free == 3585 inline/pin)
...
auto runAmanThumbsUp3630SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3585 re-pins (independence).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3585 = pinCanThumbsUp3585(c.cooldownExpired);
        ...
        ok = expect(got == pin3585, "CanThumbsUp dual-wire == pin3585") && ok;
    ...
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3585(true), "free==pin3585 true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3585(false), "free==pin3585 false") && ok;

    // Explicit residual poles free == inline == pin == pin3585 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3585 = pinCanThumbsUp3585(pole);
        ok                 = expect(got == pole, "pole free == identity") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3585, "pole free == pin3585") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3585.
    for (const bool cooldownExpired : { false, true })
    {
        ...
        ok              = expect(got == pinCanThumbsUp3585(cooldownExpired), "compose free == pin3585") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp(cooldownExpired),
             ...
             ok;
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp3585(cooldownExpired),
                    "host inject free == pin3585") &&
             ok;
    ...
    // Prior dedicated 3585 independence: free == prior 3585 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3585(true), "prior 3585 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3585(false), "prior 3585 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3585(true), "prior 3585 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3585(false), "prior 3585 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 0698 still hold under dedicated 3630.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/3541/3585/0698 under 3630") && ok;
