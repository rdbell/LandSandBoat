#include "test_petutils_pet_engage_3850.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petutils ShouldPetEngage 3850 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AttackTarget engage formula for dual-wire cross-check (dedicated 3850
// expand residual 3051; prior dedicated 3805 / 3760 / 3715 / 3670 / 3625 / 3580 / 3536 / 3484 / 3429 / 3375):
...
// Compact dual-wire pin matching Go pinShouldPetEngage3850 / C++ capacity:
//   !hasPreventActionEffect
...
// Prior dedicated 3805 pin for free == inline == pin == pin3805 cross-check.
...
// dedicated slice 3850 expand residual 3051; prior dedicated 3805 / 3760 / 3715 / 3670 / 3625 / 3580 / 3536 / 3484 / 3429 / 3375; pure 1627).
//
// Coverage:
//   - free == inline == pin == pin3805 == !hasPreventActionEffect
...
auto runPetutilsPetEngage3850SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3805 ---
    const struct
    {
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3805 = pinShouldPetEngage3805(c.hasPreventActionEffect);
        const bool wantPin = !c.hasPreventActionEffect;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3805.
        ok = expect(got == inlineF && got == pin && got == pin3805,
                    "dual-wire free == inline == pin == pin3805") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula !hasPreventActionEffect") && ok;
    }

    // Free == pin == pin3805 across residual poles.
    ...
    ok = expect(ShouldPetEngage(false) == pinShouldPetEngage3805(false), "free == pin3805 no prevent") && ok;
    ok = expect(ShouldPetEngage(true) == pinShouldPetEngage3805(true), "free == pin3805 prevent") && ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3805.
    for (const bool hasPrevent : { false, true })
    {
        ...
        const bool pin3805 = pinShouldPetEngage3805(hasPrevent);
        const bool want    = !hasPrevent;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3805,
                    "compose free == inline == pin == pin3805") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3805 for AttackTarget.
    for (const bool hasPrevent : { false, true })
    {
        const bool got     = ShouldPetEngage(hasPrevent);
        const bool inlineF = inlineShouldPetEngage(hasPrevent);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3805,
                    "host inject free == inline == pin == pin3805") &&
             ok;
