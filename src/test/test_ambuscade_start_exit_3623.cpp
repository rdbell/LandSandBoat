#include "test_ambuscade_start_exit_3623.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ambuscade start-exit 3623 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onInstanceComplete / onInstanceFailure formula for dual-wire
// checks (slice 3623): player:startEvent(10001) is always called for every char.
...
// Compact dual-wire pin matching C++ capacity formula / Go pinShouldStartExitEvent3623:
//   return true
...
// Prior dedicated pin3578 (always true; free == pin3578 always holds).
auto pinShouldStartExitEvent3578() -> bool
{
    return true;
}
...
// OmegaXI internal/ambuscade; dedicated slice 3623 expand residual 2917; prior
// dedicated 3578/3534/3488/3438/3382/3109). Formula unchanged:
//
//   ShouldStartExitEvent() = true
//
// Coverage:
//   - free == inline == pin == pin3578 == true
//   - residual 2917 / 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 1089 pins still hold
...
auto runAmbuscadeStartExit3623SelfTests() -> bool
{
    ...
    // Residual constant / 1089 / 2917 / prior dedicated 3109 / 3382 / 3438 / 3488 / 3534 / 3578 pins still hold.
    ...
    // Unconditional gate always true; free == inline == pin == pin3578 (direct return true).
    {
        ...
        const bool pin3578   = pinShouldStartExitEvent3578();
        ...
        ok = expect(got == pin3578, "dual-wire free == pin3578") && ok;
        ok = expect(got == true, "dual-wire free == true") && ok;
    }

    // Repeated calls stay true (no state); free == inline == pin == pin3578 matching C++.
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        ok             = expect(got == pinShouldStartExitEvent3578(), "free == pin3578 across calls") && ok;
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pinShouldStartExitEvent3578(), "compose free == pin3578") && ok;
    ...
    // Dense always-true poles: free == inline == pin == pin3578 for unconditional domain.
    for (int i = 0; i < 2; ++i)
    {
        const bool got = ShouldStartExitEvent();
        ok             = expect(got == pinShouldStartExitEvent(), "dense free == pin") && ok;
        ok             = expect(got == inlineShouldStartExitEvent(), "dense free == inline") && ok;
        ok             = expect(got == pinShouldStartExitEvent3578(), "dense free == pin3578") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3578(), "free == pin3578 always-true") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3578(), "host path free == pin3578") && ok;

    // Residual 2917 / prior dedicated 3578/3534/3488/3438/3382/3109 still hold under dedicated 3623.
    ok = expect(ShouldStartExitEvent(), "residual 2917/3578/3534/3488/3438/3382/3109 pins under 3623") && ok;
