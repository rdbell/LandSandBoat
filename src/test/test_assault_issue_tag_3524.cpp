#include "test_assault_issue_tag_3524.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3524 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3524 expand residual 2867 / prior 3388 stock / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3524:
//   tagStock > 0
...
// internal/assault; dedicated slice 3524 expand residual 2867 / prior 3388
// stock / 3440 ShouldIssueNewTag sibling / pure 1100 — formula unchanged).
//
// Coverage:
//   - free == inline == pin == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 stock pins still hold
...
// (ShouldIssueNewTag). Mirrors 3440 expand-residual suite layout (header +
// self-test runner; 3388 CanIssueTagFromStock content; no CMake/main).
auto runAssaultIssueTag3524SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 stock poles still hold.
    ...
    // Explicit residual poles free == inline == pin for stock 0 / 1.
    ...
    // Dense tagStock: free == pin == inline == compose.
