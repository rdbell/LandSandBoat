#include "test_mapsession_destroy_pending_ptr_3811.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapsession ShouldDestroyPendingByPointer 3811 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapSessionContainer::destroyPendingSession(MapSession*) gate for
// dual-wire cross-check (dedicated slice 3811):
...
// Compact dual-wire pin matching Go pinShouldDestroyPendingByPointer3811 /
...
// Prior dedicated slice 3766 inline/pin for independence cross-check.
auto inlineShouldDestroyPendingByPointer3766(const bool found, const bool pointerMatches) -> bool
{
    return found && pointerMatches;
}

auto pinShouldDestroyPendingByPointer3766(const bool found, const bool pointerMatches) -> bool
{
    if (!found)
    {
        return false;
...
// (found && pointerMatches; dedicated slice 3811 expand residual 3056 /
// pure 2787; prior dedicated 3766 / 3721 / 3676 / 3631 / 3586 / 3542 / 3480 / 3426 / 3376 retained).
//
// Coverage:
//   - free == inline == pin == pin3766 == (found && pointerMatches)
//   - residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 / 3542 / 3586 / 3631 / 3676 / 3721 / 3766 pins still hold
//   - full 2² bool poles (found × pointerMatches)
//   - prior dedicated 3766 independence (free == 3766 inline/pin)
...
auto runMapsessionDestroyPendingPtr3811SelfTests() -> bool
{
    ...
    // Residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 / 3542 / 3586 / 3631 / 3676 / 3721 / 3766 pins still hold under dual-wire.
    ok = expect(ShouldDestroyPendingByPointer(true, true), "residual 3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: found match destroys") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(true, false), "residual 3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: found mismatch rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, false), "residual 3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: missing rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, true), "residual 3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: missing ignores match flag") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinShouldDestroyPendingByPointer3766(c.found, c.pointerMatches);
        ...
        ok = expect(got == pinPrior, "ShouldDestroyPendingByPointer dual-wire == pin3766") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pinPrior, "host compose dual-wire free == pin3766") && ok;
        ok = expect(got == (c.found && c.pointerMatches),
                    "host compose free == found && pointerMatches") &&
             ok;
    }

    // Dense compose: free == inline == pin == pin3766 residual pins for full 2×2 (2²).
    for (const bool found : { false, true })
    {
        for (const bool pointerMatches : { false, true })
        {
            ...
            ok = expect(got == pinShouldDestroyPendingByPointer3766(found, pointerMatches),
                        "compose free == pin3766") &&
                 ok;
        }
    }

    // Residual independence (2787 / 3056 / 3066 / 3376 / 3426 / 3480 / 3542 / 3586 / 3631 / 3676 / 3721 / 3766): pointer path needs identity;
    // charID path needs presence only. Sibling dual-wires left alone.
    // Prior dedicated 3766 independence: free == prior suite inline/pin forms.
    ...
    for (const bool found : { false, true })
    {
        ...
        {
            const bool got = ShouldDestroyPendingByPointer(found, pointerMatches);
            ok             = expect(got == inlineShouldDestroyPendingByPointer3766(found, pointerMatches),
                        "prior 3766 independence: free == 3766 inline") &&
                 ok;
            ok = expect(got == pinShouldDestroyPendingByPointer3766(found, pointerMatches),
                        "prior 3766 independence: free == 3766 pin") &&
                 ok;
        }
    }
