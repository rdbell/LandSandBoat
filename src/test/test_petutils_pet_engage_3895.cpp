#include "test_petutils_pet_engage_3895.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petutils ShouldPetEngage 3895 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AttackTarget engage formula for dual-wire cross-check (dedicated 3895
// expand residual 3051; prior dedicated 3850 / 3805 / 3760 / 3715 / 3670 / 3625 / 3580 / 3536 / 3484 / 3429 / 3375):
...
// Compact dual-wire pin matching Go pinShouldPetEngage3895 / C++ capacity:
//   !hasPreventActionEffect
...
// Prior dedicated 3670 pin for free == inline == pin cross-check.
...
// Prior dedicated 3715 pin for independence cross-check.
auto pinShouldPetEngage3715(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

auto inlineShouldPetEngage3715(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

// Prior dedicated 3760 pin for independence cross-check.
auto pinShouldPetEngage3760(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

auto inlineShouldPetEngage3760(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

// Prior dedicated 3805 pin for independence cross-check.
auto pinShouldPetEngage3805(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

auto inlineShouldPetEngage3805(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

// Prior dedicated 3850 pin for free == inline == pin == pin3850 cross-check.
auto pinShouldPetEngage3850(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}

auto inlineShouldPetEngage3850(const bool hasPreventActionEffect) -> bool
{
    return !hasPreventActionEffect;
}
...
// dedicated slice 3895 expand residual 3051; prior dedicated 3850 / 3805 / 3760 / 3715 / 3670 / 3625 / 3580 / 3536 / 3484 / 3429 / 3375; pure 1627).
//
// Coverage:
//   - free == inline == pin == pin3850 == !hasPreventActionEffect
...
//   - prior 3715 dedicated poles still hold
//   - prior 3760 dedicated poles still hold
//   - prior 3805 dedicated poles still hold
//   - prior 3850 dedicated poles still hold
...
auto runPetutilsPetEngage3895SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3850 ---
    const struct
    {
        ...
        // Prior dedicated 3715 poles still hold.
        { false, true, "prior 3715 no prevent-action" },
        { true, false, "prior 3715 prevent-action" },

        // Prior dedicated 3760 poles still hold.
        { false, true, "prior 3760 no prevent-action" },
        { true, false, "prior 3760 prevent-action" },

        // Prior dedicated 3805 poles still hold.
        { false, true, "prior 3805 no prevent-action" },
        { true, false, "prior 3805 prevent-action" },

        // Prior dedicated 3850 poles still hold.
        { false, true, "prior 3850 no prevent-action" },
        { true, false, "prior 3850 prevent-action" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3850 = pinShouldPetEngage3850(c.hasPreventActionEffect);
        const bool wantPin = !c.hasPreventActionEffect;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3850.
        ok = expect(got == inlineF && got == pin && got == pin3850,
                    "dual-wire free == inline == pin == pin3850") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula !hasPreventActionEffect") && ok;
    }

    // Free == pin == pin3850 across residual poles.
    ...
    ok = expect(ShouldPetEngage(false) == pinShouldPetEngage3850(false), "free == pin3850 no prevent") && ok;
    ok = expect(ShouldPetEngage(true) == pinShouldPetEngage3850(true), "free == pin3850 prevent") && ok;
    ...
    // Prior dedicated 3715 independence: free still matches prior inline/pin.
    ok = expect(ShouldPetEngage(false) == inlineShouldPetEngage3715(false) &&
                    ShouldPetEngage(false) == pinShouldPetEngage3715(false),
                "prior 3715 independence no prevent") &&
         ok;
    ok = expect(ShouldPetEngage(true) == inlineShouldPetEngage3715(true) &&
                    ShouldPetEngage(true) == pinShouldPetEngage3715(true),
                "prior 3715 independence prevent") &&
         ok;

    // Prior dedicated 3760 independence: free still matches prior inline/pin.
    ok = expect(ShouldPetEngage(false) == inlineShouldPetEngage3760(false) &&
                    ShouldPetEngage(false) == pinShouldPetEngage3760(false),
                "prior 3760 independence no prevent") &&
         ok;
    ok = expect(ShouldPetEngage(true) == inlineShouldPetEngage3760(true) &&
                    ShouldPetEngage(true) == pinShouldPetEngage3760(true),
                "prior 3760 independence prevent") &&
         ok;

    // Prior dedicated 3805 independence: free still matches prior inline/pin.
    ok = expect(ShouldPetEngage(false) == inlineShouldPetEngage3805(false) &&
                    ShouldPetEngage(false) == pinShouldPetEngage3805(false),
                "prior 3805 independence no prevent") &&
         ok;
    ok = expect(ShouldPetEngage(true) == inlineShouldPetEngage3805(true) &&
                    ShouldPetEngage(true) == pinShouldPetEngage3805(true),
                "prior 3805 independence prevent") &&
         ok;

    // Prior dedicated 3850 independence: free still matches prior inline/pin.
    ok = expect(ShouldPetEngage(false) == inlineShouldPetEngage3850(false) &&
                    ShouldPetEngage(false) == pinShouldPetEngage3850(false),
                "prior 3850 independence no prevent") &&
         ok;
    ok = expect(ShouldPetEngage(true) == inlineShouldPetEngage3850(true) &&
                    ShouldPetEngage(true) == pinShouldPetEngage3850(true),
                "prior 3850 independence prevent") &&
         ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3850.
    for (const bool hasPrevent : { false, true })
    {
        ...
        const bool pin3850 = pinShouldPetEngage3850(hasPrevent);
        const bool want    = !hasPrevent;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3850,
                    "compose free == inline == pin == pin3850") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3850 for AttackTarget.
    for (const bool hasPrevent : { false, true })
    {
        const bool got     = ShouldPetEngage(hasPrevent);
        const bool inlineF = inlineShouldPetEngage(hasPrevent);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3850,
                    "host inject free == inline == pin == pin3850") &&
             ok;
