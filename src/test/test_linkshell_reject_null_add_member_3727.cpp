#include "test_linkshell_reject_null_add_member_3727.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "linkshell ShouldRejectNullAddMember 3727 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldRejectNullAddMember3727 / C++
// capacity identity form (formula unchanged from 1354 / 2929 / prior dedicated
// 3195 / 3489 / 3548 / 3592 / 3637 / 3682):
//   charNull
// free == inline == pin == pin3682.
...
// (charNull identity; OmegaXI internal/linkshell; dedicated slice 3727;
// residual expand 2929 / pure 1354; prior dedicated expands 3195 / 3489 / 3548 /
// 3592 / 3637 / 3682).
//
// Coverage:
//   - free == inline == pin == pin3682 identity form
//   - poles: null rejects; non-null allows
//   - residual 2929 / 1354 / prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 pins still hold
//   - host-style inject + residual independence (2958 / 3079)
//   - prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 independence (RETAINED)
auto runLinkshellRejectNullAddMember3727SelfTests() -> bool
{
    ...
    // Residual 1354 / 2929 / prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 pins still
    ...
    // --- Core poles: free == inline == pin == pin3682 identity form ---
    const struct
    {
        bool        charNull;
        bool        want;
        ...
        // Residual 2929 / 1354 / prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3682  = pinShouldRejectNullAddMember3682(c.charNull);
        ...
        ok = expect(got == pin3682, "dual-wire free==pin3682") && ok;
        ok = expect(got == wantPin, "dual-wire free==identity pin form") && ok;
    }

    // Free == pin / pin3682 across residual poles.
    ...
    ok = expect(ShouldRejectNullAddMember(true) == pinShouldRejectNullAddMember3682(true), "free==pin3682 null char") && ok;
    ok = expect(ShouldRejectNullAddMember(false) == pinShouldRejectNullAddMember3682(false), "free==pin3682 non-null char") && ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3682.
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin3682 = pinShouldRejectNullAddMember3682(charNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3682, "compose free==pin3682") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3682, "host compose free==pin3682") && ok;
    }
    ...
    // Residual independence (1354 / 2929 / 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 2958 / 3079):
    // AddMember null reject is distinct from duplicate-add and online-null gates.
    // Prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 independence (RETAINED under this expand).
