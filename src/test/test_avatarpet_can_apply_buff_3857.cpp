#include "test_avatarpet_can_apply_buff_3857.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "avatarpet CanApplyBuff 3857 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline tryBuffSpell canApplyBuff formula for dual-wire cross-check
// (slice 3857; residual 2968 / prior dedicated 3152 / 3230 / 3370 / 3425 /
// 3479 / 3543 / 3587 / 3632 / 3677 / 3722 / 3767 / 3812):
...
// Positive if/else pin matching free function / capacity body (slice 3857).
...
// Prior dedicated 3812 independence pin (positive if/else; formula unchanged).
auto pinCanApplyBuff3812(
    const bool hasStatusEffect,
    const uint8 statusTier,
    const bool spellHasTier,
    const uint8 spellTier) -> bool
...
// dedicated slice 3857 expand residual 2968).
//
// Coverage:
//   - free == inline == pin == pin3812 (positive if/else multi-branch / early-return)
//   - residual 1043 / 2968 poles still hold
//   - residual pins (one-liner + prior dedicated forms incl. 3812) still agree
...
auto runAvatarpetCanApplyBuff3857SelfTests() -> bool
{
    ...
    // Residual poles: free == inline == pin == pin3812.
    const struct
    {
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin3812 = pinCanApplyBuff3812(p.hasStatusEffect, p.statusTier, p.spellHasTier, p.spellTier);
        ...
        ok                 = expect(got == pin3812, "residual pole free == pin3812") && ok;
    }
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3812 = pinCanApplyBuff3812(c.hasStatusEffect, c.statusTier, c.spellHasTier, c.spellTier);
        ...
        ok = expect(got == pin3812, "CanApplyBuff == pin3812 (prior dedicated)") && ok;
    }

    // Dense cartesian: hasStatus × spellHasTier × tiers{0,1,2,255}
    // free == inline == pin == pin3812 == formula (+ residual pins).
            ...
            for (const bool spellHasTier : bools)
            {
                for (const uint8 spellTier : tiers)
                {
                    ...
                    ok = expect(got == pinCanApplyBuffOneLiner(hasStatus, statusTier, spellHasTier, spellTier),
                         ...
                         ok;
                    ok = expect(got == pinCanApplyBuff3812(hasStatus, statusTier, spellHasTier, spellTier),
                                "dense free == residual 3812 pin") &&
                         ok;
                }
            }
        }
    ...
    // Host-style inject poles: free == pin == pin3812 == inline.
    const struct
    {
        bool  hasStatusEffect;
        uint8 statusTier;
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3812 = pinCanApplyBuff3812(h.hasStatusEffect, h.statusTier, h.spellHasTier, h.spellTier);
        ...
        ok                 = expect(freeGot == pin3812, "host inject free == pin3812") && ok;
