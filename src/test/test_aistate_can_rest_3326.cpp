#include "test_aistate_can_rest_3326.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate CanRest 3326 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CBattleEntity::CanRest pure gate for dual-wire cross-check
// (slice 3326 dedicated expand residual 3006):
...
// Compact dual-wire pin matching free function / capacity body (slice 3326).
// Direct return only — same formula as production CanRest.
auto pinCanRest3326(const bool hasRegenDown, const bool hasNoRestFlag) -> bool
{
    return !hasRegenDown && !hasNoRestFlag;
}
...
// (!hasRegenDown && !hasNoRestFlag; OmegaXI internal/aistate; slice 3326
// dedicated expand residual 3006; prior dedicated 3296 / 3253). Formula
// unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 3006 / 1635 / prior 3253 / 3296 poles still hold
...
auto runAistateCanRest3326SelfTests() -> bool
{
    ...
    // Residual 3006 / 1635 / prior 3253 / 3296 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3006 / 1635 / prior 3253 / 3296 classic dual poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanRest3326(p.hasRegenDown, p.hasNoRestFlag);
    ...
    for (const bool hasRegenDown : { false, true })
    {
        for (const bool hasNoRestFlag : { false, true })
        {
            ...
            const bool pin     = pinCanRest3326(hasRegenDown, hasNoRestFlag);
