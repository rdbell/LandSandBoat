#include "test_salvage_claim_transport_3552.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage claim transport 3552 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanClaimTransport3552:
...
// Prior dedicated 3495 pin (same formula; residual suite identity).
auto pinCanClaimTransport3495(const uint32 transportUserID) -> bool
{
    return !salvagehelpers::TransportUserBusy(transportUserID);
}
...
// slice 3552 dedicated dual-wire expand residual 2871).
//
// Coverage:
...
//   - residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 pins still hold
//   - prior 3495 independence: free == pin3495 across poles
auto runSalvageClaimTransport3552SelfTests() -> bool
{
    ...
    // Residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 pins still hold under dual-wire.
    ...
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport(0xFFFFFFFF),
         ...
         ok;
    ok = expect(CanClaimTransport(0) == pinCanClaimTransport3495(0),
                "residual 3495: free == pin for id 0") &&
         ok;
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport3495(0xFFFFFFFF),
                "residual 3495: free == pin for max busy") &&
         ok;
    ...
    ok = expect(pinCanClaimTransport3495(0), "eligible prior pin3495 id 0 free should claim") && ok;
    ...
    ok = expect(!pinCanClaimTransport3495(0xFFFFFFFF), "prior pin3495 max busy should not claim") && ok;
    ...
    // --- Composition table: free == inline == pin == pin3495 == !TransportUserBusy ---
    // Dense poles: free / busy residual + large / max.
    const struct
    {
        uint32      id;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior   = pinCanClaimTransport3495(c.id);
        ...
        ok = expect(gotClaim == pinPrior, "dual-wire free==pin3495") && ok;
    ...
    for (const uint32 id : composeIds)
    {
        ...
        ok             = expect(got == pinCanClaimTransport3495(id), "compose pin3495") && ok;
    }
    ...
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport(99),
         ...
         ok;
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport3495(99),
                "formula busy free == pin3495") &&
         ok;
