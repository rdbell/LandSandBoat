#include "test_assault_issue_tag_3886.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3886 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3886 expand residual 2867 / prior 3841 stock / 3796 / 3751 / 3706 / 3661 / 3616 / 3571 / 3524 / 3388 / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3886:
//   tagStock > 0
...
// Prior dedicated 3841 pin (independence cross-check):
//   tagStock > 0
auto pinCanIssueTagFromStock3841(const int32 tagStock) -> bool
{
    return tagStock > 0;
}

// Prior dedicated 3796 pin (independence cross-check):
//   tagStock > 0
auto pinCanIssueTagFromStock3796(const int32 tagStock) -> bool
{
    return tagStock > 0;
}

// Prior dedicated 3751 pin (independence cross-check):
//   tagStock > 0
auto pinCanIssueTagFromStock3751(const int32 tagStock) -> bool
{
    return tagStock > 0;
}

// Prior dedicated 3706 pin (independence cross-check):
//   tagStock > 0
auto pinCanIssueTagFromStock3706(const int32 tagStock) -> bool
{
    return tagStock > 0;
}

// Prior dedicated 3661 pin (independence cross-check):
//   tagStock > 0
auto pinCanIssueTagFromStock3661(const int32 tagStock) -> bool
{
    return tagStock > 0;
}
...
// internal/assault; dedicated slice 3886 expand residual 2867 / prior 3841
// stock / 3796 / 3751 / 3706 / 3661 / 3616 / 3571 / 3524 / 3388 / 3440 ShouldIssueNewTag sibling / pure 1100 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3841 == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 / 3706 / 3751 / 3796 / 3841 stock pins still hold
...
// (prior stock), test_assault_issue_tag_3661 (prior stock),
// test_assault_issue_tag_3706 (prior stock), test_assault_issue_tag_3751
// (prior stock), test_assault_issue_tag_3796 (prior stock),
// test_assault_issue_tag_3841 (prior stock). Mirrors 3841
// expand-residual suite layout (header + self-test runner; 3841
// CanIssueTagFromStock content; no CMake/main).
auto runAssaultIssueTag3886SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 / 3706 / 3751 / 3796 / 3841 stock poles still hold.
    ...
    // --- Composition table: free == inline == pin == pin3841 == compose ---
    const struct
    {
        int32       tagStock;
        bool        want;
        ...
        // Prior dedicated stock expand residual 3661 re-pins.
        { 0, false, "prior 3661 stock 0" },
        { 1, true, "prior 3661 stock 1" },
        { 3, true, "prior 3661 MaxTagStockDefault" },
        { 4, true, "prior 3661 MaxTagStockFull" },
        // Prior dedicated stock expand residual 3706 re-pins.
        { 0, false, "prior 3706 stock 0" },
        { 1, true, "prior 3706 stock 1" },
        { 3, true, "prior 3706 MaxTagStockDefault" },
        { 4, true, "prior 3706 MaxTagStockFull" },
        // Prior dedicated stock expand residual 3751 re-pins.
        { 0, false, "prior 3751 stock 0" },
        { 1, true, "prior 3751 stock 1" },
        { 3, true, "prior 3751 MaxTagStockDefault" },
        { 4, true, "prior 3751 MaxTagStockFull" },
        // Prior dedicated stock expand residual 3796 re-pins.
        { 0, false, "prior 3796 stock 0" },
        { 1, true, "prior 3796 stock 1" },
        { 3, true, "prior 3796 MaxTagStockDefault" },
        { 4, true, "prior 3796 MaxTagStockFull" },
        // Prior dedicated stock expand residual 3841 re-pins.
        { 0, false, "prior 3841 stock 0" },
        { 1, true, "prior 3841 stock 1" },
        { 3, true, "prior 3841 MaxTagStockDefault" },
        { 4, true, "prior 3841 MaxTagStockFull" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3841 = pinCanIssueTagFromStock3841(c.tagStock);
        ...
        ok = expect(got == pin3841, "dual-wire free==pin3841") && ok;
    ...
    // Prior dedicated 3841 independence: free still matches prior pin.
    ok = expect(CanIssueTagFromStock(1) == pinCanIssueTagFromStock3841(1), "prior 3841 independence stock 1") && ok;
    ok = expect(CanIssueTagFromStock(0) == pinCanIssueTagFromStock3841(0), "prior 3841 independence stock 0") && ok;

    // Prior dedicated 3796 independence: free still matches prior pin.
    ok = expect(CanIssueTagFromStock(1) == pinCanIssueTagFromStock3796(1), "prior 3796 independence stock 1") && ok;
    ok = expect(CanIssueTagFromStock(0) == pinCanIssueTagFromStock3796(0), "prior 3796 independence stock 0") && ok;

    // Prior dedicated 3751 independence: free still matches prior pin.
    ok = expect(CanIssueTagFromStock(1) == pinCanIssueTagFromStock3751(1), "prior 3751 independence stock 1") && ok;
    ok = expect(CanIssueTagFromStock(0) == pinCanIssueTagFromStock3751(0), "prior 3751 independence stock 0") && ok;

    // Prior dedicated 3706 independence: free still matches prior pin.
    ok = expect(CanIssueTagFromStock(1) == pinCanIssueTagFromStock3706(1), "prior 3706 independence stock 1") && ok;
    ok = expect(CanIssueTagFromStock(0) == pinCanIssueTagFromStock3706(0), "prior 3706 independence stock 0") && ok;

    // Prior dedicated 3661 independence: free still matches prior pin.
    ok = expect(CanIssueTagFromStock(1) == pinCanIssueTagFromStock3661(1), "prior 3661 independence stock 1") && ok;
    ok = expect(CanIssueTagFromStock(0) == pinCanIssueTagFromStock3661(0), "prior 3661 independence stock 0") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3841 for stock 0 / 1.
    for (const int32 stock : { 0, 1 })
    {
        ...
        const bool pin3841 = pinCanIssueTagFromStock3841(stock);
        ...
        ok                 = expect(got == pin3841, "pole free == pin3841") && ok;
    }

    // Dense tagStock: free == pin == inline == pin3841 == compose.
    ...
    for (const int32 stock : denseStocks)
    {
        ...
        ok              = expect(got == pinCanIssueTagFromStock3841(stock), "dense free==prior 3841 pin") && ok;
        ok              = expect(got == pinCanIssueTagFromStock3796(stock), "dense free==prior 3796 pin") && ok;
        ok              = expect(got == pinCanIssueTagFromStock3751(stock), "dense free==prior 3751 pin") && ok;
        ok              = expect(got == pinCanIssueTagFromStock3706(stock), "dense free==prior 3706 pin") && ok;
        ok              = expect(got == pinCanIssueTagFromStock3661(stock), "dense free==prior 3661 pin") && ok;
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3841 = pinCanIssueTagFromStock3841(h.tagStock);
        ...
        ok                 = expect(got == pin3841, "host free==pin3841") && ok;
