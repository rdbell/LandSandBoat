#include "test_auctionutils_can_afford_fee_3280.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "auctionutils CanAffordFee 3280 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline production ProofOfPurchase fee formula for dual-wire cross-checks
// (dedicated slice 3280; residual expand 2924):
...
// (slice 3280). Multi-branch: positive if/else only (avoid QF1001 De Morgan
...
auto runAuctionutilsCanAffordFee3280SelfTests() -> bool
{
    ...
    // Residual 1135 / 2924 pins still hold under dual-wire.
