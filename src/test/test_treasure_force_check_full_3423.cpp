#include "test_treasure_force_check_full_3423.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3423 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3423):
...
// Compact dual-wire pin matching Go pinShouldForceCheckOnFullPoolInsert3423:
//   slotAfterFreeScan == PoolSize
...
// slice 3423 expand residual 2981; prior dedicated 3379 retained).
//
// Coverage:
