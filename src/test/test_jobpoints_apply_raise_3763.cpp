#include "test_jobpoints_apply_raise_3763.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "jobpoints ShouldApplyRaiseJobPoint 3763 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline RaiseJobPoint spend-gate formula for dual-wire cross-check
// (dedicated slice 3763):
//   cost != 0 && currentJp >= cost
auto inlineShouldApplyRaiseJobPoint3763(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

// Compact dual-wire pin matching Go pinShouldApplyRaiseJobPoint3763 / C++ capacity:
//   cost != 0 && currentJp >= cost
// Direct return of the formula (not if/else true/false).
auto pinShouldApplyRaiseJobPoint3763(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

// Prior dedicated 3718 pin surface (free == pin3718 independence).
auto pinShouldApplyRaiseJobPoint3718(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

auto inlineShouldApplyRaiseJobPoint3718(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}
...
// dedicated slice 3763 expand residual 3012 / prior dedicated 3219 /
...
//
// Coverage:
//   - free == inline == pin == pin3718 == (cost != 0 && currentJp >= cost)
...
//     prior dedicated 3673 / prior dedicated 3718 pins still hold
...
auto runJobpointsApplyRaise3763SelfTests() -> bool
{
    ...
    // prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 /
    // prior dedicated 3539 / prior dedicated 3583 / prior dedicated 3628 /
    // prior dedicated 3673 / prior dedicated 3718 pins still hold under dual-wire.
    ok = expect(ShouldApplyRaiseJobPoint(1, 1), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718: cost 1 / jp 1 applies") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(0, 100), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718: cost 0 blocks raise") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(10, 9), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718: short JP blocks raise") && ok;
    ok = expect(ShouldApplyRaiseJobPoint(10, 10), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718: exact JP applies") && ok;

    // --- Composition table: free == inline == pin == pin3718 ---
    const struct
    {
        uint8       cost;
        uint16      currentJp;
        ...
        // Residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 /
        // prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 /
        // prior dedicated 3539 / prior dedicated 3583 / prior dedicated 3628 / prior dedicated 3673 / prior dedicated 3718 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldApplyRaiseJobPoint3763(c.cost, c.currentJp);
        const bool pin     = pinShouldApplyRaiseJobPoint3763(c.cost, c.currentJp);
        const bool pin3718 = pinShouldApplyRaiseJobPoint3718(c.cost, c.currentJp);
        const bool wantPin = c.cost != 0 && c.currentJp >= c.cost;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3718.
        ok = expect(got == inlineF && got == pin && got == pin3718, "dual-wire free == inline == pin == pin3718") && ok;
    ...
    // Explicit poles free == inline == pin == pin3718 for cost 0 / exact / short-by-one.
    ...
    for (const uint8 cost : poleCosts)
    {
        if (cost == 0)
        {
            ...
            for (const uint16 jp : zeroJPs)
            {
                ...
                const bool inlineF = inlineShouldApplyRaiseJobPoint3763(cost, jp);
                const bool pin     = pinShouldApplyRaiseJobPoint3763(cost, jp);
                const bool pin3718 = pinShouldApplyRaiseJobPoint3718(cost, jp);
                ok = expect(!got && got == inlineF && got == pin && got == pin3718,
                            "pole cost0 free == inline == pin == pin3718") &&
                     ok;
            }
            continue;
        }
        // exact equality
        {
            ...
            const bool   pin3718 = pinShouldApplyRaiseJobPoint3718(cost, jp);
            ok                   = expect(got && got == inlineF && got == pin && got == pin3718,
                        "pole exact free == inline == pin == pin3718") &&
                 ok;
        }
        // short-by-one
        {
            ...
            const bool   pin3718 = pinShouldApplyRaiseJobPoint3718(cost, jp);
            ok = expect(!got && got == inlineF && got == pin && got == pin3718,
                        "pole short free == inline == pin == pin3718") &&
                 ok;
        }
    }
    ...
    for (const auto& c : hostCases)
    {
        ...
        const bool  inlineF = inlineShouldApplyRaiseJobPoint3763(cost, c.currentJp);
        const bool  pin     = pinShouldApplyRaiseJobPoint3763(cost, c.currentJp);
        const bool  pin3718 = pinShouldApplyRaiseJobPoint3718(cost, c.currentJp);
        const bool  wantPin = cost != 0 && c.currentJp >= cost;

        ok = expect(got == c.wantApply, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3718,
                    "host compose dual-wire free == inline == pin == pin3718") &&
             ok;
    ...
    for (const uint8 cost : costs)
    {
        for (const uint16 jp : jps)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3718,
                        "compose free == inline == pin == pin3718") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3718,
                        "compose extra free == inline == pin == pin3718") &&
                 ok;
        }
    }

    // Residual independence (2803 / 3012 / prior 3219 / prior 3275 / prior 3371 /
    // prior 3421 / prior 3475 / prior 3539 / prior 3583 / prior 3628 / prior 3673 / prior 3718): spend gate
    // is distinct from presence short-circuit and JobPointCost formula that compose
    // PlanRaiseJobPoint.
    // Prior dedicated 3718 independence: free == prior suite inline/pin forms
    // (local mirrors of test_jobpoints_apply_raise_3718 pin surface).
    for (const uint8 cost : costs)
    {
        for (const uint16 jp : jps)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3718(cost, jp),
                        "prior 3718 independence free == 3673 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3718(cost, jp),
                        "prior 3718 independence free == 3673 pin") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3718(cost, jp),
                        "prior 3718 independence extra free == 3673 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3718(cost, jp),
                        "prior 3718 independence extra free == 3673 pin") &&
                 ok;
        }
    }
    {
