#include "test_petutils_pet_engage_3670.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petutils ShouldPetEngage 3670 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AttackTarget engage formula for dual-wire cross-check (dedicated 3670
// expand residual 3051; prior dedicated 3625 / 3580 / 3536 / 3484 / 3429 / 3375):
...
// Compact dual-wire pin matching Go pinShouldPetEngage3670 / C++ capacity:
//   !hasPreventActionEffect
...
// Prior dedicated 3625 pin for free == inline == pin == pin3625 cross-check.
...
// dedicated slice 3670 expand residual 3051; prior dedicated 3625 / 3580 / 3536 / 3484 / 3429 / 3375; pure 1627).
//
// Coverage:
//   - free == inline == pin == pin3625 == !hasPreventActionEffect
...
auto runPetutilsPetEngage3670SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3625 ---
    const struct
    {
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3625 = pinShouldPetEngage3625(c.hasPreventActionEffect);
        const bool wantPin = !c.hasPreventActionEffect;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3625.
        ok = expect(got == inlineF && got == pin && got == pin3625,
                    "dual-wire free == inline == pin == pin3625") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula !hasPreventActionEffect") && ok;
    }

    // Free == pin == pin3625 across residual poles.
    ...
    ok = expect(ShouldPetEngage(false) == pinShouldPetEngage3625(false), "free == pin3625 no prevent") && ok;
    ok = expect(ShouldPetEngage(true) == pinShouldPetEngage3625(true), "free == pin3625 prevent") && ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3625.
    for (const bool hasPrevent : { false, true })
    {
        ...
        const bool pin3625 = pinShouldPetEngage3625(hasPrevent);
        const bool want    = !hasPrevent;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3625,
                    "compose free == inline == pin == pin3625") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3625 for AttackTarget.
    for (const bool hasPrevent : { false, true })
    {
        const bool got     = ShouldPetEngage(hasPrevent);
        const bool inlineF = inlineShouldPetEngage(hasPrevent);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3625,
                    "host inject free == inline == pin == pin3625") &&
             ok;
