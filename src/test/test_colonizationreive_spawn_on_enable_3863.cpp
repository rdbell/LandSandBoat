#include "test_colonizationreive_spawn_on_enable_3863.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "colonizationreive ShouldSpawnOnEnable 3863 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua enableReive spawn gate for dual-wire cross-check
// (dedicated 3863 expand residual 2882 / prior 3818 / prior 3773 / prior 3728 /
// prior 3683 / prior 3638 / prior 3593 / prior 3499 / prior 3465 / prior 3411 /
// prior 3335 / prior 3163):
...
// Compact dual-wire pin matching Go pinShouldSpawnOnEnable3863 / C++ capacity
// (return !isAlive):
//   !isAlive
...
// Prior dedicated 3818 pin (independence / free==inline==pin==pin3818):
//   !isAlive
auto pinShouldSpawnOnEnable3818(const bool isAlive) -> bool
{
    return !isAlive;
}

// Prior dedicated 3773 pin (independence cross-check):
//   !isAlive
auto pinShouldSpawnOnEnable3773(const bool isAlive) -> bool
{
    return !isAlive;
}

// Prior dedicated 3728 pin (independence cross-check):
//   !isAlive
auto pinShouldSpawnOnEnable3728(const bool isAlive) -> bool
{
    return !isAlive;
}

// Prior dedicated 3683 pin (independence cross-check):
//   !isAlive
...
// OmegaXI internal/colonizationreive; dedicated slice 3863 expand residual
// 2882 / prior dedicated 3818 / prior dedicated 3773 / prior dedicated 3728 /
...
//
// Coverage:
//   - free == inline == pin == pin3818 == !isAlive
//   - residual poles: isAlive true / false
//   - dense 2^1 boolean space
//   - residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 / prior 3638 / prior 3683 / prior 3728 / prior 3773 / prior 3818 pins still hold
...
auto runColonizationreiveSpawnOnEnable3863SelfTests() -> bool
{
    ...
    // Residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 / prior 3638 / prior 3683 / prior 3728 / prior 3773 / prior 3818 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3728 re-pins.
        { false, true, "prior 3728 false" },
        { true, false, "prior 3728 true" },

        // Prior dedicated 3773 re-pins.
        { false, true, "prior 3773 false" },
        { true, false, "prior 3773 true" },

        // Prior dedicated 3818 re-pins.
        { false, true, "prior 3818 false" },
        { true, false, "prior 3818 true" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3818 = pinShouldSpawnOnEnable3818(c.isAlive);
        ...
        ok = expect(got == pin3818, "ShouldSpawnOnEnable dual-wire == pin3818") && ok;
    ...
    ok = expect(ShouldSpawnOnEnable(false) == pinShouldSpawnOnEnable3818(false), "free==pin3818 false") && ok;
    ok = expect(ShouldSpawnOnEnable(true) == pinShouldSpawnOnEnable3818(true), "free==pin3818 true") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3818 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3818 = pinShouldSpawnOnEnable3818(pole);
        ok                 = expect(got == !pole, "pole free == !isAlive") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3818, "pole free == pin3818") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3818.
    for (const bool isAlive : { false, true })
    {
        ...
        ok              = expect(got == pinShouldSpawnOnEnable3818(isAlive), "compose free == pin3818") && ok;
    }

    // Prior dedicated 3818 independence: free still matches prior pin.
    for (const bool isAlive : { false, true })
    {
        const bool got      = ShouldSpawnOnEnable(isAlive);
        const bool priorPin = pinShouldSpawnOnEnable3818(isAlive);
        ok                  = expect(got == priorPin, "prior 3818 independence") && ok;
    }

    // Prior dedicated 3773 independence: free still matches prior pin.
    for (const bool isAlive : { false, true })
    {
        const bool got      = ShouldSpawnOnEnable(isAlive);
        const bool priorPin = pinShouldSpawnOnEnable3773(isAlive);
        ok                  = expect(got == priorPin, "prior 3773 independence") && ok;
    }

    // Prior dedicated 3728 independence: free still matches prior pin.
    for (const bool isAlive : { false, true })
    {
        const bool got      = ShouldSpawnOnEnable(isAlive);
        const bool priorPin = pinShouldSpawnOnEnable3728(isAlive);
        ok                  = expect(got == priorPin, "prior 3728 independence") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable(isAlive),
             ...
             ok;
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable3818(isAlive),
                    "host inject free == pin3818") &&
             ok;
