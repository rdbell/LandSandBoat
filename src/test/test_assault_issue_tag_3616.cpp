#include "test_assault_issue_tag_3616.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3616 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3616 expand residual 2867 / prior 3571 stock / 3524 / 3388 / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3616:
//   tagStock > 0
...
// internal/assault; dedicated slice 3616 expand residual 2867 / prior 3571
// stock / 3524 / 3388 / 3440 ShouldIssueNewTag sibling / pure 1100 — formula
// unchanged).
//
// Coverage:
//   - free == inline == pin == pin3571 == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 stock pins still hold
...
// test_assault_issue_tag_3571 (prior stock). Mirrors 3571 expand-residual suite
// layout (header + self-test runner; 3571 CanIssueTagFromStock content; no
// CMake/main).
auto runAssaultIssueTag3616SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 stock poles still hold.
    ...
    // --- Composition table: free == inline == pin == pin3571 == compose ---
    const struct
    {
        int32       tagStock;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3571 = pinCanIssueTagFromStock3571(c.tagStock);
        ...
        ok = expect(got == pin3571, "dual-wire free==pin3571") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3571 for stock 0 / 1.
    for (const int32 stock : { 0, 1 })
    {
        ...
        const bool pin3571 = pinCanIssueTagFromStock3571(stock);
        ...
        ok                 = expect(got == pin3571, "pole free == pin3571") && ok;
    }

    // Dense tagStock: free == pin == inline == pin3571 == compose.
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3571 = pinCanIssueTagFromStock3571(h.tagStock);
        ...
        ok                 = expect(got == pin3571, "host free==pin3571") && ok;
