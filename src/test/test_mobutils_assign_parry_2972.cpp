#include "test_mobutils_assign_parry_2972.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mobutils ShouldAssignParrySkill 2972 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CalculateMobStats / mobsetuphelpers::ShouldAssignParrySkill formula
// for dual-wire cross-check (slice 2972):
//   canParryMod > 0
...
// (MOBMOD_CAN_PARRY > 0 assignment gate; slice 2972).
auto runMobutilsAssignParry2972SelfTests() -> bool
{
    ...
    // Residual 1623 pins still hold under dual-wire.
