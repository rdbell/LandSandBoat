#include "test_artisan_can_claim_scroll_3444.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "artisan CanClaimScroll 3444 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline artisan.lua moogleOnFinish option 99 pure gate for dual-wire
// cross-check (slice 3444 dedicated expand residual 2916; prior dedicated
// 3396 / 3132):
...
// Compact dual-wire pin matching free function / capacity body (slice 3444).
// Direct return only — same formula as production CanClaimScroll.
auto pinCanClaimScroll3444(const int64 nextScroll, const int64 jstMidnight) -> bool
{
    return nextScroll < jstMidnight;
}
...
// slice 3444 dedicated expand residual 2916; prior dedicated 3396 / 3132).
// Formula unchanged.
//
// Coverage:
...
//   - residual 0948 / 2916 / prior dedicated 3396 / 3132 pins still hold
auto runArtisanCanClaimScroll3444SelfTests() -> bool
{
    ...
    // Residual 0948 / 2916 / prior dedicated 3396 / 3132 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        { 100, 200, true, "prior3132 nextScroll < midnight" },
        { 200, 200, false, "prior3132 equal timestamps" },
        { 300, 200, false, "prior3132 nextScroll > midnight" },
        { 0, 1, true, "prior3132 never claimed" },
        { 1783090799, 1783090800, true, "prior3132 JST midnight-1" },
        { 1783090800, 1783090800, false, "prior3132 exact JST midnight" },
        { 1783090801, 1783090800, false, "prior3132 JST midnight+1" },

        // Prior dedicated expand 3396 poles.
        { 100, 200, true, "prior3396 nextScroll < midnight" },
        { 200, 200, false, "prior3396 equal timestamps" },
        { 300, 200, false, "prior3396 nextScroll > midnight" },
        { 0, 1, true, "prior3396 never claimed" },
        { 1783090799, 1783090800, true, "prior3396 JST midnight-1" },
        { 1783090800, 1783090800, false, "prior3396 exact JST midnight" },
        { 1783090801, 1783090800, false, "prior3396 JST midnight+1" },
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimScroll3444(p.nextScroll, p.jstMidnight);
    ...
    ok = expect(CanClaimScroll(100, 200) == pinCanClaimScroll3444(100, 200), "free==pin nextScroll < midnight") && ok;
    ok = expect(CanClaimScroll(200, 200) == pinCanClaimScroll3444(200, 200), "free==pin equal timestamps") && ok;
    ok = expect(CanClaimScroll(300, 200) == pinCanClaimScroll3444(300, 200), "free==pin nextScroll > midnight") && ok;
    ok = expect(CanClaimScroll(0, 1) == pinCanClaimScroll3444(0, 1), "free==pin never claimed") && ok;
    ...
    for (int64 d = -50; d <= 50; ++d)
    {
        ...
        const bool  pin     = pinCanClaimScroll3444(next, midnight);
