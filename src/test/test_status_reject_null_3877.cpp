#include "test_status_reject_null_3877.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "status effect ShouldRejectNullStatusEffect 3877 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddStatusEffect null-effect formula for dual-wire cross-check
// (dedicated slice 3877 expand residual 3080; prior dedicated 3832 / 3787 / 3742 / 3697 / 3652 / 3607 / 3562 / 3526 / 3481 / 3428 / 3348):
//   isNull
...
// Compact dual-wire pin matching Go pinShouldRejectNullStatusEffect3877 /
// C++ capacity identity form (formula unchanged from 1371 / 3080 / 3348 / 3428 / 3481 / 3526 / 3562 / 3607 / 3652 / 3697 / 3742 / 3787 / 3832):
//   isNull
...
// Prior dedicated 3832 pin (retained) for free==inline==pin==pin3832.
auto pinShouldRejectNullStatusEffect3832(const bool isNull) -> bool
{
    return isNull;
}
...
// (isNull identity; OmegaXI internal/statuseffect; dedicated slice 3877
// expand residual 3080 / prior dedicated 3832 / 3787 / 3742 / 3697 / 3652 / 3607 / 3562 / 3526 / 3481 / 3428 / 3348 / pure 1371).
//
// Coverage:
//   - free == inline == pin == pin3832 identity form
//   - poles: null rejects; non-null proceeds
//   - residual 3080 / 1371 pins still hold
//   - prior 3348 / 3428 / 3481 / 3526 / 3562 / 3607 / 3652 / 3697 / 3742 / 3787 / 3832 dedicated poles still hold
...
auto runStatusRejectNull3877SelfTests() -> bool
{
    ...
    // --- Core poles: free == inline == pin == pin3832 identity form ---
    const struct
    {
        bool        isNull;
        bool        want;
        ...
        // Prior dedicated 3832 poles still hold.
        { true, true, "prior 3832 null rejects" },
        { false, false, "prior 3832 non-null proceeds" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3832 = pinShouldRejectNullStatusEffect3832(c.isNull);
        ...
        ok = expect(got == pin3832, "dual-wire free==pin3832") && ok;
    ...
    // Free == pin3832 across residual poles (prior dedicated retained).
    ok = expect(ShouldRejectNullStatusEffect(true) == pinShouldRejectNullStatusEffect3832(true),
                "free==pin3832 null effect") &&
         ok;
    ok = expect(ShouldRejectNullStatusEffect(false) == pinShouldRejectNullStatusEffect3832(false),
                "free==pin3832 non-null effect") &&
         ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3832.
    for (const bool isNull : { false, true })
    {
        ...
        const bool pin3832 = pinShouldRejectNullStatusEffect3832(isNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3832, "compose free==pin3832") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3832, "host compose free==pin3832") && ok;
    }
    ...
    // 3562 / 3607 / 3652 / 3697 / 3742 / 3787 / 3832; 3049 / 3069 / 3113 / 3135 left alone): null reject is distinct from
    ...
    for (const bool isNull : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3832,
                    "dense free==inline==pin==pin3832") &&
             ok;
        ok = expect(ShouldExpireEffect(true, 100, 100),
