#include "test_alliance_set_local_main_3077.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldSetLocalMainParty 3077 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline assignAllianceLeader local main-party formula for dual-wire
// cross-check (slice 3077):
...
// (memberFoundOnThisServer identity; slice 3077).
auto runAllianceSetLocalMain3077SelfTests() -> bool
{
    ...
    // Residual 1346 / prior dual-wire 2988 pins still hold under dual-wire.
