#include "test_abyssea_can_give_nmki_3549.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "abyssea CanGiveNMKI 3549 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.abyssea.canGiveNMKI formula for dual-wire checks (slice 3549).
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3549 / C++ capacity
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3491 (prior dedicated
// expand residual 2861 independence pins).
auto pinCanGiveNMKI3491(const int32 roll1to100, const int32 dropChance, const bool redProc) -> bool
{
    return roll1to100 <= dropChance || redProc;
}
...
// dedicated slice 3549 expand residual 2861 / prior 3491 / 3434 / 3314 /
// 3284 / 3238 / 3089 — formula unchanged).
//
// Coverage (clones 3491 density on CanGiveNMKI primary + prior 3491 independence):
...
//   - residual 1041 / 2861 / prior 3089 / prior 3238 / 3284 / 3314 / 3434 / 3491 pins still hold
...
auto runAbysseaCanGiveNMKI3549SelfTests() -> bool
{
    ...
    // Residual constant / 1041 / 2861 / 3089 / 3238 / 3284 / 3314 / 3434 / 3491 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 2861 / 3089 / 3434 / 3491 poles.
    ...
    // Prior dedicated expand residual 3491 pin helpers still agree with free dual-wire.
    ok = expect(CanGiveNMKI(20, 20, false) == pinCanGiveNMKI3491(20, 20, false),
                "free == prior 3491 pin boundary") &&
         ok;
    ok = expect(CanGiveNMKI(21, 20, false) == pinCanGiveNMKI3491(21, 20, false),
                "free == prior 3491 pin short") &&
         ok;
    ok = expect(CanGiveNMKI(100, 0, true) == pinCanGiveNMKI3491(100, 0, true),
                "free == prior 3491 pin red-proc force") &&
         ok;
    ok = expect(pinCanGiveNMKI(20, 20, false) == pinCanGiveNMKI3491(20, 20, false),
                "3549 pin == prior 3491 pin boundary") &&
         ok;
    ok = expect(pinCanGiveNMKI(21, 20, false) == pinCanGiveNMKI3491(21, 20, false),
                "3549 pin == prior 3491 pin short") &&
         ok;
    ok = expect(pinCanGiveNMKI(100, 0, true) == pinCanGiveNMKI3491(100, 0, true),
                "3549 pin == prior 3491 pin red-proc force") &&
         ok;
