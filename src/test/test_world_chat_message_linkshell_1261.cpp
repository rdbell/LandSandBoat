#include "test_world_chat_message_linkshell_1261.h"

#include "world/chat_message_linkshell.h"
...
constexpr auto linkshellEndpointsQuery = "SELECT server_addr, server_port FROM accounts_sessions "
...
auto expect(const bool condition, const char* label) -> bool
{
    if (!condition)
    {
        std::cerr << "world ChatMessageLinkshell 1261 self-test failed: " << label << '\n';
    }
    return condition;
}

auto sameMessage(const ipc::ChatMessageLinkshell& left, const ipc::ChatMessageLinkshell& right) -> bool
{
    return left.linkshellId == right.linkshellId && left.senderId == right.senderId && left.senderName == right.senderName &&
...
auto testFanout() -> bool
{
    ...
    const std::vector<IPP> endpoints{ IPP{}, IPP(0x0C0B0A09, 23456), IPP{} };
    int                    lookupCalls{};
    std::vector<IPP>       sentEndpoints{};
    std::vector<ipc::ChatMessageLinkshell> sentMessages{};
    worldipc::HandleChatMessageLinkshell(
        message,
        [&](const uint32 linkshellId)
        {
            ++lookupCalls;
            return linkshellId == message.linkshellId ? endpoints : std::vector<IPP>{};
        },
        [&](const IPP& endpoint, const ipc::ChatMessageLinkshell& delivered)
        {
    ...
    worldipc::HandleChatMessageLinkshell(
        ...
        {
            ++emptyLookups;
            return linkshellId == 0 ? std::vector<IPP>{} : endpoints;
        },
        [&](const IPP&, const ipc::ChatMessageLinkshell&) { ++emptySends; });
...
auto endpointRows(std::vector<db::LibMariaDBResultSet::Row> rows) -> std::unique_ptr<db::ResultSet>
{
    ...
    return std::make_unique<db::LibMariaDBResultSet>(linkshellEndpointsQuery, std::move(schema), std::move(rows));
}
...
auto testDatabaseLookup() -> bool
{
    DatabaseGuard guard;
    FakeDatabase  fake;
    fake.results.push_back(endpointRows({ { uint64{ 0x0C0B0A09 }, uint64{ 23456 } }, { uint64{}, uint64{} } }));
    fake.results.push_back(endpointRows({}));
    fake.results.push_back(nullptr);
    db::setDatabase(&fake);

    const auto found   = world::ipc::LookupLinkshellEndpoints(std::numeric_limits<uint32>::max());
    const auto missing = world::ipc::LookupLinkshellEndpoints(0);
    const auto failed  = world::ipc::LookupLinkshellEndpoints(7);
    const auto bound = [&](const std::size_t index, const uint32 expected)
    {
        return fake.bindings[index].size() == 2 && std::holds_alternative<uint32>(fake.bindings[index][0]) &&
               std::get<uint32>(fake.bindings[index][0]) == expected && std::holds_alternative<uint32>(fake.bindings[index][1]) &&
               std::get<uint32>(fake.bindings[index][1]) == expected;
    };

    return expect(found.size() == 2 && found[0].getIP() == 0x0C0B0A09 && found[0].getPort() == 23456 &&
                      found[1].getRawIPP() == 0,
                  "ordered database rows and zero endpoint") &&
           expect(missing.empty() && failed.empty(), "empty and failed results") &&
           expect(fake.queries == std::vector<std::string>{ linkshellEndpointsQuery, linkshellEndpointsQuery, linkshellEndpointsQuery },
                  "exact linkshell SQL") &&
           expect(bound(0, std::numeric_limits<uint32>::max()) && bound(1, 0) && bound(2, 7), "linkshell id bound twice");
}
