#include "test_treasure_force_check_full_3533.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3533 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3533):
...
// slice 3533 expand residual 2981; prior dedicated 3477 / 3423 / 3379 retained).
//
// Coverage:
//   - free == inline == pin == (slotAfterFreeScan == PoolSize)
//   - residual 1367 / 2981 pins still hold
//   - prior dedicated 3477 / 3423 / 3379 poles still hold
...
auto runTreasureForceCheckFull3533SelfTests() -> bool
{
    ...
    // Dense compose over slot domain around PoolSize (0..20) — free == inline == pin.
    ...
    for (uint8 slot = 0; slot < PoolSize; ++slot)
    {
        ...
        ok                 = expect(got == inlineF && got == pin, "host inject free-found free == inline == pin") && ok;
    }
    {
        ...
        ok                 = expect(got == inlineF && got == pin, "host inject PoolSize free == inline == pin") && ok;
    }
