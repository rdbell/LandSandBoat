#include "test_assault_auto_complete_3057.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault auto-complete 3057 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onInstanceProgressUpdate formula for dual-wire checks (slice 3057):
...
// (Lua InstanceAssault:onInstanceProgressUpdate complete gate; slice 3057).
auto runAssaultAutoComplete3057SelfTests() -> bool
{
    ...
    // Residual 1078 / 2860 pins still hold under dual-wire.
