#include "test_follow_can_follow_3405.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3405 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3405 dedicated expand residual 2883; prior dedicated 3084 / 3237 / 3331):
...
// Compact dual-wire pin matching free function / capacity body (slice 3405).
// Direct AND/EQ form only (no De Morgan) — same formula as production CanFollow.
auto pinCanFollow3405(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3405 dedicated
// expand residual 2883; prior dedicated 3084 / 3237 / 3331). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 poles still hold
...
auto runFollowCanFollow3405SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3405(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3405(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
