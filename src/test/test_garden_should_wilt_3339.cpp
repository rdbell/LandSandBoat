#include "test_garden_should_wilt_3339.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garden should wilt 3339 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// (dedicated slice 3339 expand residual 2926):
...
auto inlineShouldWilt3339(const uint32 stageDurationVanadays, const uint32 daysSinceStageChange,
...
// Compact dual-wire pin matching free function / capacity body (slice 3339).
// Positive early-return form only — avoid QF1001 De Morgan rewrites of the
// production compound OR.
auto pinShouldWilt3339(const uint32 stageDurationVanadays, const uint32 daysSinceStageChange,
                       const uint32 wiltBonus, const bool wasExamined) -> bool
{
...
// internal/gardenutils; slice 3339 dedicated expand residual 2926;
// residual pure 0795 — formula UNCHANGED / bit-identical).
//
// Coverage:
//   - free == inline == pin (positive capacity early-return form)
//   - residual 2926 / 0795 pins still hold
//   - pin matches capacity body (no De Morgan QF1001 rewrites)
// Residual suite remains: test_garden_should_wilt_2926 /
// runGardenShouldWilt2926SelfTests.
auto runGardenShouldWilt3339SelfTests() -> bool
{
    ...
    // Residual 0795 / 2926 pins still hold under dedicated dual-wire.
    ...
    const struct
    {
        ...
        // Extra boundary / zero cases (match residual 2926 table).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineGot = inlineShouldWilt3339(c.stageDuration, c.daysSince, c.wiltBonus, c.wasExamined);
        const bool pinGot    = pinShouldWilt3339(c.stageDuration, c.daysSince, c.wiltBonus, c.wasExamined);
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool inlineGot = inlineShouldWilt3339(pole.stageDuration, pole.daysSince, pole.wiltBonus, pole.wasExamined);
        const bool pinGot    = pinShouldWilt3339(pole.stageDuration, pole.daysSince, pole.wiltBonus, pole.wasExamined);
