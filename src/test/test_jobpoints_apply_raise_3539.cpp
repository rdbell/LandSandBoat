#include "test_jobpoints_apply_raise_3539.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "jobpoints ShouldApplyRaiseJobPoint 3539 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline RaiseJobPoint spend-gate formula for dual-wire cross-check
// (dedicated slice 3539):
...
// Compact dual-wire pin matching Go pinShouldApplyRaiseJobPoint3539 / C++ capacity:
//   cost != 0 && currentJp >= cost
...
// dedicated slice 3539 expand residual 3012 / prior dedicated 3219 /
// prior dedicated 3275 / prior dedicated 3371 / prior dedicated 3421 /
// prior dedicated 3475).
//
// Coverage:
//   - free == inline == pin == (cost != 0 && currentJp >= cost)
//   - residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 /
//     prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 pins
//     still hold
...
auto runJobpointsApplyRaise3539SelfTests() -> bool
{
    ...
    ok = expect(ShouldApplyRaiseJobPoint(1, 1), "residual 2803/3012/3219/3275/3371/3421/3475: cost 1 / jp 1 applies") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(0, 100), "residual 2803/3012/3219/3275/3371/3421/3475: cost 0 blocks raise") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(10, 9), "residual 2803/3012/3219/3275/3371/3421/3475: short JP blocks raise") && ok;
    ok = expect(ShouldApplyRaiseJobPoint(10, 10), "residual 2803/3012/3219/3275/3371/3421/3475: exact JP applies") && ok;
    ...
    const struct
    {
        ...
        // Residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 /
        // prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldApplyRaiseJobPoint3539(c.cost, c.currentJp);
        const bool pin     = pinShouldApplyRaiseJobPoint3539(c.cost, c.currentJp);
    ...
    // Explicit poles free == inline == pin for cost 0 / exact / short-by-one.
    ...
    for (const uint8 cost : poleCosts)
    {
        if (cost == 0)
        {
            ...
            for (const uint16 jp : zeroJPs)
            {
                ...
                const bool inlineF = inlineShouldApplyRaiseJobPoint3539(cost, jp);
                const bool pin     = pinShouldApplyRaiseJobPoint3539(cost, jp);
                ok = expect(!got && got == inlineF && got == pin, "pole cost0 free == inline == pin") && ok;
            }
            continue;
        }
        // exact equality
        {
            ...
            const bool   inlineF = inlineShouldApplyRaiseJobPoint3539(cost, jp);
            const bool   pin     = pinShouldApplyRaiseJobPoint3539(cost, jp);
            ok                   = expect(got && got == inlineF && got == pin, "pole exact free == inline == pin") && ok;
        }
        // short-by-one
        {
            ...
            const bool   inlineF = inlineShouldApplyRaiseJobPoint3539(cost, jp);
            const bool   pin     = pinShouldApplyRaiseJobPoint3539(cost, jp);
            ok = expect(!got && got == inlineF && got == pin, "pole short free == inline == pin") && ok;
        }
    }
    ...
    for (const auto& c : hostCases)
    {
        ...
        const bool  inlineF = inlineShouldApplyRaiseJobPoint3539(cost, c.currentJp);
        const bool  pin     = pinShouldApplyRaiseJobPoint3539(cost, c.currentJp);
    ...
    for (const uint8 cost : costs)
    {
        ...
        for (const uint16 jp : extraJPs)
        {
            ...
            ok                 = expect(got == inlineF && got == pin, "compose extra free == inline == pin") && ok;
        }
    }

    // Residual independence (2803 / 3012 / prior 3219 / prior 3275 / prior 3371 /
    // prior 3421 / prior 3475): spend gate is distinct from presence short-circuit
    // and JobPointCost formula that compose PlanRaiseJobPoint.
    // Prior dedicated 3475 independence: free == prior suite inline/pin forms
    // (local mirrors of test_jobpoints_apply_raise_3475 pin surface).
    auto inlineShouldApplyRaiseJobPoint3475 = [](const uint8 cost, const uint16 currentJp) -> bool {
        return cost != 0 && currentJp >= cost;
    };
    auto pinShouldApplyRaiseJobPoint3475 = [](const uint8 cost, const uint16 currentJp) -> bool {
        return cost != 0 && currentJp >= cost;
    };
    for (const uint8 cost : costs)
    {
        ...
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3475(cost, jp),
                        "prior 3475 independence free == 3475 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3475(cost, jp),
                        "prior 3475 independence free == 3475 pin") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3475(cost, jp),
                        "prior 3475 independence extra free == 3475 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3475(cost, jp),
                        "prior 3475 independence extra free == 3475 pin") &&
                 ok;
        }
    }
    {
