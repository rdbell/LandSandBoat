#include "test_alliance_attempt_leader_promote_3816.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3816 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check
// (slice 3816 dedicated dual-wire expand residual 2979):
//   isMainParty
...
// Prior dedicated expand residual 2979 pin (slice 3771; formula UNCHANGED).
// free == inline == pin == pin3771.
auto pinShouldAttemptAllianceLeaderPromote3771(const bool isMainParty) -> bool
{
    return isMainParty;
}

// This slice's dual-wire pin (slice 3816; formula isMainParty identity —
// UNCHANGED). free == inline == pin == pin3771.
...
// (isMainParty identity; slice 3816 dedicated dual-wire expand residual 2979).
//
// Coverage:
//   - main party → attempt promote true
//   - non-main → attempt promote false
//   - free == inline == pin == pin3771 (isMainParty identity)
//   - residual 1341 / 1346 / 2979 / 3144 / 3393 / 3447 / 3490 / 3547 / 3591 / 3636 / 3681 / 3726 / 3771 pins still hold
...
auto runAllianceAttemptLeaderPromote3816SelfTests() -> bool
{
    ...
    // dedicated 3681 / prior dedicated 3726 / prior dedicated 3771 pins still hold.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3816(c.isMainParty);
        const bool pin3771 = pinShouldAttemptAllianceLeaderPromote3771(c.isMainParty);

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3771,
                    "ShouldAttemptAllianceLeaderPromote free == inline == pin == pin3771") &&
             ok;
    }
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3771.
    for (const bool isMainParty : { false, true })
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3816(isMainParty);
        const bool pin3771 = pinShouldAttemptAllianceLeaderPromote3771(isMainParty);
        const bool want    = isMainParty;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3771,
                    "compose free == inline == pin == pin3771") &&
             ok;
    }
    ...
    for (const bool isMainParty : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3771,
                    "host inject dual-wire free == inline == pin == pin3771") &&
             ok;
    }
