#include "test_garrison_fail_by_death_3330.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3330 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3330:
//   allNPCsDead || allPlayersDead
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3330;
...
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 pins still hold
...
auto runGarrisonFailByDeath3330SelfTests() -> bool
{
    ...
    // Residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 pins.
    ...
    // Residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236 wipe gate pins under 3330") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3236(false, false),
                "prior 3236 pin neither dead still holds under 3330") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3236(true, false),
                "prior 3236 pin npcs dead still holds under 3330") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3236(false, true),
                "prior 3236 pin players dead still holds under 3330") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3236(true, true),
                "prior 3236 pin both dead still holds under 3330") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3330.
