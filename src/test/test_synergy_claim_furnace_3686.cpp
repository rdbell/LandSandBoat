#include "test_synergy_claim_furnace_3686.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3686 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3686 dedicated expand residual 2877; prior dedicated expand 3641):
...
// Compact dual-wire pin matching free function / capacity body (slice 3686).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3686 dedicated
// expand residual 2877; prior dedicated expand 3641). Formula unchanged:
//
//   CanClaimFurnace(state) = state == FurnaceAvailable
//
// Coverage:
//   - free == inline == pin == pin3641 (direct return formula)
//   - residual 2877 / 1149 / prior dedicated 3065 / prior expand 3239 /
//     prior expand 3285 / prior expand 3315 / prior expand 3494 /
//     prior expand 3551 / prior expand 3596 / prior expand 3641 poles
//   - residual poles for all furnaceStates + dense 0..8 out-of-range
//   - prior 3641 pin independence
auto runSynergyClaimFurnace3686SelfTests() -> bool
{
    ...
    // expand 3551 / prior expand 3596 / prior expand 3641 pins.
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285 / 3315 / 3494 / 3551 / 3596 / 3641).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3686(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3686(s);
