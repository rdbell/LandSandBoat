#include "test_artisan_can_buy_sack_3865.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "artisan CanBuySack 3865 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline artisan.lua option 1 pure gate for dual-wire cross-check
// (slice 3865 dedicated expand residual 2879; prior dedicated 3820 / 3775 / 3730 / 3685 / 3640 / 3595 / 3550 / 3493 / 3312 / 3282 / 3242 / 3090):
...
// Compact dual-wire pin matching free function / capacity body (slice 3865).
// Direct return only — same formula as production CanBuySack.
auto pinCanBuySack3865(const int32 gil, const int32 sackSize) -> bool
{
    return gil >= artisanhelpers::BuySackGilCost && sackSize == 0;
}

// Prior dedicated 3820 pin for independence cross-check.
auto pinCanBuySack3820(const int32 gil, const int32 sackSize) -> bool
{
    return gil >= artisanhelpers::BuySackGilCost && sackSize == 0;
}
...
//
// Coverage:
//   - free == inline == pin == pin3820 (direct return formula)
...
//     prior expand 3685 / prior expand 3730 / prior expand 3775 /
//     prior expand 3820 pins still hold
//   - prior dedicated 3820 independence (free == pinCanBuySack3820)
...
auto runArtisanCanBuySack3865SelfTests() -> bool
{
    ...
    // Residual constant / 0948 / 2879 / prior dedicated 3090 / 3242 / 3282 / 3312 / 3493 / 3550 / 3595 / 3640 / 3685 / 3730 / 3775 / 3820 pins still hold.
    ...
    // Residual poles: free == inline == pin == pin3820 (direct return).
    const struct
    {
        int32       gil;
        int32       sackSize;
        ...
        // Residual 2879 / 3090 / 3242 / 3282 / 3312 / 3493 / 3550 / 3595 / 3640 / 3685 / 3730 / 3775 / 3820 classic dual poles.
        ...
        // Prior dedicated 3090 / expand 3242 / expand 3282 / expand 3312 / expand 3493 / expand 3550 / expand 3595 / expand 3640 / expand 3685 / expand 3730 / expand 3775 / expand 3820 dense poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanBuySack3865(p.gil, p.sackSize);
        const bool prior   = pinCanBuySack3820(p.gil, p.sackSize);
        ...
        ok = expect(got == prior, "CanBuySack == prior pinCanBuySack3820") && ok;
    }

    // Dense gil edges around cost with empty sack: free == inline == pin == pin3820.
    for (int32 g = BuySackGilCost - 50; g <= BuySackGilCost + 50; ++g)
    {
        const bool got     = CanBuySack(g, 0);
        const bool inlineF = inlineCanBuySack(g, 0);
        const bool pin     = pinCanBuySack3865(g, 0);
        const bool prior   = pinCanBuySack3820(g, 0);
        ...
        ok = expect(got == prior, "dense gil free==prior 3820") && ok;
    }
    ...
    for (const int32 size : { 0, 1, 2, 15, 30, 80, 100, -1 })
    {
        for (const int32 g : { BuySackGilCost - 1, BuySackGilCost, BuySackGilCost + 1, 9979, 9980, 10000, 100000, 0, 1 })
        {
            ...
            const bool pin     = pinCanBuySack3865(g, size);
            const bool prior   = pinCanBuySack3820(g, size);
            ...
            ok = expect(got == prior, "dense gil×size free==prior 3820") && ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3820 for host poles.
    const struct
    {
        int32       gil;
        int32       sackSize;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == prior, "host inject free == prior pin 3820") && ok;
    }
