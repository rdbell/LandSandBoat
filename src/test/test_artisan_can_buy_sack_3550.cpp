#include "test_artisan_can_buy_sack_3550.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "artisan CanBuySack 3550 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline artisan.lua option 1 pure gate for dual-wire cross-check
// (slice 3550 dedicated expand residual 2879; prior dedicated 3493 / 3312 / 3282 / 3242 / 3090):
...
// Compact dual-wire pin matching free function / capacity body (slice 3550).
...
// Prior dedicated 3493 pin for independence cross-check.
auto pinCanBuySack3493(const int32 gil, const int32 sackSize) -> bool
{
    return gil >= artisanhelpers::BuySackGilCost && sackSize == 0;
}
...
//     prior expand 3282 / prior expand 3312 / prior expand 3493 pins still hold
//   - prior dedicated 3493 independence (free == pinCanBuySack3493)
...
auto runArtisanCanBuySack3550SelfTests() -> bool
{
    ...
    // Residual constant / 0948 / 2879 / prior dedicated 3090 / 3242 / 3282 / 3312 / 3493 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 2879 / 3090 / 3242 / 3282 / 3312 / 3493 classic dual poles.
        ...
        // Prior dedicated 3090 / expand 3242 / expand 3282 / expand 3312 / expand 3493 dense poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanBuySack3550(p.gil, p.sackSize);
        const bool prior   = pinCanBuySack3493(p.gil, p.sackSize);
        ...
        ok = expect(got == prior, "CanBuySack == prior pinCanBuySack3493") && ok;
    }

    // Dense gil edges around cost with empty sack: free == inline == pin.
    for (int32 g = BuySackGilCost - 50; g <= BuySackGilCost + 50; ++g)
    {
        const bool got     = CanBuySack(g, 0);
        const bool inlineF = inlineCanBuySack(g, 0);
        const bool pin     = pinCanBuySack3550(g, 0);
        const bool prior   = pinCanBuySack3493(g, 0);
        ...
        ok = expect(got == prior, "dense gil free==prior 3493") && ok;
    }
    ...
    for (const int32 size : { 0, 1, 2, 15, 30, 80, 100, -1 })
    {
        for (const int32 g : { BuySackGilCost - 1, BuySackGilCost, BuySackGilCost + 1, 9979, 9980, 10000, 100000, 0, 1 })
        {
            ...
            const bool pin     = pinCanBuySack3550(g, size);
            const bool prior   = pinCanBuySack3493(g, size);
            ...
            ok = expect(got == prior, "dense gil×size free==prior 3493") && ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin for host poles (+ prior 3493).
    const struct
    {
        int32       gil;
        int32       sackSize;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == prior, "host inject free == prior pin 3493") && ok;
    }
