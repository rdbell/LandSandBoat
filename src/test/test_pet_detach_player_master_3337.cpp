#include "test_pet_detach_player_master_3337.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "pet ShouldDetachPlayerMaster 3337 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CPetEntity::Die detach-player-master formula for dual-wire cross-check
// (dedicated slice 3337) — direct return AND-chain:
...
// Positive if/else pin matching free function / capacity body (slice 3337).
...
// (CPetEntity::Die player-master active-pet detach gate; dedicated slice 3337;
// residual expand 2987 / pure 1414 / 2262).
//
// Coverage:
//   - free == inline == pin (positive if/else form; avoid QF1001)
//   - residual 1414 / 2262 / 2987 pins still hold
//   - dense 2^3 poles
...
auto runPetDetachPlayerMaster3337SelfTests() -> bool
{
    ...
    // Residual 1414 / 2262 / 2987 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1414 / 2262 / 2987 single-false poles.
    ...
    // Sibling residual 3233 still holds (not re-expanded under 3337).
