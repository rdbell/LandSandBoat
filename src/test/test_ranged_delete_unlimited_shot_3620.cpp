#include "test_ranged_delete_unlimited_shot_3620.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranged ShouldDeleteUnlimitedShot 3620 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack UnlimitedShot delete gate for dual-wire cross-check
// (slice 3620 dedicated expand residual 3000; prior 3575 retained):
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto inlineShouldDeleteUnlimitedShot3620(
...
// (dedicated slice 3620). Early-return form only — same formula.
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
...
// Prior 3575 pin retained for independence cross-check (same formula).
// free == inline == pin == pin3575.
...
// slice 3620 dedicated expand residual 3000; prior dedicated 3575 retained).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3575 (early-return formula)
//   - residual poles: no US; hit always delete; miss retain <=0 / >0; residual 1390/3000/3399/3451/3500/3575
//   - dense hasUS × hit × retain compose
//   - prior 3575 independence (suite retained)
...
auto runRangedDeleteUnlimitedShot3620SelfTests() -> bool
{
    ...
    // Residual 1390 / residual dual-wire 3000 / dedicated 3399 / 3451 / 3500 / 3575 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1390 / 3000 / 3399 / 3451 / 3500 / 3575 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDeleteUnlimitedShot3620(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pinF    = pinShouldDeleteUnlimitedShot3620(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pin3575 = pinShouldDeleteUnlimitedShot3575(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
    ...
    // Prior 3575 independence: free == prior 3575 pin poles (suite retained).
    // free == inline == pin == pin3575.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3575(true, true, 5),
                "prior 3575 independence: free == 3575 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3575(false, true, 0),
                "prior 3575 independence: free == 3575 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3575(true, false, 1),
                "prior 3575 independence: free == 3575 pin retain keep") &&
         ok;
    ok = expect(pinShouldDeleteUnlimitedShot3620(true, false, 0) == pinShouldDeleteUnlimitedShot3575(true, false, 0),
                "prior 3575 independence: 3620 pin == 3575 pin miss retain 0") &&
         ok;
    ok = expect(inlineShouldDeleteUnlimitedShot3620(true, true, 5) == pinShouldDeleteUnlimitedShot3575(true, true, 5),
                "prior 3575 independence: 3620 inline == 3575 pin del hit") &&
         ok;

    // Dense compose: hasUnlimitedShot × hitOccured × retain poles — free == inline == pin == pin3575.
    for (const bool hasUS : { false, true })
    {
        for (const bool hit : { false, true })
        {
                                        ...
                                        static_cast<int16>(1), static_cast<int16>(5), static_cast<int16>(100) })
            {
                ...
                ok              = expect(got == inlineShouldDeleteUnlimitedShot3620(hasUS, hit, retain),
                            "compose free == inline") &&
                     ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3620(hasUS, hit, retain), "compose free == pin") && ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3575(hasUS, hit, retain), "compose free == pin3575") && ok;
            }
        }
    }
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pin3575, "compose free == pin3575") && ok;
    }
    ...
    // Prior 3575 independence under compose: free == 3575 pin residual poles.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3575(true, true, 5),
                "prior 3575 independence compose: free == 3575 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3575(false, true, 0),
                "prior 3575 independence compose: free == 3575 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3575(true, false, 1),
                "prior 3575 independence compose: free == 3575 pin retain keep") &&
         ok;
