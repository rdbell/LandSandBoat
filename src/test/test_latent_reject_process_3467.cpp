#include "test_latent_reject_process_3467.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "latent ShouldRejectProcessLatent 3467 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ProcessLatentEffect owner/zone early-return gate for dual-wire
// cross-check (dedicated slice 3467 expand residual 2961; prior dedicated
// 3413 / 3343). Positive OR form:
...
// Compact dual-wire pin matching Go pinShouldRejectProcessLatent3467 /
...
// dedicated slice 3467 expand residual 2961 / pure 1359; prior dedicated
// 3413 / 3343).
//
// Coverage:
//   - free == inline == pin (positive OR form)
//   - residual 1359 / 2961 / prior 3413 / 3343 poles still hold
...
auto runLatentRejectProcess3467SelfTests() -> bool
{
    ...
    // Residual 1359 / 2961 / prior 3413 / 3343 pins still hold under dual-wire.
    ok = expect(ShouldRejectProcessLatent(true, false), "residual 2961/1359/3413/3343: ownerNull must reject") && ok;
    ok = expect(ShouldRejectProcessLatent(false, true), "residual 2961/1359/3413/3343: zoneIsZero must reject") && ok;
    ok = expect(!ShouldRejectProcessLatent(false, false), "residual 2961/1359/3413/3343: neither pole must accept") && ok;
    ok = expect(ShouldRejectProcessLatent(true, true), "residual 2961/1359/3413/3343: both poles must reject") && ok;
    ...
    const struct
    {
        ...
        // Residual 2961 / 1359 / prior 3413 / 3343 re-pins.
    ...
    // Residual 2961 / prior 3413 / 3343 suite still holds under dedicated expand.
    ok = expect(ShouldRejectProcessLatent(true, false), "residual 2961 owner-null pin under 3467") && ok;
    ok = expect(ShouldRejectProcessLatent(false, true), "residual 2961 zone-zero pin under 3467") && ok;
    ok = expect(!ShouldRejectProcessLatent(false, false), "residual 2961 accept pin under 3467") && ok;
    ok = expect(ShouldRejectProcessLatent(true, true), "residual 2961 both-reject pin under 3467") && ok;
