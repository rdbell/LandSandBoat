#include "test_zone_high_targid_3384.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "zone ShouldRejectHighCharTargid 3384 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline IncreaseZoneCounter high-char-targid formula for dual-wire cross-check
// (dedicated slice 3384 expand residual 2949):
...
// Compact dual-wire pin matching Go pinShouldRejectHighCharTargid3384 /
...
// dedicated slice 3384 expand residual 2949 / pure 1363).
//
// Coverage:
//   - free == inline == pin == (targid >= CharTargidHighThreshold)
//   - residual 1363 / 2949 poles still hold
...
auto runZoneHighTargid3384SelfTests() -> bool
{
    ...
    // Residual 1363 / 2949 pins still hold under dual-wire.
