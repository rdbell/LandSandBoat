#include "test_ranged_delete_unlimited_shot_3665.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranged ShouldDeleteUnlimitedShot 3665 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack UnlimitedShot delete gate for dual-wire cross-check
// (slice 3665 dedicated expand residual 3000; prior 3620 retained):
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto inlineShouldDeleteUnlimitedShot3665(
...
// (dedicated slice 3665). Early-return form only — same formula.
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
...
// Prior 3620 pin retained for independence cross-check (same formula).
// free == inline == pin == pin3620.
auto pinShouldDeleteUnlimitedShot3620(
    const bool hasUnlimitedShot,
    const bool hitOccured,
    const int16 retainUnlimitedShotMod) -> bool
{
...
// slice 3665 dedicated expand residual 3000; prior dedicated 3620 retained).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3620 (early-return formula)
//   - residual poles: no US; hit always delete; miss retain <=0 / >0; residual 1390/3000/3399/3451/3500/3575/3620
//   - dense hasUS × hit × retain compose
//   - prior 3620 independence (suite retained)
...
auto runRangedDeleteUnlimitedShot3665SelfTests() -> bool
{
    ...
    // Residual 1390 / residual dual-wire 3000 / dedicated 3399 / 3451 / 3500 / 3575 / 3620 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1390 / 3000 / 3399 / 3451 / 3500 / 3575 / 3620 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDeleteUnlimitedShot3665(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pinF    = pinShouldDeleteUnlimitedShot3665(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pin3620 = pinShouldDeleteUnlimitedShot3620(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
    ...
    // Prior 3620 independence: free == prior 3620 pin poles (suite retained).
    // free == inline == pin == pin3620.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3620(true, true, 5),
                "prior 3620 independence: free == 3620 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3620(false, true, 0),
                "prior 3620 independence: free == 3620 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3620(true, false, 1),
                "prior 3620 independence: free == 3620 pin retain keep") &&
         ok;
    ok = expect(pinShouldDeleteUnlimitedShot3665(true, false, 0) == pinShouldDeleteUnlimitedShot3620(true, false, 0),
                "prior 3620 independence: 3665 pin == 3620 pin miss retain 0") &&
         ok;
    ok = expect(inlineShouldDeleteUnlimitedShot3665(true, true, 5) == pinShouldDeleteUnlimitedShot3620(true, true, 5),
                "prior 3620 independence: 3665 inline == 3620 pin del hit") &&
         ok;

    // Dense compose: hasUnlimitedShot × hitOccured × retain poles — free == inline == pin == pin3620.
    for (const bool hasUS : { false, true })
    {
        for (const bool hit : { false, true })
        {
                                        ...
                                        static_cast<int16>(1), static_cast<int16>(5), static_cast<int16>(100) })
            {
                ...
                ok              = expect(got == inlineShouldDeleteUnlimitedShot3665(hasUS, hit, retain),
                            "compose free == inline") &&
                     ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3665(hasUS, hit, retain), "compose free == pin") && ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3620(hasUS, hit, retain), "compose free == pin3620") && ok;
            }
        }
    }
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pin3620, "compose free == pin3620") && ok;
    }
    ...
    // Prior 3620 independence under compose: free == 3620 pin residual poles.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3620(true, true, 5),
                "prior 3620 independence compose: free == 3620 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3620(false, true, 0),
                "prior 3620 independence compose: free == 3620 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3620(true, false, 1),
                "prior 3620 independence compose: free == 3620 pin retain keep") &&
         ok;
