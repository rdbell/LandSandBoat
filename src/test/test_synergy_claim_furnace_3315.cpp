#include "test_synergy_claim_furnace_3315.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3315 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3315 dedicated expand residual 2877; prior dedicated expand 3285):
...
// Compact dual-wire pin matching free function / capacity body (slice 3315).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3315 dedicated
// expand residual 2877; prior dedicated expand 3285). Formula unchanged:
//
...
auto runSynergyClaimFurnace3315SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3315(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3315(s);
