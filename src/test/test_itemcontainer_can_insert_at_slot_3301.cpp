#include "test_itemcontainer_can_insert_at_slot_3301.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "itemcontainer CanInsertAtSlot 3301 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertItem(PItem, SlotID) range gate for dual-wire cross-check (dedicated 3301):
//   slotID <= size
...
// Compact dual-wire pin matching Go pinCanInsertAtSlot3301 / C++ capacity:
//   slotID <= size
...
// dedicated slice 3301 expand residual 2942).
//
// Coverage:
//   - free == inline == pin == (slotID <= size)
//   - residual 2802 / 2942 / prior 3270 / prior 3194 pins still hold
