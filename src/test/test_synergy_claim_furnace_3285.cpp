#include "test_synergy_claim_furnace_3285.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3285 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3285 dedicated expand residual 2877; prior dedicated expand 3239):
...
// Compact dual-wire pin matching free function / capacity body (slice 3285).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3285 dedicated
// expand residual 2877; prior dedicated expand 3239). Formula unchanged:
//
//   CanClaimFurnace(state) = state == FurnaceAvailable
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2877 / 1149 / prior dedicated 3065 / prior expand 3239 poles
...
auto runSynergyClaimFurnace3285SelfTests() -> bool
{
    ...
    // Residual 1149 / 2877 / prior dedicated 3065 / prior expand 3239 pins.
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / prior 3239).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3285(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3285(s);
