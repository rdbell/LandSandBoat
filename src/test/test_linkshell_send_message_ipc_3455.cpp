#include "test_linkshell_send_message_ipc_3455.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "linkshell ShouldSendLinkshellMessageIPC 3455 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// (slice 3455):
//   messageNonEmpty
auto inlineShouldSendLinkshellMessageIPC3455(const bool messageNonEmpty) -> bool
{
    return messageNonEmpty;
}

// Direct-return dual-wire pin matching free / capacity body (slice 3455).
// Identity form only — free == inline == pin == messageNonEmpty.
// Formula unchanged from residual 2977 / pure 1354 / 2171 / prior dedicated 3403.
auto pinShouldSendLinkshellMessageIPC3455(const bool messageNonEmpty) -> bool
{
    return messageNonEmpty;
}
...
// Prior dedicated dual-wire 3403 pin (retained suite form) for residual re-pins.
...
// (messageNonEmpty identity; dedicated slice 3455; residual expand 2977 /
// pure 1354 / 2171; prior dedicated expand 3403).
//
// Coverage:
//   - free == inline == pin (direct return identity)
//   - residual 2977 / 1354 / 2171 / prior dedicated 3403 pins still hold
...
auto runLinkshellSendMessageIPC3455SelfTests() -> bool
{
    ...
    // Residual 1354 / 2171 / prior residual dual-wire 2977 / prior dedicated
    // 3403 pins still hold.
    ...
    ok = expect(ShouldSendLinkshellMessageIPC(true), "prior dedicated 3403: non-empty sends IPC") && ok;
    ok = expect(!ShouldSendLinkshellMessageIPC(false), "prior dedicated 3403: empty skips IPC") && ok;
    ok = expect(pinShouldSendLinkshellMessageIPC3403(true), "prior dedicated 3403 pin non-empty") && ok;
    ok = expect(!pinShouldSendLinkshellMessageIPC3403(false), "prior dedicated 3403 pin empty") && ok;
    ...
    const struct
    {
        ...
        // Residual 2977 / 1354 / 2171 / prior dedicated 3403 re-pins.
        ...
        { true, true, "prior dedicated 3403 non-empty sends IPC" },
        { false, false, "prior dedicated 3403 empty skips IPC" },
    };

    for (const auto& c : cases)
    {
        const bool got     = ShouldSendLinkshellMessageIPC(c.messageNonEmpty);
        const bool inlineF = inlineShouldSendLinkshellMessageIPC3455(c.messageNonEmpty);
        const bool pin     = pinShouldSendLinkshellMessageIPC3455(c.messageNonEmpty);
    ...
    ok = expect(ShouldSendLinkshellMessageIPC(true) == pinShouldSendLinkshellMessageIPC3455(true),
                "free == pin residual non-empty send") &&
         ok;
    ok = expect(ShouldSendLinkshellMessageIPC(false) == pinShouldSendLinkshellMessageIPC3455(false),
    ...
    for (const bool messageNonEmpty : { false, true })
    {
        ...
        const bool inlineF = inlineShouldSendLinkshellMessageIPC3455(messageNonEmpty);
        const bool pin     = pinShouldSendLinkshellMessageIPC3455(messageNonEmpty);
    ...
    for (const bool messageNonEmpty : { false, true })
    {
        ...
        ok = expect(ShouldSendLinkshellMessageIPC(messageNonEmpty) ==
                        inlineShouldSendLinkshellMessageIPC3455(messageNonEmpty),
                    "host inject dual-wire free == inline") &&
             ok;
        ok = expect(ShouldSendLinkshellMessageIPC(messageNonEmpty) ==
                        pinShouldSendLinkshellMessageIPC3455(messageNonEmpty),
                    "host inject dual-wire free == pin") &&
             ok;
    }

    // Sibling dual-wire independence (2993 leave alone; do not thrash
    // push_stored_message).
