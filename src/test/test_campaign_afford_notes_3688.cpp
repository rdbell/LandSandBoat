#include "test_campaign_afford_notes_3688.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "campaign afford notes 3688 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua sigilOnEventFinish shop afford gate for dual-wire checks
// (dedicated 3688 expand residual 2946; prior dedicated 3643 retained):
...
// Compact dual-wire pin matching Go pinCanAffordAlliedNotes3688 / C++ capacity:
//   notes >= price
...
// Prior dedicated 3643 independence pin (same formula notes >= price).
auto pinCanAffordAlliedNotes3643(const int32 notes, const int32 price) -> bool
{
    return notes >= price;
}
...
// OmegaXI internal/campaign; dedicated slice 3688 expand residual 2946;
// prior dedicated 3643 independence retained).
//
// Coverage:
//   - free == inline == pin == pin3643 == (notes >= price)
//   - residual 2946 / 3072 / 3226 / 3273 / 3304 / 3496 / 3553 / 3598 / 3643 / 1115 pins still hold
//   - prior 3643 independence: free == pin3643 (same notes >= price formula)
...
auto runCampaignAffordNotes3688SelfTests() -> bool
{
    ...
    // Residual 1115 / 2946 / 3072 / 3226 / 3273 / 3304 / 3496 / 3553 / 3598 / 3643 pins still hold under dual-wire.
    ...
    // Prior dedicated 3643 independence poles (same formula notes >= price).
    ok = expect(CanAffordAlliedNotes(1000, 1000) == pinCanAffordAlliedNotes3643(1000, 1000),
                "residual 3643 independence exact") &&
         ok;
    ok = expect(CanAffordAlliedNotes(999, 1000) == pinCanAffordAlliedNotes3643(999, 1000),
                "residual 3643 independence short") &&
         ok;
    ok = expect(CanAffordAlliedNotes(0, 0) == pinCanAffordAlliedNotes3643(0, 0),
                "residual 3643 independence free item") &&
         ok;

    constexpr int32 kMaxInt = std::numeric_limits<int32>::max();
    constexpr int32 kMinInt = std::numeric_limits<int32>::min();

    // --- Composition table: free == inline == pin (incl. prior 3643 independence) ---
    const struct
    {
        int32       notes;
        int32       price;
        ...
        // Residual 2946 / 3072 / 3226 / 3273 / 3304 / 3496 / 3553 / 3598 / 3643 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3643 = pinCanAffordAlliedNotes3643(c.notes, c.price);
        const bool wantPin = c.notes >= c.price;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3643 (prior 3643 independence same formula).
        ok = expect(got == inlineF && got == pin && got == pin3643, "dual-wire free == inline == pin == pin3643") && ok;
        ok = expect(got == wantPin, "free == pin formula notes >= price") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3643.
    const struct
    {
        int32       notes;
        int32       price;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin3643 = pinCanAffordAlliedNotes3643(p.notes, p.price);
        const bool want    = p.notes >= p.price;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3643, "pole free == inline == pin == pin3643") && ok;
    }

    // Dense compose free == inline == pin == pin3643 over required pairs + neighbors.
    const int32 dense[][2] = {
        { 0, 0 },
        { 0, 1 },
        { 1, 0 },
    ...
    for (const auto& p : dense)
    {
        ...
        const bool  pin3643 = pinCanAffordAlliedNotes3643(notes, price);
        const bool  want    = notes >= price;
        ok                  = expect(got == want, "dense free == pin formula") && ok;
        ok                  = expect(got == inlineF && got == pin && got == pin3643, "dense free == inline == pin == pin3643") && ok;
    }
    ...
    for (const auto& p : inject)
    {
        ...
        ok                  = expect(got == inlineF && got == pin && got == pin3643, "inject free == inline == pin == pin3643") && ok;
    }

    // Production path semantics still hold (incl. prior 3643 independence).
    ...
    ok = expect(CanAffordAlliedNotes(1000, 1000) == pinCanAffordAlliedNotes(1000, 1000),
         ...
         ok;
    ok = expect(CanAffordAlliedNotes(1000, 1000) == pinCanAffordAlliedNotes3643(1000, 1000),
                "formula exact free == pin3643 (prior 3643 independence)") &&
         ok;
    ok = expect(CanAffordAlliedNotes(999, 1000) == pinCanAffordAlliedNotes(999, 1000),
                "formula short free == pin") &&
         ok;
    ok = expect(CanAffordAlliedNotes(999, 1000) == pinCanAffordAlliedNotes3643(999, 1000),
                "formula short free == pin3643 (prior 3643 independence)") &&
         ok;
