#include "test_einherjar_should_cycle_wave_3181.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "einherjar ShouldCycleWave 3181 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onMobDespawn cycleWave gate for dual-wire cross-check
// (dedicated slice 3181):
//   #chamberData.mobs <= 0
...
// Compact dual-wire pin matching Go pinShouldCycleWave3181 / C++ capacity:
//   remainingMobs <= 0
...
// 3181 expand residual 2864 / pure 1069).
//
// Coverage:
...
auto runEinherjarShouldCycleWave3181SelfTests() -> bool
{
    ...
    // Residual 1069 / 2864 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2864 happy / multi remaining.
    ...
    // Dense compose: remaining poles free == inline == pin.
