#include "test_nyzul_floor100_vigil_3570.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "nyzul floor100 vigil 3570 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua vigilWeaponDrop floor-100 gate for dual-wire cross-check
// (slice 3570 dedicated expand residual 2900; prior dedicated 3352 / 3449 / 3502):
...
// Compact dual-wire pin matching free function / capacity body (slice 3570).
...
// Prior dedicated 3502 pin (retained cross-check under 3570).
auto pinShouldDropFloor100VigilWeapons3502(const int32 currentFloor) -> bool
{
    return currentFloor == nyzulhelpers::Floor100;
}

// Prior dedicated 3449 pin (retained cross-check under 3570).
auto pinShouldDropFloor100VigilWeapons3449(const int32 currentFloor) -> bool
{
    return currentFloor == nyzulhelpers::Floor100;
}

// Prior dedicated 3352 pin (retained cross-check under 3570).
...
// (Lua vigilWeaponDrop floor-100 guaranteed-drop gate; slice 3570 dedicated
// expand residual 2900; prior dedicated 3352 / 3449 / 3502). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2900 / 1088 poles still hold (100 grants; non-100 misses)
//   - prior dedicated 3352 / 3449 / 3502 pins still hold
...
auto runNyzulFloor100Vigil3570SelfTests() -> bool
{
    ...
    // Residual 2900 / 1088 / prior dedicated 3352 / 3449 / 3502 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin      = pinShouldDropFloor100VigilWeapons3570(p.floor);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3570(e.floor);
    ...
    for (int32 floor = -2; floor <= Floor100 + 5; ++floor)
    {
        ...
        const bool pin     = pinShouldDropFloor100VigilWeapons3570(floor);
    ...
    ok = expect(ShouldDropFloor100VigilWeapons(99) == pinShouldDropFloor100VigilWeapons3570(99),
                "free == pin 99") &&
         ok;
    ok = expect(ShouldDropFloor100VigilWeapons(100) == pinShouldDropFloor100VigilWeapons3502(100),
                "free == prior pin3502 100") &&
         ok;
    ok = expect(ShouldDropFloor100VigilWeapons(99) == pinShouldDropFloor100VigilWeapons3502(99),
                "free == prior pin3502 99") &&
         ok;
    ...
    // Residual 2900 / prior dedicated 3352 / 3449 / 3502 still hold under dedicated 3570.
    ok = expect(ShouldDropFloor100VigilWeapons(100) && !ShouldDropFloor100VigilWeapons(99),
                "residual 2900 / prior 3352 / 3449 / 3502 pins under 3570") &&
         ok;
