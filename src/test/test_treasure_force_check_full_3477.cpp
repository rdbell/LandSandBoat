#include "test_treasure_force_check_full_3477.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3477 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3477):
...
// Compact dual-wire pin matching Go pinShouldForceCheckOnFullPoolInsert3477:
//   slotAfterFreeScan == PoolSize
...
// slice 3477 expand residual 2981; prior dedicated 3423 / 3379 retained).
//
// Coverage:
//   - free == inline == pin == (slotAfterFreeScan == PoolSize)
//   - residual 1367 / 2981 pins still hold
//   - prior dedicated 3423 / 3379 poles still hold
