#include "test_mapwire_open_socket_3511.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapwire ShouldOpenSocket 3511 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapNetworking constructor socket-gate formula for dual-wire
// cross-check (residual 2948 / prior dedicated 3169 / prior dedicated 3232 /
// prior dedicated 3412 / prior dedicated 3466 / dedicated 3511):
//   !isTestServer
...
// Compact dual-wire pin matching Go pinShouldOpenSocket3511:
//   !isTestServer
...
// (!isTestServer; OmegaXI internal/mapwire; dedicated slice 3511;
// expand residual 2948 / pure 2660; prior dedicated 3169 / 3232 / 3412 / 3466).
//
// Coverage:
//   - free == inline == pin == !isTestServer (identity-not)
//   - residual poles true/false
//   - residual 2660 / 2948 / prior 3169 / prior 3232 / prior 3412 / prior 3466 pins still hold
...
auto runMapwireOpenSocket3511SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // Prior dedicated 3169 re-pins.
        { false, true, "prior dedicated 3169 production open" },
        { true, false, "prior dedicated 3169 test skip" },

        // Prior dedicated 3232 re-pins.
        { false, true, "prior dedicated 3232 production open" },
        { true, false, "prior dedicated 3232 test skip" },

        // Prior dedicated 3412 re-pins.
        { false, true, "prior dedicated 3412 production open" },
        { true, false, "prior dedicated 3412 test skip" },

        // Prior dedicated 3466 re-pins.
        { false, true, "prior dedicated 3466 production open" },
        { true, false, "prior dedicated 3466 test skip" },
    };
    ...
    // Residual 2948 / 2660 / 3169 / 3232 / 3412 / 3466 still hold under dedicated suite.
    ok = expect(ShouldOpenSocket(false), "residual production open under 3511") && ok;
    ok = expect(!ShouldOpenSocket(true), "residual test skip under 3511") && ok;
