#include "test_jobpoints_apply_raise_3628.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "jobpoints ShouldApplyRaiseJobPoint 3628 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline RaiseJobPoint spend-gate formula for dual-wire cross-check
// (dedicated slice 3628):
...
// Compact dual-wire pin matching Go pinShouldApplyRaiseJobPoint3628 / C++ capacity:
//   cost != 0 && currentJp >= cost
...
// Prior dedicated 3583 pin surface (free == pin3583 independence).
...
// dedicated slice 3628 expand residual 3012 / prior dedicated 3219 /
...
//
// Coverage:
//   - free == inline == pin == pin3583 == (cost != 0 && currentJp >= cost)
...
//     prior dedicated 3539 / prior dedicated 3583 pins still hold
...
auto runJobpointsApplyRaise3628SelfTests() -> bool
{
    ...
    // Residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 /
    // prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 /
    // prior dedicated 3539 / prior dedicated 3583 pins still hold under dual-wire.
    ok = expect(ShouldApplyRaiseJobPoint(1, 1), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583: cost 1 / jp 1 applies") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(0, 100), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583: cost 0 blocks raise") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(10, 9), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583: short JP blocks raise") && ok;
    ok = expect(ShouldApplyRaiseJobPoint(10, 10), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583: exact JP applies") && ok;

    // --- Composition table: free == inline == pin == pin3583 ---
    const struct
    {
        uint8       cost;
        uint16      currentJp;
        ...
        // Residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 /
        // prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 /
        // prior dedicated 3539 / prior dedicated 3583 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldApplyRaiseJobPoint3628(c.cost, c.currentJp);
        const bool pin     = pinShouldApplyRaiseJobPoint3628(c.cost, c.currentJp);
        const bool pin3583 = pinShouldApplyRaiseJobPoint3583(c.cost, c.currentJp);
        const bool wantPin = c.cost != 0 && c.currentJp >= c.cost;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3583.
        ok = expect(got == inlineF && got == pin && got == pin3583, "dual-wire free == inline == pin == pin3583") && ok;
    ...
    // Explicit poles free == inline == pin == pin3583 for cost 0 / exact / short-by-one.
    ...
    for (const uint8 cost : poleCosts)
    {
        if (cost == 0)
        {
            ...
            for (const uint16 jp : zeroJPs)
            {
                ...
                const bool inlineF = inlineShouldApplyRaiseJobPoint3628(cost, jp);
                const bool pin     = pinShouldApplyRaiseJobPoint3628(cost, jp);
                const bool pin3583 = pinShouldApplyRaiseJobPoint3583(cost, jp);
                ok = expect(!got && got == inlineF && got == pin && got == pin3583,
                            "pole cost0 free == inline == pin == pin3583") &&
                     ok;
            }
            continue;
        }
        // exact equality
        {
            ...
            const bool   pin3583 = pinShouldApplyRaiseJobPoint3583(cost, jp);
            ok                   = expect(got && got == inlineF && got == pin && got == pin3583,
                        "pole exact free == inline == pin == pin3583") &&
                 ok;
        }
        // short-by-one
        {
            ...
            const bool   pin3583 = pinShouldApplyRaiseJobPoint3583(cost, jp);
            ok = expect(!got && got == inlineF && got == pin && got == pin3583,
                        "pole short free == inline == pin == pin3583") &&
                 ok;
        }
    }
    ...
    for (const auto& c : hostCases)
    {
        ...
        const bool  inlineF = inlineShouldApplyRaiseJobPoint3628(cost, c.currentJp);
        const bool  pin     = pinShouldApplyRaiseJobPoint3628(cost, c.currentJp);
        const bool  pin3583 = pinShouldApplyRaiseJobPoint3583(cost, c.currentJp);
        const bool  wantPin = cost != 0 && c.currentJp >= cost;

        ok = expect(got == c.wantApply, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3583,
                    "host compose dual-wire free == inline == pin == pin3583") &&
             ok;
    ...
    for (const uint8 cost : costs)
    {
        for (const uint16 jp : jps)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3583,
                        "compose free == inline == pin == pin3583") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3583,
                        "compose extra free == inline == pin == pin3583") &&
                 ok;
        }
    }

    // Residual independence (2803 / 3012 / prior 3219 / prior 3275 / prior 3371 /
    // prior 3421 / prior 3475 / prior 3539 / prior 3583): spend gate is distinct
    // from presence short-circuit and JobPointCost formula that compose
    // PlanRaiseJobPoint.
    // Prior dedicated 3583 independence: free == prior suite inline/pin forms
    // (local mirrors of test_jobpoints_apply_raise_3583 pin surface).
    for (const uint8 cost : costs)
    {
        for (const uint16 jp : jps)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3583(cost, jp),
                        "prior 3583 independence free == 3583 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3583(cost, jp),
                        "prior 3583 independence free == 3583 pin") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3583(cost, jp),
                        "prior 3583 independence extra free == 3583 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3583(cost, jp),
                        "prior 3583 independence extra free == 3583 pin") &&
                 ok;
        }
    }
    {
