#include "test_linkshell_reject_null_add_member_3592.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "linkshell ShouldRejectNullAddMember 3592 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldRejectNullAddMember3592 / C++
// capacity identity form (formula unchanged from 1354 / 2929 / prior dedicated
// 3195 / 3489 / 3548):
//   charNull
// free == inline == pin == pin3548.
...
// (charNull identity; OmegaXI internal/linkshell; dedicated slice 3592;
// residual expand 2929 / pure 1354; prior dedicated expands 3195 / 3489 / 3548).
//
// Coverage:
//   - free == inline == pin == pin3548 identity form
//   - poles: null rejects; non-null allows
//   - residual 2929 / 1354 / prior dedicated 3195 / 3489 / 3548 pins still hold
//   - host-style inject + residual independence (2958 / 3079)
//   - prior dedicated 3195 / 3489 / 3548 independence (RETAINED)
auto runLinkshellRejectNullAddMember3592SelfTests() -> bool
{
    ...
    // Residual 1354 / 2929 / prior dedicated 3195 / 3489 / 3548 pins still hold
    // under dedicated dual-wire expand.
    ...
    // --- Core poles: free == inline == pin == pin3548 identity form ---
    const struct
    {
        bool        charNull;
        bool        want;
        ...
        // Residual 2929 / 1354 / prior dedicated 3195 / 3489 / 3548 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3548  = pinShouldRejectNullAddMember3548(c.charNull);
        ...
        ok = expect(got == pin3548, "dual-wire free==pin3548") && ok;
        ok = expect(got == wantPin, "dual-wire free==identity pin form") && ok;
    }

    // Free == pin / pin3548 across residual poles.
    ...
    ok = expect(ShouldRejectNullAddMember(true) == pinShouldRejectNullAddMember3548(true), "free==pin3548 null char") && ok;
    ok = expect(ShouldRejectNullAddMember(false) == pinShouldRejectNullAddMember3548(false), "free==pin3548 non-null char") && ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3548.
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin3548 = pinShouldRejectNullAddMember3548(charNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3548, "compose free==pin3548") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3548, "host compose free==pin3548") && ok;
    }
    ...
    // Residual independence (1354 / 2929 / 3195 / 3489 / 3548 / 2958 / 3079):
    // AddMember null reject is distinct from duplicate-add and online-null gates.
    // Prior dedicated 3195 / 3489 / 3548 independence (RETAINED under this expand).
