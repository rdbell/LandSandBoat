#include "test_merit_lower_merit_3478.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "merit ShouldLowerMerit 3478 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline LowerMerit count-decrement formula for dual-wire cross-check
// (slice 3478 dedicated expand residual 3054; prior dedicated 3424 / 3373):
//   count > 0
...
// Retained under dedicated expand residual 3478.
...
// (count > 0 LowerMerit count-decrement gate; OmegaXI internal/merit; slice 3478
// dedicated expand residual 3054; prior dedicated 3424 / 3373). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 3054 / 2810 / prior dedicated 3424 / 3373 poles still hold
...
auto runMeritLowerMerit3478SelfTests() -> bool
{
    ...
    // Residual 3054 / 2810 / prior dedicated 3424 / 3373 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3054 / 2810 / prior 3424 / 3373 classic poles (0, 1, n).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin           = pinShouldLowerMerit3478(p.count);
        const bool priorPin3424  = pinShouldLowerMerit3424(p.count);
        ...
        ok = expect(got == priorPin3424, "ShouldLowerMerit == prior 3424 pin") && ok;
                               ...
                               uint8{ 255 } })
    {
        ...
        const bool pin          = pinShouldLowerMerit3478(count);
        const bool priorPin3424 = pinShouldLowerMerit3424(count);
        ...
        ok = expect(got == priorPin3424, "dense free == prior 3424 pin") && ok;
    ...
    for (const bool present : { false, true })
    {
        for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 15 }, uint8{ 255 } })
        {
            ...
            ok             = expect(got == inlineShouldLowerMerit(count),
                        "compose free == inline") &&
                 ok;
            ok = expect(got == pinShouldLowerMerit3478(count), "compose free == pin") && ok;
            ok = expect(got == pinShouldLowerMerit3424(count), "compose free == prior 3424 pin") && ok;
    ...
    // Prior dedicated 3424 independence: free still matches prior pin.
    for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 2 }, uint8{ 15 },
                               uint8{ 128 }, uint8{ 254 }, uint8{ 255 } })
    {
        const bool got      = ShouldLowerMerit(count);
        const bool priorPin = pinShouldLowerMerit3424(count);
        ok                  = expect(got == priorPin, "prior 3424 independence") && ok;
    }
