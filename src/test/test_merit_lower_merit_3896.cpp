#include "test_merit_lower_merit_3896.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "merit ShouldLowerMerit 3896 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline LowerMerit count-decrement formula for dual-wire cross-check
// (slice 3896 dedicated expand residual 3054; prior dedicated 3851 / 3806):
//   count > 0
...
// Retained under dedicated expand residual 3896.
...
// Compact dual-wire pin matching free function / capacity body (slice 3896).
// Direct return only — same formula as production ShouldLowerMerit.
auto pinShouldLowerMerit3896(const uint8 count) -> bool
{
    return count > 0;
}
...
// (count > 0 LowerMerit count-decrement gate; OmegaXI internal/merit; slice 3896
// dedicated expand residual 3054; prior dedicated 3851 / 3806). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3851 (direct return formula)
//   - residual 3054 / 2810 / prior dedicated 3851 / 3806 poles still hold
...
auto runMeritLowerMerit3896SelfTests() -> bool
{
    ...
    // Residual 3054 / 2810 / prior dedicated 3851 / 3806 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3054 / 2810 / prior 3851 / 3806 classic poles (0, 1, n).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin           = pinShouldLowerMerit3896(p.count);
        const bool priorPin3851  = pinShouldLowerMerit3851(p.count);
        ...
        ok = expect(got == priorPin3851, "ShouldLowerMerit == prior 3851 pin") && ok;
    ...
    // Dense poles: free == inline == pin == pin3851 (direct return) across residual + interior.
                               ...
                               uint8{ 255 } })
    {
        ...
        const bool pin          = pinShouldLowerMerit3896(count);
        const bool priorPin3851 = pinShouldLowerMerit3851(count);
        ...
        ok = expect(got == priorPin3851, "dense free == prior 3851 pin") && ok;
    ...
    for (const bool present : { false, true })
    {
        for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 15 }, uint8{ 255 } })
        {
            ...
            ok             = expect(got == inlineShouldLowerMerit(count),
                        "compose free == inline") &&
                 ok;
            ok = expect(got == pinShouldLowerMerit3896(count), "compose free == pin") && ok;
            ok = expect(got == pinShouldLowerMerit3851(count), "compose free == prior 3851 pin") && ok;
    ...
    // Prior dedicated 3851 independence: free still matches prior pin.
    for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 2 }, uint8{ 15 },
                               uint8{ 128 }, uint8{ 254 }, uint8{ 255 } })
    {
        const bool got      = ShouldLowerMerit(count);
        const bool priorPin = pinShouldLowerMerit3851(count);
        ok                  = expect(got == priorPin, "prior 3851 independence") && ok;
    }
