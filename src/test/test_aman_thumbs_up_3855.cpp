#include "test_aman_thumbs_up_3855.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3855 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3855 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3855 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3810 pin for independence cross-check.
auto pinCanThumbsUp3810(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3810(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3855 expand residual 2967; prior dedicated 3810 / 3765 / 3720 / 3675 / 3630 / 3585 / 3541 / 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == pin3810 == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 3810 pins still hold
//   - prior 3810 independence (free == 3810 inline/pin)
...
auto runAmanThumbsUp3855SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 3810 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3810 re-pins (independence).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3810 = pinCanThumbsUp3810(c.cooldownExpired);
        ...
        ok = expect(got == pin3810, "CanThumbsUp dual-wire == pin3810") && ok;
    ...
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3810(true), "free==pin3810 true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3810(false), "free==pin3810 false") && ok;

    // Explicit residual poles free == inline == pin == pin3810 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3810 = pinCanThumbsUp3810(pole);
        ok                 = expect(got == pole, "pole free == identity") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3810, "pole free == pin3810") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3810.
    for (const bool cooldownExpired : { false, true })
    {
        ...
        ok              = expect(got == pinCanThumbsUp3810(cooldownExpired), "compose free == pin3810") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp(cooldownExpired),
             ...
             ok;
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp3810(cooldownExpired),
                    "host inject free == pin3810") &&
             ok;
    ...
    // Prior dedicated 3810 independence: free == prior 3810 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3810(true), "prior 3810 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3810(false), "prior 3810 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3810(true), "prior 3810 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3810(false), "prior 3810 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 3810 / 0698 still hold under dedicated 3855.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/3541/3585/3630/3675/3720/3765/3810/0698 under 3855") && ok;
