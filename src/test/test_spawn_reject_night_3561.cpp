#include "test_spawn_reject_night_3561.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "spawn ShouldRejectAtNightSpawn 3561 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ATNIGHT spawn-type TOTD gate for dedicated dual-wire expand residual
// 3092 cross-check (slice 3561; prior dedicated 3528/3483/3435/3387 retained):
...
auto inlineShouldRejectAtNightSpawn3561(const uint8 spawnType, const vanadiel_time::TOTD totd) -> bool
{
...
// Compact dual-wire pin matching Go pinShouldRejectAtNightSpawn3561 / C++
// capacity positive form (formula unchanged from 1362 / 3092 / 3387 / 3435 / 3483 / 3528):
...
// Prior dedicated 3528 pin (retained) — free == pin3561 == pin3528.
...
// OmegaXI internal/spawnslot; dedicated dual-wire expand residual 3092 slice 3561;
// prior dedicated 3528/3483/3435/3387 retained; residual expand 3092 / pure 1362).
//
// Coverage:
//   - free == inline == pin == pin3528 positive form
//   - residual 3092 / 1362 and prior 3528 / 3483 / 3435 / 3387 pins still hold
...
auto runSpawnRejectNight3561SelfTests() -> bool
{
    ...
    // Residual 3092 / 1362 / prior 3528 / 3483 / 3435 / 3387 pins still hold under dedicated dual-wire.
    ...
    // --- Core poles: free == inline == pin == pin3528 positive form ---
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF  = inlineShouldRejectAtNightSpawn3561(c.spawnType, c.totd);
        const bool pin      = pinShouldRejectAtNightSpawn3561(c.spawnType, c.totd);
        const bool pin3528  = pinShouldRejectAtNightSpawn3528(c.spawnType, c.totd);
        ...
        ok = expect(got == inlineF && got == pin && got == pin3528 && got == wantPin,
                    "ShouldRejectAtNightSpawn free == inline == pin == pin3528 positive form") &&
             ok;
    }

    // Free == pin across residual ATNIGHT-required poles.
    ok = expect(ShouldRejectAtNightSpawn(SpawnTypeAtNight, vanadiel_time::TOTD::DAY) ==
                    pinShouldRejectAtNightSpawn3561(SpawnTypeAtNight, vanadiel_time::TOTD::DAY),
                "free == pin ATNIGHT-required + DAY") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(SpawnTypeAtNight, vanadiel_time::TOTD::NIGHT) ==
                    pinShouldRejectAtNightSpawn3561(SpawnTypeAtNight, vanadiel_time::TOTD::NIGHT),
                "free == pin ATNIGHT-required + NIGHT") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(0, vanadiel_time::TOTD::DAY) ==
                    pinShouldRejectAtNightSpawn3561(0, vanadiel_time::TOTD::DAY),
                "free == pin no ATNIGHT + DAY") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(0, vanadiel_time::TOTD::NIGHT) ==
                    pinShouldRejectAtNightSpawn3561(0, vanadiel_time::TOTD::NIGHT),
                "free == pin no ATNIGHT + NIGHT") &&
         ok;

    // Dense compose: spawnType bit space × full TOTD domain free == inline == pin == pin3528.
    ...
    for (int spawnType = 0; spawnType <= 0xFF; ++spawnType)
    {
        for (const auto totd : totds)
        {
            ...
            const bool inlineF = inlineShouldRejectAtNightSpawn3561(st, totd);
            const bool pin     = pinShouldRejectAtNightSpawn3561(st, totd);
            const bool pin3528 = pinShouldRejectAtNightSpawn3528(st, totd);
            const bool want    = HasSpawnTypeFlag(st, SpawnTypeAtNight) && !IsNightTotdWindow(totd);
            ok                 = expect(got == want, "compose free == pin formula") && ok;
            ok                 = expect(got == inlineF && got == pin && got == pin3528,
                        "compose free == inline == pin == pin3528") &&
                 ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3528 for host-style poles.
    const struct
    {
        uint8               spawnType;
        vanadiel_time::TOTD totd;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRejectAtNightSpawn3561(p.spawnType, p.totd);
        const bool pin     = pinShouldRejectAtNightSpawn3561(p.spawnType, p.totd);
        const bool pin3528 = pinShouldRejectAtNightSpawn3528(p.spawnType, p.totd);
        const bool want    = HasSpawnTypeFlag(p.spawnType, SpawnTypeAtNight) && !IsNightTotdWindow(p.totd);
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3528,
                    "host inject free == inline == pin == pin3528") &&
             ok;
    }
