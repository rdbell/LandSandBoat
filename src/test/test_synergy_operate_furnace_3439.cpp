#include "test_synergy_operate_furnace_3439.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy operate furnace 3439 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua claimedByYou gate for dual-wire checks
// (slice 3439 dedicated expand residual 2899; prior dedicated expand 3383;
...
// Compact dual-wire pin matching free function / capacity body (slice 3439).
// Direct return only — same formula as production CanOperateFurnace.
auto pinCanOperateFurnace3439(const uint32 furnacePlayerID, const uint32 playerID) -> bool
{
    return synergyhelpers::IsClaimedBy(furnacePlayerID, playerID);
}

// Prior dedicated expand 3383 pin (retained for independence check).
...
// (Lua synergyFurnaceOnTrigger claimedByYou operate gate; slice 3439 dedicated
// expand residual 2899; prior dedicated expand 3383; prior dedicated 3117).
// Formula unchanged:
//
...
//
// Coverage:
//   - free == inline == pin3439 == pin3383 == pin3117
//   - residual 2899 / 1149 / prior dedicated 3117 / prior expand 3383 poles
...
auto runSynergyOperateFurnace3439SelfTests() -> bool
{
    ...
    // Residual 1149 / 2899 / prior dedicated 3117 / prior expand 3383 pins
    ...
    // Owner match true; other poles false — free == inline == pin3439 == pin3383 == pin3117.
    const struct
    {
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinGot    = pinCanOperateFurnace3439(c.furnacePlayerID, c.playerID);
        const bool pinPrior  = pinCanOperateFurnace3383(c.furnacePlayerID, c.playerID);
        const bool pinLegacy = pinCanOperateFurnace3117(c.furnacePlayerID, c.playerID);
        ...
        ok = expect(got == pinGot, "dual-wire free == pin3439") && ok;
        ok = expect(got == pinPrior, "dual-wire free == residual pin3383") && ok;
        ok = expect(got == pinLegacy, "dual-wire free == residual pin3117") && ok;
    }
    ...
    for (const auto& p : pairs)
    {
        ...
        const bool pinGot    = pinCanOperateFurnace3439(p.furnacePlayerID, p.playerID);
        const bool pinPrior  = pinCanOperateFurnace3383(p.furnacePlayerID, p.playerID);
        const bool pinLegacy = pinCanOperateFurnace3117(p.furnacePlayerID, p.playerID);
        ...
        ok = expect(got == pinGot, "compose free == pin3439") && ok;
        ok = expect(got == pinPrior, "compose free == pin3383") && ok;
        ok = expect(got == pinLegacy, "compose free == pin3117") && ok;
    }
    ...
    // Free == pin3439 == pin3383 == pin3117 residual poles.
    ok = expect(CanOperateFurnace(5, 5) == pinCanOperateFurnace3439(5, 5),
                "free == pin3439 matching non-zero") &&
         ok;
    ok = expect(CanOperateFurnace(5, 6) == pinCanOperateFurnace3439(5, 6),
                "free == pin3439 mismatch") &&
         ok;
    ok = expect(CanOperateFurnace(0, 0) == pinCanOperateFurnace3439(0, 0),
                "free == pin3439 zero/zero") &&
         ok;
    ok = expect(pinCanOperateFurnace3439(1, 1) == pinCanOperateFurnace3383(1, 1),
                "pin3439 == pin3383 matching non-zero") &&
         ok;
    ok = expect(pinCanOperateFurnace3439(0, 0) == pinCanOperateFurnace3383(0, 0),
                "pin3439 == pin3383 zero/zero") &&
         ok;
    ok = expect(pinCanOperateFurnace3439(1, 1) == pinCanOperateFurnace3117(1, 1),
                "pin3439 == pin3117 matching non-zero") &&
         ok;
    ok = expect(pinCanOperateFurnace3439(0, 0) == pinCanOperateFurnace3117(0, 0),
                "pin3439 == pin3117 zero/zero") &&
         ok;
