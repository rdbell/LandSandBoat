#include "test_battlefield_enter_pc_3554.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "battlefield ShouldEnterPC 3554 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertEntity TYPE_PC enter-branch formula for dual-wire cross-check
// (dedicated 3554 expand residual 3024; prior dedicated 3497 / 3431 / 3381):
//   enter
...
// Compact dual-wire pin matching Go pinShouldEnterPC3554 /
// C++ capacity enter-identity form (formula unchanged from 1361 / 3024 / 3381 / 3431 / 3497):
//   enter
...
// Prior dedicated 3497 inline/pin for independence cross-check.
auto inlineShouldEnterPC3497(const bool enter) -> bool
{
    return enter;
}

auto pinShouldEnterPC3497(const bool enter) -> bool
{
    return enter;
}
...
// 3554 expand residual 3024 / prior dedicated 3497 / 3431 / 3381 / pure 1361).
//
// Coverage:
//   - free == inline == pin residual pins
//   - poles: enter-true / enter-false
//   - residual 1361 / 3024 pins still hold
//   - prior dedicated 3381 / 3431 / 3497 poles still hold
...
auto runBattlefieldEnterPC3554SelfTests() -> bool
{
    ...
    // Residual 1361 / prior dual-wire 3024 / prior dedicated 3381 / 3431 / 3497 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 1361 / 3024 / prior dedicated 3381 / 3431 / 3497 re-pins.
    ...
    // Prior dedicated 3497 independence: free still matches prior inline/pin.
    ok = expect(ShouldEnterPC(true) == inlineShouldEnterPC3497(true) &&
                    ShouldEnterPC(true) == pinShouldEnterPC3497(true),
                "prior 3497 independence enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == inlineShouldEnterPC3497(false) &&
                    ShouldEnterPC(false) == pinShouldEnterPC3497(false),
                "prior 3497 independence enter=false") &&
         ok;
    ...
    // 3302 / 3014 / 3365 / 3024 / 3381 / 3431 / 3497): enter is distinct from null-insert,
