#include "test_attackutils_da_double_damage_3249.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "attackutils ShouldApplyDADoubleDamage 3249 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CheckForDamageMultiplier DOUBLE type double-damage formula for
// dual-wire cross-check (slice 3249):
...
// (attackType == DOUBLE && rateProcs; slice 3249).
auto runAttackutilsDADoubleDamage3249SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = (c.attackType == AttackTypeDouble) && c.rateProcs;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF, "ShouldApplyDADoubleDamage dual-wire == inline LSB formula") && ok;
        ok = expect(got == pin, "free == pin formula attackType==DOUBLE && rateProcs") && ok;
    }

    // Pin composition: DOUBLE && rateProcs only.
    ok = expect(ShouldApplyDADoubleDamage(AttackTypeDouble, true),
    ...
    // Host path: attackutils::CheckForDamageMultiplier switch DOUBLE.
    const struct
    {
        uint8       attackType;
        bool        rateProcs;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == ((c.attackType == AttackTypeDouble) && c.rateProcs),
    ...
    for (const uint8 at : types)
    {
        for (const bool rateProcs : { false, true })
        {
            ...
            ok              = expect(got == inlineShouldApplyDADoubleDamage(at, rateProcs),
                        "compose free == inline") &&
                 ok;
        }
    }

    // Residual independence (1380): DA type rate is distinct from sibling
    ...
    for (const uint8 at : { AttackTypeNormal, AttackTypeDouble, AttackTypeTriple, AttackTypeZanshin })
    {
        for (const bool rateProcs : { false, true })
        {
            const bool got  = ShouldApplyDADoubleDamage(at, rateProcs);
            ...
            ok              = expect(got == inlineShouldApplyDADoubleDamage(at, rateProcs),
                 ...
                 ok;
            ok = expect(ShouldApplyZanshinDoubleDamage(AttackTypeZanshin, true),
                        "zanshin residual flipped under DA compose") &&
                 ok;
            ok = expect(ShouldApplyTATripleDamage(AttackTypeTriple, true),
                        "TA triple residual flipped under DA compose") &&
                 ok;
        }
    }
