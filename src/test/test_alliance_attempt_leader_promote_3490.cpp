#include "test_alliance_attempt_leader_promote_3490.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3490 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check
// (slice 3490 dedicated dual-wire expand residual 2979):
//   isMainParty
...
// (isMainParty identity; slice 3490 dedicated dual-wire expand residual 2979).
//
// Coverage:
//   - main party → attempt promote true
//   - non-main → attempt promote false
//   - free == inline == pin (isMainParty identity)
//   - residual 1341 / 1346 / 2979 / 3144 / 3393 / 3447 pins still hold
...
auto runAllianceAttemptLeaderPromote3490SelfTests() -> bool
{
    ...
    // dedicated 3393 / prior dedicated 3447 pins still hold.
