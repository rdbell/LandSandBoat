#include "test_synthutils_can_synthesize_hq_3839.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synthutils CanSynthesizeHQ 3839 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline canSynthesizeHQ pure half for dual-wire cross-check
// (slice 3839 dedicated expand residual 2876 / prior ~3794):
...
// Compact dual-wire pin matching free function / capacity body (slice 3839).
...
// Prior dedicated pin (slice 3794) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3794(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3749) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3749(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3704) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3704(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3659) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3659(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3614) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3614(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3569) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3569(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3508) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3508(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3313) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3313(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3283) — same formula; retained under 3839.
auto pinCanSynthesizeHQ3283(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3244) — same formula; retained under 3839.
...
// OmegaXI internal/synthutils; slice 3839 dedicated expand residual 2876 /
// prior ~3794). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3794 (direct return formula)
//   - residual 2876 / prior dedicated 3794 / 3749 / 3704 / 3659 / 3614 / 3569 / 3508 / 3313 /
...
auto runSynthutilsCanSynthesizeHQ3839SelfTests() -> bool
{
    ...
    // Residual 2876 / 1922 / prior dedicated 3244 / 3283 / 3313 / 3508 / 3569 / 3614 / 3659 / 3704 / 3749 / 3794.
    ...
    const struct
    {
        ...
        // Residual 2876 / 1922 / prior 3244 / 3283 / 3313 / 3508 / 3569 / 3614 / 3659 / 3704 / 3749 / 3794 poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3839(p.antiHQMod);
    ...
    // free == inline == pin == pin3794 == formula.
    const struct
    {
        int16       antiHQMod;
        const char* label;
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3839(e.antiHQMod);
    ...
    for (int16 m = -200; m <= 200; ++m)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3839(m);
