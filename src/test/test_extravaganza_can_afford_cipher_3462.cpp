#include "test_extravaganza_can_afford_cipher_3462.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "extravaganza CanAffordCipher 3462 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua shadowEraFinish notes gate for dual-wire cross-check
// (dedicated 3462 expand residual 2881; prior 3408):
//   notes >= 1000
auto inlineCanAffordCipher3462(const int32 notes) -> bool
{
    return notes >= 1000;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3462:
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3462(const int32 notes) -> bool
{
...
// internal/extravaganza; dedicated slice 3462 expand residual 2881;
// prior dedicated 3408 / 3322 / 3292 / 3262 / 3153 retained).
//
// Coverage:
//   - free == inline == pin == notes >= CipherPurchaseNotes
//   - CipherPurchaseNotes == 1000 pin
//   - residual 2881 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 pins still hold
...
auto runExtravaganzaCanAffordCipher3462SelfTests() -> bool
{
    ...
    // Residual 2881 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 CanAffordCipher pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3462(c.notes);
        const bool pinGot  = pinCanAffordCipher3462(c.notes);
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3462(p.notes);
        const bool pinGot  = pinCanAffordCipher3462(p.notes);
    ...
    for (const int32 notes : dense)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3462(notes);
        const bool pinGot  = pinCanAffordCipher3462(notes);
