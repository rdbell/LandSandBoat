#include "test_treasure_force_check_full_3655.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3655 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3655):
...
// (prior dedicated retained under 3655):
//   slotAfterFreeScan == PoolSize
auto pinShouldForceCheckOnFullPoolInsert3610(const uint8 slotAfterFreeScan) -> bool
{
    return slotAfterFreeScan == treasurepoolhelpers::PoolSize;
}
...
// slice 3655 expand residual 2981; prior dedicated 3610 / 3565 / 3533 / 3477 / 3423 / 3379 retained).
//
// Coverage:
//   - free == inline == pin == pin3610 == (slotAfterFreeScan == PoolSize)
//   - residual 1367 / 2981 pins still hold
//   - prior dedicated 3610 / 3565 / 3533 / 3477 / 3423 / 3379 poles still hold
...
auto runTreasureForceCheckFull3655SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3610 ---
    const struct
    {
        uint8       slot;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3610 = pinShouldForceCheckOnFullPoolInsert3610(c.slot);
        const bool wantPin = c.slot == PoolSize;

        ok = expect(got == c.want, c.label) && ok;
        // free == inline == pin == pin3610.
        ok = expect(got == inlineF && got == pin && got == pin3610, "dual-wire free == inline == pin == pin3610") && ok;
        ok = expect(got == wantPin, "free == pin formula slot == PoolSize") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3610 for classic cells.
    for (const uint8 s : { uint8{ 0 }, uint8{ 1 }, uint8{ 3 }, uint8{ 9 }, uint8{ 10 }, uint8{ 11 }, uint8{ 255 } })
    {
        ...
        const bool pin3610 = pinShouldForceCheckOnFullPoolInsert3610(s);
        const bool want    = s == PoolSize;
        ok                 = expect(got == want, "pole free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3610, "pole free == inline == pin == pin3610") && ok;
    }

    // Dense compose over slot domain around PoolSize (0..20) — free == inline == pin == pin3610.
    for (uint16 slot = 0; slot <= 20; ++slot)
    {
        ...
        ok                  = expect(got == inlineF && got == pin && got == pin3610, "compose free == inline == pin == pin3610") && ok;
    }
    ...
    // free == inline == pin == pin3610 residual pins.
    for (uint8 slot = 0; slot < PoolSize; ++slot)
    {
        ...
        const bool pin3610 = pinShouldForceCheckOnFullPoolInsert3610(slot);
        ok                 = expect(!got, "host inject free-found must not force-check") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3610, "host inject free-found free == inline == pin == pin3610") && ok;
    }
    {
        ...
        const bool pin3610 = pinShouldForceCheckOnFullPoolInsert3610(PoolSize);
        ok                 = expect(got, "host inject SlotID==PoolSize must force-check") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3610, "host inject PoolSize free == inline == pin == pin3610") && ok;
    }
