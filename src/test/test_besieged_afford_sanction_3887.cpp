#include "test_besieged_afford_sanction_3887.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "besieged CanAffordSanction 3887 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onEventFinish sanctionCost for dual-wire cross-check (slice 3887):
//   sanctionCost = 100
...
// Compact dual-wire pin matching Go pinCanAffordSanction3887 / C++ capacity
// (direct return form):
//   imperialStanding >= SanctionAffordCost(option)
...
// Prior dedicated pin matching Go pinCanAffordSanction3842 (formula identical):
//   imperialStanding >= SanctionAffordCost(option)
auto pinCanAffordSanction3842(const int32 option, const int32 imperialStanding) -> bool
{
    return imperialStanding >= besiegedhelpers::SanctionAffordCost(option);
}
...
// internal/besieged; dedicated slice 3887 expand residual 2945 —
// formula unchanged; prior dedicated ~3842).
//
// Coverage:
//   - free == inline == pin == pin3842 == (standing >= SanctionAffordCost(option))
//   - residual 1120 / 2945 / 3093 / 3227 / 3277 / 3307 / 3418 / 3472 / 3529 / 3572 / 3617 / 3662 / 3707 / 3752 / 3797 / 3842 pins still hold
...
auto runBesiegedAffordSanction3887SelfTests() -> bool
{
    ...
    // Residual constant / 1120 / 2945 / 3093 / 3227 / 3277 / 3307 / 3418 / 3472 / 3529 / 3572 / 3617 / 3662 / 3707 / 3752 / 3797 / 3842 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinCanAffordSanction3842(c.option, c.standing);
        ...
        ok = expect(got == pinPrior, "CanAffordSanction dual-wire == pin3842") && ok;
        ok = expect(got == wantPin, "CanAffordSanction == standing >= SanctionAffordCost") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3842.
    const struct
    {
        int32       option;
        int32       standing;
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pinPrior = pinCanAffordSanction3842(pole.option, pole.standing);
        ...
        ok                  = expect(got == pinPrior, "pole free == pin3842") && ok;
    }
    ...
    // Compose: free == inline == pin == pin3842 for short / exact / surplus poles per option.
    ...
    for (const int32 opt : sanctionOpts)
    {
        ...
        for (const int32 delta : { -1, 0, 1 })
        {
            ...
            const bool  pinPrior = pinCanAffordSanction3842(opt, st);
            ok                   = expect(got == inlineF, "compose free == inline") && ok;
            ok                   = expect(got == pinGot, "compose free == pin") && ok;
            ok                   = expect(got == pinPrior, "compose free == pin3842") && ok;
    ...
    for (const int32 opt : options)
    {
        for (const int32 st : standings)
        {
            ...
            ok                  = expect(got == pinCanAffordSanction3842(opt, st), "dense free == pin3842") && ok;
        }
    }
