#include "test_darkixion_can_break_horn_3325.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "darkixion CanBreakHorn 3325 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline checkHornBreak pure gate (before 5% roll) for dual-wire cross-check
// (dedicated 3325 expand residual 2885; prior 3295 / 3266 / 3154):
...
// Positive if/else pin matching free function / capacity body (slice 3325).
...
// dedicated slice 3325 expand residual 2885; prior dedicated 3295 / 3266 / 3154 retained).
//
// Coverage:
//   - free == inline == pin (positive if/else)
//   - residual 2885 / 3295 / 3266 / 3154 / 0985 pins still hold
...
auto runDarkixionCanBreakHorn3325SelfTests() -> bool
{
    ...
    // Residual 0985 / 2885 / 3154 / 3266 / 3295 CanBreakHorn pins still hold under dual-wire.
    ...
    ok = expect(!CanBreakHorn(false, 5, true), "residual anim 5 should fail") && ok;
    ok = expect(!CanBreakHorn(false, 127, true), "residual anim 127 should fail") && ok;
    ...
    ok = expect(!CanBreakHorn(true, 0, true), "residual busy literal NORMAL should fail") && ok;
    ok = expect(!CanBreakHorn(true, 3, true), "residual busy literal GLOWING should fail") && ok;
    ok = expect(!CanBreakHorn(false, 0, false), "residual literal NORMAL not front should fail") && ok;
    ok = expect(!CanBreakHorn(false, 3, false), "residual literal GLOWING not front should fail") && ok;
    ...
    ok = expect(!CanBreakHorn(true, kAnimTrample, false), "blocked busy trample not front") && ok;
    ok = expect(!CanBreakHorn(false, 5, false), "blocked anim 5 not front") && ok;
    ...
    const struct
    {
        ...
        { false, 5, true, false, "pole anim 5 blocks" },
        { false, 127, true, false, "pole anim 127 blocks" },
        ...
        { true, 4, false, false, "pole busy anim 4 not-front" },
        { true, 5, true, false, "pole busy anim 5 front" },
        // Dense host poles.
        ...
        { false, 127, false, false, "dense anim 127 not front" },
        { true, 255, true, false, "dense busy anim 255" },
    };
    ...
    const struct
    {
        ...
        { false, 5, true, "anim 5 blocks" },
        { false, 127, true, "anim 127 blocks" },
        { false, 255, true, "anim 255 blocks" },
        { true, kAnimHornBroken, false, "all poles fail" },
        { true, kAnimTrample, true, "busy trample front" },
        { false, 0, false, "literal NORMAL not front" },
        { false, 3, false, "literal GLOWING not front" },
        { true, 0, true, "busy literal NORMAL" },
        { true, 3, true, "busy literal GLOWING" },
    };
    ...
    const int32 composeAnims[] = { -1, 0, kAnimNormal, kAnimTrample, kAnimHornBroken, kAnimGlowing, 1, 2, 3, 4, 5, 6, 99, 127, 128, 255 };
    ...
    ok = expect(!CanBreakHorn(false, kAnimTrample, true), "checkHornBreak trample → blocked") && ok;
    ok = expect(!CanBreakHorn(true, 0, false), "checkHornBreak busy literal NORMAL not front → blocked") && ok;
