#include "test_notoriety_add_member_3297.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "notoriety ShouldAddNotorietyMember 3297 self-test failed: " << label << '\n';
    }
    return condition;
}
...
auto inlineShouldAddNotorietyMember3297(const bool ownerPresent, const bool entityPresent,
                                        const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}

// Compact dual-wire pin matching Go pinShouldAddNotorietyMember3297 / C++ capacity
// direct AND-chain (formula unchanged from 2818 / 2959 / 3165 / 3267):
//   ownerPresent && entityPresent && differentAllegiance
auto pinShouldAddNotorietyMember3297(const bool ownerPresent, const bool entityPresent,
                                     const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}
...
// OmegaXI internal/notoriety; dedicated slice 3297; residual expand 2959 /
// pure 2818; prior dedicated expand 3267 / 3165).
//
// Coverage:
...
auto runNotorietyAddMember3297SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3297(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool pin     = pinShouldAddNotorietyMember3297(c.ownerPresent, c.entityPresent, c.differentAllegiance);
    ...
    ok = expect(ShouldAddNotorietyMember(true, true, true) == pinShouldAddNotorietyMember3297(true, true, true),
                "free == pin admit all true") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(false, true, true) == pinShouldAddNotorietyMember3297(false, true, true),
                "free == pin nil owner") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(true, false, true) == pinShouldAddNotorietyMember3297(true, false, true),
                "free == pin nil entity") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(true, true, false) == pinShouldAddNotorietyMember3297(true, true, false),
    ...
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            for (const bool differentAllegiance : { false, true })
            {
                ...
                const bool inlineF = inlineShouldAddNotorietyMember3297(ownerPresent, entityPresent, differentAllegiance);
                const bool pin     = pinShouldAddNotorietyMember3297(ownerPresent, entityPresent, differentAllegiance);
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3297(p.owner, p.entity, p.diff);
        const bool pin     = pinShouldAddNotorietyMember3297(p.owner, p.entity, p.diff);
