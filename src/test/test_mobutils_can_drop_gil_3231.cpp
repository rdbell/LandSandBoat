#include "test_mobutils_can_drop_gil_3231.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mobutils CanDropGil 3231 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CMobEntity::CanDropGil formula for dual-wire cross-check (slice 3231):
...
// Compact dual-wire pin matching free function / capacity body (slice 3231).
// Direct return only — same formula as production CanDropGil.
auto pinCanDropGil3231(const int16 gilMin, const int16 gilMax, const int16 gilBonus) -> bool
{
    return gilMax >= 0 && (gilMin > 0 || gilMax != 0 || gilBonus > 0);
}
...
// (gilMin/gilMax/gilBonus eligibility gate; slice 3231 dedicated expand
// residual 2960). CanStealGil dual-wires through the same free function.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2960 / 2653 / prior dedicated 3158 poles still hold
...
auto runMobutilsCanDropGil3231SelfTests() -> bool
{
    ...
    // Residual 2653 / residual dual-wire 2960 / prior dedicated 3158 pins.
    ...
    const struct
    {
        ...
        // Residual 2960 / 2653 / prior 3158 re-pins.
        { 0, 0, 0, false, "residual 2960/2653/3158 all zero" },
        { 1, 0, 0, true, "residual 2960/2653/3158 positive min" },
        { 0, 1, 0, true, "residual 2960/2653/3158 positive max" },
        { 0, 0, 1, true, "residual 2960/2653/3158 positive bonus" },
        { 1, -1, 1, false, "residual 2960/2653/3158 neg max suppress" },
        { 0, 0, -1, false, "residual 2960/2653/3158 neg bonus" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinCanDropGil3231(c.gilMin, c.gilMax, c.gilBonus);
    ...
    ok = expect(CanDropGil(1, 0, 0) == pinCanDropGil3231(1, 0, 0), "free == pin residual min only") && ok;
    ok = expect(CanDropGil(0, 1, 0) == pinCanDropGil3231(0, 1, 0), "free == pin residual max only") && ok;
    ok = expect(CanDropGil(0, 0, 1) == pinCanDropGil3231(0, 0, 1), "free == pin residual bonus only") && ok;
    ok = expect(CanDropGil(0, 0, 0) == pinCanDropGil3231(0, 0, 0), "free == pin residual zero all") && ok;
    ok = expect(CanDropGil(1, -1, 1) == pinCanDropGil3231(1, -1, 1),
    ...
    for (const int16 gilMin : poles)
    {
        for (const int16 gilMax : poles)
        {
            for (const int16 gilBonus : poles)
            {
                ...
                const bool pin = pinCanDropGil3231(gilMin, gilMax, gilBonus);
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin = pinCanDropGil3231(h.gilMin, h.gilMax, h.gilBonus);
