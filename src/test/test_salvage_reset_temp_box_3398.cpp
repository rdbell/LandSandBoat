#include "test_salvage_reset_temp_box_3398.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage reset temp box 3398 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldResetTempBox3398:
...
// slice 3398 dedicated dual-wire expand residual 2898).
//
// Coverage:
...
//   - residual 1083 / 2898 / 3146 pins still hold
auto runSalvageResetTempBox3398SelfTests() -> bool
{
    ...
    // Residual 1083 / 2898 / 3146 ShouldResetTempBox pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinShouldResetTempBox3146(c.status);
        ...
        ok = expect(got == pinPrior, "dual-wire free==pinPrior3146") && ok;
