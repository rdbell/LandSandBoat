#include "test_assault_issue_tag_3258.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3258 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish option==1 gate for dual-wire cross-check
// (dedicated 3258 expand residual 2867 / prior 3145):
...
// Compact dual-wire pin matching Go pinShouldIssueNewTag3258:
...
// internal/assault; dedicated slice 3258 expand residual 2867 / prior
// dedicated 3145 — formula unchanged).
//
// Coverage:
//   - free == inline == pin == option==kRytaalOptionObtainTag&&!hasKI
//   - residual poles: option1×hasKI / wrong option
//   - dense option × hasTag space
//   - residual 1100 / 2867 / prior 3145 pins still hold
...
auto runAssaultIssueTag3258SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / prior 3145 ShouldIssueNewTag pins still hold.
