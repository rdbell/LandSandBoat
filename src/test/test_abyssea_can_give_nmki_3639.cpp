#include "test_abyssea_can_give_nmki_3639.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "abyssea CanGiveNMKI 3639 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.abyssea.canGiveNMKI formula for dual-wire checks (slice 3639).
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3639 / C++ capacity
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3594 (prior dedicated
// expand residual 2861 independence pins).
auto pinCanGiveNMKI3594(const int32 roll1to100, const int32 dropChance, const bool redProc) -> bool
{
    return roll1to100 <= dropChance || redProc;
}
...
// dedicated slice 3639 expand residual 2861 / prior 3594 / 3549 / 3491 / 3434 /
// 3314 / 3284 / 3238 / 3089 — formula unchanged).
//
// Coverage (clones 3594 density on CanGiveNMKI primary + prior 3594 independence):
//   - free == inline == pin == pin3594 == (roll <= chance || redProc)  // positive OR
...
//   - residual 1041 / 2861 / prior 3089 / prior 3238 / 3284 / 3314 / 3434 / 3491 / 3549 / 3594 pins still hold
...
auto runAbysseaCanGiveNMKI3639SelfTests() -> bool
{
    ...
    // Residual constant / 1041 / 2861 / 3089 / 3238 / 3284 / 3314 / 3434 / 3491 / 3549 / 3594 pins still hold.
    ...
    // --- Composition table: free == inline == pin == pin3594 (positive OR form) ---
    const struct
    {
        int32       roll;
        int32       chance;
        ...
        // Residual 2861 / 3089 / 3434 / 3491 / 3549 / 3594 poles.
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pin3594   = pinCanGiveNMKI3594(pole.roll, pole.chance, pole.redProc);
        const bool want      = pole.roll <= pole.chance || pole.redProc;
        ok                   = expect(got == want, pole.label) && ok;
        ok                   = expect(got == inlineGot && got == pinGot && got == pin3594,
                                      "pole free == inline == pin == pin3594") &&
             ok;
    }

    // Dense rolls 1..100 edges for normal + atma chances: free == inline == pin == pin3594.
    for (const int32 chance : { NormalNMKIDropChance, AtmaNMKIDropChance })
    {
        for (int32 roll = 1; roll <= 100; ++roll)
        {
            for (const bool red : { false, true })
            {
                ...
                const bool pin3594   = pinCanGiveNMKI3594(roll, chance, red);
                const bool want      = roll <= chance || red;
                ok                   = expect(got == want, "dense free == formula") && ok;
                ok                   = expect(got == inlineGot && got == pinGot && got == pin3594,
                                              "dense free == inline == pin == pin3594") &&
                     ok;
            }
        }
    }
    ...
    for (const int32 chance : { NormalNMKIDropChance, AtmaNMKIDropChance, 0, 50, 100 })
    {
        for (const int32 roll : { 0, -1, 101, 200 })
        {
            for (const bool red : { false, true })
            {
                ...
                const bool pin3594   = pinCanGiveNMKI3594(roll, chance, red);
                const bool want      = roll <= chance || red;
                ok                   = expect(got == want && got == inlineGot && got == pinGot && got == pin3594,
                                              "edge free == inline == pin == pin3594 == formula") &&
                     ok;
            }
        }
    }
    ...
    // Prior dedicated expand residual 3594 pin helpers still agree with free dual-wire.
    ok = expect(CanGiveNMKI(20, 20, false) == pinCanGiveNMKI3594(20, 20, false),
                "free == prior 3594 pin boundary") &&
         ok;
    ok = expect(CanGiveNMKI(21, 20, false) == pinCanGiveNMKI3594(21, 20, false),
                "free == prior 3594 pin short") &&
         ok;
    ok = expect(CanGiveNMKI(100, 0, true) == pinCanGiveNMKI3594(100, 0, true),
                "free == prior 3594 pin red-proc force") &&
         ok;
    ok = expect(pinCanGiveNMKI(20, 20, false) == pinCanGiveNMKI3594(20, 20, false),
                "3639 pin == prior 3594 pin boundary") &&
         ok;
    ok = expect(pinCanGiveNMKI(21, 20, false) == pinCanGiveNMKI3594(21, 20, false),
                "3639 pin == prior 3594 pin short") &&
         ok;
    ok = expect(pinCanGiveNMKI(100, 0, true) == pinCanGiveNMKI3594(100, 0, true),
                "3639 pin == prior 3594 pin red-proc force") &&
         ok;
