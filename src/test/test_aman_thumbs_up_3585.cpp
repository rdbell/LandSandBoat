#include "test_aman_thumbs_up_3585.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3585 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3585 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3585 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3541 pin for independence cross-check.
auto pinCanThumbsUp3541(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3541(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3585 expand residual 2967; prior dedicated 3541 / 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == pin3541 == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 pins still hold
//   - prior 3541 independence (free == 3541 inline/pin)
...
auto runAmanThumbsUp3585SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3541 re-pins (independence).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3541 = pinCanThumbsUp3541(c.cooldownExpired);
        ...
        ok = expect(got == pin3541, "CanThumbsUp dual-wire == pin3541") && ok;
    ...
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3541(true), "free==pin3541 true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3541(false), "free==pin3541 false") && ok;

    // Explicit residual poles free == inline == pin == pin3541 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3541 = pinCanThumbsUp3541(pole);
        ok                 = expect(got == pole, "pole free == identity") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3541, "pole free == pin3541") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3541.
    for (const bool cooldownExpired : { false, true })
    {
        ...
        ok              = expect(got == pinCanThumbsUp3541(cooldownExpired), "compose free == pin3541") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp(cooldownExpired),
             ...
             ok;
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp3541(cooldownExpired),
                    "host inject free == pin3541") &&
             ok;
    ...
    // Prior dedicated 3541 independence: free == prior 3541 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3541(true), "prior 3541 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3541(false), "prior 3541 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3541(true), "prior 3541 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3541(false), "prior 3541 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 0698 still hold under dedicated 3585.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/3541/0698 under 3585") && ok;
