#include "test_besieged_afford_sanction_3662.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "besieged CanAffordSanction 3662 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onEventFinish sanctionCost for dual-wire cross-check (slice 3662):
//   sanctionCost = 100
...
// Compact dual-wire pin matching Go pinCanAffordSanction3662 / C++ capacity
// (direct return form):
//   imperialStanding >= SanctionAffordCost(option)
...
// Prior dedicated pin matching Go pinCanAffordSanction3617 (formula identical):
//   imperialStanding >= SanctionAffordCost(option)
auto pinCanAffordSanction3617(const int32 option, const int32 imperialStanding) -> bool
{
    return imperialStanding >= besiegedhelpers::SanctionAffordCost(option);
}
...
// internal/besieged; dedicated slice 3662 expand residual 2945 —
// formula unchanged; prior dedicated ~3617).
//
// Coverage:
//   - free == inline == pin == pin3617 == (standing >= SanctionAffordCost(option))
//   - residual 1120 / 2945 / 3093 / 3227 / 3277 / 3307 / 3418 / 3472 / 3529 / 3572 / 3617 pins still hold
...
auto runBesiegedAffordSanction3662SelfTests() -> bool
{
    ...
    // Residual constant / 1120 / 2945 / 3093 / 3227 / 3277 / 3307 / 3418 / 3472 / 3529 / 3572 / 3617 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinCanAffordSanction3617(c.option, c.standing);
        ...
        ok = expect(got == pinPrior, "CanAffordSanction dual-wire == pin3617") && ok;
        ok = expect(got == wantPin, "CanAffordSanction == standing >= SanctionAffordCost") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3617.
    const struct
    {
        int32       option;
        int32       standing;
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pinPrior = pinCanAffordSanction3617(pole.option, pole.standing);
        ...
        ok                  = expect(got == pinPrior, "pole free == pin3617") && ok;
    }
    ...
    // Compose: free == inline == pin == pin3617 for short / exact / surplus poles per option.
    ...
    for (const int32 opt : sanctionOpts)
    {
        ...
        for (const int32 delta : { -1, 0, 1 })
        {
            ...
            const bool  pinPrior = pinCanAffordSanction3617(opt, st);
            ok                   = expect(got == inlineF, "compose free == inline") && ok;
            ok                   = expect(got == pinGot, "compose free == pin") && ok;
            ok                   = expect(got == pinPrior, "compose free == pin3617") && ok;
    ...
    for (const int32 opt : options)
    {
        for (const int32 st : standings)
        {
            ...
            ok                  = expect(got == pinCanAffordSanction3617(opt, st), "dense free == pin3617") && ok;
        }
    }
