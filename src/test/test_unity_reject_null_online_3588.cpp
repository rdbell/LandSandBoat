#include "test_unity_reject_null_online_3588.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "unitychat ShouldRejectNullOnlineMember 3588 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember / DelOnlineMember null-PChar gate for dual-wire
// cross-check (slice 3588 dedicated expand residual 3075; prior dedicated
// 3544 / 3487 / 3436 / 3386):
//   charNull
...
// Compact dual-wire pin matching free function / capacity body (slice 3588).
// Direct return only — same formula as production ShouldRejectNullOnlineMember.
// free == inline == pin == pin3544.
...
// slice 3588 dedicated expand residual 3075; prior dedicated 3544 / 3487 /
// 3436 / 3386 retained). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3544 (direct return formula)
//   - residual 1356 / 3075 pins still hold
//   - prior 3386 / 3436 / 3487 / 3544 dedicated poles still hold
//   - dense 2¹
...
//     test_unity_reject_null_online_3544
...
auto runUnityRejectNullOnline3588SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3588(c.charNull);
        const bool pin3544 = pinShouldRejectNullOnlineMember3544(c.charNull);
        ...
        ok = expect(got == pin3544, "ShouldRejectNullOnlineMember == pin3544") && ok;
    ...
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3588(charNull);
        const bool pin3544 = pinShouldRejectNullOnlineMember3544(charNull);

        ok = expect(got == want, "compose free == pin formula") && ok;
        ok = expect(got == inlineF, "compose free == inline") && ok;
        ok = expect(got == pin, "compose free == pin 3588") && ok;
        ok = expect(got == pin3544, "compose free == pin3544") && ok;
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3588(h.charNull);
        const bool pin3544 = pinShouldRejectNullOnlineMember3544(h.charNull);
        ...
        ok                 = expect(reject == pin3544, "host compose free == pin3544") && ok;
