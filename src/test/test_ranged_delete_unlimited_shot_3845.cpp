#include "test_ranged_delete_unlimited_shot_3845.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranged ShouldDeleteUnlimitedShot 3845 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack UnlimitedShot delete gate for dual-wire cross-check
// (slice 3845 dedicated expand residual 3000; prior 3800 retained):
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto inlineShouldDeleteUnlimitedShot3845(
...
// (dedicated slice 3845). Early-return form only — same formula.
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
...
// Prior 3800 pin retained for independence cross-check (same formula).
// free == inline == pin == pin3800.
...
// slice 3845 dedicated expand residual 3000; prior dedicated 3800 retained).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3800 (early-return formula)
//   - residual poles: no US; hit always delete; miss retain <=0 / >0; residual 1390/3000/3399/3451/3500/3575/3620/3665/3710/3755/3800
//   - dense hasUS × hit × retain compose
//   - prior 3800 independence (suite retained)
...
auto runRangedDeleteUnlimitedShot3845SelfTests() -> bool
{
    ...
    // Residual 1390 / residual dual-wire 3000 / dedicated 3399 / 3451 / 3500 / 3575 / 3620 / 3665 / 3710 / 3755 / 3800 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1390 / 3000 / 3399 / 3451 / 3500 / 3575 / 3620 / 3665 / 3710 / 3755 / 3800 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDeleteUnlimitedShot3845(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pinF    = pinShouldDeleteUnlimitedShot3845(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pin3800 = pinShouldDeleteUnlimitedShot3800(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
    ...
    // Prior 3800 independence: free == prior 3800 pin poles (suite retained).
    // free == inline == pin == pin3800.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3800(true, true, 5),
                "prior 3800 independence: free == 3800 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3800(false, true, 0),
                "prior 3800 independence: free == 3800 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3800(true, false, 1),
                "prior 3800 independence: free == 3800 pin retain keep") &&
         ok;
    ok = expect(pinShouldDeleteUnlimitedShot3845(true, false, 0) == pinShouldDeleteUnlimitedShot3800(true, false, 0),
                "prior 3800 independence: 3845 pin == 3800 pin miss retain 0") &&
         ok;
    ok = expect(inlineShouldDeleteUnlimitedShot3845(true, true, 5) == pinShouldDeleteUnlimitedShot3800(true, true, 5),
                "prior 3800 independence: 3845 inline == 3800 pin del hit") &&
         ok;

    // Dense compose: hasUnlimitedShot × hitOccured × retain poles — free == inline == pin == pin3800.
    for (const bool hasUS : { false, true })
    {
        for (const bool hit : { false, true })
        {
                                        ...
                                        static_cast<int16>(1), static_cast<int16>(5), static_cast<int16>(100) })
            {
                ...
                ok              = expect(got == inlineShouldDeleteUnlimitedShot3845(hasUS, hit, retain),
                            "compose free == inline") &&
                     ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3845(hasUS, hit, retain), "compose free == pin") && ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3800(hasUS, hit, retain), "compose free == pin3800") && ok;
            }
        }
    }
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pin3800, "compose free == pin3800") && ok;
    }
    ...
    // Prior 3800 independence under compose: free == 3800 pin residual poles.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3800(true, true, 5),
                "prior 3800 independence compose: free == 3800 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3800(false, true, 0),
                "prior 3800 independence compose: free == 3800 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3800(true, false, 1),
                "prior 3800 independence compose: free == 3800 pin retain keep") &&
         ok;
