#include "test_zone_high_targid_3892.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "zone ShouldRejectHighCharTargid 3892 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline IncreaseZoneCounter high-char-targid formula for dual-wire cross-check
// (dedicated slice 3892 expand residual 2949; prior dedicated 3847 / 3802 / 3757 /
// 3712 / 3667 / 3622 / 3577 / 3520 / 3457 / 3384):
...
// Compact dual-wire pin matching Go pinShouldRejectHighCharTargid3892 /
...
// Prior dedicated 3847 pin form (free == inline == pin == pin3847):
//   targid >= CharTargidHighThreshold  // pin 0x700
auto pinShouldRejectHighCharTargid3847(const uint16 targid) -> bool
{
    return targid >= zonehelpers::CharTargidHighThreshold;
}
...
// dedicated slice 3892 expand residual 2949; prior dedicated 3847 / 3802 / 3757 /
// 3712 / 3667 / 3622 / 3577 / 3520 / 3457 / 3384 / pure 1363).
//
// Coverage:
//   - free == inline == pin == pin3847 == (targid >= CharTargidHighThreshold)
//   - residual 1363 / 2949 / prior 3384 / prior 3457 / prior 3520 / prior 3577 / prior 3622 / prior 3667 / prior 3712 / prior 3757 / prior 3802 / prior 3847 poles still hold
...
auto runZoneHighTargid3892SelfTests() -> bool
{
    ...
    // Residual 1363 / 2949 / prior 3384 / prior 3457 / prior 3520 / prior 3577 / prior 3622 / prior 3667 / prior 3712 / prior 3757 / prior 3802 / prior 3847 pins still hold.
    ...
    // --- Composition table: free == inline == pin == pin3847 ---
    const struct
    {
        uint16      targid;
        bool        want;
        ...
        // Prior dedicated 3622 identity / polarity repeats.
        { 0x6FF, false, "prior 3622 identity under threshold" },
        { 0x700, true, "prior 3622 identity at threshold" },
        { 0x701, true, "prior 3622 identity over threshold" },

        // Prior dedicated 3667 identity / polarity repeats.
        { 0x6FF, false, "prior 3667 identity under threshold" },
        { 0x700, true, "prior 3667 identity at threshold" },
        { 0x701, true, "prior 3667 identity over threshold" },

        // Prior dedicated 3712 identity / polarity repeats.
        { 0x6FF, false, "prior 3712 identity under threshold" },
        { 0x700, true, "prior 3712 identity at threshold" },
        { 0x701, true, "prior 3712 identity over threshold" },

        // Prior dedicated 3757 identity / polarity repeats.
        { 0x6FF, false, "prior 3757 identity under threshold" },
        { 0x700, true, "prior 3757 identity at threshold" },
        { 0x701, true, "prior 3757 identity over threshold" },

        // Prior dedicated 3802 identity / polarity repeats.
        { 0x6FF, false, "prior 3802 identity under threshold" },
        { 0x700, true, "prior 3802 identity at threshold" },
        { 0x701, true, "prior 3802 identity over threshold" },

        // Prior dedicated 3847 identity / polarity repeats.
        { 0x6FF, false, "prior 3847 identity under threshold" },
        { 0x700, true, "prior 3847 identity at threshold" },
        { 0x701, true, "prior 3847 identity over threshold" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinShouldRejectHighCharTargid3847(c.targid);
        const bool wantPin  = c.targid >= CharTargidHighThreshold;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3847.
        ok = expect(got == inlineF && got == pin && got == pinPrior, "dual-wire free == inline == pin == pin3847") && ok;
        ok = expect(got == wantPin, "free == pin formula targid>=0x700") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3847 (0x6FF / 0x700).
    const struct
    {
        uint16      targid;
        bool        want;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pinPrior = pinShouldRejectHighCharTargid3847(p.targid);
        ok                  = expect(got == p.want, p.label) && ok;
        ok                  = expect(got == inlineF && got == pin && got == pinPrior, "pole free == inline == pin == pin3847") && ok;
    }

    // Dense dual-wire sweep around the threshold boundary:
    // free == inline == pin == pin3847 for targid in [0x6F0, 0x710].
    for (uint16 targid = 0x6F0; targid <= 0x710; ++targid)
    {
        ...
        const bool pinPrior = pinShouldRejectHighCharTargid3847(targid);
        const bool want     = targid >= 0x700;
        ok                  = expect(got == want, "boundary free == pin") && ok;
        ok                  = expect(got == inlineF && got == pin && got == pinPrior, "boundary free == inline == pin == pin3847") && ok;
    }

    // Host-style inject poles: free == inline == pin == pin3847 for IncreaseZoneCounter.
    ...
    for (const uint16 targid : samples)
    {
        ...
        ok                  = expect(got == inlineF && got == pin && got == pinPrior, "host inject free == inline == pin == pin3847") && ok;
