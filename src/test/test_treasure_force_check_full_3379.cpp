#include "test_treasure_force_check_full_3379.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3379 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3379):
...
// Compact dual-wire pin matching Go pinShouldForceCheckOnFullPoolInsert3379:
//   slotAfterFreeScan == PoolSize
...
// slice 3379 expand residual 2981).
//
// Coverage:
