#include "test_unity_reject_null_online_3813.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "unitychat ShouldRejectNullOnlineMember 3813 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember / DelOnlineMember null-PChar gate for dual-wire
// cross-check (slice 3813 dedicated expand residual 3075; prior dedicated
// 3768 / 3723 / 3678 / 3633 / 3588 / 3544 / 3487 / 3436 / 3386):
//   charNull
...
// Compact dual-wire pin matching free function / capacity body (slice 3813).
// Direct return only — same formula as production ShouldRejectNullOnlineMember.
// free == inline == pin == pin3768.
...
// slice 3813 dedicated expand residual 3075; prior dedicated 3768 / 3723 / 3678 / 3633 /
// 3588 / 3544 / 3487 / 3436 / 3386 retained). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3768 (direct return formula)
//   - residual 1356 / 3075 pins still hold
//   - prior 3386 / 3436 / 3487 / 3544 / 3588 / 3633 / 3678 / 3723 / 3768 dedicated poles still hold
//   - dense 2¹
...
auto runUnityRejectNullOnline3813SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3813(c.charNull);
        const bool pin3768 = pinShouldRejectNullOnlineMember3768(c.charNull);
        ...
        ok = expect(got == pin3768, "ShouldRejectNullOnlineMember == pin3768") && ok;
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3813(h.charNull);
        const bool pin3768 = pinShouldRejectNullOnlineMember3768(h.charNull);
        ...
        ok                 = expect(reject == pin3768, "host compose free == pin3768") && ok;
