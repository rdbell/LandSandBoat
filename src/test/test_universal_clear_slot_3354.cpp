#include "test_universal_clear_slot_3354.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "universal ShouldClearSlot 3354 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ClearSlot range gate for dual-wire cross-check (dedicated 3354):
//   slotInRange
...
// Compact dual-wire pin matching Go pinShouldClearSlot3354 / C++ capacity:
//   slotInRange
...
// dedicated slice 3354 expand residual 2813 / prior 2980).
//
// Coverage:
//   - free == inline == pin == slotInRange
//   - residual poles (2813 / prior 2980) still hold
...
auto runUniversalClearSlot3354SelfTests() -> bool
{
    ...
    // Residual 2813 / prior 2980 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Classic dual poles / residual 2813+prior 2980 surface.
        { true, true, "in-range accepts" },
        { false, false, "out-of-range rejects" },

        // Residual 2813 / prior 2980 pins (full 2^1 surface, labeled).
