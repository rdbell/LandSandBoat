#include "test_salvage_claim_transport_3687.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage claim transport 3687 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanClaimTransport3687:
...
// Prior dedicated 3642 pin (same formula; residual suite identity).
auto pinCanClaimTransport3642(const uint32 transportUserID) -> bool
{
    return !salvagehelpers::TransportUserBusy(transportUserID);
}
...
// slice 3687 dedicated dual-wire expand residual 2871).
//
// Coverage:
//   - id == 0 → claim true (positive pin form)
//   - non-zero → claim false / busy true
//   - max uint32 busy
//   - free == inline == pin == pin3642 == !TransportUserBusy
//   - dense poles: free / busy residual + large / max
//   - residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 / 3552 / 3597 / 3642 pins still hold
//   - prior 3642 independence: free == pin3642 across poles
auto runSalvageClaimTransport3687SelfTests() -> bool
{
    ...
    // Residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 / 3552 / 3597 / 3642 pins still hold under dual-wire.
    ...
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport(0xFFFFFFFF),
         ...
         ok;
    ok = expect(CanClaimTransport(0) == pinCanClaimTransport3642(0),
                "residual 3642: free == pin for id 0") &&
         ok;
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport3642(0xFFFFFFFF),
                "residual 3642: free == pin for max busy") &&
         ok;
    ...
    ok = expect(pinCanClaimTransport3642(0), "eligible prior pin3642 id 0 free should claim") && ok;
    ...
    ok = expect(!pinCanClaimTransport3642(0xFFFFFFFF), "prior pin3642 max busy should not claim") && ok;
    ...
    // --- Composition table: free == inline == pin == pin3642 == !TransportUserBusy ---
    // Dense poles: free / busy residual + large / max.
    const struct
    {
        uint32      id;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior   = pinCanClaimTransport3642(c.id);
        ...
        ok = expect(gotClaim == pinPrior, "dual-wire free==pin3642") && ok;
    ...
    for (const uint32 id : composeIds)
    {
        ...
        ok             = expect(got == pinCanClaimTransport3642(id), "compose pin3642") && ok;
    }
    ...
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport(99),
         ...
         ok;
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport3642(99),
                "formula busy free == pin3642") &&
         ok;
