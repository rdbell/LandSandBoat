#include "test_besieged_afford_sanction_3227.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "besieged CanAffordSanction 3227 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onEventFinish sanctionCost for dual-wire cross-check (slice 3227):
//   sanctionCost = 100
...
// Compact dual-wire pin matching Go pinCanAffordSanction3227 / C++ capacity
// (direct return form):
//   imperialStanding >= SanctionAffordCost(option)
...
// internal/besieged; dedicated slice 3227 expand residual 2945 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == (standing >= SanctionAffordCost(option))
//   - residual 1120 / 2945 / 3093 pins still hold
...
auto runBesiegedAffordSanction3227SelfTests() -> bool
{
    ...
    // Residual constant / 1120 / 2945 / 3093 pins still hold under dual-wire.
