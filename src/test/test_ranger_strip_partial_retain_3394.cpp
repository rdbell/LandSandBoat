#include "test_ranger_strip_partial_retain_3394.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranger ShouldStripPartialStealthOnRetain 3394 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack retain-success partial-stealth gate for dual-wire
// cross-check (dedicated 3394):
//   !stripAllDetectable
auto inlineShouldStripPartialStealthOnRetain3394(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Compact dual-wire pin matching Go pinShouldStripPartialStealthOnRetain3394 /
// C++ capacity form (formula unchanged from 1391 / 3082):
//   !stripAllDetectable
...
// dedicated slice 3394; residual expand 3082 / pure 1391).
//
// Coverage:
//   - free == inline == pin
//   - residual 3082 / 1391 pins still hold
...
auto runRangerStripPartialRetain3394SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3394(c.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3394(c.stripAllDetectable);
    ...
    for (const bool stripAll : { false, true })
    {
        ...
        const bool pin     = pinShouldStripPartialStealthOnRetain3394(stripAll);
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3394(stripAll);
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3394(h.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3394(h.stripAllDetectable);
