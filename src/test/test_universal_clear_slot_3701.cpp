#include "test_universal_clear_slot_3701.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "universal ShouldClearSlot 3701 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ClearSlot range gate for dual-wire cross-check (dedicated 3701):
//   slotInRange
...
// Compact dual-wire pin matching Go pinShouldClearSlot3701 / C++ capacity:
//   slotInRange
...
// Prior 3656 pin for independence cross-check (formula unchanged):
//   slotInRange
auto pinShouldClearSlot3656(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3701 expand residual 2813 / prior 2980 / prior 3354 /
// prior 3415 / prior 3469 / prior 3532 / prior 3566 / prior 3611 / prior 3656).
//
// Coverage:
//   - free == inline == pin == pin3656 == slotInRange
//   - residual poles (2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 / prior 3532 / prior 3566 / prior 3611 / prior 3656) still hold
//   - prior 3656 pin independence
...
auto runUniversalClearSlot3701SelfTests() -> bool
{
    ...
    // Residual 2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 / prior 3532 / prior 3566 / prior 3611 / prior 3656 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Classic dual poles / residual 2813+prior 2980+prior 3354+prior 3415+prior 3469+prior 3532+prior 3566+prior 3611+prior 3656 surface.
        { true, true, "in-range accepts" },
        { false, false, "out-of-range rejects" },

        // Residual 2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 / prior 3532 / prior 3566 / prior 3611 / prior 3656 pins (full 2^1 surface, labeled).
    ...
    for (const auto& c : cases)
    {
        ...
        // free == pin3656.
        ok = expect(got == pinShouldClearSlot3656(c.slotInRange), "free == pin3656") && ok;
    }
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3656.
    for (const bool slotInRange : { false, true })
    {
        ...
        const bool pin3656 = pinShouldClearSlot3656(slotInRange);
        const bool want    = slotInRange;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin, "compose free == inline == pin") && ok;
        ok                 = expect(got == pin3656, "compose free == pin3656") && ok;
    }
    ...
    // Prior 3656 pin independence: free == prior 3656 pin.
    for (const bool slotInRange : { false, true })
    {
        const bool got      = ShouldClearSlot(slotInRange);
        const bool priorPin = pinShouldClearSlot3656(slotInRange);
        ok                  = expect(got == priorPin, "prior 3656 pin independence") && ok;
    }
