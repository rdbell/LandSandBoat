#include "test_salvage_reset_temp_box_3146.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage reset temp box 3146 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldResetTempBox3146:
...
//   - free == inline == pin == status==kStatusNormal
//   - dense poles: 0, 1, 2, kStatusNormal, kStatusDisappear, 99, 255
//   - residual 1083 / 2898 pins still hold
auto runSalvageResetTempBox3146SelfTests() -> bool
{
    ...
    // Residual 1083 / 2898 ShouldResetTempBox pins still hold under dual-wire.
