#include "test_colonizationreive_spawn_on_enable_3638.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "colonizationreive ShouldSpawnOnEnable 3638 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua enableReive spawn gate for dual-wire cross-check
// (dedicated 3638 expand residual 2882 / prior 3593 / prior 3499 / prior 3465 /
// prior 3411 / prior 3335 / prior 3163):
...
// Compact dual-wire pin matching Go pinShouldSpawnOnEnable3638 / C++ capacity
// (return !isAlive):
//   !isAlive
...
// Prior dedicated 3593 pin (independence / free==inline==pin==pin3593):
//   !isAlive
...
// OmegaXI internal/colonizationreive; dedicated slice 3638 expand residual
// 2882 / prior dedicated 3593 / prior dedicated 3499 / prior dedicated 3465 /
// prior dedicated 3411 / prior dedicated 3335 / prior dedicated 3163 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3593 == !isAlive
//   - residual poles: isAlive true / false
//   - dense 2^1 boolean space
//   - residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 pins still hold
...
auto runColonizationreiveSpawnOnEnable3638SelfTests() -> bool
{
    ...
    // Residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3593 = pinShouldSpawnOnEnable3593(c.isAlive);
        ...
        ok = expect(got == pin3593, "ShouldSpawnOnEnable dual-wire == pin3593") && ok;
    ...
    ok = expect(ShouldSpawnOnEnable(false) == pinShouldSpawnOnEnable3593(false), "free==pin3593 false") && ok;
    ok = expect(ShouldSpawnOnEnable(true) == pinShouldSpawnOnEnable3593(true), "free==pin3593 true") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3593 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3593 = pinShouldSpawnOnEnable3593(pole);
        ok                 = expect(got == !pole, "pole free == !isAlive") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3593, "pole free == pin3593") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3593.
    for (const bool isAlive : { false, true })
    {
        ...
        ok              = expect(got == pinShouldSpawnOnEnable3593(isAlive), "compose free == pin3593") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable(isAlive),
             ...
             ok;
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable3593(isAlive),
                    "host inject free == pin3593") &&
             ok;
