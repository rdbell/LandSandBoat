#include "test_enmity_zero_out_of_range_3769.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "enmity ShouldZeroEnmityOutOfRange 3769 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline UpdateEnmity out-of-range CE/VE zero gate for dual-wire cross-check
// (slice 3769 dedicated expand residual 2927; prior dedicated expand 3724 /
// 3679 / 3634 / 3589 / 3545 / 3485 / 3427 / 3333 / prior dedicated 3157):
//   !withinRange
// Direct return form matching production free function / capacity.
auto inlineShouldZeroEnmityOutOfRange3769(const bool withinRange) -> bool
{
    return !withinRange;
}

// Compact dual-wire pin matching free function / capacity body (slice 3769).
...
// Positive form: free == inline == pin == pin3724.
...
//
// Coverage:
//   - free == inline == pin == pin3724 (direct return !withinRange identity-not)
...
//     prior expand 3724 pins still hold
auto runEnmityZeroOutOfRange3769SelfTests() -> bool
{
    ...
    // Residual poles: free == inline == pin == pin3724 (direct return).
    const struct
    {
        bool        withinRange;
        bool        want;
        ...
        // Prior dedicated expand 3724 poles (independence).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3769(p.withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3769(p.withinRange);
    ...
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3769(false), "free==pin false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3769(true), "free==pin true") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3724(false), "free==pin3724 false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3724(true), "free==pin3724 true") && ok;

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3724.
    for (const bool withinRange : { false, true })
    {
        const bool got     = ShouldZeroEnmityOutOfRange(withinRange);
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3769(withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3769(withinRange);
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok                = expect(got == inlineShouldZeroEnmityOutOfRange3769(within), "compose free == inline") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3769(within), "compose free == pin") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3724(within), "compose free == pin3724") && ok;
    }

    // Residual 2927 / prior dedicated expand 3724 / 3679 / 3634 / 3589 / 3545 / 3485 / 3427 / 3333 / prior dedicated 3157 /
    // pure 1357 still hold under expand suite.
    ok = expect(ShouldZeroEnmityOutOfRange(false) && !ShouldZeroEnmityOutOfRange(true),
                "residual 2927/3724/3679/3634/3589/3545/3485/3427/3333/3157/1357 pins failed under 3769 suite") &&
         ok;
