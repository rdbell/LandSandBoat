...
auto runSQLiteQueryTranslationSelfTests() -> bool
{
    ...
    const std::vector<QueryCase> cases{
        ...
        { "update trailing limit",
          "UPDATE chars SET playtime = ? WHERE charid = ? LIMIT 1",
          "UPDATE chars SET playtime = ? WHERE charid = ?" },
        { "delete trailing limit",
          "DELETE FROM char_vars WHERE charid = ? AND varname = ? limit 1  ",
          "DELETE FROM char_vars WHERE charid = ? AND varname = ?" },
        { "delete alias trailing limit",
          "DELETE char_equip FROM char_equip WHERE charid = ? AND slotid = ? LIMIT 1",
          "DELETE FROM char_equip WHERE charid = ? AND slotid = ?" },
        ...
        { "generic update limit rewrite is case insensitive",
          "update chars left join char_stats using(charid) set titles = ?, title = ? where charid = ? limit 1",
          "update chars left join char_stats using(charid) set titles = ?, title = ? where charid = ?" },
