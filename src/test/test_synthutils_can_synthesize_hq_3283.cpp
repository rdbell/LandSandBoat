#include "test_synthutils_can_synthesize_hq_3283.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synthutils CanSynthesizeHQ 3283 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline canSynthesizeHQ pure half for dual-wire cross-check
// (slice 3283 dedicated expand residual 2876 / prior ~3244):
...
// Compact dual-wire pin matching free function / capacity body (slice 3283).
...
// Prior dedicated pin (slice 3244) — same formula; retained under 3283.
...
// OmegaXI internal/synthutils; slice 3283 dedicated expand residual 2876 /
// prior ~3244). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2876 / prior dedicated 3244 / pure 1922 poles still hold
...
auto runSynthutilsCanSynthesizeHQ3283SelfTests() -> bool
{
    ...
    // Residual 2876 / 1922 / prior dedicated 3244 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 2876 / 1922 / prior 3244 classic dual poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3283(p.antiHQMod);
        const bool prior   = pinCanSynthesizeHQ3244(p.antiHQMod);
        ...
        ok = expect(got == prior, "CanSynthesizeHQ == prior 3244 pin") && ok;
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3283(e.antiHQMod);
        const bool prior   = pinCanSynthesizeHQ3244(e.antiHQMod);
        ...
        ok = expect(got == prior, "dense edge free == prior 3244 pin") && ok;
    }
    ...
    for (int16 m = -200; m <= 200; ++m)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3283(m);
        const bool prior   = pinCanSynthesizeHQ3244(m);
        const bool want    = m == 0;
        ...
        ok = expect(got == prior, "dense compose free == prior 3244 pin") && ok;
    }
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok = expect(got == prior, "host pole free == prior 3244 pin") && ok;
    }
