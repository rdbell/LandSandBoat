#include "test_portermoogle_can_retrieve_item_3286.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "portermoogle CanRetrieveItem 3286 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua porter_moogle hasItem / freeSlots gate for dual-wire cross-check
// (dedicated slice 3286):
//   !hasItem && freeSlots > 0
...
// Compact dual-wire pin matching Go pinCanRetrieveItem3286 / C++ capacity
...
// (Lua porter_moogle hasItem + freeSlots retrieve gate; dedicated slice 3286
// expand residual 2966 / prior dedicated ~3243 / 3171 / pure 1144 — formula
// unchanged).
//
// Coverage:
//   - free == inline == pin == (!hasItem && freeSlots > 0) (pin direct return)
//   - residual 1144 / 2966 / prior 3171 / 3243 pins still hold
...
auto runPortermoogleCanRetrieveItem3286SelfTests() -> bool
{
    ...
    // Residual 1144 / 2966 / prior 3171 / 3243 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2966 / prior 3171 / 3243 happy / multi free (n).
