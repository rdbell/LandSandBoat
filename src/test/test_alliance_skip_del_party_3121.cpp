#include "test_alliance_skip_del_party_3121.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldSkipDelPartyWhenEmpty 3121 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline delParty empty-alliance formula for dual-wire cross-check (slice 3121):
...
// (!hasAlliance || partyListEmpty; slice 3121).
auto runAllianceSkipDelParty3121SelfTests() -> bool
{
    ...
    // Residual 1341 / prior dual-wire 2941 pins still hold under dual-wire.
