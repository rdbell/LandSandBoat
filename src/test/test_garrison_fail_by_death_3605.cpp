#include "test_garrison_fail_by_death_3605.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3605 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// prior dedicated expand 3560 / dedicated 3605):
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3605:
//   allNPCsDead || allPlayersDead
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3605;
...
//
// Coverage:
//   - free == inline == pin == pin3560 (direct return positive OR; no De Morgan)
//   - 4-row truth table + residual poles + dense 2² free == inline == pin == pin3560
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
//     prior dedicated expand 3330 / prior dedicated expand 3406 /
//     prior dedicated expand 3461 / prior dedicated expand 3514 /
//     prior dedicated expand 3560 pins still hold
...
auto runGarrisonFailByDeath3605SelfTests() -> bool
{
    ...
    // prior dedicated expand 3330 / prior dedicated expand 3406 /
    // prior dedicated expand 3461 / prior dedicated expand 3514 /
    // prior dedicated expand 3560 pins.
    ...
    // 4-row truth table (+ residual re-pins): free == inline == pin == pin3560.
    const struct
    {
        bool        allNPCsDead;
        bool        allPlayersDead;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3560 = pinShouldFailByDeath3560(c.allNPCsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3560, "ShouldFailByDeath dual-wire == pin3560") && ok;
    ...
    // Free == pin == pin3560 across residual poles (4-row truth table).
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath(false, false) &&
                    ShouldFailByDeath(false, false) == pinShouldFailByDeath3560(false, false),
                "free == pin/pin3560 neither dead") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath(true, false) &&
                    ShouldFailByDeath(true, false) == pinShouldFailByDeath3560(true, false),
                "free == pin/pin3560 npcs dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath(false, true) &&
                    ShouldFailByDeath(false, true) == pinShouldFailByDeath3560(false, true),
                "free == pin/pin3560 players dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath(true, true) &&
                    ShouldFailByDeath(true, true) == pinShouldFailByDeath3560(true, true),
                "free == pin/pin3560 both dead") &&
         ok;

    // Dense compose: full 2^2 boolean space free == inline == pin == pin3560.
    for (const bool allNPCsDead : { false, true })
    {
        for (const bool allPlayersDead : { false, true })
        {
            ...
            ok = expect(got == pinShouldFailByDeath(allNPCsDead, allPlayersDead),
                        "compose free == pin") &&
                 ok;
            ok = expect(got == pinShouldFailByDeath3560(allNPCsDead, allPlayersDead),
                        "compose free == pin3560") &&
                 ok;
        }
    }
    ...
    // prior dedicated expand 3330 / prior dedicated expand 3406 /
    // prior dedicated expand 3461 / prior dedicated expand 3514 /
    // prior dedicated expand 3560 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236/3330/3406/3461/3514/3560 wipe gate pins under 3605") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3560(false, false),
                "prior 3560 pin neither dead still holds under 3605") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3560(true, false),
                "prior 3560 pin npcs dead still holds under 3605") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3560(false, true),
                "prior 3560 pin players dead still holds under 3605") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3560(true, true),
                "prior 3560 pin both dead still holds under 3605") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3605.
    ...
    // Explicit residual compose dual-wire free == inline == pin == pin3560 for host poles.
    const struct
    {
        int32       npcCount;
        int32       deadNPCCount;
    ...
    for (const auto& c : composeCases)
    {
        ...
        const bool pin3560   = pinShouldFailByDeath3560(npcsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3560, "compose dual-wire free == pin3560") && ok;
    }
