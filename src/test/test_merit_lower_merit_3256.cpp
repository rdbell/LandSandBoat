#include "test_merit_lower_merit_3256.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "merit ShouldLowerMerit 3256 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline LowerMerit count-decrement formula for dual-wire cross-check
// (slice 3256 dedicated expand residual 3054):
//   count > 0
...
// (count > 0 LowerMerit count-decrement gate; OmegaXI internal/merit; slice 3256
// dedicated expand residual 3054). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 3054 / 2810 poles still hold
...
auto runMeritLowerMerit3256SelfTests() -> bool
{
    ...
    // Residual 3054 / 2810 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3054 / 2810 classic poles (0, 1, n).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinShouldLowerMerit3256(p.count);
                               ...
                               uint8{ 255 } })
    {
        ...
        const bool pin     = pinShouldLowerMerit3256(count);
    ...
    for (const bool present : { false, true })
    {
        for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 15 }, uint8{ 255 } })
        {
            ...
            ok             = expect(got == inlineShouldLowerMerit(count),
                        "compose free == inline") &&
                 ok;
            ok = expect(got == pinShouldLowerMerit3256(count), "compose free == pin") && ok;
