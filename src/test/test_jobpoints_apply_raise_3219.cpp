#include "test_jobpoints_apply_raise_3219.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "jobpoints ShouldApplyRaiseJobPoint 3219 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline RaiseJobPoint spend-gate formula for dual-wire cross-check
// (dedicated slice 3219):
//   cost != 0 && currentJp >= cost
auto inlineShouldApplyRaiseJobPoint3219(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

// Compact dual-wire pin matching Go pinShouldApplyRaiseJobPoint3219 / C++ capacity:
//   cost != 0 && currentJp >= cost
// Direct return of the formula (not if/else true/false).
auto pinShouldApplyRaiseJobPoint3219(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}
...
// dedicated slice 3219 expand residual 3012).
//
// Coverage:
//   - free == inline == pin == (cost != 0 && currentJp >= cost)
//   - residual 2803 / 3012 pins still hold
...
auto runJobpointsApplyRaise3219SelfTests() -> bool
{
    ...
    // Residual 2803 / 3012 pins still hold under dual-wire.
    ok = expect(ShouldApplyRaiseJobPoint(1, 1), "residual 2803/3012: cost 1 / jp 1 applies") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(0, 100), "residual 2803/3012: cost 0 blocks raise") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(10, 9), "residual 2803/3012: short JP blocks raise") && ok;
    ok = expect(ShouldApplyRaiseJobPoint(10, 10), "residual 2803/3012: exact JP applies") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldApplyRaiseJobPoint3219(c.cost, c.currentJp);
        const bool pin     = pinShouldApplyRaiseJobPoint3219(c.cost, c.currentJp);
    ...
    for (const uint8 cost : poleCosts)
    {
        if (cost == 0)
        {
            ...
            for (const uint16 jp : zeroJPs)
            {
                ...
                const bool inlineF = inlineShouldApplyRaiseJobPoint3219(cost, jp);
                const bool pin     = pinShouldApplyRaiseJobPoint3219(cost, jp);
    ...
    for (const auto& c : hostCases)
    {
        ...
        const bool  inlineF = inlineShouldApplyRaiseJobPoint3219(cost, c.currentJp);
        const bool  pin     = pinShouldApplyRaiseJobPoint3219(cost, c.currentJp);
    ...
    // Residual independence (2803 / 3012): spend gate is distinct from presence
