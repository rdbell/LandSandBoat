#include "test_item_increment_count_insert_3624.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "itemcontainer ShouldIncrementCountOnInsertAt 3624 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertItem count-bump gate for dual-wire cross-check (dedicated 3624):
//   slotEmpty && slotID != 0
...
// Compact dual-wire pin matching Go pinShouldIncrementCountOnInsertAt3624 / C++ capacity:
//   slotEmpty && slotID != 0
...
// Prior dedicated 3579 pin (formula unchanged) for free == inline == pin == pin3579.
auto pinShouldIncrementCountOnInsertAt3579(const bool slotEmpty, const std::uint8_t slotID) -> bool
{
    return slotEmpty && slotID != 0;
}
...
// dedicated slice 3624 expand residual 3021; prior dedicated 3579 / 3535 / 3486 / 3432 / 3374 retained).
//
// Coverage:
//   - free == inline == pin == pin3579 == (slotEmpty && slotID != 0)
//   - residual 2802 / 3021 pins still hold
//   - prior 3579 dedicated poles still hold
...
auto runItemIncrementCountInsert3624SelfTests() -> bool
{
    ...
    // Prior dedicated 3579 poles still hold.
    ok = expect(ShouldIncrementCountOnInsertAt(true, 1), "prior 3579 inc empty nonzero") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 1), "prior 3579 no inc occupied") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(true, 0), "prior 3579 no inc empty zero") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 0), "prior 3579 no inc occupied zero") && ok;
    ok = expect(ShouldIncrementCountOnInsertAt(true, 255), "prior 3579 inc empty max") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 255), "prior 3579 no inc occupied max") && ok;

    // --- Composition table: free == inline == pin == pin3579 ---
    const struct
    {
        bool         slotEmpty;
        std::uint8_t slotID;
        ...
        // Prior dedicated 3579 poles still hold.
        { true, 1, true, "prior 3579 empty nonzero" },
        { false, 1, false, "prior 3579 occupied nonzero" },
        { true, 0, false, "prior 3579 empty zero" },
        { false, 0, false, "prior 3579 occupied zero" },
        { true, 120, true, "prior 3579 empty max-container slot" },
        { true, 255, true, "prior 3579 empty max uint8" },
        { false, 120, false, "prior 3579 occupied max-container slot" },
        { false, 255, false, "prior 3579 occupied max uint8" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3579 = pinShouldIncrementCountOnInsertAt3579(c.slotEmpty, c.slotID);
        const bool wantPin = c.slotEmpty && c.slotID != 0;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3579.
        ok = expect(got == inlineF && got == pin && got == pin3579, "dual-wire free == inline == pin == pin3579") && ok;
        ok = expect(got == wantPin, "free == pin formula empty && slotID != 0") && ok;
    }

    // Explicit poles free == inline == pin == pin3579 for empty/occupied × slot edges.
    ...
    for (const bool empty : { false, true })
    {
        for (const std::uint8_t slotID : slotEdges)
        {
            ...
            const bool pin3579 = pinShouldIncrementCountOnInsertAt3579(empty, slotID);
            const bool want    = empty && slotID != 0;
            ok                 = expect(got == want, "pole free == pin formula") && ok;
            ok                 = expect(got == inlineF && got == pin && got == pin3579, "pole free == inline == pin == pin3579") && ok;
        }
    }

    // Dense compose: empty × slot edges free == inline == pin == pin3579.
    for (const bool empty : { false, true })
    {
        for (const std::uint8_t slotID : slotEdges)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3579, "compose free == inline == pin == pin3579") && ok;
        }
    }

    // Host-style inject poles: free == inline == pin == pin3579 for InsertItem count bump.
    for (const auto& pair : {
         ...
         })
    {
        ...
        ok                = expect(got == inlineF && got == pin && got == pin3579, "host inject free == inline == pin == pin3579") && ok;
    }
    ...
    for (const std::uint8_t slotID : slotEdges)
    {
        ...
        // free == inline == pin == pin3579 still holds for the inc pole.
        ...
        const bool pin3579 = pinShouldIncrementCountOnInsertAt3579(true, slotID);
        ok                 = expect(got == inlineF && got == pin && got == pin3579, "inc/dec shape free == inline == pin == pin3579") && ok;
    }
