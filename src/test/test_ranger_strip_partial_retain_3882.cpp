#include "test_ranger_strip_partial_retain_3882.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranger ShouldStripPartialStealthOnRetain 3882 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack retain-success partial-stealth gate for dual-wire
// cross-check (dedicated 3882 expand residual 3082; prior dedicated 3837 / 3792 / 3747 / 3702 / 3657 / 3612 / 3567 / 3510 / 3445 / 3394):
//   !stripAllDetectable
auto inlineShouldStripPartialStealthOnRetain3882(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Compact dual-wire pin matching Go pinShouldStripPartialStealthOnRetain3882 /
// C++ capacity form (formula unchanged from 1391 / 3082 / 3394 / 3445 / 3510 / 3567 / 3612 / 3657 / 3702 / 3747 / 3792 / 3837):
//   !stripAllDetectable
auto pinShouldStripPartialStealthOnRetain3882(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Residual prior dedicated pin (slice 3837) — formula identical.
auto pinShouldStripPartialStealthOnRetain3837(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Residual prior dedicated pin (slice 3792) — formula identical.
auto pinShouldStripPartialStealthOnRetain3792(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Residual prior dedicated pin (slice 3747) — formula identical.
auto pinShouldStripPartialStealthOnRetain3747(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Residual prior dedicated pin (slice 3702) — formula identical.
auto pinShouldStripPartialStealthOnRetain3702(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Residual prior dedicated pin (slice 3657) — formula identical.
...
// dedicated slice 3882 expand residual 3082; prior dedicated 3837 / 3792 / 3747 / 3702 / 3657 / 3612 / 3567 / 3510 / 3445 / 3394 / pure 1391 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3837
//   - residual 3837 / 3792 / 3747 / 3702 / 3657 / 3612 / 3567 / 3510 / 3445 / 3394 / 3082 / 1391 pins still hold
...
// test_ranger_strip_partial_retain_3612, test_ranger_strip_partial_retain_3657,
// test_ranger_strip_partial_retain_3702, test_ranger_strip_partial_retain_3747,
// test_ranger_strip_partial_retain_3792, test_ranger_strip_partial_retain_3837.
...
auto runRangerStripPartialRetain3882SelfTests() -> bool
{
    ...
    // Prior dedicated 3394 edges still hold under expand residual 3882.
    ...
    // Prior dedicated 3445 edges still hold under expand residual 3882.
    ...
    // Prior dedicated 3510 edges still hold under expand residual 3882.
    ...
    // Prior dedicated 3567 edges still hold under expand residual 3882.
    ...
    // Prior dedicated 3612 edges still hold under expand residual 3882.
    ...
    // Prior dedicated 3657 edges still hold under expand residual 3882.
    ok = expect(ShouldStripPartialStealthOnRetain(false), "prior 3657: false must partial-strip") && ok;
    ok = expect(!ShouldStripPartialStealthOnRetain(true), "prior 3657: true must not partial-strip") && ok;

    // Prior dedicated 3702 edges still hold under expand residual 3882.
    ok = expect(ShouldStripPartialStealthOnRetain(false), "prior 3702: false must partial-strip") && ok;
    ok = expect(!ShouldStripPartialStealthOnRetain(true), "prior 3702: true must not partial-strip") && ok;

    // Prior dedicated 3747 edges still hold under expand residual 3882.
    ok = expect(ShouldStripPartialStealthOnRetain(false), "prior 3747: false must partial-strip") && ok;
    ok = expect(!ShouldStripPartialStealthOnRetain(true), "prior 3747: true must not partial-strip") && ok;

    // Prior dedicated 3792 edges still hold under expand residual 3882.
    ok = expect(ShouldStripPartialStealthOnRetain(false), "prior 3792: false must partial-strip") && ok;
    ok = expect(!ShouldStripPartialStealthOnRetain(true), "prior 3792: true must not partial-strip") && ok;

    // Prior dedicated 3837 edges still hold under expand residual 3882.
    ok = expect(ShouldStripPartialStealthOnRetain(false), "prior 3837: false must partial-strip") && ok;
    ok = expect(!ShouldStripPartialStealthOnRetain(true), "prior 3837: true must not partial-strip") && ok;

    // --- Core poles: free == inline == pin == pin3837 (required edges + stability) ---
    const struct
    {
        ...
        // Residual 1391 / 3082 / prior 3394 / prior 3445 / prior 3510 / prior 3567 / prior 3612 / prior 3657 / prior 3702 / prior 3747 / prior 3792 / prior 3837 re-pins.
        ...
        { false, true, "prior 3657 retain success partial" },
        { true, false, "prior 3657 retain fail no partial" },
        { false, true, "prior 3702 retain success partial" },
        { true, false, "prior 3702 retain fail no partial" },
        { false, true, "prior 3747 retain success partial" },
        { true, false, "prior 3747 retain fail no partial" },
        { false, true, "prior 3792 retain success partial" },
        { true, false, "prior 3792 retain fail no partial" },
        { false, true, "prior 3837 retain success partial" },
        { true, false, "prior 3837 retain fail no partial" },
    };

    for (const auto& c : cases)
    {
        const bool got     = ShouldStripPartialStealthOnRetain(c.stripAllDetectable);
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3882(c.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3882(c.stripAllDetectable);
        const bool pin3837 = pinShouldStripPartialStealthOnRetain3837(c.stripAllDetectable);
        const bool pin3792 = pinShouldStripPartialStealthOnRetain3792(c.stripAllDetectable);
        const bool pin3747 = pinShouldStripPartialStealthOnRetain3747(c.stripAllDetectable);
        const bool pin3702 = pinShouldStripPartialStealthOnRetain3702(c.stripAllDetectable);
        const bool pin3657 = pinShouldStripPartialStealthOnRetain3657(c.stripAllDetectable);
        ...
        ok = expect(got == inlineF && got == pin && got == wantPin,
             ...
             ok;
        ok = expect(got == pin3837, "ShouldStripPartialStealthOnRetain free == pin3837") && ok;
        ok = expect(got == pin3792, "ShouldStripPartialStealthOnRetain free == pin3792") && ok;
        ok = expect(got == pin3747, "ShouldStripPartialStealthOnRetain free == pin3747") && ok;
        ok = expect(got == pin3702, "ShouldStripPartialStealthOnRetain free == pin3702") && ok;
        ok = expect(got == pin3657, "ShouldStripPartialStealthOnRetain free == pin3657") && ok;
    ...
    for (const bool stripAll : { false, true })
    {
        ...
        const bool pin     = pinShouldStripPartialStealthOnRetain3882(stripAll);
        const bool pin3837 = pinShouldStripPartialStealthOnRetain3837(stripAll);
        const bool pin3792 = pinShouldStripPartialStealthOnRetain3792(stripAll);
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3882(stripAll);
        const bool pin3747 = pinShouldStripPartialStealthOnRetain3747(stripAll);
        const bool pin3702 = pinShouldStripPartialStealthOnRetain3702(stripAll);
        const bool pin3657 = pinShouldStripPartialStealthOnRetain3657(stripAll);
        ...
        ok                 = expect(got == pin3837, "required pole free == pin3837") && ok;
        ok                 = expect(got == pin3792, "required pole free == pin3792") && ok;
        ok                 = expect(got == pin3747, "required pole free == pin3747") && ok;
        ok                 = expect(got == pin3702, "required pole free == pin3702") && ok;
        ok                 = expect(got == pin3657, "required pole free == pin3657") && ok;
    ...
    // Dense compose over strip-all poles from retainChance/roll: free == inline == pin == pin3837.
    const struct
    {
        int16 retainChance;
        int   roll0to99;
    ...
    for (const auto& p : poles)
    {
        ...
        ok                  = expect(got == pin3837, "compose free == pin3837") && ok;
        ok                  = expect(got == pin3792, "compose free == pin3792") && ok;
        ok                  = expect(got == pin3747, "compose free == pin3747") && ok;
        ok                  = expect(got == pin3702, "compose free == pin3702") && ok;
        ok                  = expect(got == pin3657, "compose free == pin3657") && ok;
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3882(h.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3882(h.stripAllDetectable);
        const bool pin3837 = pinShouldStripPartialStealthOnRetain3837(h.stripAllDetectable);
        const bool pin3792 = pinShouldStripPartialStealthOnRetain3792(h.stripAllDetectable);
        const bool pin3747 = pinShouldStripPartialStealthOnRetain3747(h.stripAllDetectable);
        const bool pin3702 = pinShouldStripPartialStealthOnRetain3702(h.stripAllDetectable);
        const bool pin3657 = pinShouldStripPartialStealthOnRetain3657(h.stripAllDetectable);
        ...
        ok                 = expect(got == pin3837, "host inject free == pin3837") && ok;
        ok                 = expect(got == pin3792, "host inject free == pin3792") && ok;
        ok                 = expect(got == pin3747, "host inject free == pin3747") && ok;
        ok                 = expect(got == pin3702, "host inject free == pin3702") && ok;
        ok                 = expect(got == pin3657, "host inject free == pin3657") && ok;
