#include "test_trade_bump_items_count_3674.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "trade ShouldBumpItemsCountOnSetEntry 3674 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldBumpItemsCountOnSetEntry3674 / C++ capacity:
//   slotInRange
...
// Prior dedicated 3629 inline pin for independence cross-check.
auto inlineShouldBumpItemsCountOnSetEntry3629(const bool slotInRange) -> bool
{
    return slotInRange;
}

// Prior dedicated 3629 compact pin for free == inline == pin == pin3629 cross-check.
auto pinShouldBumpItemsCountOnSetEntry3629(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3674 expand residual 2997; prior dedicated 3629 / 3584 / 3540 / 3474 / 3420 / 3372).
//
// Coverage:
//   - free == inline == pin == pin3629 == slotInRange (identity)
//   - residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 pins still hold
//   - prior dedicated 3629 independence (free == prior inline == prior pin)
...
auto runTradeBumpItemsCount3674SelfTests() -> bool
{
    ...
    // Residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3629 ---
    const struct
    {
        bool        slotInRange;
        bool        want;
        ...
        // Residual 2997 / 2812 / prior 3372 / 3420 / 3474 / 3540 / 3584 / 3629 poles (full 2^1 surface).
        ...
        // Prior dedicated 3629 poles still hold.
        { true, true, "prior 3629 in range bumps" },
        { false, false, "prior 3629 out of range no bump" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3629 = pinShouldBumpItemsCountOnSetEntry3629(c.slotInRange);
        const bool wantPin = c.slotInRange;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3629.
        ok = expect(got == inlineF && got == pin && got == pin3629,
                    "dual-wire free == inline == pin == pin3629") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula identity") && ok;
    }

    // Free == pin == pin3629 across residual poles.
    ...
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry(false),
                "free == pin for false") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3629(true),
                "free == pin3629 for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3629(false),
                "free == pin3629 for false") &&
         ok;

    // Prior dedicated 3629 independence: free still matches prior inline/pin.
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == inlineShouldBumpItemsCountOnSetEntry3629(true) &&
                    ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3629(true),
                "prior 3629 free/inline/pin mismatch for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == inlineShouldBumpItemsCountOnSetEntry3629(false) &&
                    ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3629(false),
                "prior 3629 free/inline/pin mismatch for false") &&
         ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3629.
    for (const bool slotInRange : { false, true })
    {
        ...
        const bool pin3629 = pinShouldBumpItemsCountOnSetEntry3629(slotInRange);
        const bool want    = slotInRange;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3629,
                                    "compose free == inline == pin == pin3629") &&
             ok;
        // Prior 3629 independence across dense poles.
        ok = expect(got == inlineShouldBumpItemsCountOnSetEntry3629(slotInRange) &&
                        got == pinShouldBumpItemsCountOnSetEntry3629(slotInRange),
                    "compose free == prior 3629 inline/pin") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3629 for multi-arg setItem bump.
    for (const bool slotInRange : { false, true })
    {
        const bool got     = ShouldBumpItemsCountOnSetEntry(slotInRange);
        const bool inlineF = inlineShouldBumpItemsCountOnSetEntry(slotInRange);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3629,
                                    "host inject free == inline == pin == pin3629") &&
             ok;
    }
