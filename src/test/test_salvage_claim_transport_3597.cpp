#include "test_salvage_claim_transport_3597.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage claim transport 3597 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanClaimTransport3597:
...
// Prior dedicated 3552 pin (same formula; residual suite identity).
auto pinCanClaimTransport3552(const uint32 transportUserID) -> bool
{
    return !salvagehelpers::TransportUserBusy(transportUserID);
}
...
// slice 3597 dedicated dual-wire expand residual 2871).
//
// Coverage:
//   - id == 0 → claim true (positive pin form)
//   - non-zero → claim false / busy true
//   - max uint32 busy
//   - free == inline == pin == pin3552 == !TransportUserBusy
//   - dense poles: free / busy residual + large / max
//   - residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 / 3552 pins still hold
//   - prior 3552 independence: free == pin3552 across poles
auto runSalvageClaimTransport3597SelfTests() -> bool
{
    ...
    // Residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 / 3552 pins still hold under dual-wire.
    ...
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport(0xFFFFFFFF),
         ...
         ok;
    ok = expect(CanClaimTransport(0) == pinCanClaimTransport3552(0),
                "residual 3552: free == pin for id 0") &&
         ok;
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport3552(0xFFFFFFFF),
                "residual 3552: free == pin for max busy") &&
         ok;
    ...
    ok = expect(pinCanClaimTransport3552(0), "eligible prior pin3552 id 0 free should claim") && ok;
    ...
    ok = expect(!pinCanClaimTransport3552(0xFFFFFFFF), "prior pin3552 max busy should not claim") && ok;
    ...
    // --- Composition table: free == inline == pin == pin3552 == !TransportUserBusy ---
    // Dense poles: free / busy residual + large / max.
    const struct
    {
        uint32      id;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior   = pinCanClaimTransport3552(c.id);
        ...
        ok = expect(gotClaim == pinPrior, "dual-wire free==pin3552") && ok;
    ...
    for (const uint32 id : composeIds)
    {
        ...
        ok             = expect(got == pinCanClaimTransport3552(id), "compose pin3552") && ok;
    }
    ...
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport(99),
         ...
         ok;
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport3552(99),
                "formula busy free == pin3552") &&
         ok;
