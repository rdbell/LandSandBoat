#include "test_portermoogle_can_retrieve_item_3512.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "portermoogle CanRetrieveItem 3512 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua porter_moogle hasItem / freeSlots gate for dual-wire cross-check
// (dedicated slice 3512):
//   !hasItem && freeSlots > 0
...
// Compact dual-wire pin matching Go pinCanRetrieveItem3512 / C++ capacity
...
// (Lua porter_moogle hasItem + freeSlots retrieve gate; dedicated slice 3512
// expand residual 2966 / prior dedicated ~3463 / 3409 / 3316 / 3286 / 3243 / 3171 / pure 1144 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == (!hasItem && freeSlots > 0) (pin direct return)
//   - residual 1144 / 2966 / prior 3171 / 3243 / 3286 / 3316 / 3409 / 3463 pins still hold
...
auto runPortermoogleCanRetrieveItem3512SelfTests() -> bool
{
    ...
    // Residual 1144 / 2966 / prior 3171 / 3243 / 3286 / 3316 / 3409 / 3463 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2966 / prior 3171 / 3243 / 3286 / 3316 / 3409 / 3463 happy / multi free (n).
    ...
    // Dense 2×slots: hasItem × freeSlots {0, 1, n, negative} free == inline == pin.
    ...
    for (const bool hasItem : { false, true })
    {
        for (const int32 freeSlots : freePoles)
        {
            ...
            ok                 = expect(got == inlineF && got == pin, "dense 2xslots free == inline == pin") && ok;
        }
    }
