#include "test_notoriety_add_member_3635.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "notoriety ShouldAddNotorietyMember 3635 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CNotorietyContainer::add admission for dual-wire cross-check
// (dedicated 3635 expand residual 2959; prior dedicated 3590):
...
// Compact dual-wire pin matching Go pinShouldAddNotorietyMember3635 / C++ capacity
// direct AND-chain (formula unchanged from 2818 / 2959 / 3165 / 3267 / 3297 / 3327 / 3492 / 3546 / 3590):
//   ownerPresent && entityPresent && differentAllegiance
...
// Prior dedicated 3590 pin/inline for independence (formula identical).
...
// OmegaXI internal/notoriety; dedicated slice 3635 expand residual 2959 /
// pure 2818; prior dedicated expand 3590 / 3546 / 3492 / 3327 / 3297 / 3267 / 3165 retained).
//
// Coverage:
//   - free == inline == pin == pin3590 direct AND-chain
//   - residual 2959 / 2818 pins still hold
//   - prior dedicated 3590 independence (free == prior inline == prior pin)
//   - dense 2^3 boolean space free == inline == pin == pin3590
...
auto runNotorietyAddMember3635SelfTests() -> bool
{
    ...
    // Residual 2818 / 2959 / prior dedicated 3590 pins still hold under dedicated dual-wire.
    ...
    // --- Core poles: free == inline == pin == pin3590 direct AND-chain ---
    const struct
    {
        bool        ownerPresent;
        bool        entityPresent;
        ...
        // Prior dedicated 3590 re-pins.
        { true, true, true, true, "residual 3590 admit owner+entity+diff" },
        { false, true, true, false, "residual 3590 reject nil owner" },
        { true, false, true, false, "residual 3590 reject nil entity" },
        { true, true, false, false, "residual 3590 reject same allegiance" },
        { false, false, false, false, "residual 3590 reject all false" },
    };

    for (const auto& c : cases)
    {
        const bool got     = ShouldAddNotorietyMember(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool inlineF = inlineShouldAddNotorietyMember3635(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool pin     = pinShouldAddNotorietyMember3635(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool pin3590 = pinShouldAddNotorietyMember3590(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        ...
        ok = expect(got == inlineF && got == pin && got == pin3590 && got == wantPin,
                    "ShouldAddNotorietyMember free == inline == pin == pin3590 direct AND-chain") &&
             ok;
    }

    // Free == pin == pin3590 across residual poles.
    ...
    ok = expect(ShouldAddNotorietyMember(true, true, false) == pinShouldAddNotorietyMember3590(true, true, false),
                "free == pin3590 same allegiance") &&
         ok;

    // Dense compose: full 2^3 boolean space free == inline == pin == pin3590.
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            for (const bool differentAllegiance : { false, true })
            {
                ...
                const bool inlineF = inlineShouldAddNotorietyMember3635(ownerPresent, entityPresent, differentAllegiance);
                const bool pin     = pinShouldAddNotorietyMember3635(ownerPresent, entityPresent, differentAllegiance);
                const bool pin3590 = pinShouldAddNotorietyMember3590(ownerPresent, entityPresent, differentAllegiance);
                const bool want    = ownerPresent && entityPresent && differentAllegiance;
                ok                 = expect(got == want, "compose free == pin formula") && ok;
                ok                 = expect(got == inlineF && got == pin && got == pin3590,
                            "compose free == inline == pin == pin3590") &&
                     ok;
            }
        }
    }

    // Prior dedicated 3590 independence: free still matches prior pin/inline.
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            ...
            {
                const bool got         = ShouldAddNotorietyMember(ownerPresent, entityPresent, differentAllegiance);
                const bool priorInline = inlineShouldAddNotorietyMember3590(ownerPresent, entityPresent, differentAllegiance);
                const bool priorPin    = pinShouldAddNotorietyMember3590(ownerPresent, entityPresent, differentAllegiance);
                ok                     = expect(got == priorPin, "prior 3590 independence free == prior pin") && ok;
                ok                     = expect(got == priorInline, "prior 3590 independence free == prior inline") && ok;
            }
        }
    }
    ...
    ok = expect(ShouldAddNotorietyMember(true, true, true) && pinShouldAddNotorietyMember3635(true, true, true),
         ...
         ok;
    ok = expect(pinShouldAddNotorietyMember3590(true, true, true),
                "add owner+entity+diff → insert path pin3590 dual-wire") &&
         ok;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3635(p.owner, p.entity, p.diff);
        const bool pin     = pinShouldAddNotorietyMember3635(p.owner, p.entity, p.diff);
        const bool pin3590 = pinShouldAddNotorietyMember3590(p.owner, p.entity, p.diff);
        const bool want    = p.owner && p.entity && p.diff;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3590,
                    "host inject free == inline == pin == pin3590") &&
             ok;
    }
