#include "test_garrison_fail_by_death_3875.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3875 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// prior dedicated expand 3650 / prior dedicated expand 3695 /
// prior dedicated expand 3740 / prior dedicated expand 3785 /
// prior dedicated expand 3830 / dedicated 3875):
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3875:
//   allNPCsDead || allPlayersDead
...
// Compact dual-wire pin matching prior dedicated expand residual 3695.
auto pinShouldFailByDeath3695(const bool allNPCsDead, const bool allPlayersDead) -> bool
{
    return allNPCsDead || allPlayersDead;
}

// Compact dual-wire pin matching prior dedicated expand residual 3740.
auto pinShouldFailByDeath3740(const bool allNPCsDead, const bool allPlayersDead) -> bool
{
    return allNPCsDead || allPlayersDead;
}

// Compact dual-wire pin matching prior dedicated expand residual 3785.
auto pinShouldFailByDeath3785(const bool allNPCsDead, const bool allPlayersDead) -> bool
{
    return allNPCsDead || allPlayersDead;
}

// Compact dual-wire pin matching prior dedicated expand residual 3830.
auto pinShouldFailByDeath3830(const bool allNPCsDead, const bool allPlayersDead) -> bool
{
    return allNPCsDead || allPlayersDead;
}
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3875;
...
// prior dedicated expand residual 3695; prior dedicated expand residual 3740;
// prior dedicated expand residual 3785; prior dedicated expand residual 3830).
//
// Coverage:
//   - free == inline == pin == pin3830 (direct return positive OR; no De Morgan)
//   - 4-row truth table + residual poles + dense 2² free == inline == pin == pin3830
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
//     prior dedicated expand 3330 / prior dedicated expand 3406 /
//     prior dedicated expand 3461 / prior dedicated expand 3514 /
//     prior dedicated expand 3560 / prior dedicated expand 3605 /
//     prior dedicated expand 3650 / prior dedicated expand 3695 /
//     prior dedicated expand 3740 / prior dedicated expand 3785 /
//     prior dedicated expand 3830 pins still hold
...
auto runGarrisonFailByDeath3875SelfTests() -> bool
{
    ...
    // prior dedicated expand 3650 / prior dedicated expand 3695 /
    // prior dedicated expand 3740 / prior dedicated expand 3785 /
    // prior dedicated expand 3830 pins.
    ...
    // 4-row truth table (+ residual re-pins): free == inline == pin == pin3830.
    const struct
    {
        bool        allNPCsDead;
        bool        allPlayersDead;
        ...
        // Prior dedicated expand residual 3695 re-pins.
        { false, false, false, "prior dedicated 3695 neither dead" },
        { true, false, true, "prior dedicated 3695 npcs dead only" },
        { false, true, true, "prior dedicated 3695 players dead only" },
        { true, true, true, "prior dedicated 3695 both dead" },

        // Prior dedicated expand residual 3740 re-pins.
        { false, false, false, "prior dedicated 3740 neither dead" },
        { true, false, true, "prior dedicated 3740 npcs dead only" },
        { false, true, true, "prior dedicated 3740 players dead only" },
        { true, true, true, "prior dedicated 3740 both dead" },

        // Prior dedicated expand residual 3785 re-pins.
        { false, false, false, "prior dedicated 3785 neither dead" },
        { true, false, true, "prior dedicated 3785 npcs dead only" },
        { false, true, true, "prior dedicated 3785 players dead only" },
        { true, true, true, "prior dedicated 3785 both dead" },

        // Prior dedicated expand residual 3830 re-pins.
        { false, false, false, "prior dedicated 3830 neither dead" },
        { true, false, true, "prior dedicated 3830 npcs dead only" },
        { false, true, true, "prior dedicated 3830 players dead only" },
        { true, true, true, "prior dedicated 3830 both dead" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3830 = pinShouldFailByDeath3830(c.allNPCsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3830, "ShouldFailByDeath dual-wire == pin3830") && ok;
    ...
    // Free == pin == pin3830 across residual poles (4-row truth table).
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath(false, false) &&
                    ShouldFailByDeath(false, false) == pinShouldFailByDeath3830(false, false),
                "free == pin/pin3830 neither dead") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath(true, false) &&
                    ShouldFailByDeath(true, false) == pinShouldFailByDeath3830(true, false),
                "free == pin/pin3830 npcs dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath(false, true) &&
                    ShouldFailByDeath(false, true) == pinShouldFailByDeath3830(false, true),
                "free == pin/pin3830 players dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath(true, true) &&
                    ShouldFailByDeath(true, true) == pinShouldFailByDeath3830(true, true),
                "free == pin/pin3830 both dead") &&
         ok;

    // Dense compose: full 2^2 boolean space free == inline == pin == pin3830.
    for (const bool allNPCsDead : { false, true })
    {
        for (const bool allPlayersDead : { false, true })
        {
            ...
            ok = expect(got == pinShouldFailByDeath(allNPCsDead, allPlayersDead),
                        "compose free == pin") &&
                 ok;
            ok = expect(got == pinShouldFailByDeath3830(allNPCsDead, allPlayersDead),
                        "compose free == pin3830") &&
                 ok;
        }
    }
    ...
    // prior dedicated expand 3650 / prior dedicated expand 3695 /
    // prior dedicated expand 3740 / prior dedicated expand 3785 /
    // prior dedicated expand 3830 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236/3330/3406/3461/3514/3560/3605/3650/3695/3740/3785/3830 wipe gate pins under 3875") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3650(false, false),
                "prior 3650 pin neither dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3650(true, false),
                "prior 3650 pin npcs dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3650(false, true),
                "prior 3650 pin players dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3650(true, true),
                "prior 3650 pin both dead still holds under 3875") &&
         ok;

    // Residual independence: prior dedicated expand residual 3695 pin still matches free.
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3695(false, false),
                "prior 3695 pin neither dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3695(true, false),
                "prior 3695 pin npcs dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3695(false, true),
                "prior 3695 pin players dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3695(true, true),
                "prior 3695 pin both dead still holds under 3875") &&
         ok;

    // Residual independence: prior dedicated expand residual 3740 pin still matches free.
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3740(false, false),
                "prior 3740 pin neither dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3740(true, false),
                "prior 3740 pin npcs dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3740(false, true),
                "prior 3740 pin players dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3740(true, true),
                "prior 3740 pin both dead still holds under 3875") &&
         ok;

    // Residual independence: prior dedicated expand residual 3785 pin still matches free.
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3785(false, false),
                "prior 3785 pin neither dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3785(true, false),
                "prior 3785 pin npcs dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3785(false, true),
                "prior 3785 pin players dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3785(true, true),
                "prior 3785 pin both dead still holds under 3875") &&
         ok;

    // Residual independence: prior dedicated expand residual 3830 pin still matches free.
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3830(false, false),
                "prior 3830 pin neither dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3830(true, false),
                "prior 3830 pin npcs dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3830(false, true),
                "prior 3830 pin players dead still holds under 3875") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3830(true, true),
                "prior 3830 pin both dead still holds under 3875") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3875.
    ...
    // Explicit residual compose dual-wire free == inline == pin == pin3830 for host poles.
    const struct
    {
        int32       npcCount;
        int32       deadNPCCount;
    ...
    for (const auto& c : composeCases)
    {
        ...
        const bool pin3830   = pinShouldFailByDeath3830(npcsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3830, "compose dual-wire free == pin3830") && ok;
    }
