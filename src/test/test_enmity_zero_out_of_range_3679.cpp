#include "test_enmity_zero_out_of_range_3679.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "enmity ShouldZeroEnmityOutOfRange 3679 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline UpdateEnmity out-of-range CE/VE zero gate for dual-wire cross-check
// (slice 3679 dedicated expand residual 2927; prior dedicated expand 3634 /
// 3589 / 3545 / 3485 / 3427 / 3333 / prior dedicated 3157):
//   !withinRange
// Direct return form matching production free function / capacity.
auto inlineShouldZeroEnmityOutOfRange3679(const bool withinRange) -> bool
{
    return !withinRange;
}

// Compact dual-wire pin matching free function / capacity body (slice 3679).
...
// Positive form: free == inline == pin == pin3634.
...
//
// Coverage:
//   - free == inline == pin == pin3634 (direct return !withinRange identity-not)
...
//     prior expand 3589 / prior expand 3634 pins still hold
auto runEnmityZeroOutOfRange3679SelfTests() -> bool
{
    ...
    // Residual poles: free == inline == pin == pin3634 (direct return).
    const struct
    {
        bool        withinRange;
        bool        want;
        ...
        // Prior dedicated expand 3634 poles (independence).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3679(p.withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3679(p.withinRange);
    ...
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3679(false), "free==pin false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3679(true), "free==pin true") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(false) == pinShouldZeroEnmityOutOfRange3634(false), "free==pin3634 false") && ok;
    ok = expect(ShouldZeroEnmityOutOfRange(true) == pinShouldZeroEnmityOutOfRange3634(true), "free==pin3634 true") && ok;

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3634.
    for (const bool withinRange : { false, true })
    {
        const bool got     = ShouldZeroEnmityOutOfRange(withinRange);
        const bool inlineF = inlineShouldZeroEnmityOutOfRange3679(withinRange);
        const bool pin     = pinShouldZeroEnmityOutOfRange3679(withinRange);
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok                = expect(got == inlineShouldZeroEnmityOutOfRange3679(within), "compose free == inline") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3679(within), "compose free == pin") && ok;
        ok                = expect(got == pinShouldZeroEnmityOutOfRange3634(within), "compose free == pin3634") && ok;
    }

    // Residual 2927 / prior dedicated expand 3634 / 3589 / 3545 / 3485 / 3427 / 3333 / prior dedicated 3157 /
    // pure 1357 still hold under expand suite.
    ok = expect(ShouldZeroEnmityOutOfRange(false) && !ShouldZeroEnmityOutOfRange(true),
                "residual 2927/3634/3589/3545/3485/3427/3333/3157/1357 pins failed under 3679 suite") &&
         ok;
