#include "test_mapsession_destroy_pending_ptr_3586.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapsession ShouldDestroyPendingByPointer 3586 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapSessionContainer::destroyPendingSession(MapSession*) gate for
// dual-wire cross-check (dedicated slice 3586):
...
// Compact dual-wire pin matching Go pinShouldDestroyPendingByPointer3586 /
...
// Prior dedicated slice 3542 inline/pin for independence cross-check.
auto inlineShouldDestroyPendingByPointer3542(const bool found, const bool pointerMatches) -> bool
{
    return found && pointerMatches;
}

auto pinShouldDestroyPendingByPointer3542(const bool found, const bool pointerMatches) -> bool
{
    if (!found)
    {
        return false;
...
// (found && pointerMatches; dedicated slice 3586 expand residual 3056 /
// pure 2787; prior dedicated 3542 / 3480 / 3426 / 3376 retained).
//
// Coverage:
//   - free == inline == pin == pin3542 == (found && pointerMatches)
//   - residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 / 3542 pins still hold
//   - full 2² bool poles (found × pointerMatches)
//   - prior dedicated 3542 independence (free == 3542 inline/pin)
...
auto runMapsessionDestroyPendingPtr3586SelfTests() -> bool
{
    ...
    // Residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 / 3542 pins still hold under dual-wire.
    ok = expect(ShouldDestroyPendingByPointer(true, true), "residual 3542/3480/3426/3376/3056/2787: found match destroys") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(true, false), "residual 3542/3480/3426/3376/3056/2787: found mismatch rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, false), "residual 3542/3480/3426/3376/3056/2787: missing rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, true), "residual 3542/3480/3426/3376/3056/2787: missing ignores match flag") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinShouldDestroyPendingByPointer3542(c.found, c.pointerMatches);
        ...
        ok = expect(got == pinPrior, "ShouldDestroyPendingByPointer dual-wire == pin3542") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pinPrior, "host compose dual-wire free == pin3542") && ok;
        ok = expect(got == (c.found && c.pointerMatches),
                    "host compose free == found && pointerMatches") &&
             ok;
    }

    // Dense compose: free == inline == pin == pin3542 residual pins for full 2×2 (2²).
    for (const bool found : { false, true })
    {
        for (const bool pointerMatches : { false, true })
        {
            ...
            ok = expect(got == pinShouldDestroyPendingByPointer3542(found, pointerMatches),
                        "compose free == pin3542") &&
                 ok;
        }
    }

    // Residual independence (2787 / 3056 / 3066 / 3376 / 3426 / 3480 / 3542): pointer path needs identity;
    // charID path needs presence only. Sibling dual-wires left alone.
    // Prior dedicated 3542 independence: free == prior suite inline/pin forms.
    ...
    for (const bool found : { false, true })
    {
        ...
        {
            const bool got = ShouldDestroyPendingByPointer(found, pointerMatches);
            ok             = expect(got == inlineShouldDestroyPendingByPointer3542(found, pointerMatches),
                        "prior 3542 independence: free == 3542 inline") &&
                 ok;
            ok = expect(got == pinShouldDestroyPendingByPointer3542(found, pointerMatches),
                        "prior 3542 independence: free == 3542 pin") &&
                 ok;
        }
    }
