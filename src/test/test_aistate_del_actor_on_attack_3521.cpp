#include "test_aistate_del_actor_on_attack_3521.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3521 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3521 expand residual 3044):
//   emittedHostile
auto inlineShouldDelActorOnAttack3521(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3521 / C++ capacity
// identity form (formula unchanged from 1631 / 3044 / 3178 / 3401 / 3453):
//   emittedHostile
...
// OmegaXI internal/aistate; dedicated slice 3521 expand residual 3044;
// prior dedicated 3453 / 3401 / 3178; pure 1631 / 1709 / 2306). Formula unchanged.
//
// Coverage:
//   - free == inline == pin
//   - residual 3044 / 1631 / prior 3178 / prior 3401 / prior 3453 pins still hold
...
auto runAistateDelActorOnAttack3521SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 / prior 3178 / prior 3401 / prior 3453 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044/3178/3401/3453: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044/3178/3401/3453: emittedHostile false → no del") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3521(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3521(c.emittedHostile);
    ...
    for (const bool emitted : { false, true })
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3521(emitted);
        const bool pin     = pinShouldDelActorOnAttack3521(emitted);
