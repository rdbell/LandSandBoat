#include "test_unity_reject_null_online_3768.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "unitychat ShouldRejectNullOnlineMember 3768 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember / DelOnlineMember null-PChar gate for dual-wire
// cross-check (slice 3768 dedicated expand residual 3075; prior dedicated
// 3723 / 3678 / 3633 / 3588 / 3544 / 3487 / 3436 / 3386):
//   charNull
...
// Compact dual-wire pin matching free function / capacity body (slice 3768).
// Direct return only — same formula as production ShouldRejectNullOnlineMember.
// free == inline == pin == pin3723.
...
// slice 3768 dedicated expand residual 3075; prior dedicated 3723 / 3678 / 3633 /
// 3588 / 3544 / 3487 / 3436 / 3386 retained). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3723 (direct return formula)
//   - residual 1356 / 3075 pins still hold
//   - prior 3386 / 3436 / 3487 / 3544 / 3588 / 3633 / 3678 / 3723 dedicated poles still hold
//   - dense 2¹
...
//     test_unity_reject_null_online_3723
...
auto runUnityRejectNullOnline3768SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3768(c.charNull);
        const bool pin3723 = pinShouldRejectNullOnlineMember3723(c.charNull);
        ...
        ok = expect(got == pin3723, "ShouldRejectNullOnlineMember == pin3723") && ok;
    ...
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3768(charNull);
        const bool pin3723 = pinShouldRejectNullOnlineMember3723(charNull);

        ok = expect(got == want, "compose free == pin formula") && ok;
        ok = expect(got == inlineF, "compose free == inline") && ok;
        ok = expect(got == pin, "compose free == pin 3768") && ok;
        ok = expect(got == pin3723, "compose free == pin3723") && ok;
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3768(h.charNull);
        const bool pin3723 = pinShouldRejectNullOnlineMember3723(h.charNull);
        ...
        ok                 = expect(reject == pin3723, "host compose free == pin3723") && ok;
