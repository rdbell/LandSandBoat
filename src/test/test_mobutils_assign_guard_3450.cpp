#include "test_mobutils_assign_guard_3450.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mobutils ShouldAssignGuardSkill 3450 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CalculateMobStats / dual-wire formula for cross-check
// (dedicated slice 3450 expand residual 3022):
//   (mJob == 2 /*MNK*/ || mJob == 18 /*PUP*/) && cannotGuardMod == 0
auto inlineShouldAssignGuardSkill3450(const uint8 mJob, const int16 cannotGuardMod) -> bool
{
    return (mJob == 2 || mJob == 18) && cannotGuardMod == 0;
}

// Dual-wire pin matching production free function / capacity body
// (dedicated slice 3450). Direct return only — same formula.
//   (mJob == 2 /*MNK*/ || mJob == 18 /*PUP*/) && cannotGuardMod == 0
auto pinShouldAssignGuardSkill3450(const uint8 mJob, const int16 cannotGuardMod) -> bool
{
    return (mJob == 2 || mJob == 18) && cannotGuardMod == 0;
}
...
// MOBMOD_CANNOT_GUARD == 0 assignment gate; dedicated slice 3450 expand residual 3022).
// Required poles:
//   - MNK/PUP + cannotGuardMod == 0 → true
//   - non-MNK/PUP or cannotGuardMod != 0 → false
//   - free == residual == inline == pin
//   - residual 3022 / 1623 / prior re-index 3131 / prior dedicated 3392 pins still hold
auto runMobutilsAssignGuard3450SelfTests() -> bool
{
    ...
    // Residual 1623 / residual dual-wire 3022 / prior re-index 3131 /
    // prior dedicated 3392 pins.
    ...
    const struct
    {
        ...
        // Residual 1623 / 3022 / 3131 / 3392 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF  = inlineShouldAssignGuardSkill3450(c.mJob, c.cannotGuardMod);
        const bool pinF     = pinShouldAssignGuardSkill3450(c.mJob, c.cannotGuardMod);
        ...
        ok = expect(got == pinF, "ShouldAssignGuardSkill dual-wire == pin formula") && ok;
    ...
    ok = expect(ShouldAssignGuardSkill(2, 0) == pinShouldAssignGuardSkill3450(2, 0),
                "free == pin residual MNK 0") &&
         ok;
    ok = expect(ShouldAssignGuardSkill(18, 0) == pinShouldAssignGuardSkill3450(18, 0),
                "free == pin residual PUP 0") &&
         ok;
    ok = expect(ShouldAssignGuardSkill(2, 1) == pinShouldAssignGuardSkill3450(2, 1),
                "free == pin residual MNK 1") &&
         ok;
    ok = expect(ShouldAssignGuardSkill(1, 0) == pinShouldAssignGuardSkill3450(1, 0),
                "free == pin residual WAR 0") &&
         ok;
    ...
    for (const uint8 mJob : jobs)
    {
        for (const int16 cannotGuardMod : mods)
        {
            ...
            ok = expect(got == inlineShouldAssignGuardSkill3450(mJob, cannotGuardMod), "compose free == inline") && ok;
            ok = expect(got == pinShouldAssignGuardSkill3450(mJob, cannotGuardMod), "compose free == pin") && ok;
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(ShouldAssignGuardSkill(h.mJob, h.cannotGuardMod) ==
             ...
             ok;
        ok = expect(ShouldAssignGuardSkill(h.mJob, h.cannotGuardMod) ==
                        inlineShouldAssignGuardSkill3450(h.mJob, h.cannotGuardMod),
                    "host inject free == inline") &&
             ok;
        ok = expect(ShouldAssignGuardSkill(h.mJob, h.cannotGuardMod) ==
                        pinShouldAssignGuardSkill3450(h.mJob, h.cannotGuardMod),
