#include "test_assault_issue_tag_3841.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3841 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3841 expand residual 2867 / prior 3796 stock / 3751 / 3706 / 3661 / 3616 / 3571 / 3524 / 3388 / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3841:
//   tagStock > 0
...
// internal/assault; dedicated slice 3841 expand residual 2867 / prior 3796
// stock / 3751 / 3706 / 3661 / 3616 / 3571 / 3524 / 3388 / 3440 ShouldIssueNewTag sibling / pure 1100 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3796 == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 / 3706 / 3751 / 3796 stock pins still hold
...
// (prior stock), test_assault_issue_tag_3796 (prior stock). Mirrors 3796
// expand-residual suite layout (header + self-test runner; 3796
...
auto runAssaultIssueTag3841SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 / 3706 / 3751 / 3796 stock poles still hold.
    ...
    // --- Composition table: free == inline == pin == pin3796 == compose ---
    const struct
    {
        int32       tagStock;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3796 = pinCanIssueTagFromStock3796(c.tagStock);
        ...
        ok = expect(got == pin3796, "dual-wire free==pin3796") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3796 for stock 0 / 1.
    for (const int32 stock : { 0, 1 })
    {
        ...
        const bool pin3796 = pinCanIssueTagFromStock3796(stock);
        ...
        ok                 = expect(got == pin3796, "pole free == pin3796") && ok;
    }

    // Dense tagStock: free == pin == inline == pin3796 == compose.
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3796 = pinCanIssueTagFromStock3796(h.tagStock);
        ...
        ok                 = expect(got == pin3796, "host free==pin3796") && ok;
