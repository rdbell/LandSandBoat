#include "test_follow_can_follow_3557.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3557 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3557 dedicated expand residual 2883; prior dedicated 3506 / 3459 /
// 3405 / 3331 / 3237 / 3084):
...
// Compact dual-wire pin matching free function / capacity body (slice 3557).
...
// Prior dedicated 3506 pin retained for free == pin3506 dual-wire agreement.
auto pinCanFollow3506(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3557 dedicated
// expand residual 2883; prior dedicated 3506 / 3459 / 3405 / 3331 / 3237 / 3084).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3506 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 poles still hold
...
auto runFollowCanFollow3557SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3557(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3506 = pinCanFollow3506(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3557(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3506 = pinCanFollow3506(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3506, "dense free == pin3506") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3506 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3506, "host inject dual-wire free == pin3506") && ok;
    }
