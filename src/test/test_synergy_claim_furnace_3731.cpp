#include "test_synergy_claim_furnace_3731.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3731 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3731 dedicated expand residual 2877; prior dedicated expand 3686):
...
// Compact dual-wire pin matching free function / capacity body (slice 3731).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3731 dedicated
// expand residual 2877; prior dedicated expand 3686). Formula unchanged:
//
//   CanClaimFurnace(state) = state == FurnaceAvailable
//
// Coverage:
//   - free == inline == pin == pin3686 (direct return formula)
...
auto runSynergyClaimFurnace3731SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285 / 3315 / 3494 / 3551 / 3596 / 3641 / 3686).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3731(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3731(s);
