#include "test_mobtype_can_be_neutral_3076.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mobtype CanBeNeutral 3076 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// (CMobEntity::CanBeNeutral type-bit gate; OmegaXI internal/mobtype; slice 3076).
//
// Coverage:
//   - no flags → true
...
auto runMobtypeCanBeNeutral3076SelfTests() -> bool
{
    ...
    // Residual 2042 / 2655 / 2934 pins still hold under dual-wire.
    ...
    ok = expect(CanBeNeutral(Normal) == pinCanBeNeutral(Normal), "free == pin for no flags") && ok;
    ok = expect(CanBeNeutral(Notorious) == pinCanBeNeutral(Notorious), "free == pin for Notorious only") && ok;
    ok = expect(CanBeNeutral(Battlefield) == pinCanBeNeutral(Battlefield), "free == pin for Battlefield alone") && ok;
    ok = expect(CanBeNeutral(static_cast<uint8>(Notorious | Battlefield)) ==
                    pinCanBeNeutral(static_cast<uint8>(Notorious | Battlefield)),
                "free == pin for NM|BF") &&
         ok;
    ok = expect(CanBeNeutral(Fished) == pinCanBeNeutral(Fished), "free == pin for Fished alone") && ok;
