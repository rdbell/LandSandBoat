#include "test_campaign_afford_notes_3643.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "campaign afford notes 3643 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua sigilOnEventFinish shop afford gate for dual-wire checks
// (dedicated 3643 expand residual 2946; prior dedicated 3598 retained):
...
// Compact dual-wire pin matching Go pinCanAffordAlliedNotes3643 / C++ capacity:
//   notes >= price
...
// OmegaXI internal/campaign; dedicated slice 3643 expand residual 2946;
// prior dedicated 3598 independence retained).
//
// Coverage:
//   - free == inline == pin == (notes >= price)
//   - residual 2946 / 3072 / 3226 / 3273 / 3304 / 3496 / 3553 / 3598 / 1115 pins still hold
//   - prior 3598 independence: free == pin3598 (same notes >= price formula)
...
auto runCampaignAffordNotes3643SelfTests() -> bool
{
    ...
    // Residual 1115 / 2946 / 3072 / 3226 / 3273 / 3304 / 3496 / 3553 / 3598 pins still hold under dual-wire.
    ...
    // Prior dedicated 3598 independence poles (same formula notes >= price).
    ok = expect(CanAffordAlliedNotes(1000, 1000) == pinCanAffordAlliedNotes(1000, 1000),
                "residual 3598 independence exact") &&
         ok;
    ok = expect(CanAffordAlliedNotes(999, 1000) == pinCanAffordAlliedNotes(999, 1000),
                "residual 3598 independence short") &&
         ok;
    ok = expect(CanAffordAlliedNotes(0, 0) == pinCanAffordAlliedNotes(0, 0),
                "residual 3598 independence free item") &&
         ok;

    constexpr int32 kMaxInt = std::numeric_limits<int32>::max();
    constexpr int32 kMinInt = std::numeric_limits<int32>::min();

    // --- Composition table: free == inline == pin (incl. prior 3598 independence) ---
    const struct
    {
        int32       notes;
        int32       price;
        ...
        // Residual 2946 / 3072 / 3226 / 3273 / 3304 / 3496 / 3553 / 3598 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        // Positive form: free == inline == pin (prior 3598 independence same formula).
    ...
    // Production path semantics still hold (incl. prior 3598 independence).
    ...
    ok = expect(CanAffordAlliedNotes(1000, 1000) == pinCanAffordAlliedNotes(1000, 1000),
                "formula exact free == pin (prior 3598 independence)") &&
         ok;
    ok = expect(CanAffordAlliedNotes(999, 1000) == pinCanAffordAlliedNotes(999, 1000),
                "formula short free == pin (prior 3598 independence)") &&
         ok;
