#include "test_item_increment_count_insert_3374.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "itemcontainer ShouldIncrementCountOnInsertAt 3374 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertItem count-bump gate for dual-wire cross-check (dedicated 3374):
//   slotEmpty && slotID != 0
...
// Compact dual-wire pin matching Go pinShouldIncrementCountOnInsertAt3374 / C++ capacity:
//   slotEmpty && slotID != 0
...
// dedicated slice 3374 expand residual 3021).
//
// Coverage:
