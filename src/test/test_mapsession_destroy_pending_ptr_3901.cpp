#include "test_mapsession_destroy_pending_ptr_3901.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapsession ShouldDestroyPendingByPointer 3901 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapSessionContainer::destroyPendingSession(MapSession*) gate for
// dual-wire cross-check (dedicated slice 3901):
...
// Compact dual-wire pin matching Go pinShouldDestroyPendingByPointer3901 /
...
// Prior dedicated slice 3856 inline/pin for independence cross-check.
auto inlineShouldDestroyPendingByPointer3856(const bool found, const bool pointerMatches) -> bool
{
    return found && pointerMatches;
}

auto pinShouldDestroyPendingByPointer3856(const bool found, const bool pointerMatches) -> bool
{
    if (!found)
    {
        return false;
...
// (found && pointerMatches; dedicated slice 3901 expand residual 3056 /
// pure 2787; prior dedicated 3856 / 3811 / 3766 / 3721 / 3676 / 3631 / 3586 / 3542 / 3480 / 3426 / 3376 retained).
//
// Coverage:
//   - free == inline == pin == pin3856 == (found && pointerMatches)
//   - residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 / 3542 / 3586 / 3631 / 3676 / 3721 / 3766 / 3811 / 3856 pins still hold
//   - full 2² bool poles (found × pointerMatches)
//   - prior dedicated 3856 independence (free == 3856 inline/pin)
...
auto runMapsessionDestroyPendingPtr3901SelfTests() -> bool
{
    ...
    // Residual 2787 / 3056 / prior dedicated 3376 / 3426 / 3480 / 3542 / 3586 / 3631 / 3676 / 3721 / 3766 / 3811 / 3856 pins still hold under dual-wire.
    ok = expect(ShouldDestroyPendingByPointer(true, true), "residual 3856/3811/3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: found match destroys") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(true, false), "residual 3856/3811/3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: found mismatch rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, false), "residual 3856/3811/3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: missing rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, true), "residual 3856/3811/3766/3721/3676/3631/3586/3542/3480/3426/3376/3056/2787: missing ignores match flag") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior = pinShouldDestroyPendingByPointer3856(c.found, c.pointerMatches);
        ...
        ok = expect(got == pinPrior, "ShouldDestroyPendingByPointer dual-wire == pin3856") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pinPrior, "host compose dual-wire free == pin3856") && ok;
        ok = expect(got == (c.found && c.pointerMatches),
                    "host compose free == found && pointerMatches") &&
             ok;
    }

    // Dense compose: free == inline == pin == pin3856 residual pins for full 2×2 (2²).
    for (const bool found : { false, true })
    {
        for (const bool pointerMatches : { false, true })
        {
            ...
            ok = expect(got == pinShouldDestroyPendingByPointer3856(found, pointerMatches),
                        "compose free == pin3856") &&
                 ok;
        }
    }

    // Residual independence (2787 / 3056 / 3066 / 3376 / 3426 / 3480 / 3542 / 3586 / 3631 / 3676 / 3721 / 3766 / 3811 / 3856): pointer path needs identity;
    // charID path needs presence only. Sibling dual-wires left alone.
    // Prior dedicated 3856 independence: free == prior suite inline/pin forms.
    ...
    for (const bool found : { false, true })
    {
        ...
        {
            const bool got = ShouldDestroyPendingByPointer(found, pointerMatches);
            ok             = expect(got == inlineShouldDestroyPendingByPointer3856(found, pointerMatches),
                        "prior 3856 independence: free == 3856 inline") &&
                 ok;
            ok = expect(got == pinShouldDestroyPendingByPointer3856(found, pointerMatches),
                        "prior 3856 independence: free == 3856 pin") &&
                 ok;
        }
    }
