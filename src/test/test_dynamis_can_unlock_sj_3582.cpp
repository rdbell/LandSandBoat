#include "test_dynamis_can_unlock_sj_3582.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "dynamis CanUnlockSJ 3582 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua formula for dual-wire cross-check (slice 3582 expand residual 2921):
...
// Compact dual-wire pin matching Go pinCanUnlockSJ3582:
...
//   hasSJRestriction ? 1 : 0
// free==inline==pin==pin3538 required for residual expand 3582.
...
// slice 3582 dedicated expand residual 2921; prior dedicated 3538 / 3476 / 3422 / 3151 retained).
//
// Coverage:
//   - has SJ_RESTRICTION → 1; no restriction → 0
//   - free == inline == pin == pin3538 == hasSJRestriction ? 1 : 0
...
auto runDynamisCanUnlockSJ3582SelfTests() -> bool
{
    ...
    // --- Core branches: free == inline == pin == pin3538 == formula ---
    ...
    // free == pin3538 (required dual-wire identity for residual expand).
    ok = expect(CanUnlockSJ(true) == pinCanUnlockSJ3538(true), "free==pin3538 hasSJ=true") && ok;
    ok = expect(CanUnlockSJ(false) == pinCanUnlockSJ3538(false), "free==pin3538 hasSJ=false") && ok;
    ...
    // Dense 2^1 compose: free == inline == pin == pin3538 == formula
    // (and prior 3538 / 3476 / 3422 / 3151 pins).
