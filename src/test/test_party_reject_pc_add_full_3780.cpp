#include "test_party_reject_pc_add_full_3780.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "party ShouldRejectPCAddFull 3780 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddMember IsFull gate for dual-wire cross-check (dedicated 3780
// expand residual 2928; prior dedicated 3735 / 3690 / 3645 / 3600 / 3555 / 3498 / 3200):
...
// Compact dual-wire pin matching Go pinShouldRejectPCAddFull3780 / C++ capacity
// positive form (formula unchanged from 1327 / 1350 / 2928 / 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735):
//   isPCEntity && isPCParty && partyFull
...
// Prior dedicated 3735 inline/pin for free==inline==pin==pin3735 independence.
...
// OmegaXI internal/party; dedicated slice 3780; residual expand 2928 /
...
//   - prior 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735 dedicated poles still hold
...
//   - free == inline == pin == pin3735 independence
//   - prior 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735 independence (free == prior inline/pin)
auto runPartyRejectPCAddFull3780SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldRejectPCAddFull3780(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool pin     = pinShouldRejectPCAddFull3780(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool pin3735 = pinShouldRejectPCAddFull3735(c.isPCEntity, c.isPCParty, c.partyFull);
        // Positive form pin composition (explicit three-bool AND).
        const bool wantPin = c.isPCEntity && c.isPCParty && c.partyFull;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == wantPin && got == pin3735,
                    "ShouldRejectPCAddFull free == inline == pin == pin3735 positive form") &&
             ok;
    }

    // Free == pin across residual poles.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3780(true, true, true),
                "free == pin all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3780(true, true, false),
                "free == pin not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3780(false, true, true),
                "free == pin non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3780(true, false, true),
                "free == pin non-PC party") &&
         ok;

    // free == inline == pin == pin3735 independence poles.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == inlineShouldRejectPCAddFull3780(true, true, true) &&
                ...
                "free == inline == pin == pin3735 all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == inlineShouldRejectPCAddFull3780(true, true, false) &&
                ...
                "free == inline == pin == pin3735 not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == inlineShouldRejectPCAddFull3780(false, true, true) &&
                ...
                "free == inline == pin == pin3735 non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == inlineShouldRejectPCAddFull3780(true, false, true) &&
                ...
                "free == inline == pin == pin3735 non-PC party") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, false, false) == inlineShouldRejectPCAddFull3780(false, false, false) &&
                ...
                "free == inline == pin == pin3735 all false") &&
         ok;
    ...
    // Dense compose: full 2^3 boolean space free == inline == pin == pin3735.
    for (const bool isPCEntity : { false, true })
    {
        for (const bool isPCParty : { false, true })
        {
            for (const bool partyFull : { false, true })
            {
                ...
                const bool inlineF = inlineShouldRejectPCAddFull3780(isPCEntity, isPCParty, partyFull);
                const bool pin     = pinShouldRejectPCAddFull3780(isPCEntity, isPCParty, partyFull);
                const bool pin3735 = pinShouldRejectPCAddFull3735(isPCEntity, isPCParty, partyFull);
                const bool want    = isPCEntity && isPCParty && partyFull;
                ok                 = expect(got == want, "compose free == pin formula") && ok;
                ok                 = expect(got == inlineF && got == pin && got == pin3735,
                                           "compose free == inline == pin == pin3735") &&
                     ok;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRejectPCAddFull3780(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool pin     = pinShouldRejectPCAddFull3780(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool pin3735 = pinShouldRejectPCAddFull3735(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool want    = p.isPCEntity && p.isPCParty && p.partyFull;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3735,
                                    "host inject free == inline == pin == pin3735") &&
             ok;
    ...
    // Residual independence (1327 / 1350 / 2928 / 2937 / 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735):
