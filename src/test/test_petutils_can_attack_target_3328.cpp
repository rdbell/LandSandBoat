#include "test_petutils_can_attack_target_3328.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petutils CanAttackTarget 3328 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AttackTarget null-preflight for dual-wire cross-check (dedicated 3328
// expand residual 3071; prior dedicated 3298 / 3269):
...
// Compact dual-wire pin matching Go pinCanAttackTarget3328 / C++ capacity:
//   hasMaster && hasPet && hasTarget
...
// dedicated slice 3328 expand residual 3071; prior dedicated 3298 / 3269; pure 1627).
//
// Coverage:
...
auto runPetutilsCanAttackTarget3328SelfTests() -> bool
{
    ...
    // Sibling dual-wire ShouldPetEngage (3051) remains orthogonal residual.
    ok = expect(petengagehelpers::ShouldPetEngage(false), "sibling ShouldPetEngage 3051 admit") && ok;
    ok = expect(!petengagehelpers::ShouldPetEngage(true), "sibling ShouldPetEngage 3051 reject") && ok;
