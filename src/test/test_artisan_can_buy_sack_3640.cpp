#include "test_artisan_can_buy_sack_3640.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "artisan CanBuySack 3640 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline artisan.lua option 1 pure gate for dual-wire cross-check
// (slice 3640 dedicated expand residual 2879; prior dedicated 3595 / 3550 / 3493 / 3312 / 3282 / 3242 / 3090):
...
// Compact dual-wire pin matching free function / capacity body (slice 3640).
// Direct return only — same formula as production CanBuySack.
auto pinCanBuySack3640(const int32 gil, const int32 sackSize) -> bool
{
    return gil >= artisanhelpers::BuySackGilCost && sackSize == 0;
}

// Prior dedicated 3595 pin for independence cross-check.
auto pinCanBuySack3595(const int32 gil, const int32 sackSize) -> bool
{
    return gil >= artisanhelpers::BuySackGilCost && sackSize == 0;
}
...
//
// Coverage:
//   - free == inline == pin == pin3595 (direct return formula)
...
//     prior expand 3550 / prior expand 3595 pins still hold
//   - prior dedicated 3595 independence (free == pinCanBuySack3595)
...
auto runArtisanCanBuySack3640SelfTests() -> bool
{
    ...
    // Residual constant / 0948 / 2879 / prior dedicated 3090 / 3242 / 3282 / 3312 / 3493 / 3550 / 3595 pins still hold.
    ...
    // Residual poles: free == inline == pin == pin3595 (direct return).
    const struct
    {
        int32       gil;
        int32       sackSize;
        ...
        // Residual 2879 / 3090 / 3242 / 3282 / 3312 / 3493 / 3550 / 3595 classic dual poles.
        ...
        // Prior dedicated 3090 / expand 3242 / expand 3282 / expand 3312 / expand 3493 / expand 3550 / expand 3595 dense poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanBuySack3640(p.gil, p.sackSize);
        const bool prior   = pinCanBuySack3595(p.gil, p.sackSize);
        ...
        ok = expect(got == prior, "CanBuySack == prior pinCanBuySack3595") && ok;
    }

    // Dense gil edges around cost with empty sack: free == inline == pin == pin3595.
    for (int32 g = BuySackGilCost - 50; g <= BuySackGilCost + 50; ++g)
    {
        const bool got     = CanBuySack(g, 0);
        const bool inlineF = inlineCanBuySack(g, 0);
        const bool pin     = pinCanBuySack3640(g, 0);
        const bool prior   = pinCanBuySack3595(g, 0);
        ...
        ok = expect(got == prior, "dense gil free==prior 3595") && ok;
    }
    ...
    for (const int32 size : { 0, 1, 2, 15, 30, 80, 100, -1 })
    {
        for (const int32 g : { BuySackGilCost - 1, BuySackGilCost, BuySackGilCost + 1, 9979, 9980, 10000, 100000, 0, 1 })
        {
            ...
            const bool pin     = pinCanBuySack3640(g, size);
            const bool prior   = pinCanBuySack3595(g, size);
            ...
            ok = expect(got == prior, "dense gil×size free==prior 3595") && ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3595 for host poles.
    const struct
    {
        int32       gil;
        int32       sackSize;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == prior, "host inject free == prior pin 3595") && ok;
    }
