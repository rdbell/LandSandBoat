#include "test_recast_update_max_charges_3879.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "recast ShouldUpdateMaxCharges 3879 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Load maxCharges update gate for dual-wire cross-check (slice 3879):
//   maxChargesNonzero
...
// Prior dedicated 3834 pin (retained under 3879 expand residual 3122). Positive: return maxChargesNonzero.
auto pinShouldUpdateMaxCharges3834(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Dedicated 3879 pin (expand residual 3122). Positive: return maxChargesNonzero.
auto pinShouldUpdateMaxCharges3879(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3789 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3789(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3744 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3744(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3699 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3699(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3654 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3654(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3609 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3609(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3564 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3564(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3519 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3519(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3448 pin (retained under 3879 expand residual 3122).
auto pinShouldUpdateMaxCharges3448(const bool maxChargesNonzero) -> bool
{
    return maxChargesNonzero;
}

// Prior dedicated 3391 pin (retained under 3879 expand residual 3122).
...
// (maxCharges != 0 update on existing Load entry; slice 3879 dedicated expand
// residual 3122 / prior dedicated 3834 / 3789 / 3744 / 3699 / 3654 / 3609 / 3564 / 3519 / 3448 / 3391 / pure 1370).
...
// test_recast_update_max_charges_3789, test_recast_update_max_charges_3834.
// free == inline == pin == pin3834.
auto runRecastUpdateMaxCharges3879SelfTests() -> bool
{
    ...
    // Residual 3834 / 3789 / 3744 / 3699 / 3654 / 3609 / 3564 / 3519 / 3448 / 3391 / 3122 / 1370 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 3834 / 3789 / 3744 / 3699 / 3654 / 3609 / 3564 / 3519 / 3448 / 3391 / 3122 / 1370 pins restated.
        { true, true, "residual 3834/3789/3744/3699/3654/3609/3564/3519/3448/3391/3122/1370 update max" },
        { false, false, "residual 3834/3789/3744/3699/3654/3609/3564/3519/3448/3391/3122/1370 no update max" },
        // Residual 3834 re-pins.
        { true, true, "residual 3834 update max" },
        { false, false, "residual 3834 no update max" },
        // Residual 3789 re-pins.
        { true, true, "residual 3789 update max" },
        { false, false, "residual 3789 no update max" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3879 = pinShouldUpdateMaxCharges3879(c.maxChargesNonzero);
        const bool pin3834 = pinShouldUpdateMaxCharges3834(c.maxChargesNonzero);
        ...
        ok = expect(got == pin3879, "ShouldUpdateMaxCharges dual-wire == pin3879") && ok;
        ok = expect(got == pin3834, "ShouldUpdateMaxCharges dual-wire == residual pin3834") && ok;
        ok = expect(got == pin3789, "ShouldUpdateMaxCharges dual-wire == residual pin3789") && ok;
    ...
    ok = expect(ShouldUpdateMaxCharges(true) == pinShouldUpdateMaxCharges3879(true), "free == pin3879 true") && ok;
    ok = expect(ShouldUpdateMaxCharges(false) == pinShouldUpdateMaxCharges3879(false), "free == pin3879 false") && ok;
    ok = expect(ShouldUpdateMaxCharges(true) == pinShouldUpdateMaxCharges3834(true), "free == pin3834 true") && ok;
    ok = expect(ShouldUpdateMaxCharges(false) == pinShouldUpdateMaxCharges3834(false), "free == pin3834 false") && ok;
    ...
    for (const bool maxChargesNonzero : { false, true })
    {
        ...
        ok              = expect(got == pinShouldUpdateMaxCharges3879(maxChargesNonzero), "compose free == pin3879") && ok;
        ok              = expect(got == pinShouldUpdateMaxCharges3834(maxChargesNonzero), "compose free == pin3834") && ok;
    ...
    for (const bool maxNonzero : { true, false })
    {
        ...
        ok = expect(ShouldUpdateMaxCharges(inject) == pinShouldUpdateMaxCharges(inject),
             ...
             ok;
        ok = expect(ShouldUpdateMaxCharges(inject) == pinShouldUpdateMaxCharges3879(inject),
                    "host inject free == pin3879") &&
             ok;
        ok = expect(ShouldUpdateMaxCharges(inject) == pinShouldUpdateMaxCharges3834(inject),
                    "host inject free == pin3834") &&
             ok;
    ...
    // Residual 3789 independence: free still matches prior dedicated pin.
    ok = expect(ShouldUpdateMaxCharges(true) == pinShouldUpdateMaxCharges3789(true), "residual 3789 independence update") && ok;
    ok = expect(ShouldUpdateMaxCharges(false) == pinShouldUpdateMaxCharges3789(false), "residual 3789 independence no update") && ok;
    // Residual 3834 independence: free still matches prior dedicated pin.
    ok = expect(ShouldUpdateMaxCharges(true) == pinShouldUpdateMaxCharges3834(true), "residual 3834 independence update") && ok;
    ok = expect(ShouldUpdateMaxCharges(false) == pinShouldUpdateMaxCharges3834(false), "residual 3834 independence no update") && ok;
