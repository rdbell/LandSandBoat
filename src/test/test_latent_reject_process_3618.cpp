#include "test_latent_reject_process_3618.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "latent ShouldRejectProcessLatent 3618 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ProcessLatentEffect owner/zone early-return gate for dual-wire
// cross-check (dedicated slice 3618 expand residual 2961; prior dedicated
// 3573 / 3515 / 3467 / 3413 / 3343). Positive OR form:
...
// Compact dual-wire pin matching Go pinShouldRejectProcessLatent3573
// (prior dedicated retained under 3618). Positive OR form only:
//   ownerNull || zoneIsZero
auto pinShouldRejectProcessLatent3573(const bool ownerNull, const bool zoneIsZero) -> bool
{
    return ownerNull || zoneIsZero;
}
...
// dedicated slice 3618 expand residual 2961 / pure 1359; prior dedicated
// 3573 / 3515 / 3467 / 3413 / 3343).
//
// Coverage:
//   - free == inline == pin == pin3573 (positive OR form)
//   - residual 1359 / 2961 / prior 3573 / 3515 / 3467 / 3413 / 3343 poles still hold
...
auto runLatentRejectProcess3618SelfTests() -> bool
{
    ...
    // Residual 1359 / 2961 / prior 3573 / 3515 / 3467 / 3413 / 3343 pins still hold under dual-wire.
    ok = expect(ShouldRejectProcessLatent(true, false), "residual 2961/1359/3573/3515/3467/3413/3343: ownerNull must reject") && ok;
    ok = expect(ShouldRejectProcessLatent(false, true), "residual 2961/1359/3573/3515/3467/3413/3343: zoneIsZero must reject") && ok;
    ok = expect(!ShouldRejectProcessLatent(false, false), "residual 2961/1359/3573/3515/3467/3413/3343: neither pole must accept") && ok;
    ok = expect(ShouldRejectProcessLatent(true, true), "residual 2961/1359/3573/3515/3467/3413/3343: both poles must reject") && ok;

    // Core poles: free == inline == pin == pin3573. 4-row truth table.
    const struct
    {
        bool        ownerNull;
        bool        zoneIsZero;
        ...
        // Residual 2961 / 1359 / prior 3573 / 3515 / 3467 / 3413 / 3343 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3573 = pinShouldRejectProcessLatent3573(c.ownerNull, c.zoneIsZero);
        const bool wantPin = c.ownerNull || c.zoneIsZero;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pinF && got == pin3573,
                    "ShouldRejectProcessLatent dual-wire free == inline == pin == pin3573") &&
             ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == inlineF && got == pinF && got == pin3573,
                    "host compose dual-wire free == inline == pin == pin3573") &&
             ok;
        ok = expect(got == (c.ownerNull || c.zoneIsZero), "host compose free == ownerNull || zoneIsZero (positive OR)") && ok;
    }

    // Dense compose: free == inline == pin == pin3573 positive OR for full 2².
    for (const bool ownerNull : { false, true })
    {
        for (const bool zoneIsZero : { false, true })
        {
            ...
            ok = expect(got == pinShouldRejectProcessLatent3573(ownerNull, zoneIsZero), "compose free == pin3573") && ok;
        }
    }

    // Prior dedicated 3573 pin independence under 3618 expand.
    ok = expect(ShouldRejectProcessLatent(true, false) == pinShouldRejectProcessLatent3573(true, false),
                "prior 3573 pin under 3618 for owner null") &&
         ok;
    ok = expect(ShouldRejectProcessLatent(false, true) == pinShouldRejectProcessLatent3573(false, true),
                "prior 3573 pin under 3618 for zone zero") &&
         ok;
    ok = expect(ShouldRejectProcessLatent(false, false) == pinShouldRejectProcessLatent3573(false, false),
                "prior 3573 pin under 3618 for accept") &&
         ok;
    ok = expect(ShouldRejectProcessLatent(true, true) == pinShouldRejectProcessLatent3573(true, true),
                "prior 3573 pin under 3618 for both reject") &&
         ok;

    // Residual 2961 / prior 3573 / 3515 / 3467 / 3413 / 3343 suite still holds under dedicated expand.
    ok = expect(ShouldRejectProcessLatent(true, false), "residual 2961 owner-null pin under 3618") && ok;
    ok = expect(ShouldRejectProcessLatent(false, true), "residual 2961 zone-zero pin under 3618") && ok;
    ok = expect(!ShouldRejectProcessLatent(false, false), "residual 2961 accept pin under 3618") && ok;
    ok = expect(ShouldRejectProcessLatent(true, true), "residual 2961 both-reject pin under 3618") && ok;
