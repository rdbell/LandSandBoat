#include "test_garrison_fail_by_death_3740.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3740 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3740:
//   allNPCsDead || allPlayersDead
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3740;
...
// prior dedicated expand residual 3695).
//
// Coverage:
//   - free == inline == pin == pin3695 (direct return positive OR; no De Morgan)
//   - 4-row truth table + residual poles + dense 2² free == inline == pin == pin3695
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
//     prior dedicated expand 3330 / prior dedicated expand 3406 /
//     prior dedicated expand 3461 / prior dedicated expand 3514 /
//     prior dedicated expand 3560 / prior dedicated expand 3605 /
//     prior dedicated expand 3650 / prior dedicated expand 3695 pins still hold
...
auto runGarrisonFailByDeath3740SelfTests() -> bool
{
    ...
    // prior dedicated expand 3461 / prior dedicated expand 3514 /
    // prior dedicated expand 3560 / prior dedicated expand 3605 /
    // prior dedicated expand 3650 / prior dedicated expand 3695 pins.
    ...
    // 4-row truth table (+ residual re-pins): free == inline == pin == pin3695.
    const struct
    {
        bool        allNPCsDead;
        bool        allPlayersDead;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3695 = pinShouldFailByDeath3695(c.allNPCsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3695, "ShouldFailByDeath dual-wire == pin3695") && ok;
    ...
    // Free == pin == pin3695 across residual poles (4-row truth table).
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath(false, false) &&
                    ShouldFailByDeath(false, false) == pinShouldFailByDeath3695(false, false),
                "free == pin/pin3695 neither dead") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath(true, false) &&
                    ShouldFailByDeath(true, false) == pinShouldFailByDeath3695(true, false),
                "free == pin/pin3695 npcs dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath(false, true) &&
                    ShouldFailByDeath(false, true) == pinShouldFailByDeath3695(false, true),
                "free == pin/pin3695 players dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath(true, true) &&
                    ShouldFailByDeath(true, true) == pinShouldFailByDeath3695(true, true),
                "free == pin/pin3695 both dead") &&
         ok;

    // Dense compose: full 2^2 boolean space free == inline == pin == pin3695.
    for (const bool allNPCsDead : { false, true })
    {
        for (const bool allPlayersDead : { false, true })
        {
            ...
            ok = expect(got == pinShouldFailByDeath(allNPCsDead, allPlayersDead),
                        "compose free == pin") &&
                 ok;
            ok = expect(got == pinShouldFailByDeath3695(allNPCsDead, allPlayersDead),
                        "compose free == pin3695") &&
                 ok;
        }
    }
    ...
    // prior dedicated expand 3461 / prior dedicated expand 3514 /
    // prior dedicated expand 3560 / prior dedicated expand 3605 /
    // prior dedicated expand 3650 / prior dedicated expand 3695 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236/3330/3406/3461/3514/3560/3605/3650/3695 wipe gate pins under 3740") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3695(false, false),
                "prior 3695 pin neither dead still holds under 3740") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3695(true, false),
                "prior 3695 pin npcs dead still holds under 3740") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3695(false, true),
                "prior 3695 pin players dead still holds under 3740") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3695(true, true),
                "prior 3695 pin both dead still holds under 3740") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3740.
    ...
    // Explicit residual compose dual-wire free == inline == pin == pin3695 for host poles.
    const struct
    {
        int32       npcCount;
        int32       deadNPCCount;
    ...
    for (const auto& c : composeCases)
    {
        ...
        const bool pin3695   = pinShouldFailByDeath3695(npcsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3695, "compose dual-wire free == pin3695") && ok;
    }
