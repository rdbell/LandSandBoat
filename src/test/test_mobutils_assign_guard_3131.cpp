#include "test_mobutils_assign_guard_3131.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mobutils ShouldAssignGuardSkill 3131 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CalculateMobStats / dual-wire formula for cross-check (slice 3131):
...
// MOBMOD_CANNOT_GUARD == 0 assignment gate; slice 3131).
// Required poles:
...
auto runMobutilsAssignGuard3131SelfTests() -> bool
{
    ...
    // Residual 1623 / prior dual-wire 3022 pins still hold under 3131.
