#include "test_spawn_reject_night_3483.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "spawn ShouldRejectAtNightSpawn 3483 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ATNIGHT spawn-type TOTD gate for dedicated dual-wire expand residual
// 3092 cross-check (slice 3483; prior dedicated 3435/3387 retained):
...
auto inlineShouldRejectAtNightSpawn3483(const uint8 spawnType, const vanadiel_time::TOTD totd) -> bool
{
...
// Compact dual-wire pin matching Go pinShouldRejectAtNightSpawn3483 / C++
// capacity positive form (formula unchanged from 1362 / 3092 / 3387 / 3435):
//   HasSpawnTypeFlag(spawnType, SpawnTypeAtNight) && !IsNightTotdWindow(totd)
auto pinShouldRejectAtNightSpawn3483(const uint8 spawnType, const vanadiel_time::TOTD totd) -> bool
{
...
// OmegaXI internal/spawnslot; dedicated dual-wire expand residual 3092 slice 3483;
// prior dedicated 3435/3387 retained; residual expand 3092 / pure 1362).
//
// Coverage:
//   - free == inline == pin positive form
//   - residual 3092 / 1362 and prior 3435 / 3387 pins still hold
...
auto runSpawnRejectNight3483SelfTests() -> bool
{
    ...
    // Residual 3092 / 1362 / prior 3435 / 3387 pins still hold under dedicated dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldRejectAtNightSpawn3483(c.spawnType, c.totd);
        const bool pin     = pinShouldRejectAtNightSpawn3483(c.spawnType, c.totd);
    ...
    ok = expect(ShouldRejectAtNightSpawn(SpawnTypeAtNight, vanadiel_time::TOTD::DAY) ==
                    pinShouldRejectAtNightSpawn3483(SpawnTypeAtNight, vanadiel_time::TOTD::DAY),
                "free == pin ATNIGHT-required + DAY") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(SpawnTypeAtNight, vanadiel_time::TOTD::NIGHT) ==
                    pinShouldRejectAtNightSpawn3483(SpawnTypeAtNight, vanadiel_time::TOTD::NIGHT),
                "free == pin ATNIGHT-required + NIGHT") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(0, vanadiel_time::TOTD::DAY) ==
                    pinShouldRejectAtNightSpawn3483(0, vanadiel_time::TOTD::DAY),
                "free == pin no ATNIGHT + DAY") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(0, vanadiel_time::TOTD::NIGHT) ==
                    pinShouldRejectAtNightSpawn3483(0, vanadiel_time::TOTD::NIGHT),
    ...
    for (int spawnType = 0; spawnType <= 0xFF; ++spawnType)
    {
        for (const auto totd : totds)
        {
            ...
            const bool inlineF = inlineShouldRejectAtNightSpawn3483(st, totd);
            const bool pin     = pinShouldRejectAtNightSpawn3483(st, totd);
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRejectAtNightSpawn3483(p.spawnType, p.totd);
        const bool pin     = pinShouldRejectAtNightSpawn3483(p.spawnType, p.totd);
