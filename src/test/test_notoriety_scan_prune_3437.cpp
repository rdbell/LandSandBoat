#include "test_notoriety_scan_prune_3437.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "notoriety ShouldScanNotorietyForPrune 3437 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CNotorietyContainer::hasEnmity outer gate for dual-wire cross-check
// (dedicated 3437 expand residual 3020; prior dedicated 3385):
...
// Compact dual-wire pin matching Go pinShouldScanNotorietyForPrune3437 /
// C++ capacity two-bool AND form (formula unchanged from 2807 / 3020 / 3385):
//   ownerPresent && lookupNonEmpty
...
// OmegaXI internal/notoriety; dedicated slice 3437 expand residual 3020 /
// pure 2807; prior dedicated 3385 retained).
//
// Coverage:
//   - free == inline == pin residual pins
//   - poles: owner+nonempty / nil owner / empty / both false
//   - residual 2807 / 3020 / 3385 pins still hold
//   - dense 2^2 free == inline == pin residual pins
//   - host-style inject + residual independence (add/remove/prune/hasEnmity
//     left alone — do not thrash add_member / prune_mob)
auto runNotorietyScanPrune3437SelfTests() -> bool
{
    ...
    // Residual 2807 / prior dual-wire 3020 / prior dedicated 3385 pins still
    // hold under dedicated expand.
    ...
    const struct
    {
        ...
        // Residual 2807 / 3020 / 3385 re-pins.
        ...
        { true, true, true, "residual 3385 scan owner+nonempty" },
        { false, true, false, "residual 3385 no scan nil owner" },
        { true, false, false, "residual 3385 no scan empty" },
        { false, false, false, "residual 3385 no scan nil+empty" },
    };
    ...
    // 3020 / 3029 / 3034 / 3165 / 3192 / 3208 / 3267 / 3297 / 3327 / 3385): outer
    // scan is distinct from per-entry prune, add, remove, and hasEnmity-after-prune.
    // Siblings left alone beyond pins — do not thrash add_member / prune_mob.
