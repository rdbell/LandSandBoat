#include "test_artisan_can_buy_sack_3775.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "artisan CanBuySack 3775 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline artisan.lua option 1 pure gate for dual-wire cross-check
// (slice 3775 dedicated expand residual 2879; prior dedicated 3730 / 3685 / 3640 / 3595 / 3550 / 3493 / 3312 / 3282 / 3242 / 3090):
...
// Compact dual-wire pin matching free function / capacity body (slice 3775).
...
// Prior dedicated 3730 pin for independence cross-check.
...
//
// Coverage:
//   - free == inline == pin == pin3730 (direct return formula)
...
//     prior expand 3685 / prior expand 3730 pins still hold
//   - prior dedicated 3730 independence (free == pinCanBuySack3730)
...
auto runArtisanCanBuySack3775SelfTests() -> bool
{
    ...
    // Residual constant / 0948 / 2879 / prior dedicated 3090 / 3242 / 3282 / 3312 / 3493 / 3550 / 3595 / 3640 / 3685 / 3730 pins still hold.
    ...
    // Residual poles: free == inline == pin == pin3730 (direct return).
    const struct
    {
        int32       gil;
        int32       sackSize;
        ...
        // Residual 2879 / 3090 / 3242 / 3282 / 3312 / 3493 / 3550 / 3595 / 3640 / 3685 / 3730 classic dual poles.
        ...
        // Prior dedicated 3090 / expand 3242 / expand 3282 / expand 3312 / expand 3493 / expand 3550 / expand 3595 / expand 3640 / expand 3685 / expand 3730 dense poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanBuySack3775(p.gil, p.sackSize);
        const bool prior   = pinCanBuySack3730(p.gil, p.sackSize);
        ...
        ok = expect(got == prior, "CanBuySack == prior pinCanBuySack3730") && ok;
    }

    // Dense gil edges around cost with empty sack: free == inline == pin == pin3730.
    for (int32 g = BuySackGilCost - 50; g <= BuySackGilCost + 50; ++g)
    {
        const bool got     = CanBuySack(g, 0);
        const bool inlineF = inlineCanBuySack(g, 0);
        const bool pin     = pinCanBuySack3775(g, 0);
        const bool prior   = pinCanBuySack3730(g, 0);
        ...
        ok = expect(got == prior, "dense gil free==prior 3730") && ok;
    }
    ...
    for (const int32 size : { 0, 1, 2, 15, 30, 80, 100, -1 })
    {
        for (const int32 g : { BuySackGilCost - 1, BuySackGilCost, BuySackGilCost + 1, 9979, 9980, 10000, 100000, 0, 1 })
        {
            ...
            const bool pin     = pinCanBuySack3775(g, size);
            const bool prior   = pinCanBuySack3730(g, size);
            ...
            ok = expect(got == prior, "dense gil×size free==prior 3730") && ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3730 for host poles.
    const struct
    {
        int32       gil;
        int32       sackSize;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == prior, "host inject free == prior pin 3730") && ok;
    }
