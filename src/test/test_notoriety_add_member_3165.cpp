#include "test_notoriety_add_member_3165.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "notoriety ShouldAddNotorietyMember 3165 self-test failed: " << label << '\n';
    }
    return condition;
}
...
auto inlineShouldAddNotorietyMember3165(const bool ownerPresent, const bool entityPresent,
                                        const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}

// Compact dual-wire pin matching Go pinShouldAddNotorietyMember3165 / C++ capacity
// positive form (formula unchanged from 2818 / 2959):
//   ownerPresent && entityPresent && differentAllegiance
auto pinShouldAddNotorietyMember3165(const bool ownerPresent, const bool entityPresent,
                                     const bool differentAllegiance) -> bool
{
    return ownerPresent && entityPresent && differentAllegiance;
}
...
// OmegaXI internal/notoriety; dedicated slice 3165; residual expand 2959 / pure 2818).
//
// Coverage:
...
auto runNotorietyAddMember3165SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3165(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool pin     = pinShouldAddNotorietyMember3165(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        ...
        ok = expect(got == inlineF && got == pin && got == wantPin,
                    "ShouldAddNotorietyMember free == inline == pin positive form") &&
             ok;
    }

    // Free == pin across residual poles.
    ok = expect(ShouldAddNotorietyMember(true, true, true) == pinShouldAddNotorietyMember3165(true, true, true),
                "free == pin admit all true") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(false, true, true) == pinShouldAddNotorietyMember3165(false, true, true),
                "free == pin nil owner") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(true, false, true) == pinShouldAddNotorietyMember3165(true, false, true),
                "free == pin nil entity") &&
         ok;
    ok = expect(ShouldAddNotorietyMember(true, true, false) == pinShouldAddNotorietyMember3165(true, true, false),
    ...
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            for (const bool differentAllegiance : { false, true })
            {
                ...
                const bool inlineF = inlineShouldAddNotorietyMember3165(ownerPresent, entityPresent, differentAllegiance);
                const bool pin     = pinShouldAddNotorietyMember3165(ownerPresent, entityPresent, differentAllegiance);
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3165(p.owner, p.entity, p.diff);
        const bool pin     = pinShouldAddNotorietyMember3165(p.owner, p.entity, p.diff);
