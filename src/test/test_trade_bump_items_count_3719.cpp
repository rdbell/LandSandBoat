#include "test_trade_bump_items_count_3719.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "trade ShouldBumpItemsCountOnSetEntry 3719 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldBumpItemsCountOnSetEntry3719 / C++ capacity:
//   slotInRange
...
// Prior dedicated 3674 inline pin for independence cross-check.
auto inlineShouldBumpItemsCountOnSetEntry3674(const bool slotInRange) -> bool
{
    return slotInRange;
}

// Prior dedicated 3674 compact pin for free == inline == pin == pin3674 cross-check.
auto pinShouldBumpItemsCountOnSetEntry3674(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3719 expand residual 2997; prior dedicated 3674 / 3629 / 3584 / 3540 / 3474 / 3420 / 3372).
//
// Coverage:
//   - free == inline == pin == pin3674 == slotInRange (identity)
//   - residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 pins still hold
//   - prior dedicated 3674 independence (free == prior inline == prior pin)
...
auto runTradeBumpItemsCount3719SelfTests() -> bool
{
    ...
    // Residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3674 ---
    const struct
    {
        bool        slotInRange;
        bool        want;
        ...
        // Residual 2997 / 2812 / prior 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 poles (full 2^1 surface).
        ...
        // Prior dedicated 3674 poles still hold.
        { true, true, "prior 3674 in range bumps" },
        { false, false, "prior 3674 out of range no bump" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3674 = pinShouldBumpItemsCountOnSetEntry3674(c.slotInRange);
        const bool wantPin = c.slotInRange;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3674.
        ok = expect(got == inlineF && got == pin && got == pin3674,
                    "dual-wire free == inline == pin == pin3674") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula identity") && ok;
    }

    // Free == pin == pin3674 across residual poles.
    ...
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry(false),
                "free == pin for false") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3674(true),
                "free == pin3674 for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3674(false),
                "free == pin3674 for false") &&
         ok;

    // Prior dedicated 3674 independence: free still matches prior inline/pin.
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == inlineShouldBumpItemsCountOnSetEntry3674(true) &&
                    ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3674(true),
                "prior 3674 free/inline/pin mismatch for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == inlineShouldBumpItemsCountOnSetEntry3674(false) &&
                    ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3674(false),
                "prior 3674 free/inline/pin mismatch for false") &&
         ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3674.
    for (const bool slotInRange : { false, true })
    {
        ...
        const bool pin3674 = pinShouldBumpItemsCountOnSetEntry3674(slotInRange);
        const bool want    = slotInRange;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3674,
                                    "compose free == inline == pin == pin3674") &&
             ok;
        // Prior 3674 independence across dense poles.
        ok = expect(got == inlineShouldBumpItemsCountOnSetEntry3674(slotInRange) &&
                        got == pinShouldBumpItemsCountOnSetEntry3674(slotInRange),
                    "compose free == prior 3674 inline/pin") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3674 for multi-arg setItem bump.
    for (const bool slotInRange : { false, true })
    {
        const bool got     = ShouldBumpItemsCountOnSetEntry(slotInRange);
        const bool inlineF = inlineShouldBumpItemsCountOnSetEntry(slotInRange);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3674,
                                    "host inject free == inline == pin == pin3674") &&
             ok;
    }
