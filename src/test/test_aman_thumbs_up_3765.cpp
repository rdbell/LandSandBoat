#include "test_aman_thumbs_up_3765.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3765 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3765 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3765 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3720 pin for independence cross-check.
auto pinCanThumbsUp3720(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3720(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3765 expand residual 2967; prior dedicated 3720 / 3675 / 3630 / 3585 / 3541 / 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == pin3720 == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 pins still hold
//   - prior 3720 independence (free == 3720 inline/pin)
...
auto runAmanThumbsUp3765SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3720 re-pins (independence).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3720 = pinCanThumbsUp3720(c.cooldownExpired);
        ...
        ok = expect(got == pin3720, "CanThumbsUp dual-wire == pin3720") && ok;
    ...
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3720(true), "free==pin3720 true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3720(false), "free==pin3720 false") && ok;

    // Explicit residual poles free == inline == pin == pin3720 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3720 = pinCanThumbsUp3720(pole);
        ok                 = expect(got == pole, "pole free == identity") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3720, "pole free == pin3720") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3720.
    for (const bool cooldownExpired : { false, true })
    {
        ...
        ok              = expect(got == pinCanThumbsUp3720(cooldownExpired), "compose free == pin3720") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp(cooldownExpired),
             ...
             ok;
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp3720(cooldownExpired),
                    "host inject free == pin3720") &&
             ok;
    ...
    // Prior dedicated 3720 independence: free == prior 3720 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3720(true), "prior 3720 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3720(false), "prior 3720 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3720(true), "prior 3720 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3720(false), "prior 3720 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 0698 still hold under dedicated 3765.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/3541/3585/3630/3675/3720/0698 under 3765") && ok;
