#include "test_unity_reject_null_online_3678.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "unitychat ShouldRejectNullOnlineMember 3678 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember / DelOnlineMember null-PChar gate for dual-wire
// cross-check (slice 3678 dedicated expand residual 3075; prior dedicated
// 3633 / 3588 / 3544 / 3487 / 3436 / 3386):
//   charNull
...
// Compact dual-wire pin matching free function / capacity body (slice 3678).
// Direct return only — same formula as production ShouldRejectNullOnlineMember.
// free == inline == pin == pin3633.
...
// slice 3678 dedicated expand residual 3075; prior dedicated 3633 / 3588 / 3544 /
// 3487 / 3436 / 3386 retained). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3633 (direct return formula)
//   - residual 1356 / 3075 pins still hold
//   - prior 3386 / 3436 / 3487 / 3544 / 3588 / 3633 dedicated poles still hold
//   - dense 2¹
...
//     test_unity_reject_null_online_3633
...
auto runUnityRejectNullOnline3678SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3678(c.charNull);
        const bool pin3633 = pinShouldRejectNullOnlineMember3633(c.charNull);
        ...
        ok = expect(got == pin3633, "ShouldRejectNullOnlineMember == pin3633") && ok;
    ...
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3678(charNull);
        const bool pin3633 = pinShouldRejectNullOnlineMember3633(charNull);

        ok = expect(got == want, "compose free == pin formula") && ok;
        ok = expect(got == inlineF, "compose free == inline") && ok;
        ok = expect(got == pin, "compose free == pin 3678") && ok;
        ok = expect(got == pin3633, "compose free == pin3633") && ok;
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3678(h.charNull);
        const bool pin3633 = pinShouldRejectNullOnlineMember3633(h.charNull);
        ...
        ok                 = expect(reject == pin3633, "host compose free == pin3633") && ok;
