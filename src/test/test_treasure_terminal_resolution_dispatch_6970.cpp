...
// 6970). The all-voted lot that resolves an item first broadcasts its feedback,
// then broadcasts the terminal Win packet naming the highest lotter.
auto runTreasureTerminalResolutionDispatch6970SelfTests() -> bool
{
    ...
    bool ok = expect(pool.itemCount() == 0, "last vote resolves item") &&
              expect(first.getPacketCount() == 2 && second.getPacketCount() == 2, "each member receives feedback then terminal packet");
