#include "test_einherjar_should_cycle_wave_3329.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "einherjar ShouldCycleWave 3329 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onMobDespawn cycleWave gate for dual-wire cross-check
// (dedicated slice 3329):
//   #chamberData.mobs <= 0
...
// Compact dual-wire pin matching Go pinShouldCycleWave3329 / C++ capacity.
...
// 3329 expand residual 2864 / pure 1069; prior dedicated 3181 retained).
//
// Coverage:
//   - free == inline == pin == (remainingMobs <= 0)
//   - residual 1069 / 2864 pins still hold
//   - prior dedicated 3181 poles still hold
//   - poles: remainingMobs -1 / 0 / 1
//   - dense cases matching 3181 style
auto runEinherjarShouldCycleWave3329SelfTests() -> bool
{
    ...
    // Residual 1069 / 2864 / prior 3181 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2864 / prior 3181 happy / multi remaining.
    ...
    // Dense compose: remaining poles free == inline == pin (3181 style).
