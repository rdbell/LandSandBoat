#include "test_aman_thumbs_up_3900.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3900 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3900 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3900 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3855 pin for independence cross-check.
auto pinCanThumbsUp3855(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3855(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3900 expand residual 2967; prior dedicated 3855 / 3810 / 3765 / 3720 / 3675 / 3630 / 3585 / 3541 / 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == pin3855 == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 3810 / 3855 pins still hold
//   - prior 3855 independence (free == 3855 inline/pin)
...
auto runAmanThumbsUp3900SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 3810 / 3855 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3630 re-pins.
        { true, true, "prior 3630 true" },
        { false, false, "prior 3630 false" },

        // Prior dedicated 3675 re-pins.
        { true, true, "prior 3675 true" },
        { false, false, "prior 3675 false" },

        // Prior dedicated 3720 re-pins.
        { true, true, "prior 3720 true" },
        { false, false, "prior 3720 false" },

        // Prior dedicated 3765 re-pins.
        { true, true, "prior 3765 true" },
        { false, false, "prior 3765 false" },

        // Prior dedicated 3810 re-pins.
        { true, true, "prior 3810 true" },
        { false, false, "prior 3810 false" },

        // Prior dedicated 3855 re-pins (independence).
        { true, true, "prior 3855 true" },
        { false, false, "prior 3855 false" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3855 = pinCanThumbsUp3855(c.cooldownExpired);
        ...
        ok = expect(got == pin3855, "CanThumbsUp dual-wire == pin3855") && ok;
    ...
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3855(true), "free==pin3855 true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3855(false), "free==pin3855 false") && ok;

    // Explicit residual poles free == inline == pin == pin3855 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3855 = pinCanThumbsUp3855(pole);
        ok                 = expect(got == pole, "pole free == identity") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3855, "pole free == pin3855") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3855.
    for (const bool cooldownExpired : { false, true })
    {
        ...
        ok              = expect(got == pinCanThumbsUp3855(cooldownExpired), "compose free == pin3855") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp(cooldownExpired),
             ...
             ok;
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp3855(cooldownExpired),
                    "host inject free == pin3855") &&
             ok;
    ...
    // Prior dedicated 3855 independence: free == prior 3855 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3855(true), "prior 3855 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3855(false), "prior 3855 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3855(true), "prior 3855 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3855(false), "prior 3855 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 3810 / 3855 / 0698 still hold under dedicated 3900.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/3541/3585/3630/3675/3720/3765/3810/3855/0698 under 3900") && ok;
