#include "test_petutils_pet_engage_3625.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petutils ShouldPetEngage 3625 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AttackTarget engage formula for dual-wire cross-check (dedicated 3625
// expand residual 3051; prior dedicated 3580 / 3536 / 3484 / 3429 / 3375):
...
// Compact dual-wire pin matching Go pinShouldPetEngage3625 / C++ capacity:
//   !hasPreventActionEffect
...
// Prior dedicated 3580 pin for free == inline == pin == pin3580 cross-check.
...
// dedicated slice 3625 expand residual 3051; prior dedicated 3580 / 3536 / 3484 / 3429 / 3375; pure 1627).
//
// Coverage:
//   - free == inline == pin == pin3580 == !hasPreventActionEffect
...
auto runPetutilsPetEngage3625SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3580 ---
    const struct
    {
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3580 = pinShouldPetEngage3580(c.hasPreventActionEffect);
        const bool wantPin = !c.hasPreventActionEffect;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3580.
        ok = expect(got == inlineF && got == pin && got == pin3580,
                    "dual-wire free == inline == pin == pin3580") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula !hasPreventActionEffect") && ok;
    }

    // Free == pin == pin3580 across residual poles.
    ...
    ok = expect(ShouldPetEngage(false) == pinShouldPetEngage3580(false), "free == pin3580 no prevent") && ok;
    ok = expect(ShouldPetEngage(true) == pinShouldPetEngage3580(true), "free == pin3580 prevent") && ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3580.
    for (const bool hasPrevent : { false, true })
    {
        ...
        const bool pin3580 = pinShouldPetEngage3580(hasPrevent);
        const bool want    = !hasPrevent;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3580,
                    "compose free == inline == pin == pin3580") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3580 for AttackTarget.
    for (const bool hasPrevent : { false, true })
    {
        const bool got     = ShouldPetEngage(hasPrevent);
        const bool inlineF = inlineShouldPetEngage(hasPrevent);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3580,
                    "host inject free == inline == pin == pin3580") &&
             ok;
