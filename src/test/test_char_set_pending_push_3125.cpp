#include "test_char_set_pending_push_3125.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "char ShouldSetPendingPositionOnPush 3125 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnPush pending-position formula for dual-wire cross-check (slice 3125):
...
// (packetType == 0x5B && packetEntityID == ownerID; slice 3125).
auto runCharSetPendingPush3125SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        { 0x5B, 0, 1, false, "residual zero entity vs owner" },
        { 0x5B, 0, 0, true, "residual zero owner match" },
        { 0x00, 99, 99, false, "residual type zero" },
