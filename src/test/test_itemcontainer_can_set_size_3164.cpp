#include "test_itemcontainer_can_set_size_3164.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "itemcontainer CanSetSize 3164 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline SetSize / AddSize acceptance for dual-wire cross-check (dedicated 3164):
...
// Compact dual-wire pin matching Go pinCanSetSize3164 / C++ capacity:
//   newSize <= maxSize && newSize >= itemCount
auto pinCanSetSize(const std::uint8_t newSize, const std::uint8_t maxSize, const std::uint8_t itemCount) -> bool
{
    return newSize <= maxSize && newSize >= itemCount;
}
...
// (SetSize / AddSize acceptance; OmegaXI internal/itemcontainer;
// dedicated slice 3164 expand residual 3027).
//
// Coverage:
//   - free == inline == pin == (newSize <= maxSize && newSize >= itemCount)
//   - residual 2802 / 3027 pins still hold
auto runItemcontainerCanSetSize3164SelfTests() -> bool
{
    ...
    // Residual 2802 / 3027 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinCanSetSize(c.newSize, c.maxSize, c.itemCount);
        ...
        ok = expect(got == wantPin, "free == pin formula newSize <= max && newSize >= count") && ok;
    }
    ...
    for (std::uint8_t maxSize = 0; maxSize <= 8; ++maxSize)
    {
        for (std::uint8_t itemCount = 0; itemCount <= 8; ++itemCount)
        {
            for (std::uint8_t newSize = 0; newSize <= 10; ++newSize)
            {
                ...
                const bool inlineF = inlineCanSetSize(newSize, maxSize, itemCount);
                const bool pin     = pinCanSetSize(newSize, maxSize, itemCount);
    ...
    // Host-style inject poles: free == inline == pin for SetSize / AddSize.
