#include "test_follow_can_follow_3872.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3872 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3872 dedicated expand residual 2883; prior dedicated 3827 / 3782 /
// 3737 / 3692 / 3647 / 3602 / 3557 / 3506 / 3459 / 3405 / 3331 / 3237 / 3084):
...
// Compact dual-wire pin matching free function / capacity body (slice 3872).
// Direct AND/EQ form only (no De Morgan) — same formula as production CanFollow.
auto pinCanFollow3872(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}

// Prior dedicated 3827 pin retained for free == pin3827 dual-wire agreement.
auto pinCanFollow3827(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3872 dedicated
// expand residual 2883; prior dedicated 3827 / 3782 / 3737 / 3692 / 3647 /
// 3602 / 3557 / 3506 / 3459 / 3405 / 3331 / 3237 / 3084). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3827 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 / 3692 / 3737 / 3782 / 3827 poles still hold
...
auto runFollowCanFollow3872SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 / 3692 / 3737 / 3782 / 3827 pins still hold.
    ...
    const struct
    {
        ...
        // Prior dedicated 3782 expand poles.
        { true, true, 100, 100, true, "prior3782 host eligible continue path" },
        { false, true, 100, 100, false, "prior3782 host follower unspawned" },
        { true, false, 100, 100, false, "prior3782 host leader unspawned" },
        { true, true, 100, 101, false, "prior3782 host zone mismatch" },

        // Prior dedicated 3827 expand poles.
        { true, true, 100, 100, true, "prior3827 host eligible continue path" },
        { false, true, 100, 100, false, "prior3827 host follower unspawned" },
        { true, false, 100, 100, false, "prior3827 host leader unspawned" },
        { true, true, 100, 101, false, "prior3827 host zone mismatch" },
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3872(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3827 = pinCanFollow3827(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3872(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3827 = pinCanFollow3827(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3827, "dense free == pin3827") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3827 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3827, "host inject dual-wire free == pin3827") && ok;
    }
