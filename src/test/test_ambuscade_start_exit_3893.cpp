#include "test_ambuscade_start_exit_3893.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ambuscade start-exit 3893 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onInstanceComplete / onInstanceFailure formula for dual-wire
// checks (slice 3893): player:startEvent(10001) is always called for every char.
...
// Compact dual-wire pin matching C++ capacity formula / Go pinShouldStartExitEvent3893:
//   return true
...
// Prior dedicated pin3848 (always true; free == pin3848 always holds).
auto pinShouldStartExitEvent3848() -> bool
{
    return true;
}
...
// OmegaXI internal/ambuscade; dedicated slice 3893 expand residual 2917; prior
// dedicated 3848/3803/3758/3713/3668/3623/3578/3534/3488/3438/3382/3109). Formula unchanged:
//
//   ShouldStartExitEvent() = true
//
// Coverage:
//   - free == inline == pin == pin3848 == true
//   - residual 2917 / 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 3623 / 3668 / 3713 / 3758 / 3803 / 3848 / 1089 pins still hold
...
auto runAmbuscadeStartExit3893SelfTests() -> bool
{
    ...
    // Residual constant / 1089 / 2917 / prior dedicated 3109 / 3382 / 3438 / 3488 / 3534 / 3578 / 3623 / 3668 / 3713 / 3758 / 3803 / 3848 pins still hold.
    ...
    // Unconditional gate always true; free == inline == pin == pin3848 (direct return true).
    {
        ...
        const bool pin3848   = pinShouldStartExitEvent3848();
        ...
        ok = expect(got == pin3848, "dual-wire free == pin3848") && ok;
        ok = expect(got == true, "dual-wire free == true") && ok;
    }

    // Repeated calls stay true (no state); free == inline == pin == pin3848 matching C++.
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        ok             = expect(got == pinShouldStartExitEvent3848(), "free == pin3848 across calls") && ok;
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pinShouldStartExitEvent3848(), "compose free == pin3848") && ok;
    ...
    // Dense always-true poles: free == inline == pin == pin3848 for unconditional domain.
    for (int i = 0; i < 2; ++i)
    {
        const bool got = ShouldStartExitEvent();
        ok             = expect(got == pinShouldStartExitEvent(), "dense free == pin") && ok;
        ok             = expect(got == inlineShouldStartExitEvent(), "dense free == inline") && ok;
        ok             = expect(got == pinShouldStartExitEvent3848(), "dense free == pin3848") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3848(), "free == pin3848 always-true") && ok;
    ...
    ok = expect(ShouldStartExitEvent() == pinShouldStartExitEvent3848(), "host path free == pin3848") && ok;

    // Residual 2917 / prior dedicated 3848/3803/3758/3713/3668/3623/3578/3534/3488/3438/3382/3109 still hold under dedicated 3893.
    ok = expect(ShouldStartExitEvent(), "residual 2917/3848/3803/3758/3713/3668/3623/3578/3534/3488/3438/3382/3109 pins under 3893") && ok;
