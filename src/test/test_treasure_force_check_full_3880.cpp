#include "test_treasure_force_check_full_3880.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3880 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3880):
...
// Compact dual-wire pin matching Go pinShouldForceCheckOnFullPoolInsert3880:
//   slotAfterFreeScan == PoolSize
...
// Compact dual-wire pin matching Go pinShouldForceCheckOnFullPoolInsert3835
// (prior dedicated retained under 3880):
//   slotAfterFreeScan == PoolSize
auto pinShouldForceCheckOnFullPoolInsert3835(const uint8 slotAfterFreeScan) -> bool
{
    return slotAfterFreeScan == treasurepoolhelpers::PoolSize;
}
...
// slice 3880 expand residual 2981; prior dedicated 3835 / 3790 / 3745 / 3700 / 3655 / 3610 / 3565 / 3533 / 3477 / 3423 / 3379 retained).
//
// Coverage:
//   - free == inline == pin == pin3835 == (slotAfterFreeScan == PoolSize)
//   - residual 1367 / 2981 pins still hold
//   - prior dedicated 3835 / 3790 / 3745 / 3700 / 3655 / 3610 / 3565 / 3533 / 3477 / 3423 / 3379 poles still hold
...
auto runTreasureForceCheckFull3880SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3835 ---
    const struct
    {
        uint8       slot;
        bool        want;
        ...
        // Prior dedicated 3700 poles still hold.
        { 0, false, "prior 3700 first free no force" },
        { 9, false, "prior 3700 last free no force" },
        { 10, true, "prior 3700 PoolSize force" },
        { 11, false, "prior 3700 one over no force" },
        { 255, false, "prior 3700 sentinel no force" },

        // Prior dedicated 3745 poles still hold.
        { 0, false, "prior 3745 first free no force" },
        { 9, false, "prior 3745 last free no force" },
        { 10, true, "prior 3745 PoolSize force" },
        { 11, false, "prior 3745 one over no force" },
        { 255, false, "prior 3745 sentinel no force" },

        // Prior dedicated 3790 poles still hold.
        { 0, false, "prior 3790 first free no force" },
        { 9, false, "prior 3790 last free no force" },
        { 10, true, "prior 3790 PoolSize force" },
        { 11, false, "prior 3790 one over no force" },
        { 255, false, "prior 3790 sentinel no force" },

        // Prior dedicated 3835 poles still hold.
        { 0, false, "prior 3835 first free no force" },
        { 9, false, "prior 3835 last free no force" },
        { 10, true, "prior 3835 PoolSize force" },
        { 11, false, "prior 3835 one over no force" },
        { 255, false, "prior 3835 sentinel no force" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3835 = pinShouldForceCheckOnFullPoolInsert3835(c.slot);
        const bool wantPin = c.slot == PoolSize;

        ok = expect(got == c.want, c.label) && ok;
        // free == inline == pin == pin3835.
        ok = expect(got == inlineF && got == pin && got == pin3835, "dual-wire free == inline == pin == pin3835") && ok;
        ok = expect(got == wantPin, "free == pin formula slot == PoolSize") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3835 for classic cells.
    for (const uint8 s : { uint8{ 0 }, uint8{ 1 }, uint8{ 3 }, uint8{ 9 }, uint8{ 10 }, uint8{ 11 }, uint8{ 255 } })
    {
        ...
        const bool pin3835 = pinShouldForceCheckOnFullPoolInsert3835(s);
        const bool want    = s == PoolSize;
        ok                 = expect(got == want, "pole free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3835, "pole free == inline == pin == pin3835") && ok;
    }

    // Dense compose over slot domain around PoolSize (0..20) — free == inline == pin == pin3835.
    for (uint16 slot = 0; slot <= 20; ++slot)
    {
        ...
        ok                  = expect(got == inlineF && got == pin && got == pin3835, "compose free == inline == pin == pin3835") && ok;
    }
    ...
    // free == inline == pin == pin3835 residual pins.
    for (uint8 slot = 0; slot < PoolSize; ++slot)
    {
        ...
        const bool pin3835 = pinShouldForceCheckOnFullPoolInsert3835(slot);
        ok                 = expect(!got, "host inject free-found must not force-check") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3835, "host inject free-found free == inline == pin == pin3835") && ok;
    }
    {
        ...
        const bool pin3835 = pinShouldForceCheckOnFullPoolInsert3835(PoolSize);
        ok                 = expect(got, "host inject SlotID==PoolSize must force-check") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3835, "host inject PoolSize free == inline == pin == pin3835") && ok;
    }
