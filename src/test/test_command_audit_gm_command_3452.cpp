#include "test_command_audit_gm_command_3452.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "command ShouldAuditGMCommand 3452 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CCommandHandler::call audit gate for dual-wire cross-check (dedicated 3452):
...
// Compact dual-wire pin matching Go pinShouldAuditGMCommand3452 / C++ capacity
// positive form (formula unchanged from 2792 / 3011 / 3161 / 3400):
...
// OmegaXI internal/command; dedicated slice 3452; residual expand 3011 / pure 2792;
// prior dedicated expand 3400 / 3161).
//
// Coverage:
//   - free == inline == pin positive form
//   - poles: audit 0 always false; level vs permission; signed permission edge
//   - residual 3011 / 2792 / 3161 / 3400 pins still hold
...
auto runCommandAuditGMCommand3452SelfTests() -> bool
{
    ...
    // Residual 2792 / 3011 / 3161 / 3400 pins still hold under dedicated dual-wire.
    ...
    ok = expect(ShouldAuditGMCommand(3, 10), "residual 3161: audit 3 < perm 10") && ok;
    ok = expect(ShouldAuditGMCommand(127, static_cast<int8>(127)), "residual 3161: audit 127 == perm 127") && ok;
    ok = expect(!ShouldAuditGMCommand(10, 9), "residual 3161: audit 10 > perm 9") && ok;
    ok = expect(ShouldAuditGMCommand(3, 10), "residual 3400: audit 3 < perm 10") && ok;
    ok = expect(ShouldAuditGMCommand(127, static_cast<int8>(127)), "residual 3400: audit 127 == perm 127") && ok;
    ok = expect(!ShouldAuditGMCommand(10, 9), "residual 3400: audit 10 > perm 9") && ok;
    ...
    const struct
    {
        ...
        // Residual 3011 / 2792 / 3161 / 3400 re-pins.
        ...
        { 3, 10, true, "residual 3161 under mid" },
        { 127, static_cast<int8>(127), true, "residual 3161 equal max int8" },
        { 10, 9, false, "residual 3161 just over" },
        { 3, 10, true, "residual 3400 under mid" },
        { 127, static_cast<int8>(127), true, "residual 3400 equal max int8" },
        { 10, 9, false, "residual 3400 just over" },
    };
