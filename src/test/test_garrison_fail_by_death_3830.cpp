#include "test_garrison_fail_by_death_3830.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "garrison fail-by-death 3830 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldFailByDeath3830:
//   allNPCsDead || allPlayersDead
...
// (Lua xi.garrison BATTLE state fail-by-death gate; dedicated slice 3830;
...
// prior dedicated expand residual 3785).
//
// Coverage:
//   - free == inline == pin == pin3785 (direct return positive OR; no De Morgan)
//   - 4-row truth table + residual poles + dense 2² free == inline == pin == pin3785
//   - residual 2870 / prior dedicated 3073 / prior dedicated expand 3236 /
//     prior dedicated expand 3330 / prior dedicated expand 3406 /
//     prior dedicated expand 3461 / prior dedicated expand 3514 /
//     prior dedicated expand 3560 / prior dedicated expand 3605 /
//     prior dedicated expand 3650 / prior dedicated expand 3695 /
//     prior dedicated expand 3740 / prior dedicated expand 3785 pins still hold
...
auto runGarrisonFailByDeath3830SelfTests() -> bool
{
    ...
    // prior dedicated expand 3560 / prior dedicated expand 3605 /
    // prior dedicated expand 3650 / prior dedicated expand 3695 /
    // prior dedicated expand 3740 / prior dedicated expand 3785 pins.
    ...
    // 4-row truth table (+ residual re-pins): free == inline == pin == pin3785.
    const struct
    {
        bool        allNPCsDead;
        bool        allPlayersDead;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3785 = pinShouldFailByDeath3785(c.allNPCsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3785, "ShouldFailByDeath dual-wire == pin3785") && ok;
    ...
    // Free == pin == pin3785 across residual poles (4-row truth table).
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath(false, false) &&
                    ShouldFailByDeath(false, false) == pinShouldFailByDeath3785(false, false),
                "free == pin/pin3785 neither dead") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath(true, false) &&
                    ShouldFailByDeath(true, false) == pinShouldFailByDeath3785(true, false),
                "free == pin/pin3785 npcs dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath(false, true) &&
                    ShouldFailByDeath(false, true) == pinShouldFailByDeath3785(false, true),
                "free == pin/pin3785 players dead only") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath(true, true) &&
                    ShouldFailByDeath(true, true) == pinShouldFailByDeath3785(true, true),
                "free == pin/pin3785 both dead") &&
         ok;

    // Dense compose: full 2^2 boolean space free == inline == pin == pin3785.
    for (const bool allNPCsDead : { false, true })
    {
        for (const bool allPlayersDead : { false, true })
        {
            ...
            ok = expect(got == pinShouldFailByDeath(allNPCsDead, allPlayersDead),
                        "compose free == pin") &&
                 ok;
            ok = expect(got == pinShouldFailByDeath3785(allNPCsDead, allPlayersDead),
                        "compose free == pin3785") &&
                 ok;
        }
    }
    ...
    // prior dedicated expand 3560 / prior dedicated expand 3605 /
    // prior dedicated expand 3650 / prior dedicated expand 3695 /
    // prior dedicated expand 3740 / prior dedicated expand 3785 still hold.
    ok = expect(!ShouldFailByDeath(false, false) && ShouldFailByDeath(true, false) &&
                ...
                "residual 2870/3073/3236/3330/3406/3461/3514/3560/3605/3650/3695/3740/3785 wipe gate pins under 3830") &&
         ok;
    ...
    ok = expect(ShouldFailByDeath(false, false) == pinShouldFailByDeath3785(false, false),
                "prior 3785 pin neither dead still holds under 3830") &&
         ok;
    ok = expect(ShouldFailByDeath(true, false) == pinShouldFailByDeath3785(true, false),
                "prior 3785 pin npcs dead still holds under 3830") &&
         ok;
    ok = expect(ShouldFailByDeath(false, true) == pinShouldFailByDeath3785(false, true),
                "prior 3785 pin players dead still holds under 3830") &&
         ok;
    ok = expect(ShouldFailByDeath(true, true) == pinShouldFailByDeath3785(true, true),
                "prior 3785 pin both dead still holds under 3830") &&
         ok;

    // Residual AllNPCsDead compose (2870 sibling) still holds under 3830.
    ...
    // Explicit residual compose dual-wire free == inline == pin == pin3785 for host poles.
    const struct
    {
        int32       npcCount;
        int32       deadNPCCount;
    ...
    for (const auto& c : composeCases)
    {
        ...
        const bool pin3785   = pinShouldFailByDeath3785(npcsDead, c.allPlayersDead);
        ...
        ok = expect(got == pin3785, "compose dual-wire free == pin3785") && ok;
    }
