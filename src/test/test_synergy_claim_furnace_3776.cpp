#include "test_synergy_claim_furnace_3776.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3776 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3776 dedicated expand residual 2877; prior dedicated expand 3731):
...
// Compact dual-wire pin matching free function / capacity body (slice 3776).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3776 dedicated
// expand residual 2877; prior dedicated expand 3731). Formula unchanged:
//
//   CanClaimFurnace(state) = state == FurnaceAvailable
//
// Coverage:
//   - free == inline == pin == pin3731 (direct return formula)
...
//     prior expand 3686 / prior expand 3731 poles
//   - residual poles for all furnaceStates + dense 0..8 out-of-range
//   - prior 3731 pin independence
auto runSynergyClaimFurnace3776SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285 / 3315 / 3494 / 3551 / 3596 / 3641 / 3686 / 3731).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3776(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3776(s);
