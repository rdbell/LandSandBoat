#include "test_trade_bump_items_count_3540.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "trade ShouldBumpItemsCountOnSetEntry 3540 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldBumpItemsCountOnSetEntry3540 / C++ capacity:
//   slotInRange
...
// Prior dedicated 3474 inline pin for independence cross-check.
auto inlineShouldBumpItemsCountOnSetEntry3474(const bool slotInRange) -> bool
{
    return slotInRange;
}

// Prior dedicated 3474 compact pin for independence cross-check.
auto pinShouldBumpItemsCountOnSetEntry3474(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3540 expand residual 2997; prior dedicated 3474 / 3420 / 3372).
//
// Coverage:
//   - free == inline == pin == slotInRange (identity)
//   - residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 pins still hold
//   - prior dedicated 3474 independence (free == prior inline == prior pin)
...
auto runTradeBumpItemsCount3540SelfTests() -> bool
{
    ...
    // Residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2997 / 2812 / prior 3372 / 3420 / 3474 poles (full 2^1 surface).
    ...
    // Prior dedicated 3474 independence: free still matches prior inline/pin.
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == inlineShouldBumpItemsCountOnSetEntry3474(true) &&
                    ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3474(true),
                "prior 3474 free/inline/pin mismatch for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == inlineShouldBumpItemsCountOnSetEntry3474(false) &&
                    ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3474(false),
                "prior 3474 free/inline/pin mismatch for false") &&
         ok;
    ...
    for (const bool slotInRange : { false, true })
    {
        ...
        // Prior 3474 independence across dense poles.
        ok = expect(got == inlineShouldBumpItemsCountOnSetEntry3474(slotInRange) &&
                        got == pinShouldBumpItemsCountOnSetEntry3474(slotInRange),
                    "compose free == prior 3474 inline/pin") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin for multi-arg setItem bump.
    for (const bool slotInRange : { false, true })
    {
        const bool got     = ShouldBumpItemsCountOnSetEntry(slotInRange);
        const bool inlineF = inlineShouldBumpItemsCountOnSetEntry(slotInRange);
