#include "test_follow_can_follow_3647.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3647 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3647 dedicated expand residual 2883; prior dedicated 3602 / 3557 /
...
// Compact dual-wire pin matching free function / capacity body (slice 3647).
...
// Prior dedicated 3602 pin retained for free == pin3602 dual-wire agreement.
auto pinCanFollow3602(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3647 dedicated
// expand residual 2883; prior dedicated 3602 / 3557 / 3506 / 3459 / 3405 /
// 3331 / 3237 / 3084). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3602 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 poles still hold
...
auto runFollowCanFollow3647SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3647(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3602 = pinCanFollow3602(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3647(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3602 = pinCanFollow3602(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3602, "dense free == pin3602") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3602 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3602, "host inject dual-wire free == pin3602") && ok;
    }
