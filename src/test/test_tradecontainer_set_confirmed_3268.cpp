#include "test_tradecontainer_set_confirmed_3268.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "tradecontainer ShouldAllowSetConfirmedStatus 3268 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline setConfirmedStatus outer gate for dual-wire cross-check (dedicated 3268):
...
// Compact dual-wire pin matching Go pinShouldAllowSetConfirmedStatus3268 / C++ capacity:
//   slotInRange && itemNonNull && quantityGteAmount
...
// dedicated slice 3268 expand residual 2962; prior dedicated 3175).
//
// Coverage:
//   - free == inline == pin == (slotInRange && itemNonNull && quantityGteAmount)
//   - residual 2806 / 2962 / 3175 pins still hold
...
auto runTradecontainerSetConfirmed3268SelfTests() -> bool
{
    ...
    // Residual 2806 / 2962 / 3175 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2962 / 2806 / 3175 poles (full 2^3 surface).
