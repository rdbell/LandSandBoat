#include "test_extravaganza_can_afford_cipher_3408.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "extravaganza CanAffordCipher 3408 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua shadowEraFinish notes gate for dual-wire cross-check
// (dedicated 3408 expand residual 2881; prior 3322):
//   notes >= 1000
auto inlineCanAffordCipher3408(const int32 notes) -> bool
{
    return notes >= 1000;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3408:
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3408(const int32 notes) -> bool
{
...
// internal/extravaganza; dedicated slice 3408 expand residual 2881;
// prior dedicated 3322 / 3292 / 3262 / 3153 retained).
//
// Coverage:
//   - free == inline == pin == notes >= CipherPurchaseNotes
//   - CipherPurchaseNotes == 1000 pin
//   - residual 2881 / 3322 / 3292 / 3262 / 3153 / 0949 pins still hold
...
auto runExtravaganzaCanAffordCipher3408SelfTests() -> bool
{
    ...
    // Residual 2881 / 3322 / 3292 / 3262 / 3153 / 0949 CanAffordCipher pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3408(c.notes);
        const bool pinGot  = pinCanAffordCipher3408(c.notes);
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3408(p.notes);
        const bool pinGot  = pinCanAffordCipher3408(p.notes);
    ...
    for (const int32 notes : dense)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3408(notes);
        const bool pinGot  = pinCanAffordCipher3408(notes);
