#include "test_party_reject_pc_add_full_3600.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "party ShouldRejectPCAddFull 3600 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddMember IsFull gate for dual-wire cross-check (dedicated 3600
// expand residual 2928; prior dedicated 3555 / 3498 / 3200):
...
// Compact dual-wire pin matching Go pinShouldRejectPCAddFull3600 / C++ capacity
// positive form (formula unchanged from 1327 / 1350 / 2928 / 3200 / 3498 / 3555):
//   isPCEntity && isPCParty && partyFull
...
// Prior dedicated 3555 inline/pin for free==inline==pin==pin3555 independence.
...
//   - free == inline == pin == pin3555 independence
//   - prior 3200 / 3498 / 3555 independence (free == prior inline/pin)
auto runPartyRejectPCAddFull3600SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldRejectPCAddFull3600(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool pin     = pinShouldRejectPCAddFull3600(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool pin3555 = pinShouldRejectPCAddFull3555(c.isPCEntity, c.isPCParty, c.partyFull);
        // Positive form pin composition (explicit three-bool AND).
        const bool wantPin = c.isPCEntity && c.isPCParty && c.partyFull;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == wantPin && got == pin3555,
                    "ShouldRejectPCAddFull free == inline == pin == pin3555 positive form") &&
             ok;
    }

    // Free == pin across residual poles.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3600(true, true, true),
                "free == pin all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3600(true, true, false),
                "free == pin not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3600(false, true, true),
                "free == pin non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3600(true, false, true),
                "free == pin non-PC party") &&
         ok;

    // free == inline == pin == pin3555 independence poles.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == inlineShouldRejectPCAddFull3600(true, true, true) &&
                ...
                "free == inline == pin == pin3555 all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == inlineShouldRejectPCAddFull3600(true, true, false) &&
                ...
                "free == inline == pin == pin3555 not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == inlineShouldRejectPCAddFull3600(false, true, true) &&
                ...
                "free == inline == pin == pin3555 non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == inlineShouldRejectPCAddFull3600(true, false, true) &&
                ...
                "free == inline == pin == pin3555 non-PC party") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, false, false) == inlineShouldRejectPCAddFull3600(false, false, false) &&
                ...
                "free == inline == pin == pin3555 all false") &&
         ok;
    ...
    // Dense compose: full 2^3 boolean space free == inline == pin == pin3555.
    for (const bool isPCEntity : { false, true })
    {
        for (const bool isPCParty : { false, true })
        {
            for (const bool partyFull : { false, true })
            {
                ...
                const bool inlineF = inlineShouldRejectPCAddFull3600(isPCEntity, isPCParty, partyFull);
                const bool pin     = pinShouldRejectPCAddFull3600(isPCEntity, isPCParty, partyFull);
                const bool pin3555 = pinShouldRejectPCAddFull3555(isPCEntity, isPCParty, partyFull);
                const bool want    = isPCEntity && isPCParty && partyFull;
                ok                 = expect(got == want, "compose free == pin formula") && ok;
                ok                 = expect(got == inlineF && got == pin && got == pin3555,
                                           "compose free == inline == pin == pin3555") &&
                     ok;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRejectPCAddFull3600(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool pin     = pinShouldRejectPCAddFull3600(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool pin3555 = pinShouldRejectPCAddFull3555(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool want    = p.isPCEntity && p.isPCParty && p.partyFull;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3555,
                                    "host inject free == inline == pin == pin3555") &&
             ok;
    ...
    // Residual independence (1327 / 1350 / 2928 / 2937 / 3200 / 3498 / 3555):
