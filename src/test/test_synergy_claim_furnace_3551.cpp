#include "test_synergy_claim_furnace_3551.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3551 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3551 dedicated expand residual 2877; prior dedicated expand 3494):
...
// Compact dual-wire pin matching free function / capacity body (slice 3551).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3551 dedicated
// expand residual 2877; prior dedicated expand 3494). Formula unchanged:
//
...
//     prior expand 3285 / prior expand 3315 / prior expand 3494 poles
//   - residual poles for all furnaceStates + dense 0..8 out-of-range
//   - prior 3494 pin independence
auto runSynergyClaimFurnace3551SelfTests() -> bool
{
    ...
    // prior expand 3285 / prior expand 3315 / prior expand 3494 pins.
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285 / 3315 / 3494).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3551(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3551(s);
