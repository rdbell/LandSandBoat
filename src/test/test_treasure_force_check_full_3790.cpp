#include "test_treasure_force_check_full_3790.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasure ShouldForceCheckOnFullPoolInsert 3790 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline addItem full-pool force-check formula for dual-wire cross-check (dedicated 3790):
...
// Compact dual-wire pin matching Go pinShouldForceCheckOnFullPoolInsert3745
// (prior dedicated retained under 3790):
//   slotAfterFreeScan == PoolSize
auto pinShouldForceCheckOnFullPoolInsert3745(const uint8 slotAfterFreeScan) -> bool
{
    return slotAfterFreeScan == treasurepoolhelpers::PoolSize;
}
...
// slice 3790 expand residual 2981; prior dedicated 3745 / 3700 / 3655 / 3610 / 3565 / 3533 / 3477 / 3423 / 3379 retained).
//
// Coverage:
//   - free == inline == pin == pin3745 == (slotAfterFreeScan == PoolSize)
//   - residual 1367 / 2981 pins still hold
//   - prior dedicated 3745 / 3700 / 3655 / 3610 / 3565 / 3533 / 3477 / 3423 / 3379 poles still hold
...
auto runTreasureForceCheckFull3790SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3745 ---
    const struct
    {
        uint8       slot;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3745 = pinShouldForceCheckOnFullPoolInsert3745(c.slot);
        const bool wantPin = c.slot == PoolSize;

        ok = expect(got == c.want, c.label) && ok;
        // free == inline == pin == pin3745.
        ok = expect(got == inlineF && got == pin && got == pin3745, "dual-wire free == inline == pin == pin3745") && ok;
        ok = expect(got == wantPin, "free == pin formula slot == PoolSize") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3745 for classic cells.
    for (const uint8 s : { uint8{ 0 }, uint8{ 1 }, uint8{ 3 }, uint8{ 9 }, uint8{ 10 }, uint8{ 11 }, uint8{ 255 } })
    {
        ...
        const bool pin3745 = pinShouldForceCheckOnFullPoolInsert3745(s);
        const bool want    = s == PoolSize;
        ok                 = expect(got == want, "pole free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3745, "pole free == inline == pin == pin3745") && ok;
    }

    // Dense compose over slot domain around PoolSize (0..20) — free == inline == pin == pin3745.
    for (uint16 slot = 0; slot <= 20; ++slot)
    {
        ...
        ok                  = expect(got == inlineF && got == pin && got == pin3745, "compose free == inline == pin == pin3745") && ok;
    }
    ...
    // free == inline == pin == pin3745 residual pins.
    for (uint8 slot = 0; slot < PoolSize; ++slot)
    {
        ...
        const bool pin3745 = pinShouldForceCheckOnFullPoolInsert3745(slot);
        ok                 = expect(!got, "host inject free-found must not force-check") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3745, "host inject free-found free == inline == pin == pin3745") && ok;
    }
    {
        ...
        const bool pin3745 = pinShouldForceCheckOnFullPoolInsert3745(PoolSize);
        ok                 = expect(got, "host inject SlotID==PoolSize must force-check") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3745, "host inject PoolSize free == inline == pin == pin3745") && ok;
    }
