#include "test_auctionutils_can_afford_fee_3709.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "auctionutils CanAffordFee 3709 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline production ProofOfPurchase fee formula for dual-wire cross-checks
// (dedicated slice 3709; residual expand 2924; prior dedicated 3664 / 3619 / 3574 / 3503 / 3310 / 3280):
...
// (slice 3709). Multi-branch: positive if/else only (avoid QF1001 De Morgan
...
// Prior dedicated 3664 pin (positive multi-branch if/else) for
// free == pin3664 cross-check.
auto pinCanAffordFee3664(const uint32 gilQuantity, const uint32 gilReserve, const uint32 fee) -> bool
{
    if (gilQuantity < fee)
    {
        return false;
...
// dedicated slice 3709 expand residual 2924 — formula unchanged;
// prior dedicated ~3664 / 3619 / 3574 / 3503 / 3310 / 3280).
//
// Coverage:
//   - free == inline == pin == pin3664 (multi-branch positive if/else)
//   - residual poles: exact, short, surplus, zero fee, reserve block
//   - residual 1135 / 2924 / prior dedicated 3280 / 3310 / 3503 / 3574 / 3619 / 3664 pins still hold
...
auto runAuctionutilsCanAffordFee3709SelfTests() -> bool
{
    ...
    // Residual 1135 / 2924 / prior dedicated 3280 / 3310 / 3503 / 3574 / 3619 / 3664 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3664 (multi-branch if/else) ---
    ...
    const struct
    {
        ...
        // residual 2924 / prior dedicated 3280 / 3310 / 3503 / 3574 / 3619 / 3664 re-pins
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3664 = pinCanAffordFee3664(c.gilQuantity, c.gilReserve, c.fee);

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3664.
        ok = expect(got == inlineC && got == pinGot && got == pin3664, "dual-wire free==inline==pin==pin3664") && ok;
    }
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pin3664 = pinCanAffordFee3664(pole.gilQuantity, pole.gilReserve, pole.fee);
        ok                 = expect(got == inlineC && got == pinGot && got == pin3664, "pole free==inline==pin==pin3664") && ok;
    }

    // Dense compose: free == inline == pin == pin3664 over required poles + neighbors.
    const struct
    {
        uint32 gilQuantity;
        uint32 gilReserve;
    ...
    for (const auto& p : dense)
    {
        ...
        const bool pin3664 = pinCanAffordFee3664(p.gilQuantity, p.gilReserve, p.fee);
        ...
        ok                 = expect(got == pin3664, "dense free == pin3664") && ok;
    }
