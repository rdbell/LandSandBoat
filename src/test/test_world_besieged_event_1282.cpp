#include "test_world_besieged_event_1282.h"

#include "world/besieged_event.h"
...
auto expect(const bool condition, const char* label) -> bool
{
    if (!condition)
    {
        std::cerr << "world BesiegedEvent 1282 self-test failed: " << label << '\n';
    }
    return condition;
}
...
auto testDispatch() -> bool
{
    ...
    const ipc::BesiegedEvent message{
        .type = static_cast<BesiegedMessage>(std::numeric_limits<uint8>::max()),
        .payload = { 0x00, 0x7F, 0x80, 0xFF },
    };
    int calls{};
    BesiegedMessage deliveredType{};
    IPP deliveredSource{};
    std::vector<uint8> deliveredPayload{};
    worldipc::HandleBesiegedEvent(
        source,
        message,
        [&](BesiegedMessage type, IPPMessage&& delivered)
        {
            ++calls;
            deliveredType = type;
