#include "test_world_party_disband_1270.h"

#include "world/party_disband.h"
...
auto expect(const bool condition, const char* label) -> bool
{
    if (!condition)
    {
        std::cerr << "world PartyDisband 1270 self-test failed: " << label << '\n';
    }
    return condition;
}
...
{
    const ipc::PartyDisband message{ .partyId = std::numeric_limits<uint32>::max() };
    const std::vector<IPP>  endpoints{ IPP{}, IPP(0x14131211, 45678), IPP{} };
    int                     rerouteCalls{};
    int                     lookupCalls{};
    uint32                  lookedUpId{};
    std::vector<IPP>        sentEndpoints{};
    std::vector<ipc::PartyDisband> sentMessages{};

    worldipc::HandlePartyDisband(
        message,
        [&](const uint32 partyId, const ipc::PartyDisband& delivered)
        {
            ++rerouteCalls;
            worldipc::RerouteMessageToPartyMembers(
                ...
                [&](const IPP& endpoint, const ipc::PartyDisband& sent)
                {
    ...
    worldipc::HandlePartyDisband(
        ...
        [&](const uint32 partyId, const ipc::PartyDisband& delivered)
        {
            worldipc::RerouteMessageToPartyMembers(
                ...
                [&](const IPP&, const ipc::PartyDisband&) { ++emptySends; });
        });
