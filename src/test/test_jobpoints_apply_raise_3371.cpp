#include "test_jobpoints_apply_raise_3371.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "jobpoints ShouldApplyRaiseJobPoint 3371 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline RaiseJobPoint spend-gate formula for dual-wire cross-check
// (dedicated slice 3371):
//   cost != 0 && currentJp >= cost
auto inlineShouldApplyRaiseJobPoint3371(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

// Compact dual-wire pin matching Go pinShouldApplyRaiseJobPoint3371 / C++ capacity:
//   cost != 0 && currentJp >= cost
// Direct return of the formula (not if/else true/false).
auto pinShouldApplyRaiseJobPoint3371(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}
...
// dedicated slice 3371 expand residual 3012 / prior dedicated 3219 /
// prior dedicated 3275).
//
// Coverage:
//   - free == inline == pin == (cost != 0 && currentJp >= cost)
//   - residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 pins still hold
...
auto runJobpointsApplyRaise3371SelfTests() -> bool
{
    ...
    // Residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 pins
    // still hold under dual-wire.
    ok = expect(ShouldApplyRaiseJobPoint(1, 1), "residual 2803/3012/3219/3275: cost 1 / jp 1 applies") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(0, 100), "residual 2803/3012/3219/3275: cost 0 blocks raise") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(10, 9), "residual 2803/3012/3219/3275: short JP blocks raise") && ok;
    ok = expect(ShouldApplyRaiseJobPoint(10, 10), "residual 2803/3012/3219/3275: exact JP applies") && ok;
    ...
    const struct
    {
        ...
        // Residual 2803 / 3012 / prior dedicated 3219 / prior dedicated 3275 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldApplyRaiseJobPoint3371(c.cost, c.currentJp);
        const bool pin     = pinShouldApplyRaiseJobPoint3371(c.cost, c.currentJp);
    ...
    for (const uint8 cost : poleCosts)
    {
        if (cost == 0)
        {
            ...
            for (const uint16 jp : zeroJPs)
            {
                ...
                const bool inlineF = inlineShouldApplyRaiseJobPoint3371(cost, jp);
                const bool pin     = pinShouldApplyRaiseJobPoint3371(cost, jp);
    ...
    for (const auto& c : hostCases)
    {
        ...
        const bool  inlineF = inlineShouldApplyRaiseJobPoint3371(cost, c.currentJp);
        const bool  pin     = pinShouldApplyRaiseJobPoint3371(cost, c.currentJp);
    ...
    // Residual independence (2803 / 3012 / prior 3219 / prior 3275): spend gate
    // is distinct from presence short-circuit and JobPointCost formula that
    // compose PlanRaiseJobPoint.
    {
