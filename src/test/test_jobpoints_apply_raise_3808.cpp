#include "test_jobpoints_apply_raise_3808.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "jobpoints ShouldApplyRaiseJobPoint 3808 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline RaiseJobPoint spend-gate formula for dual-wire cross-check
// (dedicated slice 3808):
//   cost != 0 && currentJp >= cost
auto inlineShouldApplyRaiseJobPoint3808(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

// Compact dual-wire pin matching Go pinShouldApplyRaiseJobPoint3808 / C++ capacity:
//   cost != 0 && currentJp >= cost
// Direct return of the formula (not if/else true/false).
auto pinShouldApplyRaiseJobPoint3808(const uint8 cost, const uint16 currentJp) -> bool
{
    return cost != 0 && currentJp >= cost;
}

// Prior dedicated 3763 pin surface (free == pin3763 independence).
...
// dedicated slice 3808 expand residual 3012 / prior dedicated 3219 /
...
// prior dedicated 3763).
//
// Coverage:
//   - free == inline == pin == pin3763 == (cost != 0 && currentJp >= cost)
...
auto runJobpointsApplyRaise3808SelfTests() -> bool
{
    ...
    ok = expect(ShouldApplyRaiseJobPoint(1, 1), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718/3763: cost 1 / jp 1 applies") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(0, 100), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718/3763: cost 0 blocks raise") && ok;
    ok = expect(!ShouldApplyRaiseJobPoint(10, 9), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718/3763: short JP blocks raise") && ok;
    ok = expect(ShouldApplyRaiseJobPoint(10, 10), "residual 2803/3012/3219/3275/3371/3421/3475/3539/3583/3628/3673/3718/3763: exact JP applies") && ok;

    // --- Composition table: free == inline == pin == pin3763 ---
    const struct
    {
        uint8       cost;
        uint16      currentJp;
        ...
        // prior dedicated 3371 / prior dedicated 3421 / prior dedicated 3475 /
        // prior dedicated 3539 / prior dedicated 3583 / prior dedicated 3628 /
        // prior dedicated 3673 / prior dedicated 3718 / prior dedicated 3763 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldApplyRaiseJobPoint3808(c.cost, c.currentJp);
        const bool pin     = pinShouldApplyRaiseJobPoint3808(c.cost, c.currentJp);
        const bool pin3763 = pinShouldApplyRaiseJobPoint3763(c.cost, c.currentJp);
        const bool wantPin = c.cost != 0 && c.currentJp >= c.cost;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3763.
        ok = expect(got == inlineF && got == pin && got == pin3763, "dual-wire free == inline == pin == pin3763") && ok;
    ...
    // Explicit poles free == inline == pin == pin3763 for cost 0 / exact / short-by-one.
    ...
    for (const uint8 cost : poleCosts)
    {
        if (cost == 0)
        {
            ...
            for (const uint16 jp : zeroJPs)
            {
                ...
                const bool inlineF = inlineShouldApplyRaiseJobPoint3808(cost, jp);
                const bool pin     = pinShouldApplyRaiseJobPoint3808(cost, jp);
                const bool pin3763 = pinShouldApplyRaiseJobPoint3763(cost, jp);
                ok = expect(!got && got == inlineF && got == pin && got == pin3763,
                            "pole cost0 free == inline == pin == pin3763") &&
                     ok;
            }
            continue;
        }
        // exact equality
        {
            ...
            const bool   pin3763 = pinShouldApplyRaiseJobPoint3763(cost, jp);
            ok                   = expect(got && got == inlineF && got == pin && got == pin3763,
                        "pole exact free == inline == pin == pin3763") &&
                 ok;
        }
        // short-by-one
        {
            ...
            const bool   pin3763 = pinShouldApplyRaiseJobPoint3763(cost, jp);
            ok = expect(!got && got == inlineF && got == pin && got == pin3763,
                        "pole short free == inline == pin == pin3763") &&
                 ok;
        }
    }
    ...
    for (const auto& c : hostCases)
    {
        ...
        const bool  inlineF = inlineShouldApplyRaiseJobPoint3808(cost, c.currentJp);
        const bool  pin     = pinShouldApplyRaiseJobPoint3808(cost, c.currentJp);
        const bool  pin3763 = pinShouldApplyRaiseJobPoint3763(cost, c.currentJp);
        const bool  wantPin = cost != 0 && c.currentJp >= cost;

        ok = expect(got == c.wantApply, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3763,
                    "host compose dual-wire free == inline == pin == pin3763") &&
             ok;
    ...
    for (const uint8 cost : costs)
    {
        for (const uint16 jp : jps)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3763,
                        "compose free == inline == pin == pin3763") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3763,
                        "compose extra free == inline == pin == pin3763") &&
                 ok;
        }
    }
    ...
    // prior 3718 / prior 3763): spend gate
    // is distinct from presence short-circuit and JobPointCost formula that compose
    // PlanRaiseJobPoint.
    // Prior dedicated 3763 independence: free == prior suite inline/pin forms
    // (local mirrors of test_jobpoints_apply_raise_3763 pin surface).
    for (const uint8 cost : costs)
    {
        for (const uint16 jp : jps)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3763(cost, jp),
                        "prior 3763 independence free == 3763 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3763(cost, jp),
                        "prior 3763 independence free == 3763 pin") &&
                 ok;
        }
        for (const uint16 jp : extraJPs)
        {
            const bool got = ShouldApplyRaiseJobPoint(cost, jp);
            ok = expect(got == inlineShouldApplyRaiseJobPoint3763(cost, jp),
                        "prior 3763 independence extra free == 3763 inline") &&
                 ok;
            ok = expect(got == pinShouldApplyRaiseJobPoint3763(cost, jp),
                        "prior 3763 independence extra free == 3763 pin") &&
                 ok;
        }
    }
    {
