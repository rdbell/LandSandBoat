#include "test_battlefield_enter_pc_3869.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "battlefield ShouldEnterPC 3869 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertEntity TYPE_PC enter-branch formula for dual-wire cross-check
// (dedicated 3869 expand residual 3024; prior dedicated 3824 / 3779 / 3734 / 3689 / 3644 / 3599 / 3554 / 3497 / 3431 / 3381):
//   enter
...
// Compact dual-wire pin matching Go pinShouldEnterPC3869 /
// C++ capacity enter-identity form (formula unchanged from 1361 / 3024 / 3381 /
// 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734 / 3779 / 3824):
//   enter
...
// Prior dedicated 3824 inline/pin for independence cross-check (pin3824 chain).
auto inlineShouldEnterPC3824(const bool enter) -> bool
{
    return enter;
}

auto pinShouldEnterPC3824(const bool enter) -> bool
{
    return enter;
}
...
// 3869 expand residual 3024 / prior dedicated 3824 / 3779 / 3734 / 3689 / 3644 / 3599 / 3554 / 3497 / 3431 / 3381 / pure 1361).
//
// Coverage:
//   - free == inline == pin residual pins
//   - free == pin3824 (prior dedicated pin chain)
//   - poles: enter-true / enter-false
//   - residual 1361 / 3024 pins still hold
//   - prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734 / 3779 / 3824 poles still hold
...
auto runBattlefieldEnterPC3869SelfTests() -> bool
{
    ...
    // 3554 / 3599 / 3644 / 3689 / 3734 / 3779 / 3824 pins still hold.
    ...
    // --- Core poles: free == inline == pin residual pins (== pin3824) ---
    const struct
    {
        bool        enter;
        bool        want;
        ...
        // Residual 1361 / 3024 / prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734 / 3779 / 3824 re-pins.
        ...
        { true, true, "prior 3689 enter=true takes enter path" },
        { false, false, "prior 3689 enter=false declines enter path" },
        { true, true, "prior 3734 enter=true takes enter path" },
        { false, false, "prior 3734 enter=false declines enter path" },
        { true, true, "prior 3779 enter=true takes enter path" },
        { false, false, "prior 3779 enter=false declines enter path" },
        { true, true, "prior 3824 enter=true takes enter path" },
        { false, false, "prior 3824 enter=false declines enter path" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3824  = pinShouldEnterPC3824(c.enter);
        ...
        ok = expect(got == pin3824, "dual-wire free==pin3824") && ok;
    ...
    // free == pin3824 pin chain (constraint free==inline==pin==pin3824).
    ok = expect(ShouldEnterPC(true) == pinShouldEnterPC3824(true),
                "free==pin3824 enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == pinShouldEnterPC3824(false),
                "free==pin3824 enter=false") &&
         ok;
    ok = expect(pinShouldEnterPC(true) == pinShouldEnterPC3824(true),
                "pin3869==pin3824 enter=true") &&
         ok;
    ok = expect(pinShouldEnterPC(false) == pinShouldEnterPC3824(false),
                "pin3869==pin3824 enter=false") &&
         ok;

    // Prior dedicated 3824 independence: free still matches prior inline/pin.
    ok = expect(ShouldEnterPC(true) == inlineShouldEnterPC3824(true) &&
                    ShouldEnterPC(true) == pinShouldEnterPC3824(true),
                "prior 3824 independence enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == inlineShouldEnterPC3824(false) &&
                    ShouldEnterPC(false) == pinShouldEnterPC3824(false),
                "prior 3824 independence enter=false") &&
         ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3824, "host compose free==pin3824") && ok;
    }

    // Production path semantics:
    // enter=true → enter; enter=false → no enter.
    ok = expect(ShouldEnterPC(true) && pinShouldEnterPC(true) && pinShouldEnterPC3824(true),
                "enter=true → enter path") &&
         ok;
    ok = expect(!ShouldEnterPC(false) && !pinShouldEnterPC(false) && !pinShouldEnterPC3824(false),
                "enter=false → no enter path") &&
         ok;

    // Residual independence (1361 / 2994 / 3198 / 3002 / 3216 / 2930 / 3271 /
    // 3302 / 3014 / 3365 / 3024 / 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734 / 3779 / 3824): enter is
