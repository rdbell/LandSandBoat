#include "test_unity_reject_null_online_3633.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "unitychat ShouldRejectNullOnlineMember 3633 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember / DelOnlineMember null-PChar gate for dual-wire
// cross-check (slice 3633 dedicated expand residual 3075; prior dedicated
// 3588 / 3544 / 3487 / 3436 / 3386):
//   charNull
...
// Compact dual-wire pin matching free function / capacity body (slice 3633).
// Direct return only — same formula as production ShouldRejectNullOnlineMember.
// free == inline == pin == pin3588.
...
// slice 3633 dedicated expand residual 3075; prior dedicated 3588 / 3544 / 3487 /
// 3436 / 3386 retained). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3588 (direct return formula)
//   - residual 1356 / 3075 pins still hold
//   - prior 3386 / 3436 / 3487 / 3544 / 3588 dedicated poles still hold
//   - dense 2¹
...
auto runUnityRejectNullOnline3633SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3633(c.charNull);
        const bool pin3588 = pinShouldRejectNullOnlineMember3588(c.charNull);
        ...
        ok = expect(got == pin3588, "ShouldRejectNullOnlineMember == pin3588") && ok;
    ...
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3633(charNull);
        const bool pin3588 = pinShouldRejectNullOnlineMember3588(charNull);

        ok = expect(got == want, "compose free == pin formula") && ok;
        ok = expect(got == inlineF, "compose free == inline") && ok;
        ok = expect(got == pin, "compose free == pin 3633") && ok;
        ok = expect(got == pin3588, "compose free == pin3588") && ok;
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3633(h.charNull);
        const bool pin3588 = pinShouldRejectNullOnlineMember3588(h.charNull);
        ...
        ok                 = expect(reject == pin3588, "host compose free == pin3588") && ok;
