#include "test_alliance_attempt_leader_promote_3393.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3393 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check
// (slice 3393 dedicated dual-wire expand residual 2979):
//   isMainParty
...
// (isMainParty identity; slice 3393 dedicated dual-wire expand residual 2979).
//
// Coverage:
//   - main party → attempt promote true
//   - non-main → attempt promote false
//   - free == inline == pin (isMainParty identity)
//   - residual 1341 / 1346 / 2979 / 3144 pins still hold
...
auto runAllianceAttemptLeaderPromote3393SelfTests() -> bool
{
    ...
    // Residual 1341 / prior dual-wire 2979 / prior expansion 3144 pins still hold.
