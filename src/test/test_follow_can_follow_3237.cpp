#include "test_follow_can_follow_3237.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3237 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3237 dedicated expand residual 2883; prior dedicated 3084):
...
// Compact dual-wire pin matching free function / capacity body (slice 3237).
// Direct return only — same formula as production CanFollow.
auto pinCanFollow3237(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3237 dedicated
// expand residual 2883; prior dedicated 3084). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2883 / 0945 / prior dedicated 3084 poles still hold
...
auto runFollowCanFollow3237SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3237(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    // Dense bool combos × zone equal/unequal:
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3237(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
