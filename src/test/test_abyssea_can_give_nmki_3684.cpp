#include "test_abyssea_can_give_nmki_3684.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "abyssea CanGiveNMKI 3684 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.abyssea.canGiveNMKI formula for dual-wire checks (slice 3684).
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3684 / C++ capacity
...
// Compact dual-wire pin matching Go pinCanGiveNMKI3639 (prior dedicated
// expand residual 2861 independence pins).
auto pinCanGiveNMKI3639(const int32 roll1to100, const int32 dropChance, const bool redProc) -> bool
{
    return roll1to100 <= dropChance || redProc;
}
...
// dedicated slice 3684 expand residual 2861 / prior 3639 / 3594 / 3549 / 3491 / 3434 /
// 3314 / 3284 / 3238 / 3089 — formula unchanged).
//
// Coverage (clones 3639 density on CanGiveNMKI primary + prior 3639 independence):
//   - free == inline == pin == pin3639 == (roll <= chance || redProc)  // positive OR
...
//   - residual 1041 / 2861 / prior 3089 / prior 3238 / 3284 / 3314 / 3434 / 3491 / 3549 / 3594 / 3639 pins still hold
...
auto runAbysseaCanGiveNMKI3684SelfTests() -> bool
{
    ...
    // Residual constant / 1041 / 2861 / 3089 / 3238 / 3284 / 3314 / 3434 / 3491 / 3549 / 3594 / 3639 pins still hold.
    ...
    // --- Composition table: free == inline == pin == pin3639 (positive OR form) ---
    const struct
    {
        int32       roll;
        int32       chance;
        ...
        // Residual 2861 / 3089 / 3434 / 3491 / 3549 / 3594 / 3639 poles.
    ...
    for (const auto& pole : poles)
    {
        ...
        const bool pin3639   = pinCanGiveNMKI3639(pole.roll, pole.chance, pole.redProc);
        const bool want      = pole.roll <= pole.chance || pole.redProc;
        ok                   = expect(got == want, pole.label) && ok;
        ok                   = expect(got == inlineGot && got == pinGot && got == pin3639,
                                      "pole free == inline == pin == pin3639") &&
             ok;
    }

    // Dense rolls 1..100 edges for normal + atma chances: free == inline == pin == pin3639.
    for (const int32 chance : { NormalNMKIDropChance, AtmaNMKIDropChance })
    {
        for (int32 roll = 1; roll <= 100; ++roll)
        {
            for (const bool red : { false, true })
            {
                ...
                const bool pin3639   = pinCanGiveNMKI3639(roll, chance, red);
                const bool want      = roll <= chance || red;
                ok                   = expect(got == want, "dense free == formula") && ok;
                ok                   = expect(got == inlineGot && got == pinGot && got == pin3639,
                                              "dense free == inline == pin == pin3639") &&
                     ok;
            }
        }
    }
    ...
    for (const int32 chance : { NormalNMKIDropChance, AtmaNMKIDropChance, 0, 50, 100 })
    {
        for (const int32 roll : { 0, -1, 101, 200 })
        {
            for (const bool red : { false, true })
            {
                ...
                const bool pin3639   = pinCanGiveNMKI3639(roll, chance, red);
                const bool want      = roll <= chance || red;
                ok                   = expect(got == want && got == inlineGot && got == pinGot && got == pin3639,
                                              "edge free == inline == pin == pin3639 == formula") &&
                     ok;
            }
        }
    }
    ...
    // Prior dedicated expand residual 3639 pin helpers still agree with free dual-wire.
    ok = expect(CanGiveNMKI(20, 20, false) == pinCanGiveNMKI3639(20, 20, false),
                "free == prior 3639 pin boundary") &&
         ok;
    ok = expect(CanGiveNMKI(21, 20, false) == pinCanGiveNMKI3639(21, 20, false),
                "free == prior 3639 pin short") &&
         ok;
    ok = expect(CanGiveNMKI(100, 0, true) == pinCanGiveNMKI3639(100, 0, true),
                "free == prior 3639 pin red-proc force") &&
         ok;
    ok = expect(pinCanGiveNMKI(20, 20, false) == pinCanGiveNMKI3639(20, 20, false),
                "3684 pin == prior 3639 pin boundary") &&
         ok;
    ok = expect(pinCanGiveNMKI(21, 20, false) == pinCanGiveNMKI3639(21, 20, false),
                "3684 pin == prior 3639 pin short") &&
         ok;
    ok = expect(pinCanGiveNMKI(100, 0, true) == pinCanGiveNMKI3639(100, 0, true),
                "3684 pin == prior 3639 pin red-proc force") &&
         ok;
