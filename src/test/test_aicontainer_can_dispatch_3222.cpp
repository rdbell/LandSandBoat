#include "test_aicontainer_can_dispatch_3222.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aicontainer CanDispatch 3222 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAIContainer public controller-forward presence gate for dual-wire
// cross-check (dedicated 3222):
//   hasController
...
// Compact dual-wire pin matching Go pinCanDispatch3222 / C++ capacity
// identity form (formula unchanged from 1189 / 2947). Direct return of
// hasController (not if/else):
//   hasController
...
// (hasController identity; OmegaXI internal/aicontainer; dedicated slice 3222;
// residual expand 2947 / pure 1189).
//
// Coverage:
//   - free == inline == pin identity form
//   - residual poles: true/false
//   - host-style inject + residual independence (2952 left alone)
auto runAicontainerCanDispatch3222SelfTests() -> bool
{
    ...
    // Residual independence (1189 / 2947 / 2952 left alone):
    ...
    ok = expect(CanChangeState(false, false) && CanChangeState(false, true),
                "idle CanChangeState residual still holds under dual-wire (2952 left alone)") &&
         ok;
