#include "test_synthutils_can_synthesize_hq_3614.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synthutils CanSynthesizeHQ 3614 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline canSynthesizeHQ pure half for dual-wire cross-check
// (slice 3614 dedicated expand residual 2876 / prior ~3569):
...
// Compact dual-wire pin matching free function / capacity body (slice 3614).
...
// Prior dedicated pin (slice 3569) — same formula; retained under 3614.
auto pinCanSynthesizeHQ3569(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3508) — same formula; retained under 3614.
auto pinCanSynthesizeHQ3508(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3313) — same formula; retained under 3614.
auto pinCanSynthesizeHQ3313(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3283) — same formula; retained under 3614.
auto pinCanSynthesizeHQ3283(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3244) — same formula; retained under 3614.
...
// OmegaXI internal/synthutils; slice 3614 dedicated expand residual 2876 /
// prior ~3569). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3569 (direct return formula)
//   - residual 2876 / prior dedicated 3569 / 3508 / 3313 / 3283 / 3244 /
...
auto runSynthutilsCanSynthesizeHQ3614SelfTests() -> bool
{
    ...
    // Residual 2876 / 1922 / prior dedicated 3244 / 3283 / 3313 / 3508 / 3569.
    ...
    const struct
    {
        ...
        // Residual 2876 / 1922 / prior 3244 / 3283 / 3313 / 3508 / 3569 poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3614(p.antiHQMod);
    ...
    // free == inline == pin == pin3569 == formula.
    const struct
    {
        int16       antiHQMod;
        const char* label;
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3614(e.antiHQMod);
    ...
    for (int16 m = -200; m <= 200; ++m)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3614(m);
