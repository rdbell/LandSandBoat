#include "test_aistate_del_actor_on_attack_3846.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3846 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3846 expand residual 3044):
//   emittedHostile
auto inlineShouldDelActorOnAttack3846(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3846 / C++ capacity
// identity form (formula unchanged from 1631 / 3044 / 3178 / 3401 / 3453 / 3521 / 3576 / 3621 / 3666 / 3711 / 3756 / 3801):
//   emittedHostile
...
// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3801
// (prior dedicated retained under 3846):
//   emittedHostile
...
// OmegaXI internal/aistate; dedicated slice 3846 expand residual 3044;
// prior dedicated 3801 / 3756 / 3711 / 3666 / 3621 / 3576 / 3521 / 3453 / 3401 / 3178; pure 1631 / 1709 / 2306). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3801
//   - residual 3044 / 1631 / prior 3178 / prior 3401 / prior 3453 / prior 3521 / prior 3576 / prior 3621 / prior 3666 / prior 3711 / prior 3756 / prior 3801 pins still hold
...
auto runAistateDelActorOnAttack3846SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 / prior 3178 / prior 3401 / prior 3453 / prior 3521 / prior 3576 / prior 3621 / prior 3666 / prior 3711 / prior 3756 / prior 3801 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044/3178/3401/3453/3521/3576/3621/3666/3711/3756/3801: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044/3178/3401/3453/3521/3576/3621/3666/3711/3756/3801: emittedHostile false → no del") && ok;

    // --- Core poles: free == inline == pin == pin3801 ---
    const struct
    {
        bool        emittedHostile;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3846(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3846(c.emittedHostile);
        const bool pin3801 = pinShouldDelActorOnAttack3801(c.emittedHostile);
        // Positive form pin composition (explicit identity).
        const bool wantPin = c.emittedHostile;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3801 && got == wantPin,
                    "ShouldDelActorOnAttack free == inline == pin == pin3801") &&
             ok;
    }
    ...
    // Free == pin3801 across residual poles (prior dedicated independence).
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3801(true),
                "free == pin3801 for emittedHostile true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3801(false),
                "free == pin3801 for emittedHostile false") &&
         ok;

    // Dense compose: full 2¹ boolean space free == inline == pin == pin3801.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3846(emitted);
        const bool pin     = pinShouldDelActorOnAttack3846(emitted);
        const bool pin3801 = pinShouldDelActorOnAttack3801(emitted);
        const bool want    = emitted;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3801,
                                    "compose free == inline == pin == pin3801") &&
             ok;
    }
    ...
    // free == inline == pin == pin3801 for host inject poles.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3846(emitted);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3801,
                                    "host inject free == inline == pin == pin3801") &&
             ok;
    }

    // Prior dedicated 3801 pin independence under 3846 expand.
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3801(true),
                "prior 3801 pin holds under 3846 for true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3801(false),
                "prior 3801 pin holds under 3846 for false") &&
         ok;
