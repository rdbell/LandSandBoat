#include "test_party_reject_pc_add_full_3870.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "party ShouldRejectPCAddFull 3870 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddMember IsFull gate for dual-wire cross-check (dedicated 3870
// expand residual 2928; prior dedicated 3825 / 3780 / 3735 / 3690 / 3645 / 3600 / 3555 / 3498 / 3200):
//   isPCEntity && isPCParty && partyFull
auto inlineShouldRejectPCAddFull3870(const bool isPCEntity, const bool isPCParty, const bool partyFull) -> bool
{
    return isPCEntity && isPCParty && partyFull;
}

// Compact dual-wire pin matching Go pinShouldRejectPCAddFull3870 / C++ capacity
// positive form (formula unchanged from 1327 / 1350 / 2928 / 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735 / 3780 / 3825):
//   isPCEntity && isPCParty && partyFull
auto pinShouldRejectPCAddFull3870(const bool isPCEntity, const bool isPCParty, const bool partyFull) -> bool
{
    return isPCEntity && isPCParty && partyFull;
}
...
// Prior dedicated 3690 inline/pin for free==prior independence.
...
// Prior dedicated 3735 inline/pin for free==prior independence.
...
// Prior dedicated 3780 inline/pin for free==prior independence.
auto inlineShouldRejectPCAddFull3780(const bool isPCEntity, const bool isPCParty, const bool partyFull) -> bool
{
    return isPCEntity && isPCParty && partyFull;
}

auto pinShouldRejectPCAddFull3780(const bool isPCEntity, const bool isPCParty, const bool partyFull) -> bool
{
    return isPCEntity && isPCParty && partyFull;
}

// Prior dedicated 3825 inline/pin for free==inline==pin==pin3825 independence.
auto inlineShouldRejectPCAddFull3825(const bool isPCEntity, const bool isPCParty, const bool partyFull) -> bool
{
    return isPCEntity && isPCParty && partyFull;
}

auto pinShouldRejectPCAddFull3825(const bool isPCEntity, const bool isPCParty, const bool partyFull) -> bool
{
    return isPCEntity && isPCParty && partyFull;
}
...
// OmegaXI internal/party; dedicated slice 3870; residual expand 2928 /
...
//   - prior 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735 / 3780 / 3825 dedicated poles still hold
...
//   - free == inline == pin == pin3825 independence
//   - prior 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735 / 3780 / 3825 independence (free == prior inline/pin)
auto runPartyRejectPCAddFull3870SelfTests() -> bool
{
    ...
    const struct
    {
        ...
        // Prior dedicated 3735 poles still hold.
        { true, true, true, true, "prior 3735 PC full rejects" },
        { true, true, false, false, "prior 3735 not full allows" },
        { true, false, true, false, "prior 3735 mob party full allows" },
        { false, true, true, false, "prior 3735 mob entity full allows" },
        { false, false, false, false, "prior 3735 all false allows" },

        // Prior dedicated 3780 poles still hold.
        { true, true, true, true, "prior 3780 PC full rejects" },
        { true, true, false, false, "prior 3780 not full allows" },
        { true, false, true, false, "prior 3780 mob party full allows" },
        { false, true, true, false, "prior 3780 mob entity full allows" },
        { false, false, false, false, "prior 3780 all false allows" },

        // Prior dedicated 3825 poles still hold.
        { true, true, true, true, "prior 3825 PC full rejects" },
        { true, true, false, false, "prior 3825 not full allows" },
        { true, false, true, false, "prior 3825 mob party full allows" },
        { false, true, true, false, "prior 3825 mob entity full allows" },
        { false, false, false, false, "prior 3825 all false allows" },
    };

    for (const auto& c : cases)
    {
        const bool got     = ShouldRejectPCAddFull(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool inlineF = inlineShouldRejectPCAddFull3870(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool pin     = pinShouldRejectPCAddFull3870(c.isPCEntity, c.isPCParty, c.partyFull);
        const bool pin3825 = pinShouldRejectPCAddFull3825(c.isPCEntity, c.isPCParty, c.partyFull);
        // Positive form pin composition (explicit three-bool AND).
        const bool wantPin = c.isPCEntity && c.isPCParty && c.partyFull;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == wantPin && got == pin3825,
                    "ShouldRejectPCAddFull free == inline == pin == pin3825 positive form") &&
             ok;
    }

    // Free == pin across residual poles.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3870(true, true, true),
                "free == pin all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3870(true, true, false),
                "free == pin not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3870(false, true, true),
                "free == pin non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3870(true, false, true),
                "free == pin non-PC party") &&
         ok;

    // free == inline == pin == pin3825 independence poles.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == inlineShouldRejectPCAddFull3870(true, true, true) &&
                    ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3870(true, true, true) &&
                    ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3825(true, true, true),
                "free == inline == pin == pin3825 all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == inlineShouldRejectPCAddFull3870(true, true, false) &&
                    ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3870(true, true, false) &&
                    ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3825(true, true, false),
                "free == inline == pin == pin3825 not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == inlineShouldRejectPCAddFull3870(false, true, true) &&
                    ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3870(false, true, true) &&
                    ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3825(false, true, true),
                "free == inline == pin == pin3825 non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == inlineShouldRejectPCAddFull3870(true, false, true) &&
                    ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3870(true, false, true) &&
                    ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3825(true, false, true),
                "free == inline == pin == pin3825 non-PC party") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, false, false) == inlineShouldRejectPCAddFull3870(false, false, false) &&
                    ShouldRejectPCAddFull(false, false, false) == pinShouldRejectPCAddFull3870(false, false, false) &&
                    ShouldRejectPCAddFull(false, false, false) == pinShouldRejectPCAddFull3825(false, false, false),
                "free == inline == pin == pin3825 all false") &&
         ok;
    ...
    // Prior dedicated 3735 independence: free still matches prior inline/pin.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == inlineShouldRejectPCAddFull3735(true, true, true) &&
                ...
                "prior 3735 independence all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == inlineShouldRejectPCAddFull3735(true, true, false) &&
                ...
                "prior 3735 independence not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == inlineShouldRejectPCAddFull3735(false, true, true) &&
                ...
                "prior 3735 independence non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == inlineShouldRejectPCAddFull3735(true, false, true) &&
                ...
                "prior 3735 independence non-PC party") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, false, false) == inlineShouldRejectPCAddFull3735(false, false, false) &&
                ...
                "prior 3735 independence all false") &&
         ok;

    // Prior dedicated 3780 independence: free still matches prior inline/pin.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == inlineShouldRejectPCAddFull3780(true, true, true) &&
                    ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3780(true, true, true),
                "prior 3780 independence all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == inlineShouldRejectPCAddFull3780(true, true, false) &&
                    ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3780(true, true, false),
                "prior 3780 independence not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == inlineShouldRejectPCAddFull3780(false, true, true) &&
                    ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3780(false, true, true),
                "prior 3780 independence non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == inlineShouldRejectPCAddFull3780(true, false, true) &&
                    ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3780(true, false, true),
                "prior 3780 independence non-PC party") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, false, false) == inlineShouldRejectPCAddFull3780(false, false, false) &&
                    ShouldRejectPCAddFull(false, false, false) == pinShouldRejectPCAddFull3780(false, false, false),
                "prior 3780 independence all false") &&
         ok;

    // Prior dedicated 3825 independence: free still matches prior inline/pin.
    ok = expect(ShouldRejectPCAddFull(true, true, true) == inlineShouldRejectPCAddFull3825(true, true, true) &&
                    ShouldRejectPCAddFull(true, true, true) == pinShouldRejectPCAddFull3825(true, true, true),
                "prior 3825 independence all-true reject") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, true, false) == inlineShouldRejectPCAddFull3825(true, true, false) &&
                    ShouldRejectPCAddFull(true, true, false) == pinShouldRejectPCAddFull3825(true, true, false),
                "prior 3825 independence not full") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, true, true) == inlineShouldRejectPCAddFull3825(false, true, true) &&
                    ShouldRejectPCAddFull(false, true, true) == pinShouldRejectPCAddFull3825(false, true, true),
                "prior 3825 independence non-PC entity") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(true, false, true) == inlineShouldRejectPCAddFull3825(true, false, true) &&
                    ShouldRejectPCAddFull(true, false, true) == pinShouldRejectPCAddFull3825(true, false, true),
                "prior 3825 independence non-PC party") &&
         ok;
    ok = expect(ShouldRejectPCAddFull(false, false, false) == inlineShouldRejectPCAddFull3825(false, false, false) &&
                    ShouldRejectPCAddFull(false, false, false) == pinShouldRejectPCAddFull3825(false, false, false),
                "prior 3825 independence all false") &&
         ok;

    // Dense compose: full 2^3 boolean space free == inline == pin == pin3825.
    for (const bool isPCEntity : { false, true })
    {
        for (const bool isPCParty : { false, true })
        {
            for (const bool partyFull : { false, true })
            {
                ...
                const bool inlineF = inlineShouldRejectPCAddFull3870(isPCEntity, isPCParty, partyFull);
                const bool pin     = pinShouldRejectPCAddFull3870(isPCEntity, isPCParty, partyFull);
                const bool pin3825 = pinShouldRejectPCAddFull3825(isPCEntity, isPCParty, partyFull);
                const bool want    = isPCEntity && isPCParty && partyFull;
                ok                 = expect(got == want, "compose free == pin formula") && ok;
                ok                 = expect(got == inlineF && got == pin && got == pin3825,
                                           "compose free == inline == pin == pin3825") &&
                     ok;
                ...
                ok = expect(got == inlineShouldRejectPCAddFull3690(isPCEntity, isPCParty, partyFull) &&
                     ...
                     ok;
                ok = expect(got == inlineShouldRejectPCAddFull3735(isPCEntity, isPCParty, partyFull) &&
                                got == pinShouldRejectPCAddFull3735(isPCEntity, isPCParty, partyFull),
                            "compose free == prior 3735 inline/pin") &&
                     ok;
                ok = expect(got == inlineShouldRejectPCAddFull3780(isPCEntity, isPCParty, partyFull) &&
                                got == pinShouldRejectPCAddFull3780(isPCEntity, isPCParty, partyFull),
                            "compose free == prior 3780 inline/pin") &&
                     ok;
                ok = expect(got == inlineShouldRejectPCAddFull3825(isPCEntity, isPCParty, partyFull) &&
                                got == pinShouldRejectPCAddFull3825(isPCEntity, isPCParty, partyFull),
                            "compose free == prior 3825 inline/pin") &&
                     ok;
            }
        }
    }
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRejectPCAddFull3870(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool pin     = pinShouldRejectPCAddFull3870(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool pin3825 = pinShouldRejectPCAddFull3825(p.isPCEntity, p.isPCParty, p.partyFull);
        const bool want    = p.isPCEntity && p.isPCParty && p.partyFull;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3825,
                                    "host inject free == inline == pin == pin3825") &&
             ok;
    ...
    // Residual independence (1327 / 1350 / 2928 / 2937 / 3200 / 3498 / 3555 / 3600 / 3645 / 3690 / 3735 / 3780 / 3825):
