#include "test_synergy_claim_furnace_3641.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3641 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3641 dedicated expand residual 2877; prior dedicated expand 3596):
...
// Compact dual-wire pin matching free function / capacity body (slice 3641).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3641 dedicated
// expand residual 2877; prior dedicated expand 3596). Formula unchanged:
//
...
//     prior expand 3551 / prior expand 3596 poles
//   - residual poles for all furnaceStates + dense 0..8 out-of-range
//   - prior 3596 pin independence
auto runSynergyClaimFurnace3641SelfTests() -> bool
{
    ...
    // expand 3551 / prior expand 3596 pins.
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285 / 3315 / 3494 / 3551 / 3596).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3641(p.state);
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3641(s);
