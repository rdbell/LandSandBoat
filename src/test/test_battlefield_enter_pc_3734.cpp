#include "test_battlefield_enter_pc_3734.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "battlefield ShouldEnterPC 3734 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertEntity TYPE_PC enter-branch formula for dual-wire cross-check
// (dedicated 3734 expand residual 3024; prior dedicated 3689 / 3644 / 3599 / 3554 / 3497 / 3431 / 3381):
//   enter
...
// Compact dual-wire pin matching Go pinShouldEnterPC3734 /
// C++ capacity enter-identity form (formula unchanged from 1361 / 3024 / 3381 /
// 3431 / 3497 / 3554 / 3599 / 3644 / 3689):
//   enter
...
// Prior dedicated 3689 inline/pin for independence cross-check (pin3689 chain).
auto inlineShouldEnterPC3689(const bool enter) -> bool
{
    return enter;
}

auto pinShouldEnterPC3689(const bool enter) -> bool
{
    return enter;
}
...
// 3734 expand residual 3024 / prior dedicated 3689 / 3644 / 3599 / 3554 / 3497 / 3431 / 3381 / pure 1361).
//
// Coverage:
//   - free == inline == pin residual pins
//   - free == pin3689 (prior dedicated pin chain)
//   - poles: enter-true / enter-false
//   - residual 1361 / 3024 pins still hold
//   - prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 poles still hold
...
auto runBattlefieldEnterPC3734SelfTests() -> bool
{
    ...
    // 3554 / 3599 / 3644 / 3689 pins still hold.
    ...
    // --- Core poles: free == inline == pin residual pins (== pin3689) ---
    const struct
    {
        bool        enter;
        bool        want;
        ...
        // Residual 1361 / 3024 / prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3689  = pinShouldEnterPC3689(c.enter);
        ...
        ok = expect(got == pin3689, "dual-wire free==pin3689") && ok;
    ...
    // free == pin3689 pin chain (constraint free==inline==pin==pin3689).
    ok = expect(ShouldEnterPC(true) == pinShouldEnterPC3689(true),
                "free==pin3689 enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == pinShouldEnterPC3689(false),
                "free==pin3689 enter=false") &&
         ok;
    ok = expect(pinShouldEnterPC(true) == pinShouldEnterPC3689(true),
                "pin3734==pin3689 enter=true") &&
         ok;
    ok = expect(pinShouldEnterPC(false) == pinShouldEnterPC3689(false),
                "pin3734==pin3689 enter=false") &&
         ok;

    // Prior dedicated 3689 independence: free still matches prior inline/pin.
    ok = expect(ShouldEnterPC(true) == inlineShouldEnterPC3689(true) &&
                    ShouldEnterPC(true) == pinShouldEnterPC3689(true),
                "prior 3689 independence enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == inlineShouldEnterPC3689(false) &&
                    ShouldEnterPC(false) == pinShouldEnterPC3689(false),
                "prior 3689 independence enter=false") &&
         ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3689, "host compose free==pin3689") && ok;
    }

    // Production path semantics:
    // enter=true → enter; enter=false → no enter.
    ok = expect(ShouldEnterPC(true) && pinShouldEnterPC(true) && pinShouldEnterPC3689(true),
                "enter=true → enter path") &&
         ok;
    ok = expect(!ShouldEnterPC(false) && !pinShouldEnterPC(false) && !pinShouldEnterPC3689(false),
                "enter=false → no enter path") &&
         ok;

    // Residual independence (1361 / 2994 / 3198 / 3002 / 3216 / 2930 / 3271 /
    // 3302 / 3014 / 3365 / 3024 / 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689): enter is
