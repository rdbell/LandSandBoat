#include "test_campaign_afford_notes_3496.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "campaign afford notes 3496 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua sigilOnEventFinish shop afford gate for dual-wire checks
// (dedicated 3496 expand residual 2946; prior dedicated 3304 retained):
...
// Compact dual-wire pin matching Go pinCanAffordAlliedNotes3496 / C++ capacity:
//   notes >= price
...
// OmegaXI internal/campaign; dedicated slice 3496 expand residual 2946;
// prior dedicated 3304 independence retained).
//
// Coverage:
//   - free == inline == pin == (notes >= price)
//   - residual 2946 / 3072 / 3226 / 3273 / 3304 / 1115 pins still hold
//   - prior 3304 independence: free == pin3304 (same notes >= price formula)
...
auto runCampaignAffordNotes3496SelfTests() -> bool
{
    ...
    // Residual 1115 / 2946 / 3072 / 3226 / 3273 / 3304 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2946 / 3072 / 3226 / 3273 / 3304 poles.
    ...
    for (const auto& c : cases)
    {
        ...
        // Positive form: free == inline == pin (prior 3304 independence same formula).
    ...
    // Production path semantics still hold (incl. prior 3304 independence).
    ...
    ok = expect(CanAffordAlliedNotes(1000, 1000) == pinCanAffordAlliedNotes(1000, 1000),
                "formula exact free == pin (prior 3304 independence)") &&
         ok;
    ok = expect(CanAffordAlliedNotes(999, 1000) == pinCanAffordAlliedNotes(999, 1000),
                "formula short free == pin (prior 3304 independence)") &&
         ok;
