#include "test_darkixion_can_break_horn_3266.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "darkixion CanBreakHorn 3266 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline checkHornBreak pure gate (before 5% roll) for dual-wire cross-check
// (dedicated 3266 expand residual 2885; prior 3154):
...
// Positive if/else pin matching free function / capacity body (slice 3266).
...
// dedicated slice 3266 expand residual 2885; prior dedicated 3154 retained).
//
// Coverage:
//   - free == inline == pin (positive if/else)
//   - residual 2885 / 3154 / 0985 pins still hold
...
auto runDarkixionCanBreakHorn3266SelfTests() -> bool
{
    ...
    // Residual 0985 / 2885 / 3154 CanBreakHorn pins still hold under dual-wire.
