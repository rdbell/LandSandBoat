#include "test_alliance_attempt_leader_promote_3726.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3726 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check
// (slice 3726 dedicated dual-wire expand residual 2979):
//   isMainParty
...
// Prior dedicated expand residual 2979 pin (slice 3681; formula UNCHANGED).
// free == inline == pin == pin3681.
auto pinShouldAttemptAllianceLeaderPromote3681(const bool isMainParty) -> bool
{
    return isMainParty;
}

// This slice's dual-wire pin (slice 3726; formula isMainParty identity —
// UNCHANGED). free == inline == pin == pin3681.
...
// (isMainParty identity; slice 3726 dedicated dual-wire expand residual 2979).
//
// Coverage:
//   - main party → attempt promote true
//   - non-main → attempt promote false
//   - free == inline == pin == pin3681 (isMainParty identity)
//   - residual 1341 / 1346 / 2979 / 3144 / 3393 / 3447 / 3490 / 3547 / 3591 / 3636 / 3681 pins still hold
...
auto runAllianceAttemptLeaderPromote3726SelfTests() -> bool
{
    ...
    // dedicated 3681 pins still hold.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3726(c.isMainParty);
        const bool pin3681 = pinShouldAttemptAllianceLeaderPromote3681(c.isMainParty);

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3681,
                    "ShouldAttemptAllianceLeaderPromote free == inline == pin == pin3681") &&
             ok;
    }
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3681.
    for (const bool isMainParty : { false, true })
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3726(isMainParty);
        const bool pin3681 = pinShouldAttemptAllianceLeaderPromote3681(isMainParty);
        const bool want    = isMainParty;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3681,
                    "compose free == inline == pin == pin3681") &&
             ok;
    }
    ...
    for (const bool isMainParty : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3681,
                    "host inject dual-wire free == inline == pin == pin3681") &&
             ok;
    }
