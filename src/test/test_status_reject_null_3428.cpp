#include "test_status_reject_null_3428.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "status effect ShouldRejectNullStatusEffect 3428 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddStatusEffect null-effect formula for dual-wire cross-check
// (dedicated slice 3428 expand residual 3080; prior dedicated 3348):
//   isNull
...
// Compact dual-wire pin matching Go pinShouldRejectNullStatusEffect3428 /
// C++ capacity identity form (formula unchanged from 1371 / 3080 / 3348):
//   isNull
...
// (isNull identity; OmegaXI internal/statuseffect; dedicated slice 3428
// expand residual 3080 / prior dedicated 3348 / pure 1371).
//
// Coverage:
//   - free == inline == pin identity form
//   - poles: null rejects; non-null proceeds
//   - residual 3080 / 1371 pins still hold
//   - prior 3348 dedicated poles still hold
...
auto runStatusRejectNull3428SelfTests() -> bool
{
    ...
    // Residual independence (1371 / 2932 / 3080 / 3348; 3049 / 3069 / 3113 / 3135 left alone):
    // null reject is distinct from ID-range / clamp / expire / tick / immunity /
    // can-gain-no-existing poles.
