#include "test_petutils_pet_engage_3580.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "petutils ShouldPetEngage 3580 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AttackTarget engage formula for dual-wire cross-check (dedicated 3580
// expand residual 3051; prior dedicated 3536 / 3484 / 3429 / 3375):
...
// Compact dual-wire pin matching Go pinShouldPetEngage3580 / C++ capacity:
//   !hasPreventActionEffect
...
// Prior dedicated 3536 pin for free == inline == pin == pin3536 cross-check.
...
// dedicated slice 3580 expand residual 3051; prior dedicated 3536 / 3484 / 3429 / 3375; pure 1627).
//
// Coverage:
//   - free == inline == pin == pin3536 == !hasPreventActionEffect
...
auto runPetutilsPetEngage3580SelfTests() -> bool
{
    ...
    // --- Composition table: free == inline == pin == pin3536 ---
    const struct
    {
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3536 = pinShouldPetEngage3536(c.hasPreventActionEffect);
        const bool wantPin = !c.hasPreventActionEffect;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3536.
        ok = expect(got == inlineF && got == pin && got == pin3536,
                    "dual-wire free == inline == pin == pin3536") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula !hasPreventActionEffect") && ok;
    }

    // Free == pin == pin3536 across residual poles.
    ...
    ok = expect(ShouldPetEngage(false) == pinShouldPetEngage3536(false), "free == pin3536 no prevent") && ok;
    ok = expect(ShouldPetEngage(true) == pinShouldPetEngage3536(true), "free == pin3536 prevent") && ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3536.
    for (const bool hasPrevent : { false, true })
    {
        ...
        const bool pin3536 = pinShouldPetEngage3536(hasPrevent);
        const bool want    = !hasPrevent;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3536,
                    "compose free == inline == pin == pin3536") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3536 for AttackTarget.
    for (const bool hasPrevent : { false, true })
    {
        const bool got     = ShouldPetEngage(hasPrevent);
        const bool inlineF = inlineShouldPetEngage(hasPrevent);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3536,
                    "host inject free == inline == pin == pin3536") &&
             ok;
