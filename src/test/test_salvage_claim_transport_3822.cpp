#include "test_salvage_claim_transport_3822.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage claim transport 3822 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanClaimTransport3822:
...
// Prior dedicated 3777 pin (same formula; residual suite identity).
auto pinCanClaimTransport3777(const uint32 transportUserID) -> bool
{
    return !salvagehelpers::TransportUserBusy(transportUserID);
}
...
// slice 3822 dedicated dual-wire expand residual 2871).
//
// Coverage:
//   - id == 0 → claim true (positive pin form)
//   - non-zero → claim false / busy true
//   - max uint32 busy
//   - free == inline == pin == pin3777 == !TransportUserBusy
//   - dense poles: free / busy residual + large / max
//   - residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 / 3552 / 3597 / 3642 / 3687 / 3732 / 3777 pins still hold
//   - prior 3777 independence: free == pin3777 across poles
auto runSalvageClaimTransport3822SelfTests() -> bool
{
    ...
    // Residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 / 3495 / 3552 / 3597 / 3642 / 3687 / 3732 / 3777 pins still hold under dual-wire.
    ...
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport(0xFFFFFFFF),
         ...
         ok;
    ok = expect(CanClaimTransport(0) == pinCanClaimTransport3777(0),
                "residual 3777: free == pin for id 0") &&
         ok;
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport3777(0xFFFFFFFF),
                "residual 3777: free == pin for max busy") &&
         ok;
    ...
    ok = expect(pinCanClaimTransport3777(0), "eligible prior pin3777 id 0 free should claim") && ok;
    ...
    ok = expect(!pinCanClaimTransport3777(0xFFFFFFFF), "prior pin3777 max busy should not claim") && ok;
    ...
    // --- Composition table: free == inline == pin == pin3777 == !TransportUserBusy ---
    // Dense poles: free / busy residual + large / max.
    const struct
    {
        uint32      id;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior   = pinCanClaimTransport3777(c.id);
        ...
        ok = expect(gotClaim == pinPrior, "dual-wire free==pin3777") && ok;
    ...
    for (const uint32 id : composeIds)
    {
        ...
        ok             = expect(got == pinCanClaimTransport3777(id), "compose pin3777") && ok;
    }
    ...
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport(99),
         ...
         ok;
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport3777(99),
                "formula busy free == pin3777") &&
         ok;
