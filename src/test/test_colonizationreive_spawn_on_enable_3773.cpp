#include "test_colonizationreive_spawn_on_enable_3773.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "colonizationreive ShouldSpawnOnEnable 3773 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua enableReive spawn gate for dual-wire cross-check
// (dedicated 3773 expand residual 2882 / prior 3728 / prior 3683 / prior 3638 /
...
// Compact dual-wire pin matching Go pinShouldSpawnOnEnable3773 / C++ capacity
// (return !isAlive):
//   !isAlive
...
// Prior dedicated 3728 pin (independence / free==inline==pin==pin3728):
//   !isAlive
...
// OmegaXI internal/colonizationreive; dedicated slice 3773 expand residual
// 2882 / prior dedicated 3728 / prior dedicated 3683 / prior dedicated 3638 /
...
//
// Coverage:
//   - free == inline == pin == pin3728 == !isAlive
//   - residual poles: isAlive true / false
//   - dense 2^1 boolean space
//   - residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 / prior 3638 / prior 3683 / prior 3728 pins still hold
...
auto runColonizationreiveSpawnOnEnable3773SelfTests() -> bool
{
    ...
    // Residual 1038 / 2882 / prior 3163 / prior 3335 / prior 3411 / prior 3465 / prior 3499 / prior 3593 / prior 3638 / prior 3683 / prior 3728 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3728 = pinShouldSpawnOnEnable3728(c.isAlive);
        ...
        ok = expect(got == pin3728, "ShouldSpawnOnEnable dual-wire == pin3728") && ok;
    ...
    ok = expect(ShouldSpawnOnEnable(false) == pinShouldSpawnOnEnable3728(false), "free==pin3728 false") && ok;
    ok = expect(ShouldSpawnOnEnable(true) == pinShouldSpawnOnEnable3728(true), "free==pin3728 true") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3728 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3728 = pinShouldSpawnOnEnable3728(pole);
        ok                 = expect(got == !pole, "pole free == !isAlive") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3728, "pole free == pin3728") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3728.
    for (const bool isAlive : { false, true })
    {
        ...
        ok              = expect(got == pinShouldSpawnOnEnable3728(isAlive), "compose free == pin3728") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable(isAlive),
             ...
             ok;
        ok = expect(ShouldSpawnOnEnable(isAlive) == pinShouldSpawnOnEnable3728(isAlive),
                    "host inject free == pin3728") &&
             ok;
