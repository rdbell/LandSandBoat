#include "test_dynamis_can_unlock_sj_3717.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "dynamis CanUnlockSJ 3717 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua formula for dual-wire cross-check (slice 3717 expand residual 2921):
...
// Compact dual-wire pin matching Go pinCanUnlockSJ3717:
...
//   hasSJRestriction ? 1 : 0
// free==inline==pin==pin3672 required for residual expand 3717.
...
// slice 3717 dedicated expand residual 2921; prior dedicated 3672 / 3627 / 3582 / 3538 / 3476 / 3422 / 3151 retained).
//
// Coverage:
//   - has SJ_RESTRICTION → 1; no restriction → 0
//   - free == inline == pin == pin3672 == hasSJRestriction ? 1 : 0
...
auto runDynamisCanUnlockSJ3717SelfTests() -> bool
{
    ...
    // --- Core branches: free == inline == pin == pin3672 == formula ---
    ...
    // free == pin3672 (required dual-wire identity for residual expand).
    ok = expect(CanUnlockSJ(true) == pinCanUnlockSJ3672(true), "free==pin3672 hasSJ=true") && ok;
    ok = expect(CanUnlockSJ(false) == pinCanUnlockSJ3672(false), "free==pin3672 hasSJ=false") && ok;
    ...
    // Dense 2^1 compose: free == inline == pin == pin3672 == formula
    // (and prior 3672 / 3627 / 3582 / 3538 / 3476 / 3422 / 3151 pins).
