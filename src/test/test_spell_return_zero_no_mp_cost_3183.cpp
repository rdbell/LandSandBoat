#include "test_spell_return_zero_no_mp_cost_3183.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "spell ShouldReturnZeroNoMPCost 3183 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CalculateSpellCost hasMPCost gate for dual-wire cross-check
// (dedicated slice 3183):
//   !hasMPCost
...
// Compact dual-wire pin matching Go pinShouldReturnZeroNoMPCost3183 /
// C++ capacity:
//   !hasMPCost
...
// (!hasMPCost; dedicated slice 3183 expand residual 2969 / pure 2104).
//
// Coverage:
...
auto runSpellReturnZeroNoMPCost3183SelfTests() -> bool
{
    ...
    // Residual 2104 / 2969 pins still hold under dual-wire.
    ...
    // Residual independence (2104 / 2964 / 2969): null zero and no-MP-cost
    // zero both block via different pure helpers. Sibling 2964 left alone.
