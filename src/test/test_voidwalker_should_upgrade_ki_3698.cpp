#include "test_voidwalker_should_upgrade_ki_3698.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "voidwalker ShouldUpgradeKI 3698 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline voidwalker.lua checkUpgrade roll gate for dual-wire cross-check
// (dedicated expand residual 3698):
//   rand == 5  // math.random(1, 10) == 5
auto inlineShouldUpgradeKI3698(const int32 roll) -> bool
{
    return roll == 5;
}

// Compact dual-wire pin matching Go pinShouldUpgradeKI3698 / C++ capacity
// UpgradeRollSuccess form (formula unchanged from 0987 / 2884 / 3173 / 3377 /
// 3433 / 3482 / 3527 / 3563 / 3608 / 3653):
//   roll == UpgradeRollSuccess  // == 5
auto pinShouldUpgradeKI3698(const int32 roll) -> bool
{
    using voidwalkerhelpers::UpgradeRollSuccess;
    return roll == UpgradeRollSuccess;
}

// Prior dedicated 3173 pin form (residual pin under 3698).
...
// Prior dedicated 3377 pin form (residual pin under 3698).
...
// Prior dedicated 3433 pin form (residual pin under 3698).
...
// Prior dedicated 3482 pin form (residual pin under 3698).
...
// Prior dedicated 3527 pin form (residual pin under 3698).
...
// Prior dedicated 3563 pin form (residual pin under 3698).
...
// Prior dedicated 3608 pin form (residual pin under 3698).
...
// Prior dedicated 3653 pin form (residual pin under 3698).
...
// Residual 2884 inline form (residual pin under 3698).
...
// OmegaXI internal/voidwalker; dedicated expand residual 3698 of residual
// 2884; prior dedicated 3653 / 3608 / 3563 / 3527 / 3482 / 3433 / 3377 / 3173 /
// pure 0987).
//
// Coverage:
//   - free == inline == pin == pin3653
//   - positive pin: return roll == UpgradeRollSuccess
//   - UpgradeRollSuccess == 5 pin
//   - residual 2884 / 0987 / prior dedicated 3173 / 3377 / 3433 / 3482 / 3527 / 3563 / 3608 / 3653 pins still hold
...
auto runVoidwalkerShouldUpgradeKI3698SelfTests() -> bool
{
    ...
    // --- Core poles: free == inline == pin == pin3653 ---
    const struct
    {
        int32       roll;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldUpgradeKI3698(c.roll);
        const bool pin     = pinShouldUpgradeKI3698(c.roll);
        // Positive form pin composition (explicit roll == UpgradeRollSuccess).
        const bool wantPin = c.roll == UpgradeRollSuccess;
        // Residual free == prior dedicated 3173 / 3377 / 3433 / 3482 / 3527 / 3563 / 3608 / 3653 pin forms.
        ...
        ok = expect(got == inlineF && got == pin && got == wantPin && got == pin3653,
                    "ShouldUpgradeKI free == inline == pin == pin3653") &&
             ok;
    ...
    ok = expect(ShouldUpgradeKI(UpgradeRollSuccess) == pinShouldUpgradeKI3698(UpgradeRollSuccess),
                "free == pin for UpgradeRollSuccess") &&
         ok;
    ok = expect(!ShouldUpgradeKI(UpgradeRollSuccess - 1), "UpgradeRollSuccess-1 should not upgrade") && ok;
    ok = expect(!ShouldUpgradeKI(UpgradeRollSuccess + 1), "UpgradeRollSuccess+1 should not upgrade") && ok;
    ok = expect(ShouldUpgradeKI(UpgradeRollMin) == pinShouldUpgradeKI3698(UpgradeRollMin),
                "free == pin for UpgradeRollMin") &&
         ok;
    ok = expect(ShouldUpgradeKI(UpgradeRollMax) == pinShouldUpgradeKI3698(UpgradeRollMax),
                "free == pin for UpgradeRollMax") &&
         ok;

    // Dense compose across inject range poles 1..10 and neighbors:
    // free == inline == pin == pin3653 (+ residual 2884 inline + prior pin forms).
    for (int32 roll = UpgradeRollMin - 2; roll <= UpgradeRollMax + 2; ++roll)
    {
        const bool got     = ShouldUpgradeKI(roll);
        const bool inlineF = inlineShouldUpgradeKI3698(roll);
        const bool pin     = pinShouldUpgradeKI3698(roll);
        const bool pin3653 = pinShouldUpgradeKI3653Residual(roll);
        const bool want    = roll == UpgradeRollSuccess;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3653,
                                    "compose free == inline == pin == pin3653") &&
             ok;
    ...
    for (int32 roll = UpgradeRollMin; roll <= UpgradeRollMax; ++roll)
    {
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3653,
                                    "host inject free == inline == pin == pin3653") &&
             ok;
    }
