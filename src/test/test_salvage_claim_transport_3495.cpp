#include "test_salvage_claim_transport_3495.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "salvage claim transport 3495 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanClaimTransport3495:
...
// Prior dedicated 3318 pin (same formula; residual suite identity).
auto pinCanClaimTransport3318(const uint32 transportUserID) -> bool
{
    return !salvagehelpers::TransportUserBusy(transportUserID);
}
...
// slice 3495 dedicated dual-wire expand residual 2871).
//
// Coverage:
...
//   - residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 pins still hold
//   - prior 3318 independence: free == pin3318 across poles
auto runSalvageClaimTransport3495SelfTests() -> bool
{
    ...
    // Residual 0977 / 1083 / 2871 / 3085 / 3259 / 3290 / 3318 pins still hold under dual-wire.
    ...
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport(0xFFFFFFFF),
         ...
         ok;
    ok = expect(CanClaimTransport(0) == pinCanClaimTransport3318(0),
                "residual 3318: free == pin for id 0") &&
         ok;
    ok = expect(CanClaimTransport(0xFFFFFFFF) == pinCanClaimTransport3318(0xFFFFFFFF),
                "residual 3318: free == pin for max busy") &&
         ok;
    ...
    ok = expect(pinCanClaimTransport3318(0), "eligible prior pin3318 id 0 free should claim") && ok;
    ...
    ok = expect(!pinCanClaimTransport3318(0xFFFFFFFF), "prior pin3318 max busy should not claim") && ok;
    ...
    // --- Composition table: free == inline == pin == pin3318 == !TransportUserBusy ---
    // Dense poles: free / busy residual + large / max.
    const struct
    {
        uint32      id;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pinPrior   = pinCanClaimTransport3318(c.id);
        ...
        ok = expect(gotClaim == pinPrior, "dual-wire free==pin3318") && ok;
    ...
    for (const uint32 id : composeIds)
    {
        ...
        ok             = expect(got == pinCanClaimTransport3318(id), "compose pin3318") && ok;
    }
    ...
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport(99),
         ...
         ok;
    ok = expect(CanClaimTransport(99) == pinCanClaimTransport3318(99),
                "formula busy free == pin3318") &&
         ok;
