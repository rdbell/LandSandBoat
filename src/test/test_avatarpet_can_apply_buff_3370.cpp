#include "test_avatarpet_can_apply_buff_3370.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "avatarpet CanApplyBuff 3370 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline tryBuffSpell canApplyBuff formula for dual-wire cross-check
// (slice 3370; residual 2968 / prior dedicated 3152 / 3230):
...
// Positive if/else pin matching free function / capacity body (slice 3370).
// Multi-branch: positive if/else only (avoid De Morgan rewrites).
...
// dedicated slice 3370 expand residual 2968).
//
// Coverage:
//   - free == inline == pin (positive if/else multi-branch)
//   - residual 1043 / 2968 poles still hold
//   - residual pins (one-liner + prior dedicated forms) still agree
...
auto runAvatarpetCanApplyBuff3370SelfTests() -> bool
{
    ...
    // free == inline == pin == formula (+ residual pins).
