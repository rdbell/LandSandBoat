#include "test_extravaganza_can_afford_cipher_3694.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "extravaganza CanAffordCipher 3694 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua shadowEraFinish notes gate for dual-wire cross-check
// (dedicated 3694 expand residual 2881; prior 3649):
//   notes >= 1000
auto inlineCanAffordCipher3694(const int32 notes) -> bool
{
    return notes >= 1000;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3694:
...
// Compact dual-wire pin matching Go pinCanAffordCipher3649 (prior dedicated):
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3649(const int32 notes) -> bool
{
    return notes >= extravaganzahelpers::CipherPurchaseNotes;
}
...
// internal/extravaganza; dedicated slice 3694 expand residual 2881;
// prior dedicated 3649 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 retained).
//
// Coverage:
//   - free == inline == pin == pin3649 == notes >= CipherPurchaseNotes
//   - CipherPurchaseNotes == 1000 pin
//   - residual 2881 / 3649 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 pins still hold
...
auto runExtravaganzaCanAffordCipher3694SelfTests() -> bool
{
    ...
    // Residual 2881 / 3649 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 CanAffordCipher pins still hold under dual-wire.
    ...
    // Residual poles (short / exact / surplus) + free==inline==pin==pin3649 table.
    const struct
    {
        int32       notes;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF   = inlineCanAffordCipher3694(c.notes);
        const bool pinGot    = pinCanAffordCipher3694(c.notes);
        const bool pin3649   = pinCanAffordCipher3649(c.notes);
        const bool compose   = c.notes >= CipherPurchaseNotes;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3649.
        ok = expect(got == inlineF && got == pinGot && got == pin3649, "dual-wire free==inline==pin==pin3649") && ok;
        ok = expect(got == compose, "formula free==notes>=CipherPurchaseNotes") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3649 (short / exact / surplus).
    const struct
    {
        int32       notes;
        const char* label;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3694(p.notes);
        const bool pinGot  = pinCanAffordCipher3694(p.notes);
        const bool pin3649 = pinCanAffordCipher3649(p.notes);
        const bool want    = p.notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3649, "pole free==inline==pin==pin3649") && ok;
    }

    // Dense compose free == inline == pin == pin3649 over representative note values.
    const int32 dense[] = {
        -1000,
        -1,
        0,
    ...
    for (const int32 notes : dense)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3694(notes);
        const bool pinGot  = pinCanAffordCipher3694(notes);
        const bool pin3649 = pinCanAffordCipher3649(notes);
        const bool want    = notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, "dense free==pin formula") && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3649, "dense free==inline==pin==pin3649") && ok;
    }
    ...
    for (int32 notes = CipherPurchaseNotes - 5; notes <= CipherPurchaseNotes + 5; ++notes)
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3649, "neighborhood free==inline==pin==pin3649") && ok;
    }
