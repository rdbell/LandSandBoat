#include "test_battlefield_enter_pc_3644.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "battlefield ShouldEnterPC 3644 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertEntity TYPE_PC enter-branch formula for dual-wire cross-check
// (dedicated 3644 expand residual 3024; prior dedicated 3599 / 3554 / 3497 / 3431 / 3381):
//   enter
...
// Compact dual-wire pin matching Go pinShouldEnterPC3644 /
...
// Prior dedicated 3599 inline/pin for independence cross-check (pin3599 chain).
auto inlineShouldEnterPC3599(const bool enter) -> bool
{
    return enter;
}

auto pinShouldEnterPC3599(const bool enter) -> bool
{
    return enter;
}
...
// 3644 expand residual 3024 / prior dedicated 3599 / 3554 / 3497 / 3431 / 3381 / pure 1361).
//
// Coverage:
//   - free == inline == pin residual pins
//   - free == pin3599 (prior dedicated pin chain)
//   - poles: enter-true / enter-false
//   - residual 1361 / 3024 pins still hold
//   - prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 poles still hold
...
auto runBattlefieldEnterPC3644SelfTests() -> bool
{
    ...
    // --- Core poles: free == inline == pin residual pins (== pin3599) ---
    const struct
    {
        bool        enter;
        bool        want;
        ...
        // Residual 1361 / 3024 / prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3599  = pinShouldEnterPC3599(c.enter);
        ...
        ok = expect(got == pin3599, "dual-wire free==pin3599") && ok;
    ...
    // free == pin3599 pin chain (constraint free==inline==pin==pin3599).
    ok = expect(ShouldEnterPC(true) == pinShouldEnterPC3599(true),
                "free==pin3599 enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == pinShouldEnterPC3599(false),
                "free==pin3599 enter=false") &&
         ok;
    ok = expect(pinShouldEnterPC(true) == pinShouldEnterPC3599(true),
                "pin3644==pin3599 enter=true") &&
         ok;
    ok = expect(pinShouldEnterPC(false) == pinShouldEnterPC3599(false),
                "pin3644==pin3599 enter=false") &&
         ok;

    // Prior dedicated 3599 independence: free still matches prior inline/pin.
    ok = expect(ShouldEnterPC(true) == inlineShouldEnterPC3599(true) &&
                    ShouldEnterPC(true) == pinShouldEnterPC3599(true),
                "prior 3599 independence enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == inlineShouldEnterPC3599(false) &&
                    ShouldEnterPC(false) == pinShouldEnterPC3599(false),
                "prior 3599 independence enter=false") &&
         ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3599, "host compose free==pin3599") && ok;
    }

    // Production path semantics:
    // enter=true → enter; enter=false → no enter.
    ok = expect(ShouldEnterPC(true) && pinShouldEnterPC(true) && pinShouldEnterPC3599(true),
                "enter=true → enter path") &&
         ok;
    ok = expect(!ShouldEnterPC(false) && !pinShouldEnterPC(false) && !pinShouldEnterPC3599(false),
                "enter=false → no enter path") &&
         ok;

    // Residual independence (1361 / 2994 / 3198 / 3002 / 3216 / 2930 / 3271 /
    // 3302 / 3014 / 3365 / 3024 / 3381 / 3431 / 3497 / 3554 / 3599): enter is
