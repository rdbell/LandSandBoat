#include "test_aistate_del_actor_on_attack_3576.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3576 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3576 expand residual 3044):
//   emittedHostile
auto inlineShouldDelActorOnAttack3576(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3576 / C++ capacity
// identity form (formula unchanged from 1631 / 3044 / 3178 / 3401 / 3453 / 3521):
//   emittedHostile
auto pinShouldDelActorOnAttack3576(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3521
// (prior dedicated retained under 3576):
//   emittedHostile
auto pinShouldDelActorOnAttack3521(const bool emittedHostile) -> bool
{
    return emittedHostile;
}
...
// OmegaXI internal/aistate; dedicated slice 3576 expand residual 3044;
// prior dedicated 3521 / 3453 / 3401 / 3178; pure 1631 / 1709 / 2306). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3521
//   - residual 3044 / 1631 / prior 3178 / prior 3401 / prior 3453 / prior 3521 pins still hold
...
auto runAistateDelActorOnAttack3576SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 / prior 3178 / prior 3401 / prior 3453 / prior 3521 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044/3178/3401/3453/3521: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044/3178/3401/3453/3521: emittedHostile false → no del") && ok;

    // --- Core poles: free == inline == pin == pin3521 ---
    const struct
    {
        bool        emittedHostile;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3576(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3576(c.emittedHostile);
        const bool pin3521 = pinShouldDelActorOnAttack3521(c.emittedHostile);
        // Positive form pin composition (explicit identity).
        const bool wantPin = c.emittedHostile;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3521 && got == wantPin,
                    "ShouldDelActorOnAttack free == inline == pin == pin3521") &&
             ok;
    }

    // Free == pin across residual poles + identity composition.
    ok = expect(ShouldDelActorOnAttack(true), "emittedHostile true must del") && ok;
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3576(true),
                "free == pin for emittedHostile true") &&
         ok;
    ok = expect(!ShouldDelActorOnAttack(false), "emittedHostile false must not del") && ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3576(false),
                "free == pin for emittedHostile false") &&
         ok;

    // Free == pin3521 across residual poles (prior dedicated independence).
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3521(true),
                "free == pin3521 for emittedHostile true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3521(false),
                "free == pin3521 for emittedHostile false") &&
         ok;

    // Dense compose: full 2¹ boolean space free == inline == pin == pin3521.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3576(emitted);
        const bool pin     = pinShouldDelActorOnAttack3576(emitted);
        const bool pin3521 = pinShouldDelActorOnAttack3521(emitted);
        const bool want    = emitted;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3521,
                                    "compose free == inline == pin == pin3521") &&
             ok;
    }
    ...
    // free == inline == pin == pin3521 for host inject poles.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3576(emitted);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3521,
                                    "host inject free == inline == pin == pin3521") &&
             ok;
    }

    // Prior dedicated 3521 pin independence under 3576 expand.
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3521(true),
                "prior 3521 pin holds under 3576 for true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3521(false),
                "prior 3521 pin holds under 3576 for false") &&
         ok;
