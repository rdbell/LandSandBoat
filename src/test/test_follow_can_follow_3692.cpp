#include "test_follow_can_follow_3692.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3692 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3692 dedicated expand residual 2883; prior dedicated 3647 / 3602 /
...
// Compact dual-wire pin matching free function / capacity body (slice 3692).
// Direct AND/EQ form only (no De Morgan) — same formula as production CanFollow.
auto pinCanFollow3692(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}

// Prior dedicated 3647 pin retained for free == pin3647 dual-wire agreement.
auto pinCanFollow3647(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3692 dedicated
// expand residual 2883; prior dedicated 3647 / 3602 / 3557 / 3506 / 3459 /
// 3405 / 3331 / 3237 / 3084). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3647 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 poles still hold
...
auto runFollowCanFollow3692SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 / 3602 / 3647 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3692(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3647 = pinCanFollow3647(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3692(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3647 = pinCanFollow3647(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3647, "dense free == pin3647") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3647 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3647, "host inject dual-wire free == pin3647") && ok;
    }
