#include "test_aman_thumbs_up_3810.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3810 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3810 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3810 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3765 pin for independence cross-check.
auto pinCanThumbsUp3765(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3765(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3810 expand residual 2967; prior dedicated 3765 / 3720 / 3675 / 3630 / 3585 / 3541 / 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == pin3765 == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 pins still hold
//   - prior 3765 independence (free == 3765 inline/pin)
...
auto runAmanThumbsUp3810SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3765 re-pins (independence).
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3765 = pinCanThumbsUp3765(c.cooldownExpired);
        ...
        ok = expect(got == pin3765, "CanThumbsUp dual-wire == pin3765") && ok;
    ...
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3765(true), "free==pin3765 true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3765(false), "free==pin3765 false") && ok;

    // Explicit residual poles free == inline == pin == pin3765 for true / false.
    for (const bool pole : { false, true })
    {
        ...
        const bool pin3765 = pinCanThumbsUp3765(pole);
        ok                 = expect(got == pole, "pole free == identity") && ok;
        ok                 = expect(got == inlineF, "pole free == inline") && ok;
        ok                 = expect(got == pinGot, "pole free == pin") && ok;
        ok                 = expect(got == pin3765, "pole free == pin3765") && ok;
    }

    // Dense compose: full 2^1 boolean space — free == pin == inline == pin3765.
    for (const bool cooldownExpired : { false, true })
    {
        ...
        ok              = expect(got == pinCanThumbsUp3765(cooldownExpired), "compose free == pin3765") && ok;
    }
    ...
    for (const auto& h : hostPoles)
    {
        ...
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp(cooldownExpired),
             ...
             ok;
        ok = expect(CanThumbsUp(cooldownExpired) == pinCanThumbsUp3765(cooldownExpired),
                    "host inject free == pin3765") &&
             ok;
    ...
    // Prior dedicated 3765 independence: free == prior 3765 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3765(true), "prior 3765 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3765(false), "prior 3765 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3765(true), "prior 3765 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3765(false), "prior 3765 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 3541 / 3585 / 3630 / 3675 / 3720 / 3765 / 0698 still hold under dedicated 3810.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/3541/3585/3630/3675/3720/3765/0698 under 3810") && ok;
