#include "test_aman_thumbs_up_3541.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aman CanThumbsUp 3541 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CAMANContainer::canThumbsUp pure half for dual-wire cross-check
// (dedicated 3541 expand residual 2967):
//   cooldownExpired
...
// Compact dual-wire pin matching Go pinCanThumbsUp3541 / C++ capacity
// (direct return):
//   cooldownExpired
...
// Prior dedicated 3473 pin for independence cross-check.
auto pinCanThumbsUp3473(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}

auto inlineCanThumbsUp3473(const bool cooldownExpired) -> bool
{
    return cooldownExpired;
}
...
// dedicated slice 3541 expand residual 2967; prior dedicated 3473 / 3419 / 3308).
//
// Coverage:
//   - cooldownExpired true → permit / false → reject
//   - free == inline == pin == cooldownExpired (direct return)
//   - residual 0698 / 2967 / prior dedicated 3150 / 3229 / 3278 / 3308 / 3419 / 3473 pins still hold
//   - prior 3473 independence (free == 3473 inline/pin)
...
auto runAmanThumbsUp3541SelfTests() -> bool
{
    ...
    // Residual 0698 / 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3473 re-pins (independence).
    ...
    // Prior dedicated 3473 independence: free == prior 3473 inline/pin.
    ok = expect(CanThumbsUp(true) == inlineCanThumbsUp3473(true), "prior 3473 independence free==inline true") && ok;
    ok = expect(CanThumbsUp(false) == inlineCanThumbsUp3473(false), "prior 3473 independence free==inline false") && ok;
    ok = expect(CanThumbsUp(true) == pinCanThumbsUp3473(true), "prior 3473 independence free==pin true") && ok;
    ok = expect(CanThumbsUp(false) == pinCanThumbsUp3473(false), "prior 3473 independence free==pin false") && ok;

    // Residual 2967 / 3150 / 3229 / 3278 / 3308 / 3419 / 3473 / 0698 still hold under dedicated 3541.
    ok = expect(CanThumbsUp(true) && !CanThumbsUp(false), "residual 2967/3150/3229/3278/3308/3419/3473/0698 under 3541") && ok;
