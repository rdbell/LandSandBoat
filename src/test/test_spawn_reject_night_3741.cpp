#include "test_spawn_reject_night_3741.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "spawn ShouldRejectAtNightSpawn 3741 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ATNIGHT spawn-type TOTD gate for dedicated dual-wire expand residual
// 3092 cross-check (slice 3741; prior dedicated 3696/3651/3606/3561/3528/3483/3435/3387 retained):
...
auto inlineShouldRejectAtNightSpawn3741(const uint8 spawnType, const vanadiel_time::TOTD totd) -> bool
{
...
// Compact dual-wire pin matching Go pinShouldRejectAtNightSpawn3741 / C++
// capacity positive form (formula unchanged from 1362 / 3092 / 3387 / 3435 / 3483 / 3528 / 3561 / 3606 / 3651 / 3696):
...
// Prior dedicated 3696 pin (retained) — free == pin3741 == pin3696.
auto pinShouldRejectAtNightSpawn3696(const uint8 spawnType, const vanadiel_time::TOTD totd) -> bool
{
    return spawnhelpers::HasSpawnTypeFlag(spawnType, spawnhelpers::SpawnTypeAtNight) &&
           !spawnhelpers::IsNightTotdWindow(totd);
}
...
// OmegaXI internal/spawnslot; dedicated dual-wire expand residual 3092 slice 3741;
// prior dedicated 3696/3651/3606/3561/3528/3483/3435/3387 retained; residual expand 3092 / pure 1362).
//
// Coverage:
//   - free == inline == pin == pin3696 positive form
//   - residual 3092 / 1362 and prior 3696 / 3651 / 3606 / 3561 / 3528 / 3483 / 3435 / 3387 pins still hold
...
auto runSpawnRejectNight3741SelfTests() -> bool
{
    ...
    // Residual 3092 / 1362 / prior 3696 / 3651 / 3606 / 3561 / 3528 / 3483 / 3435 / 3387 pins still hold under dedicated dual-wire.
    ...
    // --- Core poles: free == inline == pin == pin3696 positive form ---
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF  = inlineShouldRejectAtNightSpawn3741(c.spawnType, c.totd);
        const bool pin      = pinShouldRejectAtNightSpawn3741(c.spawnType, c.totd);
        const bool pin3696  = pinShouldRejectAtNightSpawn3696(c.spawnType, c.totd);
        ...
        ok = expect(got == inlineF && got == pin && got == pin3696 && got == wantPin,
                    "ShouldRejectAtNightSpawn free == inline == pin == pin3696 positive form") &&
             ok;
    }

    // Free == pin across residual ATNIGHT-required poles.
    ok = expect(ShouldRejectAtNightSpawn(SpawnTypeAtNight, vanadiel_time::TOTD::DAY) ==
                    pinShouldRejectAtNightSpawn3741(SpawnTypeAtNight, vanadiel_time::TOTD::DAY),
                "free == pin ATNIGHT-required + DAY") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(SpawnTypeAtNight, vanadiel_time::TOTD::NIGHT) ==
                    pinShouldRejectAtNightSpawn3741(SpawnTypeAtNight, vanadiel_time::TOTD::NIGHT),
                "free == pin ATNIGHT-required + NIGHT") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(0, vanadiel_time::TOTD::DAY) ==
                    pinShouldRejectAtNightSpawn3741(0, vanadiel_time::TOTD::DAY),
                "free == pin no ATNIGHT + DAY") &&
         ok;
    ok = expect(ShouldRejectAtNightSpawn(0, vanadiel_time::TOTD::NIGHT) ==
                    pinShouldRejectAtNightSpawn3741(0, vanadiel_time::TOTD::NIGHT),
                "free == pin no ATNIGHT + NIGHT") &&
         ok;

    // Dense compose: spawnType bit space × full TOTD domain free == inline == pin == pin3696.
    ...
    for (int spawnType = 0; spawnType <= 0xFF; ++spawnType)
    {
        for (const auto totd : totds)
        {
            ...
            const bool inlineF = inlineShouldRejectAtNightSpawn3741(st, totd);
            const bool pin     = pinShouldRejectAtNightSpawn3741(st, totd);
            const bool pin3696 = pinShouldRejectAtNightSpawn3696(st, totd);
            const bool want    = HasSpawnTypeFlag(st, SpawnTypeAtNight) && !IsNightTotdWindow(totd);
            ok                 = expect(got == want, "compose free == pin formula") && ok;
            ok                 = expect(got == inlineF && got == pin && got == pin3696,
                        "compose free == inline == pin == pin3696") &&
                 ok;
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3696 for host-style poles.
    const struct
    {
        uint8               spawnType;
        vanadiel_time::TOTD totd;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRejectAtNightSpawn3741(p.spawnType, p.totd);
        const bool pin     = pinShouldRejectAtNightSpawn3741(p.spawnType, p.totd);
        const bool pin3696 = pinShouldRejectAtNightSpawn3696(p.spawnType, p.totd);
        const bool want    = HasSpawnTypeFlag(p.spawnType, SpawnTypeAtNight) && !IsNightTotdWindow(p.totd);
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3696,
                    "host inject free == inline == pin == pin3696") &&
             ok;
    }
