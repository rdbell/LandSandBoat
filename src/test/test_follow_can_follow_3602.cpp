#include "test_follow_can_follow_3602.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "follow CanFollow 3602 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua xi.follow.follow spawn+zone gate for dual-wire cross-check
// (slice 3602 dedicated expand residual 2883; prior dedicated 3557 / 3506 /
...
// Compact dual-wire pin matching free function / capacity body (slice 3602).
...
// Prior dedicated 3557 pin retained for free == pin3557 dual-wire agreement.
auto pinCanFollow3557(const bool followerSpawned, const bool leaderSpawned, const uint16 followerZone, const uint16 leaderZone) -> bool
{
    return followerSpawned && leaderSpawned && followerZone == leaderZone;
}
...
// (Lua xi.follow.follow spawn+zone eligibility gate; slice 3602 dedicated
// expand residual 2883; prior dedicated 3557 / 3506 / 3459 / 3405 / 3331 / 3237 / 3084).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3557 (direct AND/EQ form, no De Morgan)
//   - residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 poles still hold
...
auto runFollowCanFollow3602SelfTests() -> bool
{
    ...
    // Residual 2883 / 0945 / prior dedicated 3084 / 3237 / 3331 / 3405 / 3459 / 3506 / 3557 pins still hold.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanFollow3602(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
        const bool pin3557 = pinCanFollow3557(p.followerSpawned, p.leaderSpawned, p.followerZone, p.leaderZone);
    ...
    for (const bool followerSpawned : { false, true })
    {
        for (const bool leaderSpawned : { false, true })
        {
            for (const auto& zp : zonePairs)
            {
                ...
                const bool pin     = pinCanFollow3602(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                const bool pin3557 = pinCanFollow3557(followerSpawned, leaderSpawned, zp.followerZone, zp.leaderZone);
                ...
                ok = expect(got == pin3557, "dense free == pin3557") && ok;
            }
        }
    }
    ...
    // Explicit dual-wire: free == inline == pin == pin3557 for host poles.
    const struct
    {
        bool        followerSpawned;
        bool        leaderSpawned;
    ...
    for (const auto& p : hostPoles)
    {
        ...
        ok                 = expect(got == pin3557, "host inject dual-wire free == pin3557") && ok;
    }
