#include "test_synergy_claim_furnace_3866.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 3866 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua AVAILABLE gate for dual-wire checks
// (slice 3866 dedicated expand residual 2877; prior dedicated expand 3821):
...
// Compact dual-wire pin matching free function / capacity body (slice 3866).
// Direct return only — same formula as production CanClaimFurnace.
auto pinCanClaimFurnace3866(const uint8 state) -> bool
{
    return state == synergyhelpers::FurnaceAvailable;
}

// Prior dedicated expand 3821 pin (retained for independence check).
auto pinCanClaimFurnace3821(const uint8 state) -> bool
{
    return state == synergyhelpers::FurnaceAvailable;
}

// Prior dedicated expand 3776 pin (retained for independence check).
auto pinCanClaimFurnace3776(const uint8 state) -> bool
{
    return state == synergyhelpers::FurnaceAvailable;
}

// Prior dedicated expand 3731 pin (retained for independence check).
auto pinCanClaimFurnace3731(const uint8 state) -> bool
{
    return state == synergyhelpers::FurnaceAvailable;
}

// Prior dedicated expand 3686 pin (retained for independence check).
auto pinCanClaimFurnace3686(const uint8 state) -> bool
{
    return state == synergyhelpers::FurnaceAvailable;
}

// Prior dedicated expand 3641 pin (retained for independence check).
auto pinCanClaimFurnace3641(const uint8 state) -> bool
{
    return state == synergyhelpers::FurnaceAvailable;
}

// Prior dedicated expand 3596 pin (retained for independence check).
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate; slice 3866 dedicated
// expand residual 2877; prior dedicated expand 3821). Formula unchanged:
//
//   CanClaimFurnace(state) = state == FurnaceAvailable
//
// Coverage:
//   - free == inline == pin == pin3821 (direct return formula)
//   - residual 2877 / 1149 / prior dedicated 3065 / prior expand 3239 /
//     prior expand 3285 / prior expand 3315 / prior expand 3494 /
//     prior expand 3551 / prior expand 3596 / prior expand 3641 /
//     prior expand 3686 / prior expand 3731 / prior expand 3776 /
//     prior expand 3821 poles
//   - residual poles for all furnaceStates + dense 0..8 out-of-range
//   - prior 3821 pin independence
auto runSynergyClaimFurnace3866SelfTests() -> bool
{
    ...
    // expand 3551 / prior expand 3596 / prior expand 3641 / prior expand
    // 3686 / prior expand 3731 / prior expand 3776 / prior expand 3821 pins.
    ...
    const struct
    {
        ...
        // All named furnaceStates (residual 2877 / prior 3065 / 3239 / 3285 / 3315 / 3494 / 3551 / 3596 / 3641 / 3686 / 3731 / 3776 / 3821).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanClaimFurnace3866(p.state);
        ...
        ok = expect(got == pinCanClaimFurnace3596(p.state), "free == prior pinCanClaimFurnace3596") && ok;
        ok = expect(got == pinCanClaimFurnace3641(p.state), "free == prior pinCanClaimFurnace3641") && ok;
        ok = expect(got == pinCanClaimFurnace3686(p.state), "free == prior pinCanClaimFurnace3686") && ok;
        ok = expect(got == pinCanClaimFurnace3731(p.state), "free == prior pinCanClaimFurnace3731") && ok;
        ok = expect(got == pinCanClaimFurnace3776(p.state), "free == prior pinCanClaimFurnace3776") && ok;
        ok = expect(got == pinCanClaimFurnace3821(p.state), "free == prior pinCanClaimFurnace3821") && ok;
    }
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        const bool  pin     = pinCanClaimFurnace3866(s);
        ...
        ok = expect(got == pinCanClaimFurnace3596(s), "dense free == prior pin3596") && ok;
        ok = expect(got == pinCanClaimFurnace3641(s), "dense free == prior pin3641") && ok;
        ok = expect(got == pinCanClaimFurnace3686(s), "dense free == prior pin3686") && ok;
        ok = expect(got == pinCanClaimFurnace3731(s), "dense free == prior pin3731") && ok;
        ok = expect(got == pinCanClaimFurnace3776(s), "dense free == prior pin3776") && ok;
        ok = expect(got == pinCanClaimFurnace3821(s), "dense free == prior pin3821") && ok;
    }
