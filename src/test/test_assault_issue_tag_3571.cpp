#include "test_assault_issue_tag_3571.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3571 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3571 expand residual 2867 / prior 3524 stock / 3388 / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3571:
//   tagStock > 0
...
// internal/assault; dedicated slice 3571 expand residual 2867 / prior 3524
// stock / 3388 / 3440 ShouldIssueNewTag sibling / pure 1100 — formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3524 == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 stock pins still hold
...
// (ShouldIssueNewTag), test_assault_issue_tag_3524 (prior stock). Mirrors 3524
// expand-residual suite layout (header + self-test runner; 3524
...
auto runAssaultIssueTag3571SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 stock poles still hold.
    ...
    // --- Composition table: free == inline == pin == pin3524 == compose ---
    const struct
    {
        int32       tagStock;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3524 = pinCanIssueTagFromStock3524(c.tagStock);
        ...
        ok = expect(got == pin3524, "dual-wire free==pin3524") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3524 for stock 0 / 1.
    for (const int32 stock : { 0, 1 })
    {
        ...
        const bool pin3524 = pinCanIssueTagFromStock3524(stock);
        ...
        ok                 = expect(got == pin3524, "pole free == pin3524") && ok;
    }

    // Dense tagStock: free == pin == inline == pin3524 == compose.
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3524 = pinCanIssueTagFromStock3524(h.tagStock);
        ...
        ok                 = expect(got == pin3524, "host free==pin3524") && ok;
