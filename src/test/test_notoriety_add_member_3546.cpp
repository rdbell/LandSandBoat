#include "test_notoriety_add_member_3546.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "notoriety ShouldAddNotorietyMember 3546 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline CNotorietyContainer::add admission for dual-wire cross-check
// (dedicated 3546 expand residual 2959; prior dedicated 3492):
...
// Compact dual-wire pin matching Go pinShouldAddNotorietyMember3546 / C++ capacity
// direct AND-chain (formula unchanged from 2818 / 2959 / 3165 / 3267 / 3297 / 3327 / 3492):
//   ownerPresent && entityPresent && differentAllegiance
...
// Prior dedicated 3492 pin/inline for independence (formula identical).
...
// OmegaXI internal/notoriety; dedicated slice 3546 expand residual 2959 /
// pure 2818; prior dedicated expand 3492 / 3327 / 3297 / 3267 / 3165 retained).
//
// Coverage:
//   - free == inline == pin direct AND-chain
//   - residual 2959 / 2818 pins still hold
//   - prior dedicated 3492 independence (free == prior inline == prior pin)
...
auto runNotorietyAddMember3546SelfTests() -> bool
{
    ...
    // Residual 2818 / 2959 / prior dedicated 3492 pins still hold under dedicated dual-wire.
    ...
    const struct
    {
        ...
        // Prior dedicated 3492 re-pins.
        { true, true, true, true, "residual 3492 admit owner+entity+diff" },
        { false, true, true, false, "residual 3492 reject nil owner" },
        { true, false, true, false, "residual 3492 reject nil entity" },
        { true, true, false, false, "residual 3492 reject same allegiance" },
        { false, false, false, false, "residual 3492 reject all false" },
    };

    for (const auto& c : cases)
    {
        const bool got     = ShouldAddNotorietyMember(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool inlineF = inlineShouldAddNotorietyMember3546(c.ownerPresent, c.entityPresent, c.differentAllegiance);
        const bool pin     = pinShouldAddNotorietyMember3546(c.ownerPresent, c.entityPresent, c.differentAllegiance);
    ...
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            for (const bool differentAllegiance : { false, true })
            {
                ...
                const bool inlineF = inlineShouldAddNotorietyMember3546(ownerPresent, entityPresent, differentAllegiance);
                const bool pin     = pinShouldAddNotorietyMember3546(ownerPresent, entityPresent, differentAllegiance);
    ...
    // Prior dedicated 3492 independence: free still matches prior pin/inline.
    for (const bool ownerPresent : { false, true })
    {
        for (const bool entityPresent : { false, true })
        {
            ...
            {
                const bool got         = ShouldAddNotorietyMember(ownerPresent, entityPresent, differentAllegiance);
                const bool priorInline = inlineShouldAddNotorietyMember3492(ownerPresent, entityPresent, differentAllegiance);
                const bool priorPin    = pinShouldAddNotorietyMember3492(ownerPresent, entityPresent, differentAllegiance);
                ok                     = expect(got == priorPin, "prior 3492 independence free == prior pin") && ok;
                ok                     = expect(got == priorInline, "prior 3492 independence free == prior inline") && ok;
            }
        }
    }
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldAddNotorietyMember3546(p.owner, p.entity, p.diff);
        const bool pin     = pinShouldAddNotorietyMember3546(p.owner, p.entity, p.diff);
