#include "test_artisan_can_buy_sack_3282.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "artisan CanBuySack 3282 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline artisan.lua option 1 pure gate for dual-wire cross-check
// (slice 3282 dedicated expand residual 2879; prior dedicated 3242 / 3090):
...
// Compact dual-wire pin matching free function / capacity body (slice 3282).
// Direct return only — same formula as production CanBuySack.
auto pinCanBuySack3282(const int32 gil, const int32 sackSize) -> bool
{
...
//   - residual 0948 / 2879 / prior dedicated 3090 / prior expand 3242 pins still hold
auto runArtisanCanBuySack3282SelfTests() -> bool
{
    ...
    // Residual constant / 0948 / 2879 / prior dedicated 3090 / 3242 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 2879 / 3090 / 3242 classic dual poles.
        ...
        // Prior dedicated 3090 / expand 3242 dense poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanBuySack3282(p.gil, p.sackSize);
    ...
    for (int32 g = BuySackGilCost - 50; g <= BuySackGilCost + 50; ++g)
    {
        ...
        const bool pin     = pinCanBuySack3282(g, 0);
    ...
    for (const int32 size : { 0, 1, 2, 15, 30, 80, 100, -1 })
    {
        for (const int32 g : { BuySackGilCost - 1, BuySackGilCost, BuySackGilCost + 1, 9979, 9980, 10000, 100000, 0, 1 })
        {
            ...
            const bool pin     = pinCanBuySack3282(g, size);
