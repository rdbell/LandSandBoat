#include "test_unity_reject_null_online_3544.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "unitychat ShouldRejectNullOnlineMember 3544 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember / DelOnlineMember null-PChar gate for dual-wire
// cross-check (slice 3544 dedicated expand residual 3075; prior dedicated
// 3487 / 3436 / 3386):
//   charNull
...
// Compact dual-wire pin matching free function / capacity body (slice 3544).
...
// slice 3544 dedicated expand residual 3075; prior dedicated 3487 / 3436 / 3386
// retained). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 1356 / 3075 pins still hold
//   - prior 3386 / 3436 / 3487 dedicated poles still hold
//   - dense 2¹
...
auto runUnityRejectNullOnline3544SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3544(c.charNull);
    ...
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3544(charNull);

        ok = expect(got == want, "compose free == pin formula") && ok;
        ok = expect(got == inlineF, "compose free == inline") && ok;
        ok = expect(got == pin, "compose free == pin 3544") && ok;
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool pin     = pinShouldRejectNullOnlineMember3544(h.charNull);
