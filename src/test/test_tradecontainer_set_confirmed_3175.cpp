#include "test_tradecontainer_set_confirmed_3175.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "tradecontainer ShouldAllowSetConfirmedStatus 3175 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline setConfirmedStatus outer gate for dual-wire cross-check (dedicated 3175):
...
// Compact dual-wire pin matching Go pinShouldAllowSetConfirmedStatus3175 / C++ capacity:
//   slotInRange && itemNonNull && quantityGteAmount
...
// dedicated slice 3175 expand residual 2962).
//
// Coverage:
//   - free == inline == pin == (slotInRange && itemNonNull && quantityGteAmount)
//   - residual 2806 / 2962 pins still hold
...
auto runTradecontainerSetConfirmed3175SelfTests() -> bool
{
    ...
    // Residual 2806 / 2962 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 2962 / 2806 poles (full 2^3 surface).
