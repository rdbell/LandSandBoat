#include "test_aistate_del_actor_on_attack_3178.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3178 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3178):
//   emittedHostile
auto inlineShouldDelActorOnAttack3178(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3178 / C++ capacity
// identity form (formula unchanged from 1631 / 3044):
//   emittedHostile
auto pinShouldDelActorOnAttack3178(const bool emittedHostile) -> bool
{
    return emittedHostile;
}
...
// OmegaXI internal/aistate; dedicated slice 3178; residual expand 3044 /
// pure 1631 / 1709 / 2306).
//
// Coverage:
//   - free == inline == pin
//   - residual 3044 / 1631 pins still hold
...
auto runAistateDelActorOnAttack3178SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044: emittedHostile false → no del") && ok;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3178(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3178(c.emittedHostile);
    ...
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3178(true),
                "free == pin for emittedHostile true") &&
         ok;
    ok = expect(!ShouldDelActorOnAttack(false), "emittedHostile false must not del") && ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3178(false),
    ...
    for (const bool emitted : { false, true })
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3178(emitted);
        const bool pin     = pinShouldDelActorOnAttack3178(emitted);
