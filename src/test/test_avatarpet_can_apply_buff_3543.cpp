#include "test_avatarpet_can_apply_buff_3543.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "avatarpet CanApplyBuff 3543 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline tryBuffSpell canApplyBuff formula for dual-wire cross-check
// (slice 3543; residual 2968 / prior dedicated 3152 / 3230 / 3370 / 3425 / 3479):
...
// Positive if/else pin matching free function / capacity body (slice 3543).
...
// Prior dedicated 3479 independence pin (positive if/else; formula unchanged).
auto pinCanApplyBuff3479(
    const bool hasStatusEffect,
    const uint8 statusTier,
    const bool spellHasTier,
    const uint8 spellTier) -> bool
...
// dedicated slice 3543 expand residual 2968).
//
// Coverage:
//   - free == inline == pin (positive if/else multi-branch / early-return)
//   - residual 1043 / 2968 poles still hold
//   - residual pins (one-liner + prior dedicated forms incl. 3479) still agree
...
auto runAvatarpetCanApplyBuff3543SelfTests() -> bool
{
    ...
    // free == inline == pin == formula (+ residual pins incl. prior 3479).
            ...
            for (const bool spellHasTier : bools)
            {
                for (const uint8 spellTier : tiers)
                {
                    ...
                    ok = expect(got == pinCanApplyBuffOneLiner(hasStatus, statusTier, spellHasTier, spellTier),
                         ...
                         ok;
                    ok = expect(got == pinCanApplyBuff3479(hasStatus, statusTier, spellHasTier, spellTier),
                                "dense free == residual 3479 pin") &&
                         ok;
                }
            }
        }
    ...
    // Host-style inject poles: free == pin == inline.
    const struct
    {
        bool  hasStatusEffect;
        uint8 statusTier;
