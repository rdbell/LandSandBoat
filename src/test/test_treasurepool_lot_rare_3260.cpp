#include "test_treasurepool_lot_rare_3260.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "treasurepool CanLotRareItem 3260 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline lotItem rare-owned formula for dual-wire cross-check (dedicated 3260):
//   !itemIsRare || !alreadyHasItem  (De Morgan of !(rare && has))
auto inlineCanLotRareItem(const bool itemIsRare, const bool alreadyHasItem) -> bool
{
    return !itemIsRare || !alreadyHasItem;
}

// Compact dual-wire pin matching Go pinCanLotRareItem3260 / C++ capacity:
//   !(itemIsRare && alreadyHasItem)
auto pinCanLotRareItem(const bool itemIsRare, const bool alreadyHasItem) -> bool
{
    if (itemIsRare && alreadyHasItem)
    {
        return false;
    }
    return true;
...
// (!(rare && alreadyHas) rare-owned lot gate; OmegaXI internal/treasurepool;
// dedicated slice 3260 expand residual 2998).
//
// Coverage:
//   - free == inline == pin == (!itemIsRare || !alreadyHasItem)
//   - residual 1367 / 2998 pins still hold
...
auto runTreasurepoolLotRare3260SelfTests() -> bool
{
    ...
    // Residual 1367 / 2998 pins still hold under dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool wantAnd = !(c.itemIsRare && c.alreadyHasItem);
        ...
        ok = expect(got == wantAnd, "free == pin form !(rare && has)") && ok;
        ok = expect(wantPin == wantAnd, "De Morgan pin forms agree") && ok;
    }
    ...
    for (const bool rare : { false, true })
    {
        for (const bool has : { false, true })
        {
            ...
            ok                 = expect(got == !(rare && has), "compose free == !(rare && has)") && ok;
        }
    }
    ...
    // free == inline == pin across dense 2².
    for (const bool rare : { false, true })
    {
        for (const bool has : { false, true })
        {
