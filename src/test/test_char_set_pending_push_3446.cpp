#include "test_char_set_pending_push_3446.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "char ShouldSetPendingPositionOnPush 3446 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnPush pending-position formula for dual-wire cross-check (dedicated slice 3446):
...
// Compact dual-wire pin matching Go pinShouldSetPendingPositionOnPush3446 /
// C++ capacity:
//   packetType == 0x5B && packetEntityID == ownerID
...
// Prior dedicated 3395 pin (retained under expand 3446).
...
// (packetType == 0x5B && packetEntityID == ownerID; dedicated slice 3446
// expand residual 3125; prior dedicated 3395 / pure 2842).
//
// Coverage:
//   - free == inline == pin == (packetType == 0x5B && packetEntityID == ownerID)
//   - residual 2842 / 3125 + prior dedicated 3395 pins still hold
...
auto runCharSetPendingPush3446SelfTests() -> bool
{
    ...
    // Residual 2842 / 3125 / prior dedicated 3395 pins still hold under dual-wire.
