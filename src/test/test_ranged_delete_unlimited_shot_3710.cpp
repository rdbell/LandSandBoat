#include "test_ranged_delete_unlimited_shot_3710.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranged ShouldDeleteUnlimitedShot 3710 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack UnlimitedShot delete gate for dual-wire cross-check
// (slice 3710 dedicated expand residual 3000; prior 3665 retained):
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto inlineShouldDeleteUnlimitedShot3710(
...
// (dedicated slice 3710). Early-return form only — same formula.
//   if !hasUnlimitedShot → false
//   else → hitOccured || retainUnlimitedShotMod <= 0
auto pinShouldDeleteUnlimitedShot3710(
    const bool hasUnlimitedShot,
    const bool hitOccured,
    const int16 retainUnlimitedShotMod) -> bool
{
...
// Prior 3665 pin retained for independence cross-check (same formula).
// free == inline == pin == pin3665.
auto pinShouldDeleteUnlimitedShot3665(
    const bool hasUnlimitedShot,
    const bool hitOccured,
    const int16 retainUnlimitedShotMod) -> bool
{
...
// slice 3710 dedicated expand residual 3000; prior dedicated 3665 retained).
// Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3665 (early-return formula)
//   - residual poles: no US; hit always delete; miss retain <=0 / >0; residual 1390/3000/3399/3451/3500/3575/3620/3665
//   - dense hasUS × hit × retain compose
//   - prior 3665 independence (suite retained)
...
auto runRangedDeleteUnlimitedShot3710SelfTests() -> bool
{
    ...
    // Residual 1390 / residual dual-wire 3000 / dedicated 3399 / 3451 / 3500 / 3575 / 3620 / 3665 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Residual 1390 / 3000 / 3399 / 3451 / 3500 / 3575 / 3620 / 3665 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDeleteUnlimitedShot3710(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pinF    = pinShouldDeleteUnlimitedShot3710(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
        const bool pin3665 = pinShouldDeleteUnlimitedShot3665(c.hasUnlimitedShot, c.hitOccured, c.retainUnlimitedShotMod);
    ...
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3710(false, true, 0),
                "free == pin residual no US") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3710(true, true, 5),
                "free == pin residual hit delete") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3710(true, false, 1),
                "free == pin residual retain keep") &&
         ok;

    // Prior 3665 independence: free == prior 3665 pin poles (suite retained).
    // free == inline == pin == pin3665.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3665(true, true, 5),
                "prior 3665 independence: free == 3665 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3665(false, true, 0),
                "prior 3665 independence: free == 3665 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3665(true, false, 1),
                "prior 3665 independence: free == 3665 pin retain keep") &&
         ok;
    ok = expect(pinShouldDeleteUnlimitedShot3710(true, false, 0) == pinShouldDeleteUnlimitedShot3665(true, false, 0),
                "prior 3665 independence: 3710 pin == 3665 pin miss retain 0") &&
         ok;
    ok = expect(inlineShouldDeleteUnlimitedShot3710(true, true, 5) == pinShouldDeleteUnlimitedShot3665(true, true, 5),
                "prior 3665 independence: 3710 inline == 3665 pin del hit") &&
         ok;

    // Dense compose: hasUnlimitedShot × hitOccured × retain poles — free == inline == pin == pin3665.
    for (const bool hasUS : { false, true })
    {
        for (const bool hit : { false, true })
        {
                                        ...
                                        static_cast<int16>(1), static_cast<int16>(5), static_cast<int16>(100) })
            {
                ...
                ok              = expect(got == inlineShouldDeleteUnlimitedShot3710(hasUS, hit, retain),
                            "compose free == inline") &&
                     ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3710(hasUS, hit, retain), "compose free == pin") && ok;
                ok = expect(got == pinShouldDeleteUnlimitedShot3665(hasUS, hit, retain), "compose free == pin3665") && ok;
            }
        }
    }
    ...
    for (const auto& c : composeCases)
    {
        ...
        ok = expect(got == pin3665, "compose free == pin3665") && ok;
    }
    ...
    // Prior 3665 independence under compose: free == 3665 pin residual poles.
    ok = expect(ShouldDeleteUnlimitedShot(true, true, 5) == pinShouldDeleteUnlimitedShot3665(true, true, 5),
                "prior 3665 independence compose: free == 3665 pin del hit") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(false, true, 0) == pinShouldDeleteUnlimitedShot3665(false, true, 0),
                "prior 3665 independence compose: free == 3665 pin no us") &&
         ok;
    ok = expect(ShouldDeleteUnlimitedShot(true, false, 1) == pinShouldDeleteUnlimitedShot3665(true, false, 1),
                "prior 3665 independence compose: free == 3665 pin retain keep") &&
         ok;
