#include "test_trade_bump_items_count_3764.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "trade ShouldBumpItemsCountOnSetEntry 3764 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldBumpItemsCountOnSetEntry3764 / C++ capacity:
//   slotInRange
...
// Prior dedicated 3719 inline pin for independence cross-check.
auto inlineShouldBumpItemsCountOnSetEntry3719(const bool slotInRange) -> bool
{
    return slotInRange;
}

// Prior dedicated 3719 compact pin for free == inline == pin == pin3719 cross-check.
auto pinShouldBumpItemsCountOnSetEntry3719(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3764 expand residual 2997; prior dedicated 3719 / 3674 / 3629 / 3584 / 3540 / 3474 / 3420 / 3372).
//
// Coverage:
//   - free == inline == pin == pin3719 == slotInRange (identity)
//   - residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 / 3719 pins still hold
//   - prior dedicated 3719 independence (free == prior inline == prior pin)
...
auto runTradeBumpItemsCount3764SelfTests() -> bool
{
    ...
    // Residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 / 3719 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3719 ---
    const struct
    {
        bool        slotInRange;
        bool        want;
        ...
        // Residual 2997 / 2812 / prior 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 / 3719 poles (full 2^1 surface).
        ...
        // Prior dedicated 3719 poles still hold.
        { true, true, "prior 3719 in range bumps" },
        { false, false, "prior 3719 out of range no bump" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3719 = pinShouldBumpItemsCountOnSetEntry3719(c.slotInRange);
        const bool wantPin = c.slotInRange;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3719.
        ok = expect(got == inlineF && got == pin && got == pin3719,
                    "dual-wire free == inline == pin == pin3719") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula identity") && ok;
    }

    // Free == pin == pin3719 across residual poles.
    ...
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry(false),
                "free == pin for false") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3719(true),
                "free == pin3719 for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3719(false),
                "free == pin3719 for false") &&
         ok;

    // Prior dedicated 3719 independence: free still matches prior inline/pin.
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == inlineShouldBumpItemsCountOnSetEntry3719(true) &&
                    ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3719(true),
                "prior 3719 free/inline/pin mismatch for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == inlineShouldBumpItemsCountOnSetEntry3719(false) &&
                    ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3719(false),
                "prior 3719 free/inline/pin mismatch for false") &&
         ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3719.
    for (const bool slotInRange : { false, true })
    {
        ...
        const bool pin3719 = pinShouldBumpItemsCountOnSetEntry3719(slotInRange);
        const bool want    = slotInRange;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3719,
                                    "compose free == inline == pin == pin3719") &&
             ok;
        // Prior 3719 independence across dense poles.
        ok = expect(got == inlineShouldBumpItemsCountOnSetEntry3719(slotInRange) &&
                        got == pinShouldBumpItemsCountOnSetEntry3719(slotInRange),
                    "compose free == prior 3719 inline/pin") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3719 for multi-arg setItem bump.
    for (const bool slotInRange : { false, true })
    {
        const bool got     = ShouldBumpItemsCountOnSetEntry(slotInRange);
        const bool inlineF = inlineShouldBumpItemsCountOnSetEntry(slotInRange);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3719,
                                    "host inject free == inline == pin == pin3719") &&
             ok;
    }
