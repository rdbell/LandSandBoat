#include "test_merit_lower_merit_3671.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "merit ShouldLowerMerit 3671 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline LowerMerit count-decrement formula for dual-wire cross-check
// (slice 3671 dedicated expand residual 3054; prior dedicated 3626 / 3581):
//   count > 0
...
// Retained under dedicated expand residual 3671.
...
// (count > 0 LowerMerit count-decrement gate; OmegaXI internal/merit; slice 3671
// dedicated expand residual 3054; prior dedicated 3626 / 3581). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3626 (direct return formula)
//   - residual 3054 / 2810 / prior dedicated 3626 / 3581 poles still hold
...
auto runMeritLowerMerit3671SelfTests() -> bool
{
    ...
    // Residual 3054 / 2810 / prior dedicated 3626 / 3581 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 3054 / 2810 / prior 3626 / 3581 classic poles (0, 1, n).
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin           = pinShouldLowerMerit3671(p.count);
        const bool priorPin3626  = pinShouldLowerMerit3626(p.count);
        const bool priorPin3581  = pinShouldLowerMerit3581(p.count);
        ...
        ok = expect(got == priorPin3581, "ShouldLowerMerit == prior 3581 pin") && ok;
        ok = expect(got == wantPin, "ShouldLowerMerit == pin formula count > 0") && ok;
    }

    // Dense poles: free == inline == pin == pin3626 (direct return) across residual + interior.
                               ...
                               uint8{ 255 } })
    {
        ...
        const bool pin          = pinShouldLowerMerit3671(count);
        const bool priorPin3626 = pinShouldLowerMerit3626(count);
        const bool priorPin3581 = pinShouldLowerMerit3581(count);
        ...
        ok = expect(got == priorPin3581, "dense free == prior 3581 pin") && ok;
    }
    ...
    for (const bool present : { false, true })
    {
        for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 15 }, uint8{ 255 } })
        {
            ...
            ok             = expect(got == inlineShouldLowerMerit(count),
                        "compose free == inline") &&
                 ok;
            ok = expect(got == pinShouldLowerMerit3671(count), "compose free == pin") && ok;
            ok = expect(got == pinShouldLowerMerit3626(count), "compose free == prior 3626 pin") && ok;
            ok = expect(got == pinShouldLowerMerit3581(count), "compose free == prior 3581 pin") && ok;
    ...
    // Prior dedicated 3581 independence: free still matches prior pin.
    for (const uint8 count : { uint8{ 0 }, uint8{ 1 }, uint8{ 2 }, uint8{ 15 },
                               uint8{ 128 }, uint8{ 254 }, uint8{ 255 } })
    {
        const bool got      = ShouldLowerMerit(count);
        const bool priorPin = pinShouldLowerMerit3581(count);
        ok                  = expect(got == priorPin, "prior 3581 independence") && ok;
    }
