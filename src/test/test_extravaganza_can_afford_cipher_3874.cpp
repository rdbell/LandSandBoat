#include "test_extravaganza_can_afford_cipher_3874.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "extravaganza CanAffordCipher 3874 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua shadowEraFinish notes gate for dual-wire cross-check
// (dedicated 3874 expand residual 2881; prior 3829):
//   notes >= 1000
auto inlineCanAffordCipher3874(const int32 notes) -> bool
{
    return notes >= 1000;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3874:
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3874(const int32 notes) -> bool
{
    return notes >= extravaganzahelpers::CipherPurchaseNotes;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3829 (prior dedicated):
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3829(const int32 notes) -> bool
{
    return notes >= extravaganzahelpers::CipherPurchaseNotes;
}
...
// internal/extravaganza; dedicated slice 3874 expand residual 2881;
// prior dedicated 3829 / 3784 / 3739 / 3694 / 3649 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 retained).
//
// Coverage:
//   - free == inline == pin == pin3829 == notes >= CipherPurchaseNotes
//   - CipherPurchaseNotes == 1000 pin
//   - residual 2881 / 3829 / 3784 / 3739 / 3694 / 3649 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 pins still hold
//   - residual poles: short / exact / surplus (0, 999, 1000, 1001, MaxInt)
...
auto runExtravaganzaCanAffordCipher3874SelfTests() -> bool
{
    ...
    // Residual 2881 / 3829 / 3784 / 3739 / 3694 / 3649 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 CanAffordCipher pins still hold under dual-wire.
    ...
    ok = expect(CanAffordCipher(std::numeric_limits<int32>::max()), "residual: MaxInt notes must afford") && ok;
    ...
    // Residual poles (short / exact / surplus) + free==inline==pin==pin3829 table.
    const struct
    {
        int32       notes;
        bool        want;
        ...
        { std::numeric_limits<int32>::max(), true, "pole surplus MaxInt" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF   = inlineCanAffordCipher3874(c.notes);
        const bool pinGot    = pinCanAffordCipher3874(c.notes);
        const bool pin3829   = pinCanAffordCipher3829(c.notes);
        const bool compose   = c.notes >= CipherPurchaseNotes;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3829.
        ok = expect(got == inlineF && got == pinGot && got == pin3829, "dual-wire free==inline==pin==pin3829") && ok;
        ok = expect(got == compose, "formula free==notes>=CipherPurchaseNotes") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3829 (short / exact / surplus).
    const struct
    {
        int32       notes;
        const char* label;
        ...
        { std::numeric_limits<int32>::max(), "surplus MaxInt" },
        { -1, "short negative" },
    };
    for (const auto& p : poles)
    {
        const bool got     = CanAffordCipher(p.notes);
        const bool inlineF = inlineCanAffordCipher3874(p.notes);
        const bool pinGot  = pinCanAffordCipher3874(p.notes);
        const bool pin3829 = pinCanAffordCipher3829(p.notes);
        const bool want    = p.notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3829, "pole free==inline==pin==pin3829") && ok;
    }

    // Dense compose free == inline == pin == pin3829 over representative note values.
    const int32 dense[] = {
        -1000,
        -1,
        0,
    ...
    for (const int32 notes : dense)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3874(notes);
        const bool pinGot  = pinCanAffordCipher3874(notes);
        const bool pin3829 = pinCanAffordCipher3829(notes);
        const bool want    = notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, "dense free==pin formula") && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3829, "dense free==inline==pin==pin3829") && ok;
    }
    ...
    for (int32 notes = CipherPurchaseNotes - 5; notes <= CipherPurchaseNotes + 5; ++notes)
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3829, "neighborhood free==inline==pin==pin3829") && ok;
    }
