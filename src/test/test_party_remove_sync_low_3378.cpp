#include "test_party_remove_sync_low_3378.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "party ShouldRemoveSyncForLowLevel 3378 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldRemoveSyncForLowLevel3378 / C++ capacity
// exclusive lower-bound form (formula unchanged from 1330 / 2974):
...
// OmegaXI internal/party; dedicated slice 3378; residual expand 2974 /
// pure 1330).
//
// Coverage:
...
auto runPartyRemoveSyncLow3378SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldRemoveSyncForLowLevel3378(c.syncLevel);
        const bool pin     = pinShouldRemoveSyncForLowLevel3378(c.syncLevel);
    ...
    ok = expect(ShouldRemoveSyncForLowLevel(0) == pinShouldRemoveSyncForLowLevel3378(0),
                "free == pin level 0 remove") &&
         ok;
    ok = expect(ShouldRemoveSyncForLowLevel(9) == pinShouldRemoveSyncForLowLevel3378(9),
                "free == pin level 9 remove") &&
         ok;
    ok = expect(ShouldRemoveSyncForLowLevel(10) == pinShouldRemoveSyncForLowLevel3378(10),
                "free == pin level 10 keep") &&
         ok;
    ok = expect(ShouldRemoveSyncForLowLevel(255) == pinShouldRemoveSyncForLowLevel3378(255),
                                     ...
                                     uint8_t{ 11 }, uint8_t{ 50 }, uint8_t{ 99 }, uint8_t{ 255 } })
    {
        ...
        const bool inlineF = inlineShouldRemoveSyncForLowLevel3378(syncLevel);
        const bool pin     = pinShouldRemoveSyncForLowLevel3378(syncLevel);
    ...
    for (const auto& p : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldRemoveSyncForLowLevel3378(p.syncLevel);
        const bool pin     = pinShouldRemoveSyncForLowLevel3378(p.syncLevel);
    ...
    // Residual independence (1330 / 2974 / 2999):
