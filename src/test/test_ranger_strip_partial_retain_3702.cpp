#include "test_ranger_strip_partial_retain_3702.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranger ShouldStripPartialStealthOnRetain 3702 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack retain-success partial-stealth gate for dual-wire
// cross-check (dedicated 3702 expand residual 3082; prior dedicated 3657 / 3612 / 3567 / 3510 / 3445 / 3394):
//   !stripAllDetectable
auto inlineShouldStripPartialStealthOnRetain3702(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Compact dual-wire pin matching Go pinShouldStripPartialStealthOnRetain3702 /
// C++ capacity form (formula unchanged from 1391 / 3082 / 3394 / 3445 / 3510 / 3567 / 3612 / 3657):
//   !stripAllDetectable
...
// dedicated slice 3702 expand residual 3082; prior dedicated 3657 / 3612 / 3567 / 3510 / 3445 / 3394 / pure 1391 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3657
//   - residual 3657 / 3612 / 3567 / 3510 / 3445 / 3394 / 3082 / 1391 pins still hold
...
auto runRangerStripPartialRetain3702SelfTests() -> bool
{
    ...
    // Prior dedicated 3394 edges still hold under expand residual 3702.
    ...
    // Prior dedicated 3445 edges still hold under expand residual 3702.
    ...
    // Prior dedicated 3510 edges still hold under expand residual 3702.
    ...
    // Prior dedicated 3567 edges still hold under expand residual 3702.
    ...
    // Prior dedicated 3612 edges still hold under expand residual 3702.
    ...
    // Prior dedicated 3657 edges still hold under expand residual 3702.
    ...
    // --- Core poles: free == inline == pin == pin3657 (required edges + stability) ---
    const struct
    {
        ...
        // Residual 1391 / 3082 / prior 3394 / prior 3445 / prior 3510 / prior 3567 / prior 3612 / prior 3657 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3702(c.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3702(c.stripAllDetectable);
    ...
    for (const bool stripAll : { false, true })
    {
        ...
        const bool pin     = pinShouldStripPartialStealthOnRetain3702(stripAll);
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3702(stripAll);
    ...
    // Dense compose over strip-all poles from retainChance/roll: free == inline == pin == pin3657.
    const struct
    {
        int16 retainChance;
        int   roll0to99;
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3702(h.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3702(h.stripAllDetectable);
