#include "test_auctionutils_can_afford_fee_3310.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "auctionutils CanAffordFee 3310 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline production ProofOfPurchase fee formula for dual-wire cross-checks
// (dedicated slice 3310; residual expand 2924; prior dedicated 3280):
...
// (slice 3310). Multi-branch: positive if/else only (avoid QF1001 De Morgan
...
// dedicated slice 3310 expand residual 2924 — formula unchanged;
// prior dedicated ~3280).
//
// Coverage:
//   - free == inline == pin (multi-branch positive if/else)
//   - residual poles: exact, short, surplus, zero fee, reserve block
//   - residual 1135 / 2924 / prior dedicated 3280 pins still hold
...
auto runAuctionutilsCanAffordFee3310SelfTests() -> bool
{
    ...
    // Residual 1135 / 2924 / prior dedicated 3280 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // residual 2924 / prior dedicated 3280 re-pins
