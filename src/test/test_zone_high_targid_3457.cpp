#include "test_zone_high_targid_3457.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "zone ShouldRejectHighCharTargid 3457 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline IncreaseZoneCounter high-char-targid formula for dual-wire cross-check
// (dedicated slice 3457 expand residual 2949; prior dedicated 3384):
...
// Compact dual-wire pin matching Go pinShouldRejectHighCharTargid3457 /
...
// dedicated slice 3457 expand residual 2949; prior dedicated 3384 / pure 1363).
//
// Coverage:
//   - free == inline == pin == (targid >= CharTargidHighThreshold)
//   - residual 1363 / 2949 / prior 3384 poles still hold
...
auto runZoneHighTargid3457SelfTests() -> bool
{
    ...
    // Residual 1363 / 2949 / prior 3384 pins still hold under dual-wire.
    ...
    // free == inline == pin for targid in [0x6F0, 0x710].
    for (uint16 targid = 0x6F0; targid <= 0x710; ++targid)
    {
        ...
        ok                 = expect(got == inlineF && got == pin, "boundary free == inline == pin") && ok;
    }
