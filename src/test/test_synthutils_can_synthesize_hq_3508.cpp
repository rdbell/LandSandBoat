#include "test_synthutils_can_synthesize_hq_3508.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synthutils CanSynthesizeHQ 3508 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline canSynthesizeHQ pure half for dual-wire cross-check
// (slice 3508 dedicated expand residual 2876 / prior ~3313):
...
// Compact dual-wire pin matching free function / capacity body (slice 3508).
...
// Prior dedicated pin (slice 3313) — same formula; retained under 3508.
auto pinCanSynthesizeHQ3313(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3283) — same formula; retained under 3508.
auto pinCanSynthesizeHQ3283(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3244) — same formula; retained under 3508.
...
// OmegaXI internal/synthutils; slice 3508 dedicated expand residual 2876 /
// prior ~3313). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2876 / prior dedicated 3313 / 3283 / 3244 / pure 1922 poles still hold
...
auto runSynthutilsCanSynthesizeHQ3508SelfTests() -> bool
{
    ...
    // Residual 2876 / 1922 / prior dedicated 3244 / 3283 / 3313 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 2876 / 1922 / prior 3244 / 3283 / 3313 classic dual poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3508(p.antiHQMod);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3508(e.antiHQMod);
    ...
    for (int16 m = -200; m <= 200; ++m)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3508(m);
