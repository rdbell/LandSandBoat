#include "test_item_increment_count_insert_3804.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "itemcontainer ShouldIncrementCountOnInsertAt 3804 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertItem count-bump gate for dual-wire cross-check (dedicated 3804):
//   slotEmpty && slotID != 0
...
// Compact dual-wire pin matching Go pinShouldIncrementCountOnInsertAt3804 / C++ capacity:
//   slotEmpty && slotID != 0
...
// Prior dedicated 3759 pin (formula unchanged) for free == inline == pin == pin3759.
auto pinShouldIncrementCountOnInsertAt3759(const bool slotEmpty, const std::uint8_t slotID) -> bool
{
    return slotEmpty && slotID != 0;
}
...
// dedicated slice 3804 expand residual 3021; prior dedicated 3759 / 3714 / 3669 / 3624 / 3579 / 3535 / 3486 / 3432 / 3374 retained).
//
// Coverage:
//   - free == inline == pin == pin3759 == (slotEmpty && slotID != 0)
//   - residual 2802 / 3021 pins still hold
//   - prior 3759 dedicated poles still hold
...
auto runItemIncrementCountInsert3804SelfTests() -> bool
{
    ...
    // Prior dedicated 3759 poles still hold.
    ok = expect(ShouldIncrementCountOnInsertAt(true, 1), "prior 3759 inc empty nonzero") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 1), "prior 3759 no inc occupied") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(true, 0), "prior 3759 no inc empty zero") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 0), "prior 3759 no inc occupied zero") && ok;
    ok = expect(ShouldIncrementCountOnInsertAt(true, 255), "prior 3759 inc empty max") && ok;
    ok = expect(!ShouldIncrementCountOnInsertAt(false, 255), "prior 3759 no inc occupied max") && ok;

    // --- Composition table: free == inline == pin == pin3759 ---
    const struct
    {
        bool         slotEmpty;
        std::uint8_t slotID;
        ...
        // Prior dedicated 3759 poles still hold.
        { true, 1, true, "prior 3759 empty nonzero" },
        { false, 1, false, "prior 3759 occupied nonzero" },
        { true, 0, false, "prior 3759 empty zero" },
        { false, 0, false, "prior 3759 occupied zero" },
        { true, 120, true, "prior 3759 empty max-container slot" },
        { true, 255, true, "prior 3759 empty max uint8" },
        { false, 120, false, "prior 3759 occupied max-container slot" },
        { false, 255, false, "prior 3759 occupied max uint8" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3759 = pinShouldIncrementCountOnInsertAt3759(c.slotEmpty, c.slotID);
        const bool wantPin = c.slotEmpty && c.slotID != 0;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3759.
        ok = expect(got == inlineF && got == pin && got == pin3759, "dual-wire free == inline == pin == pin3759") && ok;
        ok = expect(got == wantPin, "free == pin formula empty && slotID != 0") && ok;
    }

    // Explicit poles free == inline == pin == pin3759 for empty/occupied × slot edges.
    ...
    for (const bool empty : { false, true })
    {
        for (const std::uint8_t slotID : slotEdges)
        {
            ...
            const bool pin3759 = pinShouldIncrementCountOnInsertAt3759(empty, slotID);
            const bool want    = empty && slotID != 0;
            ok                 = expect(got == want, "pole free == pin formula") && ok;
            ok                 = expect(got == inlineF && got == pin && got == pin3759, "pole free == inline == pin == pin3759") && ok;
        }
    }

    // Dense compose: empty × slot edges free == inline == pin == pin3759.
    for (const bool empty : { false, true })
    {
        for (const std::uint8_t slotID : slotEdges)
        {
            ...
            ok                 = expect(got == inlineF && got == pin && got == pin3759, "compose free == inline == pin == pin3759") && ok;
        }
    }

    // Host-style inject poles: free == inline == pin == pin3759 for InsertItem count bump.
    for (const auto& pair : {
         ...
         })
    {
        ...
        ok                = expect(got == inlineF && got == pin && got == pin3759, "host inject free == inline == pin == pin3759") && ok;
    }
    ...
    for (const std::uint8_t slotID : slotEdges)
    {
        ...
        // free == inline == pin == pin3759 still holds for the inc pole.
        ...
        const bool pin3759 = pinShouldIncrementCountOnInsertAt3759(true, slotID);
        ok                 = expect(got == inlineF && got == pin && got == pin3759, "inc/dec shape free == inline == pin == pin3759") && ok;
    }
