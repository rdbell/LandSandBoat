#include "test_alliance_set_local_main_3355.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldSetLocalMainParty 3355 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline assignAllianceLeader local main-party formula for dual-wire
// cross-check (slice 3355 dedicated dual-wire expand residual 2988):
...
// (memberFoundOnThisServer identity; slice 3355 dedicated dual-wire
// expand residual 2988).
auto runAllianceSetLocalMain3355SelfTests() -> bool
{
    ...
    // Residual 1346 / prior dual-wire 2988 / prior expansion 3077 pins still hold.
    ok = expect(ShouldSetLocalMainParty(true), "residual: admit local member") && ok;
    ok = expect(!ShouldSetLocalMainParty(false), "residual: reject remote / missing member") && ok;
    ...
    // Host inject identity: free function is the memberFoundOnThisServer identity.
    for (const bool memberFoundOnThisServer : { false, true })
    {
        ok = expect(ShouldSetLocalMainParty(memberFoundOnThisServer) == memberFoundOnThisServer,
                    "host inject identity") &&
             ok;
        ok = expect(ShouldSetLocalMainParty(memberFoundOnThisServer) ==
                        inlineShouldSetLocalMainParty(memberFoundOnThisServer),
