#include "test_extravaganza_can_afford_cipher_3559.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "extravaganza CanAffordCipher 3559 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua shadowEraFinish notes gate for dual-wire cross-check
// (dedicated 3559 expand residual 2881; prior 3504):
//   notes >= 1000
auto inlineCanAffordCipher3559(const int32 notes) -> bool
{
    return notes >= 1000;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3559:
...
// Compact dual-wire pin matching Go pinCanAffordCipher3504 (prior dedicated):
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3504(const int32 notes) -> bool
{
    return notes >= extravaganzahelpers::CipherPurchaseNotes;
}
...
// internal/extravaganza; dedicated slice 3559 expand residual 2881;
// prior dedicated 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 retained).
//
// Coverage:
//   - free == inline == pin == pin3504 == notes >= CipherPurchaseNotes
//   - CipherPurchaseNotes == 1000 pin
//   - residual 2881 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 pins still hold
...
auto runExtravaganzaCanAffordCipher3559SelfTests() -> bool
{
    ...
    // Residual 2881 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 CanAffordCipher pins still hold under dual-wire.
    ...
    // Residual poles (short / exact / surplus) + free==inline==pin==pin3504 table.
    const struct
    {
        int32       notes;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF   = inlineCanAffordCipher3559(c.notes);
        const bool pinGot    = pinCanAffordCipher3559(c.notes);
        const bool pin3504   = pinCanAffordCipher3504(c.notes);
        const bool compose   = c.notes >= CipherPurchaseNotes;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3504.
        ok = expect(got == inlineF && got == pinGot && got == pin3504, "dual-wire free==inline==pin==pin3504") && ok;
        ok = expect(got == compose, "formula free==notes>=CipherPurchaseNotes") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3504 (short / exact / surplus).
    const struct
    {
        int32       notes;
        const char* label;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3559(p.notes);
        const bool pinGot  = pinCanAffordCipher3559(p.notes);
        const bool pin3504 = pinCanAffordCipher3504(p.notes);
        const bool want    = p.notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3504, "pole free==inline==pin==pin3504") && ok;
    }

    // Dense compose free == inline == pin == pin3504 over representative note values.
    const int32 dense[] = {
        -1000,
        -1,
        0,
    ...
    for (const int32 notes : dense)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3559(notes);
        const bool pinGot  = pinCanAffordCipher3559(notes);
        const bool pin3504 = pinCanAffordCipher3504(notes);
        const bool want    = notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, "dense free==pin formula") && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3504, "dense free==inline==pin==pin3504") && ok;
    }
    ...
    for (int32 notes = CipherPurchaseNotes - 5; notes <= CipherPurchaseNotes + 5; ++notes)
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3504, "neighborhood free==inline==pin==pin3504") && ok;
    }
