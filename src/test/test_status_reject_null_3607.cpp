#include "test_status_reject_null_3607.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "status effect ShouldRejectNullStatusEffect 3607 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddStatusEffect null-effect formula for dual-wire cross-check
// (dedicated slice 3607 expand residual 3080; prior dedicated 3562 / 3526 / 3481 / 3428 / 3348):
//   isNull
...
// Compact dual-wire pin matching Go pinShouldRejectNullStatusEffect3607 /
// C++ capacity identity form (formula unchanged from 1371 / 3080 / 3348 / 3428 / 3481 / 3526 / 3562):
//   isNull
...
// Prior dedicated 3562 pin (retained) for free==inline==pin==pin3562.
auto pinShouldRejectNullStatusEffect3562(const bool isNull) -> bool
{
    return isNull;
}
...
// (isNull identity; OmegaXI internal/statuseffect; dedicated slice 3607
// expand residual 3080 / prior dedicated 3562 / 3526 / 3481 / 3428 / 3348 / pure 1371).
//
// Coverage:
//   - free == inline == pin == pin3562 identity form
//   - poles: null rejects; non-null proceeds
//   - residual 3080 / 1371 pins still hold
//   - prior 3348 / 3428 / 3481 / 3526 / 3562 dedicated poles still hold
...
auto runStatusRejectNull3607SelfTests() -> bool
{
    ...
    // --- Core poles: free == inline == pin == pin3562 identity form ---
    const struct
    {
        bool        isNull;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3562 = pinShouldRejectNullStatusEffect3562(c.isNull);
        ...
        ok = expect(got == pin3562, "dual-wire free==pin3562") && ok;
    ...
    // Free == pin3562 across residual poles (prior dedicated retained).
    ok = expect(ShouldRejectNullStatusEffect(true) == pinShouldRejectNullStatusEffect3562(true),
                "free==pin3562 null effect") &&
         ok;
    ok = expect(ShouldRejectNullStatusEffect(false) == pinShouldRejectNullStatusEffect3562(false),
                "free==pin3562 non-null effect") &&
         ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3562.
    for (const bool isNull : { false, true })
    {
        ...
        const bool pin3562 = pinShouldRejectNullStatusEffect3562(isNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3562, "compose free==pin3562") && ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3562, "host compose free==pin3562") && ok;
    }
    ...
    // 3562; 3049 / 3069 / 3113 / 3135 left alone): null reject is distinct from
    ...
    for (const bool isNull : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3562,
                    "dense free==inline==pin==pin3562") &&
             ok;
        ok = expect(ShouldExpireEffect(true, 100, 100),
