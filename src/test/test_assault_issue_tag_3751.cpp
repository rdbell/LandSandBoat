#include "test_assault_issue_tag_3751.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3751 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3751 expand residual 2867 / prior 3706 stock / 3661 / 3616 / 3571 / 3524 / 3388 / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3751:
//   tagStock > 0
...
// internal/assault; dedicated slice 3751 expand residual 2867 / prior 3706
// stock / 3661 / 3616 / 3571 / 3524 / 3388 / 3440 ShouldIssueNewTag sibling / pure 1100 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3706 == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 / 3706 stock pins still hold
...
// test_assault_issue_tag_3706 (prior stock). Mirrors 3706
// expand-residual suite layout (header + self-test runner; 3706
...
auto runAssaultIssueTag3751SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 / 3706 stock poles still hold.
    ...
    // --- Composition table: free == inline == pin == pin3706 == compose ---
    const struct
    {
        int32       tagStock;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3706 = pinCanIssueTagFromStock3706(c.tagStock);
        ...
        ok = expect(got == pin3706, "dual-wire free==pin3706") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3706 for stock 0 / 1.
    for (const int32 stock : { 0, 1 })
    {
        ...
        const bool pin3706 = pinCanIssueTagFromStock3706(stock);
        ...
        ok                 = expect(got == pin3706, "pole free == pin3706") && ok;
    }

    // Dense tagStock: free == pin == inline == pin3706 == compose.
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3706 = pinCanIssueTagFromStock3706(h.tagStock);
        ...
        ok                 = expect(got == pin3706, "host free==pin3706") && ok;
