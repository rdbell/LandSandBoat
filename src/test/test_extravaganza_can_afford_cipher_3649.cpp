#include "test_extravaganza_can_afford_cipher_3649.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "extravaganza CanAffordCipher 3649 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua shadowEraFinish notes gate for dual-wire cross-check
// (dedicated 3649 expand residual 2881; prior 3604):
//   notes >= 1000
auto inlineCanAffordCipher3649(const int32 notes) -> bool
{
    return notes >= 1000;
}

// Compact dual-wire pin matching Go pinCanAffordCipher3649:
...
// Compact dual-wire pin matching Go pinCanAffordCipher3604 (prior dedicated):
//   notes >= CipherPurchaseNotes
auto pinCanAffordCipher3604(const int32 notes) -> bool
{
    return notes >= extravaganzahelpers::CipherPurchaseNotes;
}
...
// internal/extravaganza; dedicated slice 3649 expand residual 2881;
// prior dedicated 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 retained).
//
// Coverage:
//   - free == inline == pin == pin3604 == notes >= CipherPurchaseNotes
//   - CipherPurchaseNotes == 1000 pin
//   - residual 2881 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 pins still hold
...
auto runExtravaganzaCanAffordCipher3649SelfTests() -> bool
{
    ...
    // Residual 2881 / 3604 / 3559 / 3504 / 3462 / 3408 / 3322 / 3292 / 3262 / 3153 / 0949 CanAffordCipher pins still hold under dual-wire.
    ...
    // Residual poles (short / exact / surplus) + free==inline==pin==pin3604 table.
    const struct
    {
        int32       notes;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF   = inlineCanAffordCipher3649(c.notes);
        const bool pinGot    = pinCanAffordCipher3649(c.notes);
        const bool pin3604   = pinCanAffordCipher3604(c.notes);
        const bool compose   = c.notes >= CipherPurchaseNotes;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3604.
        ok = expect(got == inlineF && got == pinGot && got == pin3604, "dual-wire free==inline==pin==pin3604") && ok;
        ok = expect(got == compose, "formula free==notes>=CipherPurchaseNotes") && ok;
    }

    // Explicit residual poles free == inline == pin == pin3604 (short / exact / surplus).
    const struct
    {
        int32       notes;
        const char* label;
    ...
    for (const auto& p : poles)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3649(p.notes);
        const bool pinGot  = pinCanAffordCipher3649(p.notes);
        const bool pin3604 = pinCanAffordCipher3604(p.notes);
        const bool want    = p.notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, p.label) && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3604, "pole free==inline==pin==pin3604") && ok;
    }

    // Dense compose free == inline == pin == pin3604 over representative note values.
    const int32 dense[] = {
        -1000,
        -1,
        0,
    ...
    for (const int32 notes : dense)
    {
        ...
        const bool inlineF = inlineCanAffordCipher3649(notes);
        const bool pinGot  = pinCanAffordCipher3649(notes);
        const bool pin3604 = pinCanAffordCipher3604(notes);
        const bool want    = notes >= CipherPurchaseNotes;
        ok                 = expect(got == want, "dense free==pin formula") && ok;
        ok                 = expect(got == inlineF && got == pinGot && got == pin3604, "dense free==inline==pin==pin3604") && ok;
    }
    ...
    for (int32 notes = CipherPurchaseNotes - 5; notes <= CipherPurchaseNotes + 5; ++notes)
    {
        ...
        ok                 = expect(got == inlineF && got == pinGot && got == pin3604, "neighborhood free==inline==pin==pin3604") && ok;
    }
