#include "test_ranger_strip_partial_retain_3445.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "ranger ShouldStripPartialStealthOnRetain 3445 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline OnRangedAttack retain-success partial-stealth gate for dual-wire
// cross-check (dedicated 3445 expand residual 3082; prior dedicated 3394):
//   !stripAllDetectable
auto inlineShouldStripPartialStealthOnRetain3445(const bool stripAllDetectable) -> bool
{
    return !stripAllDetectable;
}

// Compact dual-wire pin matching Go pinShouldStripPartialStealthOnRetain3445 /
// C++ capacity form (formula unchanged from 1391 / 3082 / 3394):
//   !stripAllDetectable
...
// dedicated slice 3445 expand residual 3082; prior dedicated 3394 / pure 1391 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin
//   - residual 3394 / 3082 / 1391 pins still hold
...
// test_ranger_strip_partial_retain_3394.
...
auto runRangerStripPartialRetain3445SelfTests() -> bool
{
    ...
    // Prior dedicated 3394 edges still hold under expand residual 3445.
    ...
    const struct
    {
        ...
        // Residual 1391 / 3082 / prior 3394 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3445(c.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3445(c.stripAllDetectable);
    ...
    for (const bool stripAll : { false, true })
    {
        ...
        const bool pin     = pinShouldStripPartialStealthOnRetain3445(stripAll);
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3445(stripAll);
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool inlineF = inlineShouldStripPartialStealthOnRetain3445(h.stripAllDetectable);
        const bool pin     = pinShouldStripPartialStealthOnRetain3445(h.stripAllDetectable);
