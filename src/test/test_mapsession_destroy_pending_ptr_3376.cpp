#include "test_mapsession_destroy_pending_ptr_3376.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "mapsession ShouldDestroyPendingByPointer 3376 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline MapSessionContainer::destroyPendingSession(MapSession*) gate for
// dual-wire cross-check (dedicated slice 3376):
...
// Compact dual-wire pin matching Go pinShouldDestroyPendingByPointer3376 /
...
// (found && pointerMatches; dedicated slice 3376 expand residual 3056 /
// pure 2787).
//
// Coverage:
//   - free == inline == pin == (found && pointerMatches)
//   - residual 2787 / 3056 pins still hold
...
auto runMapsessionDestroyPendingPtr3376SelfTests() -> bool
{
    ...
    // Residual 2787 / 3056 pins still hold under dual-wire.
    ok = expect(ShouldDestroyPendingByPointer(true, true), "residual 3056/2787: found match destroys") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(true, false), "residual 3056/2787: found mismatch rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, false), "residual 3056/2787: missing rejects") && ok;
    ok = expect(!ShouldDestroyPendingByPointer(false, true), "residual 3056/2787: missing ignores match flag") && ok;
    ...
    // Residual independence (2787 / 3056 / 3066): pointer path needs identity;
