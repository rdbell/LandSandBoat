#include "test_alliance_attempt_leader_promote_3861.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "alliance ShouldAttemptAllianceLeaderPromote 3861 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline removeParty promote-gate formula for dual-wire cross-check
// (slice 3861 dedicated dual-wire expand residual 2979):
//   isMainParty
...
// Prior dedicated expand residual 2979 pin (slice 3816; formula UNCHANGED).
// free == inline == pin == pin3816.
auto pinShouldAttemptAllianceLeaderPromote3816(const bool isMainParty) -> bool
{
    return isMainParty;
}

// This slice's dual-wire pin (slice 3861; formula isMainParty identity —
// UNCHANGED). free == inline == pin == pin3816.
auto pinShouldAttemptAllianceLeaderPromote3861(const bool isMainParty) -> bool
{
    return isMainParty;
}
...
// (isMainParty identity; slice 3861 dedicated dual-wire expand residual 2979).
//
// Coverage:
//   - main party → attempt promote true
//   - non-main → attempt promote false
//   - free == inline == pin == pin3816 (isMainParty identity)
//   - residual 1341 / 1346 / 2979 / 3144 / 3393 / 3447 / 3490 / 3547 / 3591 / 3636 / 3681 / 3726 / 3771 / 3816 pins still hold
...
auto runAllianceAttemptLeaderPromote3861SelfTests() -> bool
{
    ...
    // dedicated 3547 / prior dedicated 3591 / prior dedicated 3636 / prior
    // dedicated 3681 / prior dedicated 3726 / prior dedicated 3771 / prior
    // dedicated 3816 pins still hold.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3861(c.isMainParty);
        const bool pin3816 = pinShouldAttemptAllianceLeaderPromote3816(c.isMainParty);

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3816,
                    "ShouldAttemptAllianceLeaderPromote free == inline == pin == pin3816") &&
             ok;
    }
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3816.
    for (const bool isMainParty : { false, true })
    {
        ...
        const bool pin     = pinShouldAttemptAllianceLeaderPromote3861(isMainParty);
        const bool pin3816 = pinShouldAttemptAllianceLeaderPromote3816(isMainParty);
        const bool want    = isMainParty;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3816,
                    "compose free == inline == pin == pin3816") &&
             ok;
    }
    ...
    for (const bool isMainParty : { false, true })
    {
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3816,
                    "host inject dual-wire free == inline == pin == pin3816") &&
             ok;
    }
