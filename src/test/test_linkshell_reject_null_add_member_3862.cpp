#include "test_linkshell_reject_null_add_member_3862.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "linkshell ShouldRejectNullAddMember 3862 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldRejectNullAddMember3862 / C++
// capacity identity form (formula unchanged from 1354 / 2929 / prior dedicated
// 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817):
//   charNull
// free == inline == pin == pin3817.
...
// Prior dedicated dual-wire 3817 pin (retained suite form) for residual re-pins.
// free == inline == pin == pin3817.
auto pinShouldRejectNullAddMember3817(const bool charNull) -> bool
{
    return charNull;
}
...
// (charNull identity; OmegaXI internal/linkshell; dedicated slice 3862;
// residual expand 2929 / pure 1354; prior dedicated expands 3195 / 3489 / 3548 /
// 3592 / 3637 / 3682 / 3727 / 3772 / 3817).
//
// Coverage:
//   - free == inline == pin == pin3817 identity form
//   - poles: null rejects; non-null allows
//   - residual 2929 / 1354 / prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817 pins still hold
//   - host-style inject + residual independence (2958 / 3079)
//   - prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817 independence (RETAINED)
auto runLinkshellRejectNullAddMember3862SelfTests() -> bool
{
    ...
    // Residual 1354 / 2929 / prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817 pins still
    ...
    ok = expect(ShouldRejectNullAddMember(true), "prior dedicated 3817: null char rejects") && ok;
    ok = expect(!ShouldRejectNullAddMember(false), "prior dedicated 3817: non-null char allows") && ok;
    ok = expect(pinShouldRejectNullAddMember3817(true), "prior dedicated 3817 pin null rejects") && ok;
    ok = expect(!pinShouldRejectNullAddMember3817(false), "prior dedicated 3817 pin non-null allows") && ok;

    // --- Core poles: free == inline == pin == pin3817 identity form ---
    const struct
    {
        bool        charNull;
        bool        want;
        ...
        // Residual 2929 / 1354 / prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817 re-pins.
        ...
        { true, true, "prior dedicated 3817 null rejects" },
        { false, false, "prior dedicated 3817 non-null allows" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3817  = pinShouldRejectNullAddMember3817(c.charNull);
        ...
        ok = expect(got == pin3817, "dual-wire free==pin3817") && ok;
        ok = expect(got == wantPin, "dual-wire free==identity pin form") && ok;
    }

    // Free == pin / pin3817 across residual poles.
    ...
    ok = expect(ShouldRejectNullAddMember(true) == pinShouldRejectNullAddMember3817(true), "free==pin3817 null char") && ok;
    ok = expect(ShouldRejectNullAddMember(false) == pinShouldRejectNullAddMember3817(false), "free==pin3817 non-null char") && ok;

    // Dense compose: full 2^1 boolean space free == inline == pin == pin3817.
    for (const bool charNull : { false, true })
    {
        ...
        const bool pin3817 = pinShouldRejectNullAddMember3817(charNull);
        ok                 = expect(got == inlineF, "compose free==inline") && ok;
        ok                 = expect(got == pinGot, "compose free==pin") && ok;
        ok                 = expect(got == pin3817, "compose free==pin3817") && ok;
        ...
        ok                 = expect(got == pinShouldRejectNullAddMember3772(charNull), "compose free==prior pin 3772") && ok;
    }
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3817, "host compose free==pin3817") && ok;
    }
    ...
    // Residual independence (1354 / 2929 / 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817 / 2958 / 3079):
    // AddMember null reject is distinct from duplicate-add and online-null gates.
    // Prior dedicated 3195 / 3489 / 3548 / 3592 / 3637 / 3682 / 3727 / 3772 / 3817 independence (RETAINED under this expand).
    ...
    ok = expect(pinShouldRejectNullAddMember3817(true), "prior dedicated 3817 pin still rejects null") && ok;
    ok = expect(!pinShouldRejectNullAddMember3817(false), "prior dedicated 3817 pin still allows non-null") && ok;
