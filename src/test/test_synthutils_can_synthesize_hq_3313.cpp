#include "test_synthutils_can_synthesize_hq_3313.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synthutils CanSynthesizeHQ 3313 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline canSynthesizeHQ pure half for dual-wire cross-check
// (slice 3313 dedicated expand residual 2876 / prior ~3283):
...
// Compact dual-wire pin matching free function / capacity body (slice 3313).
...
// Prior dedicated pin (slice 3283) — same formula; retained under 3313.
auto pinCanSynthesizeHQ3283(const int16 antiHQMod) -> bool
{
    return antiHQMod == 0;
}

// Prior dedicated pin (slice 3244) — same formula; retained under 3313.
...
// OmegaXI internal/synthutils; slice 3313 dedicated expand residual 2876 /
// prior ~3283). Formula unchanged.
//
// Coverage:
//   - free == inline == pin (direct return formula)
//   - residual 2876 / prior dedicated 3283 / 3244 / pure 1922 poles still hold
...
auto runSynthutilsCanSynthesizeHQ3313SelfTests() -> bool
{
    ...
    // Residual 2876 / 1922 / prior dedicated 3244 / 3283 pins still hold.
    ...
    const struct
    {
        ...
        // Residual 2876 / 1922 / prior 3244 / 3283 classic dual poles.
    ...
    for (const auto& p : poles)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3313(p.antiHQMod);
    ...
    for (const auto& e : edges)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3313(e.antiHQMod);
    ...
    for (int16 m = -200; m <= 200; ++m)
    {
        ...
        const bool pin     = pinCanSynthesizeHQ3313(m);
