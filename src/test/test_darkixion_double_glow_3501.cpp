#include "test_darkixion_double_glow_3501.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "darkixion ShouldDoubleGlowSkill 3501 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline DI_GLOW double-up pure gate for dual-wire cross-check
// (dedicated 3501 expand residual 2915; prior dedicated 3389 / 3443):
//   animSub == GLOWING
...
// Compact dual-wire pin matching Go pinShouldDoubleGlowSkill3501 /
// free function / capacity body (slice 3501). Equality form only — avoid
...
// Residual prior dedicated pin (slice 3443) — formula identical.
auto pinShouldDoubleGlowSkill3443(const int32 animSub) -> bool
{
    return animSub == darkixionhelpers::kAnimGlowing;
}

// Residual prior dedicated pin (slice 3389) — formula identical.
auto pinShouldDoubleGlowSkill3389(const int32 animSub) -> bool
{
    return animSub == darkixionhelpers::kAnimGlowing;
}
...
// 3501 expand residual 2915; prior dedicated 3389 / 3443 / pure 0985 — formula
// unchanged).
//
// Coverage:
...
//   - dense animSub space
//   - residual 0985 / 2915 / 3389 / 3443 pins still hold
//   - host-owned skill pick / useMobAbility left outside pure surface
//
// Sibling suites RETAINED: test_darkixion_double_glow_2915,
// test_darkixion_double_glow_3389, test_darkixion_double_glow_3443.
//
...
auto runDarkixionDoubleGlow3501SelfTests() -> bool
{
    ...
    // Residual 0985 / 2915 / 3389 / 3443 ShouldDoubleGlowSkill pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // residual 3443 re-pins
        { kAnimGlowing, true, "residual 3443 glowing" },
        { kAnimNormal, false, "residual 3443 normal" },
        // residual 3389 re-pins
        { kAnimGlowing, true, "residual 3389 glowing" },
        { kAnimNormal, false, "residual 3389 normal" },
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3443 = pinShouldDoubleGlowSkill3443(c.animSub);
        const bool pin3389 = pinShouldDoubleGlowSkill3389(c.animSub);
        ...
        ok = expect(got == pin3443, "dual-wire free==pin3443") && ok;
        ok = expect(got == pin3389, "dual-wire free==pin3389") && ok;
    ...
    for (const int32 anim : composeAnims)
    {
        ...
        ok              = expect(got == pinShouldDoubleGlowSkill3443(anim), "compose pin3443") && ok;
        ok              = expect(got == pinShouldDoubleGlowSkill3389(anim), "compose pin3389") && ok;
    }
    ...
    // Residual 3389 independence: free still matches prior dedicated pin.
    ok = expect(ShouldDoubleGlowSkill(kAnimGlowing) == pinShouldDoubleGlowSkill3389(kAnimGlowing),
                "residual 3389 independence glowing") &&
         ok;
    ok = expect(ShouldDoubleGlowSkill(kAnimNormal) == pinShouldDoubleGlowSkill3389(kAnimNormal),
                "residual 3389 independence normal") &&
         ok;

    // Residual 3443 independence: free still matches prior dedicated pin.
    ok = expect(ShouldDoubleGlowSkill(kAnimGlowing) == pinShouldDoubleGlowSkill3443(kAnimGlowing),
                "residual 3443 independence glowing") &&
         ok;
    ok = expect(ShouldDoubleGlowSkill(kAnimNormal) == pinShouldDoubleGlowSkill3443(kAnimNormal),
                "residual 3443 independence normal") &&
         ok;
