#include "test_voidwalker_should_upgrade_ki_3433.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "voidwalker ShouldUpgradeKI 3433 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline voidwalker.lua checkUpgrade roll gate for dual-wire cross-check
// (dedicated expand residual 3433):
//   rand == 5  // math.random(1, 10) == 5
auto inlineShouldUpgradeKI3433(const int32 roll) -> bool
{
    return roll == 5;
}

// Compact dual-wire pin matching Go pinShouldUpgradeKI3433 / C++ capacity
// UpgradeRollSuccess form (formula unchanged from 0987 / 2884 / 3173 / 3377):
//   roll == UpgradeRollSuccess  // == 5
auto pinShouldUpgradeKI3433(const int32 roll) -> bool
{
    using voidwalkerhelpers::UpgradeRollSuccess;
    return roll == UpgradeRollSuccess;
}

// Prior dedicated 3173 pin form (residual pin under 3433).
...
// Prior dedicated 3377 pin form (residual pin under 3433).
...
// Residual 2884 inline form (residual pin under 3433).
...
// OmegaXI internal/voidwalker; dedicated expand residual 3433 of residual
// 2884; prior dedicated 3377 / 3173 / pure 0987).
//
// Coverage:
//   - free == inline == pin
//   - UpgradeRollSuccess == 5 pin
//   - residual 2884 / 0987 / prior dedicated 3173 / 3377 pins still hold
...
auto runVoidwalkerShouldUpgradeKI3433SelfTests() -> bool
{
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldUpgradeKI3433(c.roll);
        const bool pin     = pinShouldUpgradeKI3433(c.roll);
        // Positive form pin composition (explicit roll == UpgradeRollSuccess).
        const bool wantPin = c.roll == UpgradeRollSuccess;
        // Residual free == prior dedicated 3173 / 3377 pin forms as well.
    ...
    ok = expect(ShouldUpgradeKI(UpgradeRollSuccess) == pinShouldUpgradeKI3433(UpgradeRollSuccess),
                "free == pin for UpgradeRollSuccess") &&
         ok;
    ok = expect(!ShouldUpgradeKI(UpgradeRollSuccess - 1), "UpgradeRollSuccess-1 should not upgrade") && ok;
    ok = expect(!ShouldUpgradeKI(UpgradeRollSuccess + 1), "UpgradeRollSuccess+1 should not upgrade") && ok;
    ok = expect(ShouldUpgradeKI(UpgradeRollMin) == pinShouldUpgradeKI3433(UpgradeRollMin),
                "free == pin for UpgradeRollMin") &&
         ok;
    ok = expect(ShouldUpgradeKI(UpgradeRollMax) == pinShouldUpgradeKI3433(UpgradeRollMax),
                "free == pin for UpgradeRollMax") &&
         ok;

    // Dense compose across inject range poles 1..10 and neighbors:
    // free == inline == pin (+ residual 2884 inline + prior 3377 pin).
    for (int32 roll = UpgradeRollMin - 2; roll <= UpgradeRollMax + 2; ++roll)
    {
        const bool got     = ShouldUpgradeKI(roll);
        const bool inlineF = inlineShouldUpgradeKI3433(roll);
        const bool pin     = pinShouldUpgradeKI3433(roll);
