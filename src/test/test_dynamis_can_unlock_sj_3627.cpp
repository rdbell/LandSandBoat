#include "test_dynamis_can_unlock_sj_3627.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "dynamis CanUnlockSJ 3627 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua formula for dual-wire cross-check (slice 3627 expand residual 2921):
...
// Compact dual-wire pin matching Go pinCanUnlockSJ3627:
...
//   hasSJRestriction ? 1 : 0
// free==inline==pin==pin3582 required for residual expand 3627.
...
// slice 3627 dedicated expand residual 2921; prior dedicated 3582 / 3538 / 3476 / 3422 / 3151 retained).
//
// Coverage:
//   - has SJ_RESTRICTION → 1; no restriction → 0
//   - free == inline == pin == pin3582 == hasSJRestriction ? 1 : 0
...
auto runDynamisCanUnlockSJ3627SelfTests() -> bool
{
    ...
    // --- Core branches: free == inline == pin == pin3582 == formula ---
    ...
    // free == pin3582 (required dual-wire identity for residual expand).
    ok = expect(CanUnlockSJ(true) == pinCanUnlockSJ3582(true), "free==pin3582 hasSJ=true") && ok;
    ok = expect(CanUnlockSJ(false) == pinCanUnlockSJ3582(false), "free==pin3582 hasSJ=false") && ok;
    ...
    // Dense 2^1 compose: free == inline == pin == pin3582 == formula
    // (and prior 3582 / 3538 / 3476 / 3422 / 3151 pins).
