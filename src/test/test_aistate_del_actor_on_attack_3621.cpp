#include "test_aistate_del_actor_on_attack_3621.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3621 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3621 expand residual 3044):
//   emittedHostile
auto inlineShouldDelActorOnAttack3621(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3621 / C++ capacity
// identity form (formula unchanged from 1631 / 3044 / 3178 / 3401 / 3453 / 3521 / 3576):
//   emittedHostile
...
// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3576
// (prior dedicated retained under 3621):
//   emittedHostile
...
// OmegaXI internal/aistate; dedicated slice 3621 expand residual 3044;
// prior dedicated 3576 / 3521 / 3453 / 3401 / 3178; pure 1631 / 1709 / 2306). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3576
//   - residual 3044 / 1631 / prior 3178 / prior 3401 / prior 3453 / prior 3521 / prior 3576 pins still hold
...
auto runAistateDelActorOnAttack3621SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 / prior 3178 / prior 3401 / prior 3453 / prior 3521 / prior 3576 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044/3178/3401/3453/3521/3576: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044/3178/3401/3453/3521/3576: emittedHostile false → no del") && ok;

    // --- Core poles: free == inline == pin == pin3576 ---
    const struct
    {
        bool        emittedHostile;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3621(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3621(c.emittedHostile);
        const bool pin3576 = pinShouldDelActorOnAttack3576(c.emittedHostile);
        // Positive form pin composition (explicit identity).
        const bool wantPin = c.emittedHostile;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3576 && got == wantPin,
                    "ShouldDelActorOnAttack free == inline == pin == pin3576") &&
             ok;
    }
    ...
    // Free == pin3576 across residual poles (prior dedicated independence).
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3576(true),
                "free == pin3576 for emittedHostile true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3576(false),
                "free == pin3576 for emittedHostile false") &&
         ok;

    // Dense compose: full 2¹ boolean space free == inline == pin == pin3576.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3621(emitted);
        const bool pin     = pinShouldDelActorOnAttack3621(emitted);
        const bool pin3576 = pinShouldDelActorOnAttack3576(emitted);
        const bool want    = emitted;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3576,
                                    "compose free == inline == pin == pin3576") &&
             ok;
    }
    ...
    // free == inline == pin == pin3576 for host inject poles.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3621(emitted);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3576,
                                    "host inject free == inline == pin == pin3576") &&
             ok;
    }

    // Prior dedicated 3576 pin independence under 3621 expand.
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3576(true),
                "prior 3576 pin holds under 3621 for true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3576(false),
                "prior 3576 pin holds under 3621 for false") &&
         ok;
