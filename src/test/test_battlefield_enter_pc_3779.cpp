#include "test_battlefield_enter_pc_3779.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "battlefield ShouldEnterPC 3779 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline InsertEntity TYPE_PC enter-branch formula for dual-wire cross-check
// (dedicated 3779 expand residual 3024; prior dedicated 3734 / 3689 / 3644 / 3599 / 3554 / 3497 / 3431 / 3381):
//   enter
...
// Compact dual-wire pin matching Go pinShouldEnterPC3779 /
// C++ capacity enter-identity form (formula unchanged from 1361 / 3024 / 3381 /
// 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734):
//   enter
...
// Prior dedicated 3734 inline/pin for independence cross-check (pin3734 chain).
auto inlineShouldEnterPC3734(const bool enter) -> bool
{
    return enter;
}

auto pinShouldEnterPC3734(const bool enter) -> bool
{
    return enter;
}
...
// 3779 expand residual 3024 / prior dedicated 3734 / 3689 / 3644 / 3599 / 3554 / 3497 / 3431 / 3381 / pure 1361).
//
// Coverage:
//   - free == inline == pin residual pins
//   - free == pin3734 (prior dedicated pin chain)
//   - poles: enter-true / enter-false
//   - residual 1361 / 3024 pins still hold
//   - prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734 poles still hold
...
auto runBattlefieldEnterPC3779SelfTests() -> bool
{
    ...
    // 3554 / 3599 / 3644 / 3689 / 3734 pins still hold.
    ...
    // --- Core poles: free == inline == pin residual pins (== pin3734) ---
    const struct
    {
        bool        enter;
        bool        want;
        ...
        // Residual 1361 / 3024 / prior dedicated 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734 re-pins.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3734  = pinShouldEnterPC3734(c.enter);
        ...
        ok = expect(got == pin3734, "dual-wire free==pin3734") && ok;
    ...
    // free == pin3734 pin chain (constraint free==inline==pin==pin3734).
    ok = expect(ShouldEnterPC(true) == pinShouldEnterPC3734(true),
                "free==pin3734 enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == pinShouldEnterPC3734(false),
                "free==pin3734 enter=false") &&
         ok;
    ok = expect(pinShouldEnterPC(true) == pinShouldEnterPC3734(true),
                "pin3779==pin3734 enter=true") &&
         ok;
    ok = expect(pinShouldEnterPC(false) == pinShouldEnterPC3734(false),
                "pin3779==pin3734 enter=false") &&
         ok;

    // Prior dedicated 3734 independence: free still matches prior inline/pin.
    ok = expect(ShouldEnterPC(true) == inlineShouldEnterPC3734(true) &&
                    ShouldEnterPC(true) == pinShouldEnterPC3734(true),
                "prior 3734 independence enter=true") &&
         ok;
    ok = expect(ShouldEnterPC(false) == inlineShouldEnterPC3734(false) &&
                    ShouldEnterPC(false) == pinShouldEnterPC3734(false),
                "prior 3734 independence enter=false") &&
         ok;
    ...
    for (const auto& c : hostCases)
    {
        ...
        ok = expect(got == pin3734, "host compose free==pin3734") && ok;
    }

    // Production path semantics:
    // enter=true → enter; enter=false → no enter.
    ok = expect(ShouldEnterPC(true) && pinShouldEnterPC(true) && pinShouldEnterPC3734(true),
                "enter=true → enter path") &&
         ok;
    ok = expect(!ShouldEnterPC(false) && !pinShouldEnterPC(false) && !pinShouldEnterPC3734(false),
                "enter=false → no enter path") &&
         ok;

    // Residual independence (1361 / 2994 / 3198 / 3002 / 3216 / 2930 / 3271 /
    // 3302 / 3014 / 3365 / 3024 / 3381 / 3431 / 3497 / 3554 / 3599 / 3644 / 3689 / 3734): enter is
