#include "test_universal_clear_slot_3532.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "universal ShouldClearSlot 3532 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ClearSlot range gate for dual-wire cross-check (dedicated 3532):
//   slotInRange
...
// Compact dual-wire pin matching Go pinShouldClearSlot3532 / C++ capacity:
//   slotInRange
...
// Prior 3469 pin for independence cross-check (formula unchanged):
//   slotInRange
auto pinShouldClearSlot3469(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3532 expand residual 2813 / prior 2980 / prior 3354 /
// prior 3415 / prior 3469).
//
// Coverage:
//   - free == inline == pin == slotInRange
//   - residual poles (2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469) still hold
//   - prior 3469 pin independence
...
auto runUniversalClearSlot3532SelfTests() -> bool
{
    ...
    // Residual 2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 pins still hold under dual-wire.
    ...
    const struct
    {
        ...
        // Classic dual poles / residual 2813+prior 2980+prior 3354+prior 3415+prior 3469 surface.
        { true, true, "in-range accepts" },
        { false, false, "out-of-range rejects" },

        // Residual 2813 / prior 2980 / prior 3354 / prior 3415 / prior 3469 pins (full 2^1 surface, labeled).
    ...
    // Prior 3469 pin independence: free == prior 3469 pin.
    for (const bool slotInRange : { false, true })
    {
        const bool got      = ShouldClearSlot(slotInRange);
        const bool priorPin = pinShouldClearSlot3469(slotInRange);
        ok                  = expect(got == priorPin, "prior 3469 pin independence") && ok;
    }
