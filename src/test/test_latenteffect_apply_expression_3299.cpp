#include "test_latenteffect_apply_expression_3299.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "latenteffect ShouldApplyLatentExpression 3299 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline ProcessLatentEffect latentFound → ApplyLatentEffect gate for dual-wire
// cross-check (dedicated slice 3299 expand residual 2983). Direct return:
//   latentFound
...
// Compact dual-wire pin matching Go pinShouldApplyLatentExpression3299 /
...
// dedicated slice 3299 expand residual 2983 / pure 1359; prior dedicated 3235).
//
// Coverage:
...
auto runLatenteffectApplyExpression3299SelfTests() -> bool
{
    ...
    ok = expect(ShouldApplyLatentExpression(true), "residual 2983 found-apply pin under 3299") && ok;
    ok = expect(!ShouldApplyLatentExpression(false), "residual 2983 not-found-skip pin under 3299") && ok;

    // Prior dedicated expand 3235 poles still hold under 3299.
    ok = expect(ShouldApplyLatentExpression(true), "prior dedicated 3235 found-apply pin under 3299") && ok;
    ok = expect(!ShouldApplyLatentExpression(false), "prior dedicated 3235 not-found-skip pin under 3299") && ok;
