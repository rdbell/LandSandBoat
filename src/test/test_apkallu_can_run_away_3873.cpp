#include "test_apkallu_can_run_away_3873.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "apkallu can run away 3873 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinCanRunAway3873:
...
// Prior dedicated 3407 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3407(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3460 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3460(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3505 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3505(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3558 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3558(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3603 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3603(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3648 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3648(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3693 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3693(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3738 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3738(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3783 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3783(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}

// Prior dedicated 3828 pin (independence cross-check):
//   GetHateTier(hate) >= 3  ≡  hate >= kHateTier3Min
auto pinCanRunAway3828(const int32 hate) -> bool
{
    return apkalluhelpers::GetHateTier(hate) >= 3;
}
...
// dedicated slice 3873 expand residual 2880 / prior dedicated ~3828 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3828 == GetHateTier>=3 == hate>=45
//   - residual poles across hate tiers 0..3+
//   - dense hate range compose
//   - residual 0925 / 2880 / prior 3149 / prior 3245 / prior 3287 / prior 3317 / prior 3407 / prior 3460 / prior 3505 / prior 3558 / prior 3603 / prior 3648 / prior 3693 / prior 3738 / prior 3783 / prior 3828 pins still hold
auto runApkalluCanRunAway3873SelfTests() -> bool
{
    ...
    // prior 3407 / prior 3460 / prior 3505 / prior 3558 / prior 3603 / prior 3648 / prior 3693 / prior 3738 / prior 3783 / prior 3828 CanRunAway pins still hold under dual-wire
    ...
    // --- Composition table: free == inline == pin == pin3828 == compose ---
    ...
    for (const auto& c : runCases)
    {
        ...
        ok = expect(got == pin3828, "CanRunAway dual-wire free == pin3828") && ok;
    ...
    // Free == pin3828 across residual poles (positive pin form).
    ok = expect(CanRunAway(45) == pinCanRunAway3828(45), "free == pin3828 residual 45") && ok;
    ok = expect(CanRunAway(100) == pinCanRunAway3828(100), "free == pin3828 residual 100") && ok;
    ok = expect(CanRunAway(44) == pinCanRunAway3828(44), "free == pin3828 residual 44") && ok;
    ok = expect(CanRunAway(0) == pinCanRunAway3828(0), "free == pin3828 hate 0") && ok;

    // Dense compose: free CanRunAway matches GetHateTier(hate) >= 3, inline, pin, pin3828, and hate >= 45.
    for (int32 hate = -20; hate <= 120; ++hate)
    {
        ...
        ok = expect(got == pinCanRunAway3828(hate), "dense free == pin3828") && ok;
        ok = expect(got == (hate >= kHateTier3Min), "dense free == hate>=45") && ok;
    }

    // Prior dedicated 3149 / 3245 / 3287 / 3317 / 3407 / 3460 / 3505 / 3558 / 3603 / 3648 / 3693 / 3738 / 3783 / 3828 independence:
    // free still matches prior pins.
    for (int32 hate = -20; hate <= 120; ++hate)
    {
        ...
        ok = expect(CanRunAway(hate) == pinCanRunAway3407(hate), "prior 3407 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3460(hate), "prior 3460 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3505(hate), "prior 3505 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3558(hate), "prior 3558 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3603(hate), "prior 3603 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3648(hate), "prior 3648 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3693(hate), "prior 3693 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3738(hate), "prior 3738 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3783(hate), "prior 3783 independence") && ok;
        ok = expect(CanRunAway(hate) == pinCanRunAway3828(hate), "prior 3828 independence") && ok;
    }
