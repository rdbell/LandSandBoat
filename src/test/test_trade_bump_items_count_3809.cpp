#include "test_trade_bump_items_count_3809.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "trade ShouldBumpItemsCountOnSetEntry 3809 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// Compact dual-wire pin matching Go pinShouldBumpItemsCountOnSetEntry3809 / C++ capacity:
//   slotInRange
...
// Prior dedicated 3764 inline pin for independence cross-check.
auto inlineShouldBumpItemsCountOnSetEntry3764(const bool slotInRange) -> bool
{
    return slotInRange;
}

// Prior dedicated 3764 compact pin for free == inline == pin == pin3764 cross-check.
auto pinShouldBumpItemsCountOnSetEntry3764(const bool slotInRange) -> bool
{
    return slotInRange;
}
...
// dedicated slice 3809 expand residual 2997; prior dedicated 3764 / 3719 / 3674 / 3629 / 3584 / 3540 / 3474 / 3420 / 3372).
//
// Coverage:
//   - free == inline == pin == pin3764 == slotInRange (identity)
//   - residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 / 3719 / 3764 pins still hold
//   - prior dedicated 3764 independence (free == prior inline == prior pin)
...
auto runTradeBumpItemsCount3809SelfTests() -> bool
{
    ...
    // Residual 2812 / 2997 / prior dedicated 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 / 3719 / 3764 pins still hold under dual-wire.
    ...
    // --- Composition table: free == inline == pin == pin3764 ---
    const struct
    {
        bool        slotInRange;
        bool        want;
        ...
        // Residual 2997 / 2812 / prior 3372 / 3420 / 3474 / 3540 / 3584 / 3629 / 3674 / 3719 / 3764 poles (full 2^1 surface).
        ...
        // Prior dedicated 3764 poles still hold.
        { true, true, "prior 3764 in range bumps" },
        { false, false, "prior 3764 out of range no bump" },
    };
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3764 = pinShouldBumpItemsCountOnSetEntry3764(c.slotInRange);
        const bool wantPin = c.slotInRange;

        ok = expect(got == c.want, c.label) && ok;
        // Positive form: free == inline == pin == pin3764.
        ok = expect(got == inlineF && got == pin && got == pin3764,
                    "dual-wire free == inline == pin == pin3764") &&
             ok;
        ok = expect(got == wantPin, "free == pin formula identity") && ok;
    }

    // Free == pin == pin3764 across residual poles.
    ...
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry(false),
                "free == pin for false") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3764(true),
                "free == pin3764 for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3764(false),
                "free == pin3764 for false") &&
         ok;

    // Prior dedicated 3764 independence: free still matches prior inline/pin.
    ok = expect(ShouldBumpItemsCountOnSetEntry(true) == inlineShouldBumpItemsCountOnSetEntry3764(true) &&
                    ShouldBumpItemsCountOnSetEntry(true) == pinShouldBumpItemsCountOnSetEntry3764(true),
                "prior 3764 free/inline/pin mismatch for true") &&
         ok;
    ok = expect(ShouldBumpItemsCountOnSetEntry(false) == inlineShouldBumpItemsCountOnSetEntry3764(false) &&
                    ShouldBumpItemsCountOnSetEntry(false) == pinShouldBumpItemsCountOnSetEntry3764(false),
                "prior 3764 free/inline/pin mismatch for false") &&
         ok;
    ...
    // Dense compose: full 2^1 boolean space free == inline == pin == pin3764.
    for (const bool slotInRange : { false, true })
    {
        ...
        const bool pin3764 = pinShouldBumpItemsCountOnSetEntry3764(slotInRange);
        const bool want    = slotInRange;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3764,
                                    "compose free == inline == pin == pin3764") &&
             ok;
        // Prior 3764 independence across dense poles.
        ok = expect(got == inlineShouldBumpItemsCountOnSetEntry3764(slotInRange) &&
                        got == pinShouldBumpItemsCountOnSetEntry3764(slotInRange),
                    "compose free == prior 3764 inline/pin") &&
             ok;
    }

    // Host-style inject poles: free == inline == pin == pin3764 for multi-arg setItem bump.
    for (const bool slotInRange : { false, true })
    {
        const bool got     = ShouldBumpItemsCountOnSetEntry(slotInRange);
        const bool inlineF = inlineShouldBumpItemsCountOnSetEntry(slotInRange);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3764,
                                    "host inject free == inline == pin == pin3764") &&
             ok;
    }
