#include "test_assault_issue_tag_3706.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "assault issue tag 3706 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline Lua onRytaalEventFinish tagStock gate for dual-wire cross-check
// (dedicated 3706 expand residual 2867 / prior 3661 stock / 3616 / 3571 / 3524 / 3388 / 3440 sibling):
//   tagStock > 0
...
// Compact dual-wire pin matching Go pinCanIssueTagFromStock3706:
//   tagStock > 0
...
// internal/assault; dedicated slice 3706 expand residual 2867 / prior 3661
// stock / 3616 / 3571 / 3524 / 3388 / 3440 ShouldIssueNewTag sibling / pure 1100 —
// formula unchanged).
//
// Coverage:
//   - free == inline == pin == pin3661 == tagStock > 0
//   - residual poles: stock 0 / 1
//   - dense tagStock space
//   - residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 stock pins still hold
...
// (prior stock), test_assault_issue_tag_3661 (prior stock). Mirrors 3661
// expand-residual suite layout (header + self-test runner; 3661
...
auto runAssaultIssueTag3706SelfTests() -> bool
{
    ...
    // Residual 1100 / 2867 / 3145 / 3258 / 3388 / 3524 / 3571 / 3616 / 3661 stock poles still hold.
    ...
    // --- Composition table: free == inline == pin == pin3661 == compose ---
    const struct
    {
        int32       tagStock;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool pin3661 = pinCanIssueTagFromStock3661(c.tagStock);
        ...
        ok = expect(got == pin3661, "dual-wire free==pin3661") && ok;
    ...
    // Explicit residual poles free == inline == pin == pin3661 for stock 0 / 1.
    for (const int32 stock : { 0, 1 })
    {
        ...
        const bool pin3661 = pinCanIssueTagFromStock3661(stock);
        ...
        ok                 = expect(got == pin3661, "pole free == pin3661") && ok;
    }

    // Dense tagStock: free == pin == inline == pin3661 == compose.
    ...
    for (const auto& h : hostPoles)
    {
        ...
        const bool pin3661 = pinCanIssueTagFromStock3661(h.tagStock);
        ...
        ok                 = expect(got == pin3661, "host free==pin3661") && ok;
