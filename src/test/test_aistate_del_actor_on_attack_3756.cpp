#include "test_aistate_del_actor_on_attack_3756.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "aistate ShouldDelActorOnAttack 3756 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline processActionEffectFlags actor ON_ATTACK gate for dual-wire
// cross-check (dedicated 3756 expand residual 3044):
//   emittedHostile
auto inlineShouldDelActorOnAttack3756(const bool emittedHostile) -> bool
{
    return emittedHostile;
}

// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3756 / C++ capacity
// identity form (formula unchanged from 1631 / 3044 / 3178 / 3401 / 3453 / 3521 / 3576 / 3621 / 3666 / 3711):
//   emittedHostile
...
// Compact dual-wire pin matching Go pinShouldDelActorOnAttack3711
// (prior dedicated retained under 3756):
//   emittedHostile
...
// OmegaXI internal/aistate; dedicated slice 3756 expand residual 3044;
// prior dedicated 3711 / 3666 / 3621 / 3576 / 3521 / 3453 / 3401 / 3178; pure 1631 / 1709 / 2306). Formula unchanged.
//
// Coverage:
//   - free == inline == pin == pin3711
//   - residual 3044 / 1631 / prior 3178 / prior 3401 / prior 3453 / prior 3521 / prior 3576 / prior 3621 / prior 3666 / prior 3711 pins still hold
...
auto runAistateDelActorOnAttack3756SelfTests() -> bool
{
    ...
    // Residual 1631 / 3044 / prior 3178 / prior 3401 / prior 3453 / prior 3521 / prior 3576 / prior 3621 / prior 3666 / prior 3711 pins still hold under dedicated dual-wire.
    ok = expect(ShouldDelActorOnAttack(true), "residual 1631/3044/3178/3401/3453/3521/3576/3621/3666/3711: emittedHostile true → del") && ok;
    ok = expect(!ShouldDelActorOnAttack(false), "residual 1631/3044/3178/3401/3453/3521/3576/3621/3666/3711: emittedHostile false → no del") && ok;

    // --- Core poles: free == inline == pin == pin3711 ---
    const struct
    {
        bool        emittedHostile;
        bool        want;
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldDelActorOnAttack3756(c.emittedHostile);
        const bool pin     = pinShouldDelActorOnAttack3756(c.emittedHostile);
        const bool pin3711 = pinShouldDelActorOnAttack3711(c.emittedHostile);
        // Positive form pin composition (explicit identity).
        const bool wantPin = c.emittedHostile;

        ok = expect(got == c.want, c.label) && ok;
        ok = expect(got == inlineF && got == pin && got == pin3711 && got == wantPin,
                    "ShouldDelActorOnAttack free == inline == pin == pin3711") &&
             ok;
    }
    ...
    // Free == pin3711 across residual poles (prior dedicated independence).
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3711(true),
                "free == pin3711 for emittedHostile true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3711(false),
                "free == pin3711 for emittedHostile false") &&
         ok;

    // Dense compose: full 2¹ boolean space free == inline == pin == pin3711.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3756(emitted);
        const bool pin     = pinShouldDelActorOnAttack3756(emitted);
        const bool pin3711 = pinShouldDelActorOnAttack3711(emitted);
        const bool want    = emitted;
        ok                 = expect(got == want, "compose free == pin formula") && ok;
        ok                 = expect(got == inlineF && got == pin && got == pin3711,
                                    "compose free == inline == pin == pin3711") &&
             ok;
    }
    ...
    // free == inline == pin == pin3711 for host inject poles.
    for (const bool emitted : { false, true })
    {
        const bool got     = ShouldDelActorOnAttack(emitted);
        const bool inlineF = inlineShouldDelActorOnAttack3756(emitted);
        ...
        ok                 = expect(got == inlineF && got == pin && got == pin3711,
                                    "host inject free == inline == pin == pin3711") &&
             ok;
    }

    // Prior dedicated 3711 pin independence under 3756 expand.
    ok = expect(ShouldDelActorOnAttack(true) == pinShouldDelActorOnAttack3711(true),
                "prior 3711 pin holds under 3756 for true") &&
         ok;
    ok = expect(ShouldDelActorOnAttack(false) == pinShouldDelActorOnAttack3711(false),
                "prior 3711 pin holds under 3756 for false") &&
         ok;
