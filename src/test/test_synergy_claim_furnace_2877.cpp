#include "test_synergy_claim_furnace_2877.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "synergy claim furnace 2877 self-test failed: " << label << '\n';
    }
    return condition;
}
...
// (Lua synergyFurnaceOnTrigger AVAILABLE claim gate).
auto runSynergyClaimFurnace2877SelfTests() -> bool
{
    ...
    for (uint16 state = 0; state <= 8; ++state)
    {
        ...
        ok = expect(got == inlineCanClaimFurnace(s), "compose range free == inline") && ok;
    }
