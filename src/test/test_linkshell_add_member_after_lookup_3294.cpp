#include "test_linkshell_add_member_after_lookup_3294.h"
...
auto expect(const bool condition, const char* const label) -> bool
{
    if (!condition)
    {
        std::cerr << "linkshell ShouldAddMemberAfterOnlineLookup 3294 self-test failed: " << label << '\n';
    }
    return condition;
}

// Inline AddOnlineMember post-lookup AddMember gate for dual-wire
// cross-check (slice 3294):
//   linkshellLoaded
auto inlineShouldAddMemberAfterOnlineLookup3294(const bool linkshellLoaded) -> bool
{
    return linkshellLoaded;
}

// Direct-return dual-wire pin matching free / capacity body (slice 3294).
// Identity form only — free == inline == pin == linkshellLoaded.
auto pinShouldAddMemberAfterOnlineLookup3294(const bool linkshellLoaded) -> bool
{
    return linkshellLoaded;
}
...
// (linkshellLoaded identity; dedicated slice 3294; residual expand 3050 /
// pure 1355).
//
// Coverage:
//   - free == inline == pin (direct return identity)
//   - residual 3050 / 1355 pins still hold
...
auto runLinkshellAddMemberAfterLookup3294SelfTests() -> bool
{
    ...
    // Residual 1355 / 3050 pins still hold under dedicated dual-wire.
    ...
    for (const auto& c : cases)
    {
        ...
        const bool inlineF = inlineShouldAddMemberAfterOnlineLookup3294(c.linkshellLoaded);
        const bool pin     = pinShouldAddMemberAfterOnlineLookup3294(c.linkshellLoaded);
    ...
    ok = expect(ShouldAddMemberAfterOnlineLookup(true) == pinShouldAddMemberAfterOnlineLookup3294(true),
                "free == pin residual loaded add") &&
         ok;
    ok = expect(ShouldAddMemberAfterOnlineLookup(false) == pinShouldAddMemberAfterOnlineLookup3294(false),
    ...
    for (const bool linkshellLoaded : { false, true })
    {
        ...
        const bool inlineF = inlineShouldAddMemberAfterOnlineLookup3294(linkshellLoaded);
        const bool pin     = pinShouldAddMemberAfterOnlineLookup3294(linkshellLoaded);
    ...
    for (const auto& h : hosts)
    {
        ...
        const bool inlineF = inlineShouldAddMemberAfterOnlineLookup3294(h.linkshellLoaded);
        const bool pin     = pinShouldAddMemberAfterOnlineLookup3294(h.linkshellLoaded);
